...
object data {
  ...
  def service[S[_]](
    ...
    case req @ GET -> AsPath(path) :? Offset(offsetParam) +& Limit(limitParam) =>
      respond_((offsetOrInvalid(offsetParam) |@| limitOrInvalid(limitParam)) {
        (offset, limit) =>
    ...
    case req @ Method.MOVE -> AsPath(path) =>
      respond((for {
        ...
        _ <- M
          .move(scn, MoveSemantics.FailIfExists)
  ...
  private def parseDestination(dstString: String): ApiError \/ APath = {
    def absPathRequired(rf: pathy.Path[Rel, _, _]) =
      ApiError
        .fromMsg(BadRequest withReason "Illegal move.",
    ...
    refineType(src).fold(
      srcDir =>
        refineType(dst).swap.bimap(
          df =>
            ApiError.fromMsg(BadRequest withReason "Illegal move.",
  ...
  private def upload[S[_]](
      ...
      OptionT(
        decodeErrors.toList.toNel
          .map(
            errs =>
              respond_[S, ApiError](
                ApiError.apiError(BadRequest withReason "Malformed upload data.",
    ...
    def write(fPath: AFile,
              xs: IndexedSeq[(DecodeError \/ Data)]): FreeS[QResponse[S] \/ Unit] =
      if (xs.isEmpty) {
        respond_[S, ApiError](
          ApiError.fromStatus(BadRequest withReason "Request has no body.")).map(_.left)
      } else {
      ...
      files.toList.traverse {
        case (rFile, contentBytes) =>
          for {
            ...
            fileMetadata <- EitherT((meta.files.get(rFile) \/> InvalidMessageBodyFailure(
            ...
            fmt <- EitherT((MessageFormat
              .fromMediaType(mdType) \/> (InvalidMessageBodyFailure(
      ...
      .fold[EitherT[FreeS, QResponse[S], Unit]](
        ...
        aDir =>
          for {
            ...
            _ <- EitherT((req.headers.get(`Content-Type`) \/> (MediaTypeMissing(
              uploadFormats): DecodeFailure))
              .flatMap { contentType =>
            ...
            tuple <- filesToContent
              ...
              .cata(
                meta =>
                  decodeUtf8(meta).strengthR(filesToContent - ArchiveMetadata.HiddenFile),
                EitherT.left[FreeS,
                             QResponse[S],
                             (String, Map[RelFile[Sandboxed], ByteVector])](
            ...
            meta <- EitherT(
              (Parse
                .decodeOption[ArchiveMetadata](metaString) \/> (InvalidMessageBodyFailure(
        ...
        aFile =>
          for {
            ...
            fmt <- EitherT(MessageFormat.forMessage(req).point[FreeS])
              .leftMap(_.toResponse[S])
            // Write a single file with the specified format
  ...
  private def zippedContents[S[_]](
    ...
    Process.await(Q.descendantFiles(dir)) { quasarFiles =>
      val metadata = ArchiveMetadata(
        quasarFiles.toList
          .strengthR(FileMetadata(`Content-Type`(format.mediaType)))
          .toMap)
