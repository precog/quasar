...
final class EquiJoinPlanner[
    T[_[_]]: BirecursiveT: ShowT, F[_]: Monad: ContextReader: NameGenerator: PlannerErrorME]
    ...
    def unapply[A](mf: FreeMapA[T, A]): Boolean =
      (
        mf.resume.swap.toOption >>= {
          case MFC(mfs.Guard(Meta(), _, _, _)) => ().some; case _ => none
        }
      ).isDefined
  ...
  object BranchCollection {
    def unapply(qs: FreeQS[T]): Option[DocTypeValue] =
      (qs match {
        ...
        case Embed(
            CoEnv(\/-(QC(qscript.Filter(Embed(CoEnv(\/-(CShiftedRead(c)))), MetaGuard()))))) =>
          c.some
        case _ => none
  ...
  object KeyMetaId {
    def unapply(mf: FreeMap[T]): Boolean = mf.resume match {
      case -\/(MFC(mfs.ProjectField(Embed(CoEnv(\/-(MFC(mfs.Meta(_))))), mfs.StrLit("id")))) =>
        true
      case _ => false
    }
  }
