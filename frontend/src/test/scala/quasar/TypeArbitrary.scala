...
trait TypeArbitrary {
  implicit def arbitraryType: Arbitrary[Type] = Arbitrary {
    Gen.sized(depth => typeGen(depth / 25))
  }

  def arbitrarySimpleType = Arbitrary {
    Gen.sized(depth => complexGen(depth / 25, simpleGen))
  }
  ...
  def arbitraryNonnestedType = Arbitrary {
    Gen.oneOf(Gen.const(Top), Gen.const(Bottom), simpleGen)
  }
  ...
  def typeGen(depth: Int): Gen[Type] = {
    ...
    val gens = Gen.oneOf(Top, Bottom) :: List(terminalGen, constGen, objectGen, arrayGen)
      .map(complexGen(depth, _))
  ...
  def coproductGen(depth: Int, gen: Gen[Type]): Gen[Type] =
    for {
  ...
  def terminalGen: Gen[Type] =
    Gen.oneOf(Null, Str, Type.Int, Dec, Bool, Binary, Timestamp, Date, Time, Interval)
  ...
  def fieldGen: Gen[(String, Type)] =
    for {
      c <- Gen.alphaChar
      t <- Gen.oneOf(terminalGen, constGen)
    } yield (c.toString(), t)

  def objectGen: Gen[Type] =
    for {
