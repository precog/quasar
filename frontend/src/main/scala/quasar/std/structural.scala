...
trait StructuralLib extends Library {
    ...
    partialTyperV[nat._2] {
      ...
      case Sized(a1, FlexArr(min2, max2, elem2)) =>
        (a1.arrayMinLength |@| a1.arrayType)(
          (min1, typ1) =>
            success(
              FlexArr(min1 + min2, (a1.arrayMaxLength |@| max2)(_ + _), Type.lub(typ1, elem2))))
      ...
      case Sized(FlexArr(min1, max1, elem1), a2) =>
        (a2.arrayMinLength |@| a2.arrayType)(
          (min2, typ2) =>
            success(
              FlexArr(min1 + min2, (max1 |@| a2.arrayMaxLength)(_ + _), Type.lub(elem1, typ2))))
          .getOrElse(failure(NonEmptyList(GenericError(a2.shows + " is not an array."))))
    },
    partialUntyperV[nat._2] {
      case x if x.arrayLike =>
        x.arrayType.fold[Func.VDomain[nat._2]](failure(NonEmptyList(
          GenericError("internal error: " + x.shows + " is arrayLike, but no arrayType")))) {
          typ =>
  ...
  val ShiftArray = UnaryFunc(
    ...
    new Func.Simplifier {
      def apply[T](orig: LP[T])(implicit TR: Recursive.Aux[T, LP], TC: Corecursive.Aux[T, LP]) =
        orig match {
          case InvokeUnapply(_, Sized(Embed(InvokeUnapply(UnshiftArray, Sized(Embed(set)))))) =>
            set.some
          case _ => None
        }
    },
  ...
  val UnshiftArray: UnaryFunc = UnaryFunc(
    ...
    new Func.Simplifier {
      def apply[T](orig: LP[T])(implicit TR: Recursive.Aux[T, LP], TC: Corecursive.Aux[T, LP]) =
        orig match {
          case InvokeUnapply(_, Sized(Embed(InvokeUnapply(ShiftArray, Sized(Embed(array)))))) =>
            array.some
          case InvokeUnapply(
              _,
              Sized(Embed(
                InvokeUnapply(ShiftArrayIndices, Sized(Embed(Constant(Data.Arr(array)))))))) =>
            Constant(Data.Arr((0 until array.length).toList ∘ (Data.Int(_)))).some
          case InvokeUnapply(
              _,
              Sized(Embed(InvokeUnapply(ShiftMap, Sized(Embed(Constant(Data.Obj(map)))))))) =>
            Constant(Data.Arr(map.values.toList)).some
          case InvokeUnapply(
              _,
              Sized(
                Embed(InvokeUnapply(ShiftMapKeys, Sized(Embed(Constant(Data.Obj(map)))))))) =>
