...
abstract class MongoDbQStdLibSpec extends StdLibSpec {
  ...
  def compile(
      ...
      coll: Collection,
      lp: FreeMap[Fix]): FileSystemError \/ (Crystallized[WorkflowF], BsonField.Name)
    ...
    val runner = new MapFuncStdLibTestRunner with MongoDbDomain {
      def run(args: List[Data], prg: List[Fix[LP]] => Fix[LP], expected: Data): Result =
        check(args, prg).getOrElse((for {
          ...
          _ <- insert(coll, List(Bson.Doc(argsBson.toListMap)).map(_.repr))
            .run(setupClient)
          ...
          lp = prg(
            (0 until args.length).toList.map(idx =>
              Fix(StructuralLib.ObjectProject(lpf.free('hole),
                                              lpf.constant(Data.Str("arg" + idx))))))
          t <- compile(qm, coll, translate[Hole](lp, _ => SrcHole))
            .point[Task]
            .unattemptRuntime
          (wf, resultField) = t
      ...
      def binaryMapFunc(prg: FreeMapA[Fix, BinaryArg],
                        arg1: Data,
                        arg2: Data,
                        expected: Data) =
        sys.error("impossible!")

      def ternaryMapFunc(prg: FreeMapA[Fix, TernaryArg],
                         arg1: Data,
                         arg2: Data,
                         arg3: Data,
                         expected: Data) =
      ...
      override def nullary(prg: Fix[LP], expected: Data): Result =
        run(Nil, κ(prg), expected)

      override def unary(prg: Fix[LP] => Fix[LP], arg: Data, expected: Data): Result =
        run(List(arg), { case List(arg) => prg(arg) }, expected)

      override def binary(prg: (Fix[LP], Fix[LP]) => Fix[LP],
                          arg1: Data,
                          arg2: Data,
                          expected: Data): Result =
        run(List(arg1, arg2), { case List(arg1, arg2) => prg(arg1, arg2) }, expected)

      override def ternary(prg: (Fix[LP], Fix[LP], Fix[LP]) => Fix[LP],
                           arg1: Data,
                           arg2: Data,
                           arg3: Data,
                           expected: Data): Result =
