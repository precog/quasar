...
package object shapeless {
  ...
  implicit class FuncUtils[A, N <: Nat](val self: Sized[List[A], N])
    ...
    def traverse[G[_], B](f: A => G[B])(
        implicit G: Applicative[G]): G[Sized[List[B], N]] =
      G.map(self.unsized.traverse(f))(bs => Sized.wrap[List[B], N](bs))

    // Input[Option[B], N] -> Option[Input[B, N]]
    def sequence[G[_], B](implicit ev: A === G[B],
                          G: Applicative[G]): G[Sized[List[B], N]] =
    ...
    def unzip3[X, Y, Z](implicit ev: A === (X, (Y, Z)))
      ...
      Unzip[List].unzip3[X, Y, Z](ev.subst(self.unsized)) match {
        case (x, y, z) =>
          (Sized.wrap[List[X], N](x),
           Sized.wrap[List[Y], N](y),
           Sized.wrap[List[Z], N](z))
      }
  }
}
