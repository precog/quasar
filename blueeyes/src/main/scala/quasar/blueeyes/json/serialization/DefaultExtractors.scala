...
trait DefaultExtractors {
  ...
  implicit def OptionExtractor[T](
      implicit extractor: Extractor[T]): Extractor[Option[T]] = new Extractor[Option[T]] {
    def validated(jvalue: JValue) = jvalue match {
  ...
  implicit def Tuple2Extractor[A, B](implicit ea: Extractor[A],
                                     eb: Extractor[B]): Extractor[(A, B)] =
    new Extractor[(A, B)] {
      def validated(jvalue: JValue) = jvalue match {
  ...
  implicit def Tuple3Extractor[A, B, C](implicit ea: Extractor[A],
                                        eb: Extractor[B],
                                        ec: Extractor[C]): Extractor[(A, B, C)] =
    new Extractor[(A, B, C)] {
      def validated(jvalue: JValue) = jvalue match {
        case JArray(values) if (values.length == 3) =>
          ^^[VE, A, B, C, (A, B, C)](ea.validated(values(0)),
  ...
  implicit def Tuple4Extractor[A, B, C, D](implicit ea: Extractor[A],
                                           eb: Extractor[B],
                                           ec: Extractor[C],
                                           ed: Extractor[D]): Extractor[(A, B, C, D)] =
    new Extractor[(A, B, C, D)] {
      def validated(jvalue: JValue) = jvalue match {
        case JArray(values) if (values.length == 4) =>
          ^^^[VE, A, B, C, D, (A, B, C, D)](ea.validated(values(0)),
                                            eb.validated(values(1)),
                                            ec.validated(values(2)),
  ...
  implicit def Tuple5Extractor[A, B, C, D, E](
      implicit ea: Extractor[A],
      eb: Extractor[B],
      ec: Extractor[C],
      ed: Extractor[D],
    ...
    def validated(jvalue: JValue) = jvalue match {
      case JArray(values) if (values.length == 5) =>
        ^^^^[VE, A, B, C, D, E, (A, B, C, D, E)](ea.validated(values(0)),
                                                 eb.validated(values(1)),
                                                 ec.validated(values(2)),
                                                 ed.validated(values(3)),
  ...
  implicit def ArrayExtractor[T](implicit m: CTag[T],
                                 elementExtractor: Extractor[T]): Extractor[Array[T]] =
    new Extractor[Array[T]] {
      def validated(jvalue: JValue) = jvalue match {
        case JArray(values) =>
          values.map(elementExtractor.validated _).sequence[VE, T].map(_.toArray)
        case _ => invalidv("Expected JArray but found: " + jvalue)
      }
    }

  implicit def SetExtractor[T](
      implicit elementExtractor: Extractor[T]): Extractor[Set[T]] =
    new Extractor[Set[T]] {
      def validated(jvalue: JValue) = jvalue match {
        case JArray(values) =>
          values.map(elementExtractor.validated _).sequence[VE, T].map(_.toSet)
        case _ => invalidv("Expected JArray but found: " + jvalue)
      }
    }

  implicit def ListExtractor[T](
      implicit elementExtractor: Extractor[T]): Extractor[List[T]] =
  ...
  implicit def VectorExtractor[T](
      implicit elementExtractor: Extractor[T]): Extractor[Vector[T]] =
    new Extractor[Vector[T]] {
      def validated(jvalue: JValue) = jvalue match {
        case JArray(values) =>
  ...
  implicit def MapExtractor[K, V](implicit keyExtractor: Extractor[K],
                                  valueExtractor: Extractor[V]): Extractor[Map[K, V]] =
    new Extractor[Map[K, V]] {
      def validated(jvalue: JValue) = {
        ListExtractor(Tuple2Extractor(keyExtractor, valueExtractor))
          .validated(jvalue)
          .map(_.toMap)
      }
    }

  implicit def StringMapExtractor[V](
      implicit valueExtractor: Extractor[V]): Extractor[Map[String, V]] =
    new Extractor[Map[String, V]] {
      ...
      def validated(jvalue: JValue) = jvalue match {
        ...
        case other =>
          ListExtractor(Tuple2Extractor(StringExtractor, valueExtractor))
            .validated(other)
            .map(_.toMap)
      }
    }
