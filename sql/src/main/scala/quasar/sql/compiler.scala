...
final case class TableContext[T](root: Option[T], full: () => T, subtables: Map[String, T])(
...
final case class BindingContext[T](subbindings: Map[String, T]) {
...
final case class CompilerState[T](fields: List[String], context: Context[T], nameGen: Int)
...
private object CompilerState {
   ...
   */
  def contextual[M[_], T, A](bc: BindingContext[T], tc: TableContext[T])(compM: M[A])(
      implicit m: MonadState[M, CompilerState[T]]): M[A] = {
  ...
  def addFields[M[_], T, A](add: List[String])(f: M[A])(
      implicit m: MonadState[M, CompilerState[T]]): M[A] =
    for {
      curr <- fields
  ...
  def fields[M[_], T](implicit m: MonadState[M, CompilerState[T]]): M[List[String]] =
    m.get ∘ (_.fields)

  def rootTable[M[_], T](implicit m: MonadState[M, CompilerState[T]]): M[Option[T]] =
    m.get ∘ (_.context.tableContext.headOption.flatMap(_.root))

  def rootTableReq[M[_], T](
      implicit
      MErr: MonadError_[M, SemanticError],
      MState: MonadState[M, CompilerState[T]]): M[T] =
    rootTable >>=
  ...
  def subtable[M[_], T](name: String)(
      implicit m: MonadState[M, CompilerState[T]]): M[Option[T]] =
    m.get ∘ { state =>
  ...
  def subtableReq[M[_], T](name: String)(
      implicit
      MErr: MonadError_[M, SemanticError],
      MState: MonadState[M, CompilerState[T]]): M[T] =
    subtable(name) >>=
      (_.fold(MErr.raiseError[T](CompiledSubtableMissing(name)))(_.point[M]))

  def fullTable[M[_], T](implicit m: MonadState[M, CompilerState[T]]): M[Option[T]] =
    m.get ∘ (_.context.tableContext.headOption.map(_.full()))

  /** Generates a fresh name for use as an identifier, e.g. tmp321. */
  def freshName[M[_], T](prefix: String)(
      implicit m: MonadState[M, CompilerState[T]]): M[scala.Symbol] =
...
final class Compiler[M[_], T: Equal](
  ...
  private def fail[A](error: SemanticError)(implicit m: MonadError_[M, SemanticError]): M[A] =
    m.raiseError(error)
  ...
  private def compile0(node: CoExpr)(
      implicit
      MErr: MonadError_[M, SemanticError],
      MState: MonadState[M, CompilerState[T]]): M[T] = {
    ...
    def compileDistinct(t: T) =
      CompilerState.freshName("distinct") ∘ (name =>
        lpr.let(
          name,
          t,
          agg.Arbitrary(set.GroupBy(lpr.free(name), lpr.free(name)).embed).embed))

    def compileCases(cases: List[Case[CoExpr]], default: T)(f: Case[CoExpr] => M[(T, T)]) =
      cases
        .traverse(f)
        .map(_.foldRight(default) {
          case ((cond, expr), default) =>
            relations.Cond(cond, expr, default).embed
        })

    @SuppressWarnings(Array("org.wartremover.warts.Recursion"))
    def flattenJoins(term: T, relations: SqlRelation[CoExpr]): T = relations match {
    ...
    def buildJoinDirectionMap(relations: SqlRelation[CoExpr]): Map[String, List[JoinDir]] = {
      @SuppressWarnings(Array("org.wartremover.warts.Recursion"))
      def loop(rel: SqlRelation[CoExpr], acc: List[JoinDir]): Map[String, List[JoinDir]] =
        rel match {
    ...
    def compileTableRefs(joined: T, relations: SqlRelation[CoExpr]): Map[String, T] =
      buildJoinDirectionMap(relations).map {
        case (name, dirs) =>
          name -> dirs.foldRight(joined)((dir, acc) => dir.projectFrom(acc))
      }

    def tableContext(joined: T, relations: SqlRelation[CoExpr]): TableContext[T] =
      TableContext(
        Some(joined),
        () => flattenJoins(joined, relations),
        compileTableRefs(joined, relations))

    def step(relations: SqlRelation[CoExpr]): (Option[M[T]] => M[T] => M[T]) = {
      (current: Option[M[T]]) => (next: M[T]) =>
        current.map { current =>
          for {
            ...
            bc = relations match {
              ...
              case TableRelationAST(_, Some(name)) =>
                BindingContext(Map(name -> lpr.free(stepName)))
              case id @ IdentRelationAST(_, _) =>
                BindingContext(Map(id.aliasName -> lpr.free(stepName)))
              case r => BindingContext[T](Map())
            }
            next2 <- CompilerState.contextual(bc, tableContext(lpr.free(stepName), relations))(
              next)
    ...
    def relationName(node: CoExpr): SemanticError \/ String = {
      ...
      val relations =
        if (namedRel.size <= 1) namedRel
          ...
          val filtered = namedRel.filter(x =>
            x._1 ≟ pprint(forgetAnnotation[CoExpr, Fix[Sql], Sql, SA.Annotations](node)))
          if (filtered.isEmpty) namedRel else filtered
        }
      relations.toList match {
        case Nil =>
          -\/(NoTableDefined(forgetAnnotation[CoExpr, Fix[Sql], Sql, SA.Annotations](node)))
        case List((name, _)) => \/-(name)
        case x =>
          -\/(
            AmbiguousReference(
              forgetAnnotation[CoExpr, Fix[Sql], Sql, SA.Annotations](node),
              x.map(_._2).join))
      }
    }

    def compileFunction[N <: Nat](func: GenericFunc[N], args: Func.Input[CoExpr, N]): M[T] =
      args.traverse(compile0).map(func.applyGeneric(_).embed)

    def buildRecord(names: List[Option[String]], values: List[T]): T = {
      ...
      fields
        .reduceOption(structural.ObjectConcat(_, _).embed)
    ...
    def compileRelation(r: SqlRelation[CoExpr]): M[T] =
      r match {
        ...
        case TableRelationAST(path, _) =>
          sandboxCurrent(canonicalize(path))
            .cata(p => emit(lpr.read(p)), fail(InvalidPathError(path, None)))

        case ExprRelationAST(expr, _) => compile0(expr)

        case JoinRelation(left, right, tpe, clause) =>
          (CompilerState.freshName("left") ⊛ CompilerState.freshName("right"))(
            (leftName, rightName) => {
    ...
    def temporalPartFunc[A](
      ...
      args.traverse(compile0).flatMap {
        case Embed(Constant(Data.Str(part))) :: expr :: Nil =>
          f1(part).cata(f2(_, expr), fail(UnexpectedDatePart("\"" + part + "\"")))
        case _ :: _ :: Nil =>
          fail(
            UnexpectedDatePart(
              pprint(forgetAnnotation[CoExpr, Fix[Sql], Sql, SA.Annotations](args(0)))))
        case _ =>
    ...
    node.tail match {
      case s @ Select(isDistinct, projections, relations, filter, groupBy, orderBy) =>
        ...
        val namesOrError: SemanticError \/ List[Option[String]] =
          projectionNames[Fix[Sql]](
            projections.map(_.map(forgetAnnotation[CoExpr, Fix[Sql], Sql, SA.Annotations])),
            relationName(node).toOption).map(_.map {
            case (name, Embed(expr)) =>
              expr match {
                case Splice(_) => None
                case _ => name.some
              }
        ...
        namesOrError.fold(
          ...
          names => {
            ...
            val (nam, initial) =
              projections match {
                case List(Proj(Cofree(_, Splice(_)), None)) =>
                  (
                    names.some,
                    projections.map(_.expr).traverse(compile0).map(buildRecord(names, _)))
            ...
            relations.foldRight(initial)((relations, select) => {
              val stepBuilder = step(relations)
              stepBuilder(compileRelation(relations).some) {
                val filtered = filter.map(filter =>
                  (CompilerState.rootTableReq[M, T] ⊛ compile0(filter))(set.Filter(_, _).embed))

                stepBuilder(filtered) {
                  val grouped = groupBy.map(
                    groupBy =>
                      ...
                      stepBuilder(squashed.some) {
                        def sort(ord: OrderBy[CoExpr], nu: T) =
                          CompilerState.rootTableReq[M, T] >>= (preSorted =>
                            nam.fold(ord.keys.map(p => (nu, p._1)).point[M])(na =>
                              CompilerState.addFields(na.unite)(ord.keys.traverse {
                                case (ot, key) =>
                                  compile0(key)
                                    .map(_.transApoT(substitute(preSorted, nu))) strengthR ot
                              })) ∘
                              (ks =>
                                lpr.sort(nu, ks.map(_.map {
                                  case ASC => SortDir.Ascending
                                  case DESC => SortDir.Descending
                                }))))

                        val sorted = orderBy ∘ (ord =>
                          CompilerState.rootTableReq[M, T] >>= (sort(ord, _)))
                        ...
                        stepBuilder(sorted) {
                          def distinct(name: Symbol, aggregation: UnaryFunc, t: T) = {
                            ...
                            lpr.let(
                              name,
                              t,
                              (syntheticNames.nonEmpty)
                                .fold(
                                  aggregation(
                                    set
                                      .GroupBy(
                                        fvar,
                                        syntheticNames.foldLeft(fvar)((acc, field) =>
                                          structural
                                            .DeleteField(acc, lpr.constant(Data.Str(field)))
                                            .embed))
                                      .embed),
                                  agg.Arbitrary(set.GroupBy(fvar, fvar).embed)
                                )
                                .embed
                            )
                          }
                          ...
                          val distincted = isDistinct match {
                            case SelectDistinct =>
                              ((CompilerState.rootTableReq[M, T] ⊛ CompilerState.freshName(
                                "distinct"))((t, name) =>
                                orderBy.fold(distinct(name, agg.Arbitrary, t).point[M])(
                          ...
                          stepBuilder(distincted) {
                            val pruned =
                              CompilerState
                                .rootTableReq[M, T]
                                .map(syntheticNames.foldLeft(_)((acc, field) =>
                                  structural
                                    .DeleteField(acc, lpr.constant(Data.Str(field)))
                                    .embed))
      ...
      case SetLiteral(values0) =>
        values0
          .traverse(compile0)
          .map(vs => structural.ShiftArray(structural.MakeArrayN(vs: _*).embed).embed)
      ...
      case Splice(expr) =>
        expr.fold(
          CompilerState.fullTable.flatMap(_.map(emit _).getOrElse(fail(GenericError(
            "Not within a table context so could not find table expression for wildcard")))))(
          compile0)
      ...
      case Binop(left, right, op) =>
        ((op match {
          ...
          case f @ Union =>
            fail(GenericError(
              "Should not have encountered a union at this point in compilation")).right
          case f @ Intersect =>
            fail(
              GenericError(
                "Should not have encountered an intersect at this point in compilation")).right
      ...
      case Unop(expr, op) =>
        ((op match {
          ...
          case f @ Exists =>
            fail(
              GenericError(
                "Should not have encountered an exists at this point in compilation")).right
        ...
        }): GenericFunc[nat._1] \/ M[T]).valueOr(compileFunction[nat._1](_, Func.Input1(expr)))

      case Ident(name) =>
        CompilerState.fields.flatMap(
          fields =>
      ...
      case InvokeFunction(name, args)
          if name ≟ CIName("date_part") || name ≟ CIName("temporal_part") =>
        temporalPartFunc(
          name,
          args,
          extractFunc,
          (f: UnaryFunc, expr: T) => emit(f(expr).embed))

      case InvokeFunction(name, args)
          if name ≟ CIName("date_trunc") || name ≟ CIName("temporal_trunc") =>
        temporalPartFunc(
          name,
          args,
          temporalPart,
          (p: TemporalPart, expr: T) => emit(TemporalTrunc(p, expr).embed))
      ...
      case InvokeFunction(name, args) if name ≟ CIName("coalesce") =>
        args match {
          case List(a1, a2) =>
            (CompilerState.freshName("c") ⊛ compile0(a1) ⊛ compile0(a2))(
              (name, c1, c2) =>
                lpr.let(
                  name,
                  c1,
      ...
      case InvokeFunction(name, args) =>
        val function: Option[HomomorphicFunction[T, T]] =
          functionMapping.mapValues(_.toFunction[T].andThen(_.embed)).lift.apply(name)
        function.cata[M[T]](
          func =>
            args
              .traverse(compile0)
      ...
      case Switch(cases, default0) =>
        default0
          .fold(emit(lpr.constant(Data.Null)))(compile0)
          .flatMap(compileCases(cases, _) {
...
object Compiler {
  def apply[M[_], T: Equal](implicit TR: Recursive.Aux[T, LP], TC: Corecursive.Aux[T, LP]) =
    new Compiler[M, T]

  def trampoline[T: Equal](implicit TR: Recursive.Aux[T, LP], TC: Corecursive.Aux[T, LP]) =
    apply[StateT[EitherT[scalaz.Free.Trampoline, SemanticError, ?], CompilerState[T], ?], T]

  def compile[T: Equal](tree: Cofree[Sql, SA.Annotations])(
      implicit TR: Recursive.Aux[T, LP],
      TC: Corecursive.Aux[T, LP]): SemanticError \/ T =
  ...
  def reduceGroupKeys[T: Equal](
      tree: T)(implicit TR: Recursive.Aux[T, LP], TC: Corecursive.Aux[T, LP]): T = {
    ...
    def keysƒ(t: LP[(T, List[T])]): (T, List[T]) = {
      @SuppressWarnings(Array("org.wartremover.warts.Recursion"))
    ...
    def rewriteƒ: Coalgebra[LP, Cofree[LP, Boolean]] = {
      ...
      t =>
        t.tail match {
          case InvokeUnapply(func @ UnaryFunc(_, _, _, _, _, _, _), Sized(arg))
