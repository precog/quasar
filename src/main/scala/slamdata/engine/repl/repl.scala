...
object Repl {
  ...
  object Command {
    ...
    case class Save(dir: String, value: String) extends Command
  ...
  private def parseCommand(input: String): Command = {
    ...
    input match {
      ...
      case SavePattern(path, value) => Save(path, value)
      case _                    => Unknown
    }
  }
  ...
  def save(state: RunState, path: String, value: String): Process[Task, Unit] = Process.eval({
    state.mounted.lookup(state.path).map { case (backend, relPath) =>
      backend.dataSource.write(relPath, RenderedJson(value) :: Nil)
