...
import quasar.{Data, SemanticError, TermLogicalPlanMatchers}
...
trait CompilerHelpers extends TermLogicalPlanMatchers {
  ...
  val parseAndAnnotate
    : Fix[Sql] => NonEmptyList[SemanticError] \/ Cofree[Sql, SemanticAnalysis.Annotations] =
    query => {
  ...
  val parseAndAnnotateUnsafe: Fix[Sql] => Cofree[Sql, SemanticAnalysis.Annotations] = query => {
    parseAndAnnotate(query).valueOr(
      err =>
  ...
  def renameJoinSides(result: Fix[LP])(
      oldLeft: Symbol,
      newLeft: Symbol,
      oldRight: Symbol,
  ...
  def read(file: String): Fix[LP] =
    lpf.read(
      sandboxCurrent(posixCodec.parsePath(Some(_), Some(_), κ(None), κ(None))(file).get).get)
