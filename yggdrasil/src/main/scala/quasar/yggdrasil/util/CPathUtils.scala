...
object CPathUtils {
    ...
    (cpath.nodes, value) match {
      ...
      case (CPathArray :: tail, es: CArray[_]) =>
        ...
        es.value.toList.zipWithIndex flatMap {
          case (e, i) => addComponent(JPathIndex(i), cPathToJPaths(CPath(tail), elemType(e)))
        }
  ...
  private def addComponent(c: JPathNode, xs: List[(JPath, CValue)]): List[(JPath, CValue)] =
    xs map {
  ...
  def union(cPath1: CPath, cPath2: CPath): Option[CPath] = {
    def loop(ps1: List[CPathNode], ps2: List[CPathNode], acc: List[CPathNode]): Option[CPath] =
      (ps1, ps2) match {
        case (Nil, Nil) =>
