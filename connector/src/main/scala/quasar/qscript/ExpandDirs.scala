...
private[qscript] final class ExpandDirsPath[T[_[_]]: BirecursiveT, O[_]: Functor](
  ...
  def readDir(implicit R: Const[Read[AFile], ?] :<: O)
    : ExpandDirs.Aux[T, Const[Read[ADir], ?], O] =
  ...
  def shiftedReadDir(implicit SR: Const[ShiftedRead[AFile], ?] :<: O)
    ...
    new ExpandDirs[Const[ShiftedRead[ADir], ?]] {
      ...
      def expandDirs[M[_]: Monad: MonadFsErr, F[_]: Functor](
          ...
          g: DiscoverPath.ListContents[M]) =
        r =>
          unionAll(OutToF, g, wrapRead[OUT](_, r.getConst.idStatus)) apply r.getConst.path
    }
}
...
private[qscript] final class ExpandDirsBranch[T[_[_]]: BirecursiveT] extends TTypes[T] {
  ...
  def applyToBranch[M[_]: Monad: MonadFsErr](listContents: DiscoverPath.ListContents[M],
                                             branch: FreeQS): M[FreeQS] =
    branch.transCataM[M, T[CoEnvQS], CoEnvQS](
      liftCoM[T, M, QScriptTotal, Hole, T[CoEnvQS]](
        ExpandDirsTotal.expandDirs(coenvPrism[QScriptTotal, Hole].reverseGet,
                                   listContents))
    ) ∘ (_.convertTo[FreeQS])

  def qscriptCore[O[_]](
      implicit QC: QScriptCore :<: O): ExpandDirs.Aux[T, QScriptCore, O] =
