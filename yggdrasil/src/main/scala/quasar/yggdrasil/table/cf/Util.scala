...
object util {
  ...
  def Sparsen(idx: Array[Int], toSize: Int) = CF1P("builtin::ct::sparsen") {
    case c: BoolColumn =>
      new SparsenColumn(c, idx, toSize) with BoolColumn { def apply(row: Int) = c(remap(row)) }
    case c: LongColumn =>
      new SparsenColumn(c, idx, toSize) with LongColumn { def apply(row: Int) = c(remap(row)) }
    case c: DoubleColumn =>
      new SparsenColumn(c, idx, toSize) with DoubleColumn {
        def apply(row: Int) = c(remap(row))
      }
    case c: NumColumn =>
      new SparsenColumn(c, idx, toSize) with NumColumn { def apply(row: Int) = c(remap(row)) }
    case c: StrColumn =>
      new SparsenColumn(c, idx, toSize) with StrColumn { def apply(row: Int) = c(remap(row)) }
    case c: DateColumn =>
      new SparsenColumn(c, idx, toSize) with DateColumn { def apply(row: Int) = c(remap(row)) }
    case c: PeriodColumn =>
  ...
  def Remap(f: Int => Int) = CF1P("builtin::ct::remap") {
    ...
    case c: DoubleColumn =>
      new RemapColumn(c, f) with DoubleColumn { def apply(row: Int) = c(f(row)) }
    case c: NumColumn =>
    ...
    case c: PeriodColumn =>
      new RemapColumn(c, f) with PeriodColumn { def apply(row: Int) = c(f(row)) }
    case c: HomogeneousArrayColumn[a] =>
  ...
  def filter(from: Int, to: Int, definedAt: BitSet) = CF1P("builtin::ct::filter") {
    ...
    case c: EmptyArrayColumn =>
      new BitsetColumn(definedAt & c.definedAt(from, to)) with EmptyArrayColumn
    case c: EmptyObjectColumn =>
      new BitsetColumn(definedAt & c.definedAt(from, to)) with EmptyObjectColumn
  ...
  def filterBy(p: Int => Boolean) = CF1P("builtin::ct::filterBy") {
    case c: BoolColumn =>
      new BoolColumn {
        def apply(row: Int) = c(row); def isDefinedAt(row: Int) = c.isDefinedAt(row) && p(row)
      }
    case c: LongColumn =>
      new LongColumn {
        def apply(row: Int) = c(row); def isDefinedAt(row: Int) = c.isDefinedAt(row) && p(row)
    ...
    case c: EmptyArrayColumn =>
      new EmptyArrayColumn { def isDefinedAt(row: Int) = c.isDefinedAt(row) && p(row) }
    case c: EmptyObjectColumn =>
      new EmptyObjectColumn { def isDefinedAt(row: Int) = c.isDefinedAt(row) && p(row) }
    case c: NullColumn =>
      new NullColumn { def isDefinedAt(row: Int) = c.isDefinedAt(row) && p(row) }
  }
  ...
  def MaskedUnion(leftMask: BitSet) = CF2P("builtin::ct::maskedUnion") {
    ...
    case (left: HomogeneousArrayColumn[a], right: HomogeneousArrayColumn[b])
      ...
      new UnionColumn(left, right) with HomogeneousArrayColumn[a] {
        val tpe = left.tpe
        def apply(row: Int) =
          if (leftMask.get(row)) left(row) else right(row).asInstanceOf[Array[a]]
      }
