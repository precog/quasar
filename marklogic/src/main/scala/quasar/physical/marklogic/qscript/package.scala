...
import quasar.qscript.MapFuncsCore.{Constant, Eq, Neq, TypeOf}
...
package object qscript {
  ...
  def rebaseXQuery[T[_[_]]: BirecursiveT, F[_]: Monad, FMT, Q](
      ...
      QTP: Planner[F, FMT, QScriptTotal[T, ?], T[EJson]]): F[Search[Q] \/ XQuery] =
  ...
  def rewriteNullCheck[T[_[_]]: BirecursiveT, U, E](
    ...
    fa =>
      CoEnv(fa.run.map(totally {
        case MFC(Eq(lhs, Embed(NullLit()))) =>
          MFC(Eq(UC.embed(CoEnv(\/-(MFC(TypeOf(lhs))))), nullString))
        case MFC(Eq(Embed(NullLit()), rhs)) =>
          MFC(Eq(UC.embed(CoEnv(MFC[T, U](TypeOf(rhs)).right)), nullString))
        case MFC(Neq(lhs, Embed(NullLit()))) =>
          MFC(Neq(UC.embed(CoEnv(MFC[T, U](TypeOf(lhs)).right)), nullString))
        case MFC(Neq(Embed(NullLit()), rhs)) =>
          MFC(Neq(UC.embed(CoEnv(MFC[T, U](TypeOf(rhs)).right)), nullString))
      }))
  }
  ...
  private def invalidQName[F[_]: MonadPlanErr, A](s: String): F[A] =
    MonadError_[F, MarkLogicPlannerError].raiseError(MarkLogicPlannerError.invalidQName(s))
}
