...
object Module {
  ...
  final case class InvokeModuleFunction(
      path: AFile,
  ...
  final case class More(handle: ResultHandle) extends Module[FileSystemError \/ Vector[Data]]
  ...
  final class Ops[S[_]](implicit val unsafe: Unsafe[S]) {
    ...
    def invokeFunction(
      ...
      Process.bracket(unsafe.invokeFunction(path, args, offset, limit))(closeHandle) {
        dataOrHandle =>
          dataOrHandle.fold(data => Process.emitAll(data), handle => readUntilEmpty(handle))
      }
    }
    ...
    def invokeFunction_(
        ...
        limit: Option[Positive])(
        implicit
        SO: Failure :<: S): Process[Free[S, ?], Data] = {
      val nat: M ~> Free[S, ?] = λ[M ~> Free[S, ?]] { x =>
        Failure.Ops[Error, S].unattempt(x.run)
      }
  ...
  object impl {
    ...
    def default[S[_]](
        implicit query: QueryFile.Unsafe[S],
      ...
      λ[Module ~> Free[S, ?]] {
        case InvokeModuleFunction(file, args, offset, limit) =>
          ...
          (for {
            moduleConfig <- EitherT(
              mount
                .lookupModuleConfig(currentDir)
            ...
            userArgs <- EitherT(
              maybeAllArgs.toRightDisjunction(argumentsMissing(missingArgs)).point[Free[S, ?]])
            parsedArgs <- EitherT(
              userArgs
