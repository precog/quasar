...
sealed abstract class ToApiErrorInstances extends ToApiErrorInstances0 {
  ...
  implicit def fileSystemErrorResponse: ToApiError[FileSystemError] = {
    ...
    error {
      ...
      case UnknownResultHandle(ResultHandle(id)) =>
        apiError(InternalServerError withReason "Unknown result handle.", "handleId" := id)
  ...
  implicit def physicalErrorToApiError: ToApiError[PhysicalError] =
    error(
      err => fromMsg_(InternalServerError withReason "Physical filesystem error.", err.shows))
    ...
    error(err =>
      err match {
        ...
        case ObjectIdFormatError(oid) =>
          fromMsg(BadRequest withReason "Invalid ObjectId.", err.message, "objectId" := oid)
    ...
    error(err =>
      err match {
        ...
        case NoTableDefined(node) =>
          fromMsg(BadRequest withReason "No table defined.", err.message, "sql" := node.render)
        case MissingField(name) =>
          fromMsg(BadRequest withReason "Missing field.", err.message, "fieldName" := name)
        case DuplicateAlias(name) =>
          fromMsg(BadRequest withReason "Duplicate alias name.", err.message, "name" := name)
        case MissingIndex(i) =>
