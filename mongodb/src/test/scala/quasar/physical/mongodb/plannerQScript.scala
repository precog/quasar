...
class PlannerQScriptSpec
  ...
  def emit[A: RenderTree](label: String, v: A): EitherWriter[A] =
    EitherT[Writer[PhaseResults, ?], FileSystemError, A](
      Writer(Vector(PhaseResult.tree(label, v)), v.right))
  ...
  val listContents: DiscoverPath.ListContents[EitherWriter] =
    dir =>
      (if (dir ≟ rootDir)
         Set(DirName("db").left[FileName], DirName("db1").left, DirName("db2").left)
       else
         Set(
           ...
           FileName("logs").right,
           FileName("usa_factbook").right,
           FileName("cities").right
         )).point[EitherWriter]

  implicit val monadTell
    : MonadTell[EitherT[PhaseResultT[Id, ?], FileSystemError, ?], PhaseResults] =
    EitherT
      .monadListen[WriterT[Id, Vector[PhaseResult], ?], PhaseResults, FileSystemError](
  ...
  def queryPlanner(expr: Fix[Sql],
    ...
    queryPlan(expr, Variables.empty, basePath, 0L, None)
      ...
      .flatMap(_.fold(
        _ =>
          scala.sys.error("query evaluated to a constant, this won’t get to the backend"),
        MongoDbQScriptPlanner.plan(_,
                                   fs.QueryContext(model, stats, indexes, listContents))
      ))

  def plan0(query: String,
            model: MongoQueryModel,
            stats: Collection => Option[CollectionStatistics],
            indexes: Collection => Option[Set[Index]])
    ...
    fixParser
      .parseExpr(Query(query))
      .fold(e => scala.sys.error("parsing error: " + e.message),
            queryPlanner(_, model, stats, indexes).run)
      .value
      .toEither
  }
  ...
  def indexes(ps: (Collection, BsonField)*): Collection => Option[Set[Index]] = {
    val map: Map[Collection, Set[Index]] = ps.toList.foldMap {
      case (c, f) =>
        Map(
          c -> Set(Index(f.asText + "_", NonEmptyList(f -> IndexType.Ascending), false)))
    }
    c =>
      map.get(c).orElse(defaultIndexes(c))
  }
  ...
  def plan(logical: Fix[LP]): Either[FileSystemError, Crystallized[WorkflowF]] = {
    (for {
      ...
      phys <- MongoDbQScriptPlanner.plan[Fix, EitherWriter](
        simplified,
        fs.QueryContext(MongoQueryModel.`3.2`,
                        defaultStats,
                        defaultIndexes,
                        listContents))
  ...
  def planLog(query: String): ParsingError \/ Vector[PhaseResult] =
    for {
      ...
      queryPlanner(expr, MongoQueryModel.`3.2`, defaultStats, defaultIndexes).run.written
  ...
  def isNumeric(field: BsonField): Selector =
    Selector.Or(
      ...
      Selector.Or(Selector.Doc(field -> Selector.Type(BsonType.Date)),
                  Selector.Doc(field -> Selector.Type(BsonType.Bool)))
    )

  def divide(a1: Fix[ExprOp], a2: Fix[ExprOp]) =
    $cond(
      $eq(a2, $literal(Bson.Int32(0))),
      $cond(
        $eq(a1, $literal(Bson.Int32(0))),
  ...
  "plan from query string" should {
    "plan simple select *" in {
      plan("select * from foo") must beWorkflow($read[WorkflowF](collection("db", "foo")))
    }
    ...
    "plan count(*)" in {
      plan("select count(*) from foo") must beWorkflow(
        chain[Workflow](
          ...
          $group(grouped("f0" -> $sum($literal(Bson.Int32(1)))),
                 \/-($literal(Bson.Null))),
          $project(reshape("value" -> $field("f0")), ExcludeId)
        ))
    }
    ...
    "plan simple field projection on single set" in {
      plan("select foo.bar from foo") must
        beWorkflow(
          chain[Workflow]($read(collection("db", "foo")),
                          $project(reshape("value" -> $field("bar")), ExcludeId)))
    }
    ...
    "plan multiple field projection on single set when table name is inferred" in {
      plan("select bar, baz from foo") must
        beWorkflow(
          chain[Workflow](
            $read(collection("db", "foo")),
            $project(reshape("bar" -> $field("bar"), "baz" -> $field("baz")), IgnoreId)))
    }
          ...
          chain[Workflow](
            ...
            $project(
              reshape(
                "value" ->
                  $cond(
                    $and($lt($literal(Bson.Null), $field("bar")),
                         $lt($field("bar"), $literal(Bson.Text("")))),
                    $cond(
                      $or(
                        $and($lt($literal(Bson.Null), $field("foo")),
                             $lt($field("foo"), $literal(Bson.Text("")))),
                        $and($lte($literal(Check.minDate), $field("foo")),
          ...
          chain[Workflow](
            ...
            $project(
              reshape(
                "value" ->
                  $cond(
                    $and($lte($literal(Bson.Text("")), $field("baz")),
                         $lt($field("baz"), $literal(Bson.Doc()))),
                    $cond(
                      $and($lte($literal(Bson.Text("")), $field("bar")),
            ...
            $project(
              reshape(
                "value" ->
                  $cond(
                    $or(
                      $and($lte($literal(Bson.Arr()), $field("state")),
                           $lt($field("state"),
                               $literal(Bson.Binary.fromArray(scala.Array[Byte]())))),
                      $and($lte($literal(Bson.Text("")), $field("state")),
                           $lt($field("state"), $literal(Bson.Doc())))
                    ),
                    $cond(
                      $or(
                        $and($lte($literal(Bson.Arr()), $field("city")),
                             $lt($field("city"),
                                 $literal(Bson.Binary.fromArray(scala.Array[Byte]())))),
                        $and($lte($literal(Bson.Text("")), $field("city")),
            ...
            $project(
              reshape(
                "0" ->
                  $cond(
                    $or(
                      $and($lte($literal(Bson.Arr()), $field("b")),
                           $lt($field("b"),
                               $literal(Bson.Binary.fromArray(scala.Array[Byte]())))),
                      $and($lte($literal(Bson.Text("")), $field("b")),
                           $lt($field("b"), $literal(Bson.Doc())))
                    ),
                    $cond(
                      $or(
                        $and($lte($literal(Bson.Arr()), $field("a")),
                             $lt($field("a"),
                                 $literal(Bson.Binary.fromArray(scala.Array[Byte]())))),
                        $and($lte($literal(Bson.Text("")), $field("a")),
        ...
        beWorkflow(
          chain[Workflow](
            ...
            $project(
              reshape(
                "value" ->
                  $cond($and($lte($literal(Bson.Text("")), $field("bar")),
                             $lt($field("bar"), $literal(Bson.Doc()))),
                        $toLower($field("bar")),
                        $literal(Bson.Undefined))),
              ExcludeId
      ...
      plan("select coalesce(bar, baz) from foo") must
        beWorkflow(
          chain[Workflow]($read(collection("db", "foo")),
                          $project(reshape(
                                     "value" ->
                                       $cond($eq($field("bar"), $literal(Bson.Null)),
                                             $field("baz"),
                                             $field("bar"))),
                                   ExcludeId)))
    }
        ...
        beWorkflow(
          chain[Workflow](
            ...
            $project(
              reshape(
                "value" ->
                  $cond($and($lte($literal(Check.minDate), $field("baz")),
          ...
          chain[Workflow](
            ...
            $project(
              reshape(
                "value" ->
                  $cond(
                    $and($lte($literal(Check.minDate), $field("baz")),
                         $lt($field("baz"), $literal(Bson.Regex("", "")))),
                    $trunc(
                      $add($divide($subtract($month($field("baz")),
          ...
          chain[Workflow](
            ...
            $project(
              reshape(
                "__tmp2" ->
                  $cond(
                    $and($lte($literal(Check.minDate), $field("ts")),
            ...
            $match(
              Selector.Doc(BsonField.Name("__tmp2") -> Selector.Eq(Bson.Bool(true)))),
            $project(reshape("value"                -> $field("__tmp3")), ExcludeId)
          ))
    ...
    "plan filter array element" in {
      plan("select loc from zips where loc[0] < -73") must
        beWorkflow(
          chain[Workflow](
            $read(collection("db", "zips")),
            $match(Selector.Where(If(
              BinOp(
                And,
                Call(Select(ident("Array"), "isArray"),
                     List(Select(ident("this"), "loc"))),
                BinOp(
                  Or,
                  BinOp(
                    Or,
                    BinOp(
                      jscore.Or,
                      Call(ident("isNumber"),
                           List(Access(Select(ident("this"), "loc"),
                                       Literal(Js.Num(0, false))))),
                      BinOp(
                        jscore.Or,
                        BinOp(Instance,
                              Access(Select(ident("this"), "loc"),
                                     Literal(Js.Num(0, false))),
                              ident("NumberInt")),
                        BinOp(Instance,
                              Access(Select(ident("this"), "loc"),
                  ...
                  BinOp(
                    Or,
                    BinOp(Instance,
                          Access(Select(ident("this"), "loc"), Literal(Js.Num(0, false))),
                          ident("Date")),
                    BinOp(Eq,
                          UnOp(TypeOf,
                               Access(Select(ident("this"), "loc"),
                                      Literal(Js.Num(0, false)))),
                          jscore.Literal(Js.Str("boolean")))
                  )
                )
              ),
              BinOp(Lt,
                    Access(Select(ident("this"), "loc"), Literal(Js.Num(0, false))),
                    Literal(Js.Num(-73, false))),
              ident("undefined")
            ).toJs)),
            $project(reshape("value" -> $field("loc")), ExcludeId)
          ))
    }.pendingUntilFixed(notOnPar)
      ...
      plan("select loc[0] from zips") must
        beWorkflow(
          chain[Workflow](
            ...
            $project(reshape("value" -> $field("__tmp0")), ExcludeId)
          ))
    }
          ...
          chain[Workflow](
            ...
            $group(
              grouped(
                "f0" ->
                  $sum(
                    $cond($and($lt($literal(Bson.Null), $field("pop")),
            ...
            $project(
              reshape(
                "value" ->
                  $cond(
                    $or(
                      $and($lt($literal(Bson.Null), $field("pop")),
                           $lt($field("pop"), $literal(Bson.Doc()))),
                      $and($lte($literal(Bson.Bool(false)), $field("pop")),
          ...
          chain[Workflow](
            ...
            $project(
              reshape(
                "value" ->
                  $cond(
                    $and($lt($literal(Bson.Null), $field("b")),
                         $lt($field("b"), $literal(Bson.Text("")))),
                    $cond(
                      $or(
                        $and($lt($literal(Bson.Null), $field("a")),
                             $lt($field("a"), $literal(Bson.Text("")))),
                        $and($lte($literal(Check.minDate), $field("a")),
      ...
      plan("select * from zips where length(city) < 4 and pop < 20000") must
        beWorkflow(
          chain[Workflow](
            ...
            $match(
              Selector.Doc(BsonField.Name("__tmp6") -> Selector.Eq(Bson.Bool(true)))),
            $project(reshape("value"                -> $field("__tmp7")), ExcludeId)
          ))
    }.pendingUntilFixed(notOnPar)
        ...
        beWorkflow(
          chain[Workflow](
            ...
            $match(Selector.And(
              isNumeric(BsonField.Name("bar")),
              Selector.And(
                Selector.Doc(BsonField.Name("bar") -> Selector.Gte(Bson.Int32(10))),
                Selector.Doc(BsonField.Name("bar") -> Selector.Lte(Bson.Int32(100))))
            ))
          ))
    }.pendingUntilFixed(notOnPar)
      ...
      plan("select * from foo where bar like \"A%\" or bar like \"Z%\"") must
        beWorkflow(
          chain[Workflow](
            $read(collection("db", "foo")),
            $match(Selector.Or(
              Selector.And(Selector.Doc(BsonField.Name("bar") ->
                             Selector.Type(BsonType.Text)),
                           Selector.Doc(BsonField.Name("bar") ->
                             Selector.Regex("^A.*$", false, true, false, false))),
              Selector.And(Selector.Doc(BsonField.Name("bar") ->
            ...
            $match(Selector.Where(If(
              ...
              BinOp(Neq,
                    ...
                    Call(Select(Select(ident("this"), "loc"), "indexOf"),
                         List(jscore.Literal(Js.Num(43.058514, true))))),
              ident("undefined")
            ).toJs))
          ))
    }.pendingUntilFixed(notOnPar)
    ...
    "plan filter with ~" in {
      plan("select * from zips where city ~ \"^B[AEIOU]+LD.*\"") must beWorkflow(
        chain[Workflow](
          $read(collection("db", "zips")),
          $match(Selector.And(
            Selector.Doc(BsonField.Name("city") -> Selector.Type(BsonType.Text)),
            Selector.Doc(BsonField.Name("city") -> Selector
              .Regex("^B[AEIOU]+LD.*", false, true, false, false))
          ))
        ))
    }.pendingUntilFixed(notOnPar)
    ...
    "plan filter with ~*" in {
      plan("select * from zips where city ~* \"^B[AEIOU]+LD.*\"") must beWorkflow(
        chain[Workflow](
          ...
          $match(Selector.And(
            Selector.Doc(BsonField.Name("city") -> Selector.Type(BsonType.Text)),
            Selector.Doc(BsonField
              .Name("city") -> Selector.Regex("^B[AEIOU]+LD.*", true, true, false, false))
          ))
        ))
    }.pendingUntilFixed(notOnPar)
      ...
      plan("select * from zips where city !~ \"^B[AEIOU]+LD.*\"") must beWorkflow(
        chain[Workflow](
          ...
          $match(Selector.And(
            Selector.Doc(BsonField.Name("city") -> Selector.Type(BsonType.Text)),
            Selector.Doc(
              ListMap[BsonField, Selector.SelectorExpr](BsonField.Name("city") -> Selector
                .NotExpr(Selector.Regex("^B[AEIOU]+LD.*", false, true, false, false))))
          ))
        ))
    }.pendingUntilFixed(notOnPar)
      ...
      plan("select * from zips where city !~* \"^B[AEIOU]+LD.*\"") must beWorkflow(
        chain[Workflow](
          ...
          $match(Selector.And(
            ...
            Selector.Doc(
              ListMap[BsonField, Selector.SelectorExpr](BsonField.Name("city") -> Selector
                .NotExpr(Selector.Regex("^B[AEIOU]+LD.*", true, true, false, false))))
          ))
        ))
    }.pendingUntilFixed(notOnPar)

    "plan filter with alternative ~" in {
      plan("select * from a where \"foo\" ~ pattern or target ~ pattern") must beWorkflow(
        chain[Workflow](
          ...
          $simpleMap(
            NonEmptyList(MapExpr(JsFn(
              Name("x"),
              obj(
                "__tmp8" -> Call(Select(New(Name("RegExp"),
                                            List(Select(ident("x"), "pattern"),
                                                 jscore.Literal(Js.Str("m")))),
                                        "test"),
                                 List(jscore.Literal(Js.Str("foo")))),
                "__tmp9"  -> ident("x"),
                "__tmp10" -> Select(ident("x"), "pattern"),
                "__tmp11" -> Select(ident("x"), "target"),
                "__tmp12" -> Call(Select(New(Name("RegExp"),
                                             List(Select(ident("x"), "pattern"),
                                                  jscore.Literal(Js.Str("m")))),
                                         "test"),
          ...
          $match(Selector.Or(
            Selector.And(
              Selector.Doc(BsonField.Name("__tmp9") \ BsonField.Name("pattern") ->
                Selector.Type(BsonType.Text)),
              Selector.Doc(BsonField.Name("__tmp8") -> Selector.Eq(Bson.Bool(true)))
            ),
            Selector.And(
              Selector.Doc(BsonField.Name("__tmp10") -> Selector.Type(BsonType.Text)),
              Selector.And(
                Selector.Doc(BsonField.Name("__tmp11") -> Selector.Type(BsonType.Text)),
                Selector.Doc(BsonField.Name("__tmp12") -> Selector.Eq(Bson.Bool(true))))
            )
          )),
          $project(reshape("value" -> $field("__tmp9")), ExcludeId)
        ))
    }.pendingUntilFixed(notOnPar)
        ...
        beWorkflow(
          chain[Workflow](
            ...
            $match(Selector.And(
              ...
              Selector.And(
                Selector.Doc(BsonField.Name("bar") -> Selector.Neq(Bson.Int32(-10))),
            ...
            $match(
              Selector.And(
                ...
                Selector.And(
                  isNumeric(BsonField.Name("pop")),
                  Selector.Or(
                    Selector.Doc(ListMap[BsonField, Selector.SelectorExpr](BsonField.Name(
                      "pop") -> Selector.NotExpr(Selector.Gt(Bson.Int32(0))))),
                    Selector.Doc(ListMap[BsonField, Selector.SelectorExpr](BsonField.Name(
                      "pop") -> Selector.NotExpr(Selector.Lt(Bson.Int32(1000)))))
                  )
                )
              ))
          ))
    ...
    "plan filter with both index and field projections" in {
      plan(
        "select count(parents[0].sha) as count from slamengine_commits where parents[0].sha = \"56d1caf5d082d1a6840090986e277d36d03f1859\"") must
        beWorkflow(
          chain[Workflow](
            ...
            $match(Selector.Where(If(
              BinOp(
                jscore.And,
                Call(Select(ident("Array"), "isArray"),
                     List(Select(ident("this"), "parents"))),
                BinOp(
                  jscore.And,
                  Call(ident("isObject"),
                       List(Access(Select(ident("this"), "parents"),
              ...
              BinOp(
                jscore.Eq,
                Select(Access(Select(ident("this"), "parents"),
                              jscore.Literal(Js.Num(0, false))),
                       "sha"),
            ...
            //     above.
            $simpleMap(NonEmptyList(MapExpr(JsFn(Name("x"), obj()))), ListMap()),
            $group(grouped("count" -> $sum($literal(Bson.Int32(1)))),
                   \/-($literal(Bson.Null)))
          ))
    }.pendingUntilFixed(notOnPar)
    ...
    "plan simple having filter" in {
      plan("select city from zips group by city having count(*) > 10") must
        beWorkflow(
          chain[Workflow](
            $read(collection("db", "zips")),
            $group(grouped("__tmp1" -> $sum($literal(Bson.Int32(1)))),
                   -\/(reshape("0" -> $field("city")))),
            $match(Selector.Doc(BsonField.Name("__tmp1") -> Selector.Gt(Bson.Int32(10)))),
            $project(reshape("value"                     -> $field("_id", "0")), ExcludeId)
          ))
    }.pendingUntilFixed(notOnPar)
      ...
      plan("select city, sum(pop) from zips group by city having sum(pop) > 50000") must
        beWorkflow(
          chain[Workflow](
            ...
            $match(Selector.Doc(BsonField.Name("1") -> Selector.Gt(Bson.Int32(50000)))),
            $project(reshape("city"                 -> $field("_id", "0"), "1" -> $include()), IgnoreId)
          ))
    }.pendingUntilFixed(notOnPar)
    ...
    "prefer projection+filter over JS filter" in {
      plan("select * from zips where city <> state") must
        beWorkflow(
          chain[Workflow](
            $read(collection("db", "zips")),
            $project(reshape("__tmp0" -> $neq($field("city"), $field("state")),
                             "__tmp1" -> $$ROOT),
                     IgnoreId),
            $match(
              Selector.Doc(BsonField.Name("__tmp0") -> Selector.Eq(Bson.Bool(true)))),
            $project(reshape("value"                -> $field("__tmp1")), ExcludeId)
          ))
    }.pendingUntilFixed(notOnPar)
      ...
      plan("select * from zips where city <> state and pop < 10000") must
        beWorkflow(
          chain[Workflow](
              ...
              reshape(
                "__tmp4" ->
                  $cond(
                    ...
                    $and($neq($field("city"), $field("state")),
            ...
            $match(
              Selector.Doc(BsonField.Name("__tmp4") -> Selector.Eq(Bson.Bool(true)))),
            $project(reshape("value"                -> $field("__tmp5")), ExcludeId)
          ))
    }.pendingUntilFixed(notOnPar)
              ...
              reshape(
                "value" ->
                  $cond(
                    $and($lt($literal(Bson.Null), $field("pop")),
                         $lt($field("pop"), $literal(Bson.Text("")))),
                    $substr($literal(Bson.Text("fghijklmnop")),
    ...
    "drop nothing" in {
      plan("select * from zips limit 5 offset 0") must
        beWorkflow(chain[Workflow]($read(collection("db", "zips")), $limit(5)))
    }
      ...
      plan("select baz from foo order by bar/10") must
        beWorkflow(
          chain[Workflow](
                ...
                "__tmp2" ->
                  $cond(
                    $or(
                      $and($lt($literal(Bson.Null), $field("bar")),
                           $lt($field("bar"), $literal(Bson.Text("")))),
                      $and($lte($literal(Check.minDate), $field("bar")),
            ...
            $project(reshape("baz"                      -> $field("baz")), ExcludeId)
          ))
    }.pendingUntilFixed(notOnPar)
            ...
            $simpleMap(
              NonEmptyList(
                MapExpr(
                  JsFn(Name("x"),
                       SpliceObjects(
                         List(ident("x"), obj("pop" -> Select(ident("x"), "pop"))))))),
              ListMap())
          ))
    }.pendingUntilFixed(notOnPar)
          ...
          chain[Workflow](
            ...
            $simpleMap(
              NonEmptyList(
                MapExpr(
                  JsFn(Name("x"),
                       SpliceObjects(List(ident("x"),
                                          obj("city2" -> Select(ident("x"), "city")),
                                          obj("pop2"  -> Select(ident("x"), "pop"))))))),
              ListMap()
            )
          ))
    }.pendingUntilFixed(notOnPar)
            ...
            $simpleMap(
              NonEmptyList(
                MapExpr(
                  JsFn(Name("x"),
                       SpliceObjects(List(ident("x"),
                                          obj("1" -> jscore.Literal(Js.Str("1"))),
                                          obj("2" -> jscore.Literal(Js.Str("2")))))))),
              ListMap()
            )
          ))
    }.pendingUntilFixed(notOnPar)
    ...
    "plan select with multiple wildcards and fields" in {
      plan(
        "select state as state2, *, city as city2, *, pop as pop2 from zips where pop < 1000") must
        beWorkflow(
          chain[Workflow](
            $read(collection("db", "zips")),
            $match(
              Selector.And(
                isNumeric(BsonField.Name("pop")),
                Selector.Doc(BsonField.Name("pop") -> Selector.Lt(Bson.Int32(1000))))),
            $simpleMap(
              NonEmptyList(MapExpr(JsFn(
                Name("x"),
                obj("__tmp2" -> SpliceObjects(
                  List(obj("state2" -> Select(ident("x"), "state")),
                       ident("x"),
                       obj("city2" -> Select(ident("x"), "city")),
                       ident("x"),
                       obj("pop2" -> Select(ident("x"), "pop")))))
            ...
            $project(reshape("value" -> $field("__tmp2")), ExcludeId)
          ))
    }.pendingUntilFixed(notOnPar)
                ...
                SpliceObjects(List(
                  ...
                  obj("__sd__0" ->
                    jscore.If(
                      ...
                      BinOp(Div,
                            Select(ident("__val"), "pop"),
                            jscore.Literal(Js.Num(10, false))),
                      ident("undefined")
                    ))
                ))
              ))),
      ...
      plan("select name from person order by height") must
        beWorkflow(
          chain[Workflow](
            ...
            $project(reshape("name" -> $field("name"), "__tmp0" -> $field("height")),
                     IgnoreId),
            $sort(NonEmptyList(BsonField.Name("__tmp0") -> SortDir.Ascending)),
            $project(reshape("name"                     -> $field("name")), ExcludeId)
          ))
    }.pendingUntilFixed(notOnPar)
                ...
                "popInK" ->
                  $cond(
                    $or(
                      $and($lt($literal(Bson.Null), $field("pop")),
                           $lt($field("pop"), $literal(Bson.Text("")))),
                      $and($lte($literal(Check.minDate), $field("pop")),
                           $lt($field("pop"), $literal(Bson.Regex("", ""))))
                    ),
        ...
        beWorkflow(
          chain[Workflow](
            ...
            $match(
              Selector.And(
                isNumeric(BsonField.Name("pop")),
                Selector.Doc(BsonField.Name("pop") -> Selector.Lte(Bson.Int32(1000))))),
            $project(reshape("city"                   -> $field("city"), "pop" -> $field("pop")), IgnoreId),
    ...
    "plan multiple column sort with wildcard" in {
      plan("select * from zips order by pop, city desc") must
        beWorkflow(
          chain[Workflow]($read(collection("db", "zips")),
                          $sort(
                            NonEmptyList(BsonField.Name("pop")  -> SortDir.Ascending,
    ...
    "plan efficient count and field ref" in {
      plan("SELECT city, COUNT(*) AS cnt FROM zips ORDER BY cnt DESC") must
        beWorkflow {
          chain[Workflow](
            $read(collection("db", "zips")),
            $group(grouped("city" -> $push($field("city")),
      ...
      plan("SELECT COUNT(*) as cnt, LENGTH(city) FROM zips") must
        beWorkflow {
          chain[Workflow](
            ...
            $group(grouped("cnt" -> $sum($literal(Bson.Int32(1))),
                           "1"   -> $push($field("1"))),
                   \/-($literal(Bson.Null))),
            $unwind(DocField("1"))
          )
    ...
    "plan trivial group by" in {
      plan("select city from zips group by city") must
        beWorkflow(
          chain[Workflow]($read(collection("db", "zips")),
                          $group(grouped(), -\/(reshape("0" -> $field("city")))),
                          $project(reshape("value" -> $field("_id", "0")), ExcludeId)))
    }.pendingUntilFixed(notOnPar)
    ...
    "plan useless group by expression" in {
      plan("select city from zips group by lower(city)") must
        beWorkflow(
          chain[Workflow]($read(collection("db", "zips")),
                          $project(reshape("value" -> $field("city")), ExcludeId)))
    }
        ...
        beWorkflow(
          chain[Workflow](
            ...
            $group(
              ...
              -\/(reshape("0" -> $field("city"), "1" -> $field("state")))
            ),
            $project(reshape("0" ->
                               $concat($concat($field("__tmp10"),
                                               $literal(Bson.Text(", "))),
                                       $field("__tmp11")),
                             "1" -> $field("1")),
                     IgnoreId)
          ...
          chain[Workflow](
            ...
            $group(
              ...
              -\/(
                reshape(
                  "0" ->
                    $cond($and($lte($literal(Bson.Text("")), $field("city")),
                               $lt($field("city"), $literal(Bson.Doc()))),
                          $toLower($field("city")),
                          $literal(Bson.Undefined))))
            ),
    ...
    "plan group by month" in {
      plan(
        "select avg(score) as a, DATE_PART(\"month\", `date`) as m from caloriesBurnedData group by DATE_PART(\"month\", `date`)") must
        beWorkflow(
          chain[Workflow](
            ...
            $group(
              grouped(
                "a" ->
                  $avg(
                    $cond($and($lt($literal(Bson.Null), $field("score")),
              ...
              -\/(
                reshape(
                  "0" ->
                    $cond($and($lte($literal(Check.minDate), $field("date")),
                               $lt($field("date"), $literal(Bson.Regex("", "")))),
                          $month($field("date")),
                          $literal(Bson.Undefined))))
            ),
            $project(reshape("a" -> $include(), "m" -> $field("_id", "0")), IgnoreId)
          ))
    }.pendingUntilFixed(notOnPar)

    "plan expr3 with grouping" in {
      plan(
        "select case when pop > 1000 then city else lower(city) end, count(*) from zips group by city") must
        beRight
    }.pendingUntilFixed(notOnPar)
      ...
      plan("select count(*) from bar group by baz") must
        beWorkflow {
          chain[Workflow](
            ...
            $group(grouped("__tmp0" -> $sum($literal(Bson.Int32(1)))),
                   -\/(reshape("0" -> $field("baz")))),
            $project(reshape("value" -> $field("__tmp0")), ExcludeId)
          )
        }
          ...
          chain[Workflow](
            ...
            $group(
              grouped(
                ...
                "sm" ->
                  $sum(
                    $cond($and($lt($literal(Bson.Null), $field("biz")),
    ...
    "plan sum grouped by single field with filter" in {
      plan("select sum(pop) as sm from zips where state=\"CO\" group by city") must
        beWorkflow {
          chain[Workflow](
            $read(collection("db", "zips")),
            $match(Selector.Doc(BsonField.Name("state") -> Selector.Eq(Bson.Text("CO")))),
            $group(
              grouped(
                "sm" ->
                  $sum(
      ...
      plan("select count(*) as cnt, city from zips group by city") must
        beWorkflow {
          chain[Workflow](
            ...
            $group(grouped("cnt" -> $sum($literal(Bson.Int32(1)))),
                   -\/(reshape("0" -> $field("city")))),
            $project(reshape("cnt" -> $include(), "city" -> $field("_id", "0")), IgnoreId)
          )
        }
    }.pendingUntilFixed(notOnPar)
      ...
      plan("select city, state, sum(pop) from zips") must
        beWorkflow(
          chain[Workflow](
            ...
            $project(
              reshape(
                "__tmp3" -> reshape("city" -> $field("city"), "state" -> $field("state")),
                "__tmp4" -> reshape(
                  "__tmp2" ->
                    $cond($and($lt($literal(Bson.Null), $field("pop")),
                               $lt($field("pop"), $literal(Bson.Text("")))),
            ...
            $group(grouped("2"      -> $sum($field("__tmp4", "__tmp2")),
                           "__tmp3" -> $push($field("__tmp3"))),
                   \/-($literal(Bson.Null))),
            $unwind(DocField("__tmp3")),
            $project(reshape("city"  -> $field("__tmp3", "city"),
      ...
      plan("select max(pop)/1000, pop from zips") must
        beWorkflow {
          chain[Workflow](
            ...
            $unwind(DocField("pop")),
            $project(reshape("0"   -> $divide($field("__tmp3"), $literal(Bson.Int32(1000))),
                             "pop" -> $field("pop")),
                     IgnoreId)
          )
        }
      ...
      plan("select sum(avg(pop)), min(city) from zips group by foo") must
        beWorkflow {
          chain[Workflow](
            ...
            $group(
              grouped(
                "1" ->
                  $min($cond(
                    $or(
                      $and($lt($literal(Bson.Null), $field("city")),
                           $lt($field("city"), $literal(Bson.Doc()))),
                      $and($lte($literal(Bson.Bool(false)), $field("city")),
            ...
            $group(grouped("0" -> $sum($field("__tmp10")), "1" -> $push($field("1"))),
                   \/-($literal(Bson.Null))),
            $unwind(DocField("1"))
          )
        }
    ...
    "plan aggregation on grouped field" in {
      plan("select city, count(city) from zips group by city") must
        beWorkflow {
          chain[Workflow](
            $read(collection("db", "zips")),
            $group(grouped("1" -> $sum($literal(Bson.Int32(1)))),
                   -\/(reshape("0" -> $field("city")))),
            $project(reshape("city" -> $field("_id", "0"), "1" -> $include()), IgnoreId)
          )
        }
      ...
      plan("select pop, sum(pop), pop/1000 from zips") must
        beWorkflow(
          chain[Workflow](
            ...
            $group(grouped("1"      -> $sum($field("__tmp6", "__tmp2")),
                           "__tmp5" -> $push($field("__tmp5"))),
                   \/-($literal(Bson.Null))),
            $unwind(DocField("__tmp5")),
            $project(reshape("pop" -> $field("__tmp5", "pop"),
                             "1"   -> $field("1"),
                             "2"   -> $field("__tmp5", "2")),
                     IgnoreId)
          ))
      ...
      plan("select city, sum(pop-1)/1000 from zips group by city") must
        beWorkflow(
          chain[Workflow](
            ...
            $project(reshape("city" -> $field("_id", "0"),
                             "1" -> $divide($field("__tmp6"),
      ...
      plan("select state, length(min(city)) as shortest from zips group by state") must
        beWorkflow(
          chain[Workflow](
            ...
            $project(reshape("state" -> $field("_id", "0"), "__tmp6" -> $include()),
                     IgnoreId),
            $simpleMap(
              NonEmptyList(MapExpr(
                JsFn(Name("x"),
                     obj("state" -> Select(ident("x"), "state"),
                         "shortest" ->
                           Call(ident("NumberLong"),
                                List(Select(Select(ident("x"), "__tmp6"), "length"))))))),
              ListMap()
            ),
            $project(reshape("state" -> $include(), "shortest" -> $include()), IgnoreId)
          ))
    }.pendingUntilFixed(notOnPar)
      ...
      plan("select length(city) as len, count(*) as cnt from zips group by length(city)") must
        beWorkflow(
          chain[Workflow](
            ...
            $project(reshape("len" -> $field("_id", "0"), "cnt" -> $include()), IgnoreId)
          ))
    }.pendingUntilFixed(notOnPar)
    ...
    "plan expressions with ~" in {
      plan(
        "select foo ~ \"bar.*\", \"abc\" ~ \"a|b\", \"baz\" ~ regex, target ~ regex from a") must beWorkflow(
        chain[Workflow](
          $read(collection("db", "a")),
          $simpleMap(
            NonEmptyList(MapExpr(JsFn(
              ...
              obj(
                "0" -> If(
                  Call(ident("isString"), List(Select(ident("x"), "foo"))),
                  Call(Select(New(Name("RegExp"),
                                  List(jscore.Literal(Js.Str("bar.*")),
                                       jscore.Literal(Js.Str("m")))),
                              "test"),
      ...
      plan("select city, loc[0] from zips") must
        beWorkflow {
          chain[Workflow](
            ...
            $project(reshape("city" -> $include(), "1" -> $include()), IgnoreId)
          )
        }
    }
      ...
      plan("select loc || [ pop ] from zips where city = \"BOULDER\"") must
        beWorkflow {
          chain[Workflow](
              ...
              NonEmptyList(MapExpr(JsFn(
                ...
                obj("__tmp4" ->
                  If(
                    ...
                    SpliceArrays(
                      List(jscore.Select(ident("x"), "loc"),
            ...
            $project(reshape("value" -> $field("__tmp4")), ExcludeId)
          )
        }
    }.pendingUntilFixed(notOnPar)
          ...
          chain[Workflow](
            ...
            $project(
              reshape(
                "__tmp2" ->
                  $cond(
                    $and($lte($literal(Bson.Arr(List())), $field("loc")),
                         $lt($field("loc"),
                             $literal(Bson.Binary.fromArray(scala.Array[Byte]())))),
                    $field("loc"),
      ...
      plan("select loc || [ 0, 1, 2 ] from zips") must beWorkflow {
        chain[Workflow](
          ...
          $simpleMap(
            NonEmptyList(MapExpr(JsFn(
              Name("x"),
              Obj(ListMap(Name("__tmp4") ->
                If(
                  BinOp(jscore.Or,
                        Call(Select(ident("Array"), "isArray"),
                             List(Select(ident("x"), "loc"))),
                        Call(ident("isString"), List(Select(ident("x"), "loc")))),
                  SpliceArrays(List(Select(ident("x"), "loc"),
                                    Arr(List(jscore.Literal(Js.Num(0, false)),
                                             jscore.Literal(Js.Num(1, false)),
      ...
      plan("SELECT `_id` as zip, loc as loc, loc[*] as coord FROM zips") must
        beWorkflow {
          chain[Workflow](
            ...
            $unwind(DocField(BsonField.Name("__tmp2"))),
            $project(reshape("zip"   -> $field("__tmp3", "_id"),
      ...
      plan("select loc[*] from zips where loc[*] < 0") must
        beWorkflow(
          chain[Workflow](
            ...
            $match(Selector.Doc(BsonField.Name("__tmp6") -> Selector.Lt(Bson.Int32(0)))),
            $project(reshape("value"                     -> $field("__tmp6")), ExcludeId)
          ))
    }.pendingUntilFixed(notOnPar)

    "group by flattened field" in {
      plan(
        "select substring(parents[*].sha, 0, 1), count(*) from slamengine_commits group by substring(parents[*].sha, 0, 1)") must
        beWorkflow(
          chain[Workflow](
            $read(collection("db", "slamengine_commits")),
            $project(
              reshape(
                "__tmp12" ->
                  $cond(
                    $and($lte($literal(Bson.Arr(List())), $field("parents")),
                         $lt($field("parents"),
                             $literal(Bson.Binary.fromArray(scala.Array[Byte]())))),
                    $field("parents"),
                    $literal(Bson.Arr(List(Bson.Undefined)))
            ...
            $group(
              ...
              -\/(
                reshape("0" ->
                  $cond(
                    $and($lte($literal(Bson.Text("")), $field("__tmp12", "sha")),
                         $lt($field("__tmp12", "sha"), $literal(Bson.Doc()))),
                    $substr($field("__tmp12", "sha"),
                            $literal(Bson.Int32(0)),
                            $literal(Bson.Int32(1))),
                    $literal(Bson.Undefined)
                  )))
            ),
            $project(reshape("0" -> $field("_id", "0"), "1" -> $include()), IgnoreId)
          ))
    }.pendingUntilFixed(notOnPar)
      ...
      plan("select `_id` as zip, loc[*] from zips order by loc[*]") must
        beWorkflow(
          chain[Workflow](
            ...
            $unwind(DocField(BsonField.Name("__tmp2"))),
            $project(reshape("zip" -> $field("__tmp3", "_id"), "loc" -> $field("__tmp2")),
                     IgnoreId),
    ...
    "unify flattened with double-flattened" in {
      plan(
        "select * from user_comments where (comments[*].id LIKE \"%Dr%\" OR comments[*].replyTo[*] LIKE \"%Dr%\")") must
        beWorkflow(
          chain[Workflow](
            ...
            $project(
              reshape(
                "__tmp14" ->
                  $cond(
                    $and($lte($literal(Bson.Arr(List())), $field("comments")),
                         $lt($field("comments"),
                             $literal(Bson.Binary.fromArray(scala.Array[Byte]())))),
                    $field("comments"),
                    $literal(Bson.Arr(List(Bson.Undefined)))
            ...
            $project(
              reshape(
                "__tmp18" ->
                  $cond(
                    $and($lte($literal(Bson.Arr(List())), $field("__tmp14", "replyTo")),
                         $lt($field("__tmp14", "replyTo"),
                             $literal(Bson.Binary.fromArray(scala.Array[Byte]())))),
            ...
            $match(Selector.Or(
              Selector.And(
                Selector.Doc(
                  BsonField.Name("__tmp19") \ BsonField.Name("__tmp14") \ BsonField.Name(
                    "id") -> Selector.Type(BsonType.Text)),
                Selector.Doc(
                  BsonField.Name("__tmp19") \ BsonField.Name("__tmp14") \ BsonField.Name(
                    "id") -> Selector.Regex("^.*Dr.*$", false, true, false, false))
              ),
              Selector.Doc(BsonField
                .Name("__tmp18") -> Selector.Regex("^.*Dr.*$", false, true, false, false))
            )),
            $project(reshape("value" -> $field("__tmp19", "__tmp15")), ExcludeId)
          ))
    }.pendingUntilFixed(notOnPar)
    ...
    "plan limit with offset" in {
      plan("SELECT * FROM zips OFFSET 100 LIMIT 5") must
        beWorkflow(
          chain[Workflow]($read(collection("db", "zips")), $limit(105), $skip(100)))
    }
      ...
      plan("SELECT city, SUM(pop) AS pop FROM zips GROUP BY city ORDER BY pop") must
        beWorkflow {
          chain[Workflow](
            ...
            $project(reshape("city" -> $field("_id", "0"), "pop" -> $include()),
                     IgnoreId),
            $sort(NonEmptyList(BsonField.Name("pop") -> SortDir.Ascending))
          )
        }
    ...
    "plan filter and expressions with IS NULL" in {
      plan("select foo is null from zips where foo is null") must
        beWorkflow(
          chain[Workflow](
            $read(collection("db", "zips")),
            $match(Selector.Doc(BsonField.Name("foo") -> Selector.Eq(Bson.Null))),
            $project(reshape("value"                  -> $eq($field("foo"), $literal(Bson.Null))),
                     ExcludeId)
          ))
    }.pendingUntilFixed(notOnPar)
      ...
      plan("select distinct city, state from zips") must
        beWorkflow(
          chain[Workflow](
            ...
            $group(grouped(),
                   -\/(reshape("0" -> $field("city"), "1" -> $field("state")))),
            $project(reshape("city" -> $field("_id", "0"), "state" -> $field("_id", "1")),
                     IgnoreId)
          ))
    }.pendingUntilFixed(notOnPar)
      ...
      plan("select count(distinct(city)) from zips") must
        beWorkflow(
          chain[Workflow](
            $read(collection("db", "zips")),
            $group(grouped(), -\/(reshape("0" -> $field("city")))),
            $group(grouped("__tmp2" -> $sum($literal(Bson.Int32(1)))),
                   \/-($literal(Bson.Null))),
            $project(reshape("value" -> $field("__tmp2")), ExcludeId)
          ))
    }.pendingUntilFixed(notOnPar)
      ...
      plan("select count(distinct substring(city, 0, 1)) from zips") must
        beWorkflow(
          chain[Workflow](
            ...
            $group(grouped("__tmp8" -> $sum($literal(Bson.Int32(1)))),
                   \/-($literal(Bson.Null))),
            $project(reshape("value" -> $field("__tmp8")), ExcludeId)
          ))
    }.pendingUntilFixed(notOnPar)
        ...
        beWorkflow(
          chain[Workflow](
            ...
            $simpleMap(
              ...
              ListMap()),
            $group(grouped(), -\/(reshape("0" -> $field("__tmp1")))),
            $project(reshape("value" -> $field("_id", "0")), ExcludeId)
          ))
    }.pendingUntilFixed(notOnPar)
                  ...
                  JsFn(Name("x"),
                       obj("__tmp4" ->
                         Call(ident("remove"),
                              List(ident("x"), jscore.Literal(Js.Str("_id")))))))),
              ListMap()),
            $group(grouped(), -\/(reshape("0" -> $field("__tmp4")))),
            $group(grouped("__tmp6" -> $sum($literal(Bson.Int32(1)))),
                   \/-($literal(Bson.Null))),
            $project(reshape("value" -> $field("__tmp6")), ExcludeId)
          ))
    }.pendingUntilFixed(notOnPar)
    ...
    "plan distinct with simple order by" in {
      plan("select distinct city from zips order by city") must
        beWorkflow(
          chain[Workflow](
            $read(collection("db", "zips")),
            $project(reshape("city"                   -> $field("city")), IgnoreId),
            $sort(NonEmptyList(BsonField.Name("city") -> SortDir.Ascending)),
            $group(grouped(), -\/(reshape("0" -> $field("city")))),
            $project(reshape("city"                   -> $field("_id", "0")), IgnoreId),
            $sort(NonEmptyList(BsonField.Name("city") -> SortDir.Ascending))
          ))
    ...
    "plan distinct with unrelated order by" in {
      plan("select distinct city from zips order by pop desc") must
        beWorkflow(
          chain[Workflow](
            $read(collection("db", "zips")),
            $project(reshape("__sd__0" -> $field("pop"), "city" -> $field("city")),
                     IgnoreId),
            $sort(NonEmptyList(BsonField.Name("__sd__0") -> SortDir.Descending)),
            $group(grouped("__tmp2"                      -> $first($$ROOT)),
                   -\/(reshape("city" -> $field("city")))),
            $project(reshape("city"   -> $field("__tmp2", "city"),
                             "__tmp0" -> $field("__tmp2", "__sd__0")),
                     IgnoreId),
            $sort(NonEmptyList(BsonField.Name("__tmp0") -> SortDir.Descending)),
            $project(reshape("city"                     -> $field("city")), ExcludeId)
          ))
    }.pendingUntilFixed(notOnPar)
    ...
    "plan distinct as function with group" in {
      plan("select state, count(distinct(city)) from zips group by state") must
        beWorkflow(
          chain[Workflow](
            $read(collection("db", "zips")),
            $group(grouped("__tmp0" -> $first($$ROOT)),
                   -\/(reshape("0" -> $field("city")))),
            $group(grouped("state" -> $first($field("__tmp0", "state")),
      ...
      plan("SELECT DISTINCT SUM(pop) AS totalPop, city, state FROM zips GROUP BY city") must
        beWorkflow(
          chain[Workflow](
            ...
            $project(reshape("totalPop" -> $include(),
                             "city"     -> $field("_id", "0"),
                             "state"    -> $include()),
                     IgnoreId),
            ...
            $group(grouped(),
                   -\/(
                     reshape("0" -> $field("totalPop"),
                             "1" -> $field("city"),
                             "2" -> $field("state")))),
            $project(reshape("totalPop" -> $field("_id", "0"),
                             "city"     -> $field("_id", "1"),
                             "state"    -> $field("_id", "2")),
                     IgnoreId)
          ))
    ...
    "plan distinct with sum, group, and orderBy" in {
      plan(
        "SELECT DISTINCT SUM(pop) AS totalPop, city, state FROM zips GROUP BY city ORDER BY totalPop DESC") must
        beWorkflow(
          chain[Workflow](
            $read(collection("db", "zips")),
            $group(
        ...
        beWorkflow(
          chain[Workflow](
            ...
            $match(
              Selector.And(
                isNumeric(BsonField.Name("pop")),
                Selector.Doc(BsonField.Name("pop") -> Selector.Gt(Bson.Int32(1000))))),
            $simpleMap(
              NonEmptyList(MapExpr(JsFn(
                Name("x"),
                obj(
            ...
            $project(reshape("city"                     -> $field("city"), "pop" -> $field("pop")), ExcludeId)
          ))
    }.pendingUntilFixed(notOnPar)
    ...
    "plan filter with timestamp and interval" in {
      ...
      plan(
        """select * from days where date < timestamp("2014-11-17T22:00:00Z") and date - interval("PT12H") > timestamp("2014-11-17T00:00:00Z")""") must
        beWorkflow(
          chain[Workflow](
            ...
            $project(
              reshape(
                "__tmp6" ->
                  $cond(
                    $or(
                      $and($lt($literal(Bson.Null), $field("date")),
                           $lt($field("date"), $literal(Bson.Text("")))),
                      $and($lte($literal(Check.minDate), $field("date")),
                           $lt($field("date"), $literal(Bson.Regex("", ""))))
                    ),
                    $cond(
                      $or(
                        ...
                        $and($lte($literal(Bson.Bool(false)), $field("date")),
                             $lt($field("date"), $literal(Bson.Regex("", ""))))
                      ),
                      $and($lt($field("date"), $literal(date22)),
                           $gt($subtract($field("date"),
                                         $literal(Bson.Dec(12 * 60 * 60 * 1000))),
                               $literal(date0))),
                      $literal(Bson.Undefined)
                    ),
                    $literal(Bson.Undefined)
                ...
                "value" ->
                  $cond(
                    $and($lte($literal(Check.minDate), $field("ts")),
                         $lt($field("ts"), $literal(Bson.Regex("", "")))),
                    $dateToString(
                      Hour :: ":" :: Minute :: ":" :: Second :: "." :: Millisecond :: FormatString.empty,
                      $field("ts")),
                    $literal(Bson.Undefined)
                  )),
              ExcludeId
              ...
              Selector.Or(
                ...
                Selector.And(
                  ...
                  Selector.And(
                    ...
                    Selector.And(
                      Selector.And(
                        Selector.Doc(BsonField.Name("ts") -> Selector.Gte(date23)),
                        Selector.Doc(BsonField.Name("ts") -> Selector.Lt(date28))),
                      Selector.Or(
                        Selector.Doc(BsonField.Name("ts") -> Selector.Lt(date25)),
                        Selector.Doc(BsonField.Name("ts") -> Selector.Gte(date26)))
                    )
                  )
                ),
                Selector.And(Selector.Doc(BsonField.Name("ts") -> Selector.Gte(date29)),
                             Selector.Doc(BsonField.Name("ts") -> Selector.Lt(date30)))
              ))
          ))
    }.pendingUntilFixed(notOnPar)
    ...
    "plan js and filter with id" in {
      Bson.ObjectId
        .fromString("0123456789abcdef01234567")
        .fold[Result](failure("Couldn’t create ObjectId."))(oid =>
          plan("""select length(city), foo = oid("0123456789abcdef01234567") from days where `_id` = oid("0123456789abcdef01234567")""") must
            beWorkflow(chain[Workflow](
              $read(collection("db", "days")),
              $match(Selector.Doc(BsonField.Name("_id") -> Selector.Eq(oid))),
              $simpleMap(
                NonEmptyList(MapExpr(JsFn(
                  Name("x"),
                  obj(
                    ...
                    "1" ->
                      BinOp(jscore.Eq,
                            Select(ident("x"), "foo"),
                            New(Name("ObjectId"),
                                List(jscore.Literal(Js.Str("0123456789abcdef01234567")))))
                  )
                ))),
                ListMap()
              ),
              $project(reshape("0" -> $field("0"), "1" -> $field("1")), ExcludeId)
            )))
    }.pendingUntilFixed(notOnPar)
        ...
        chain[Workflow](
          ...
          $project(
            reshape(
              "value" ->
                $cond(
                  $and($lt($literal(Bson.Null), $field("epoch")),
    ...
    "plan convert to timestamp in map-reduce" in {
      ...
      plan("select length(name), to_timestamp(epoch) from foo") must beWorkflow {
        chain[Workflow](
          ...
          $project(reshape("0" -> $include(), "1" -> $include()), IgnoreId)
        )
      }
    }

    def joinStructure0(left: Workflow,
                       leftName: String,
                       leftBase: Fix[ExprOp],
                       right: Workflow,
                       leftKey: Reshape.Shape[ExprOp],
                       rightKey: (String, Fix[ExprOp], Reshape.Shape[ExprOp]) \/ JsCore,
                       fin: FixOp[WorkflowF],
                       swapped: Boolean) = {

      val (leftLabel, rightLabel) =
        if (swapped) (JoinDir.Right.name, JoinDir.Left.name)
        else (JoinDir.Left.name, JoinDir.Right.name)
      def initialPipeOps(src: Workflow,
                         name: String,
                         base: Fix[ExprOp],
                         key: Reshape.Shape[ExprOp],
                         mainLabel: String,
                         otherLabel: String): Workflow =
        chain[Workflow](
          src,
          $group(grouped(name         -> $push(base)), key),
          $project(reshape(mainLabel  -> $field(name),
              ...
              rk =>
                $map(
                  $MapF.mapKeyVal(
                    ...
                    rk.toJs,
                    Js.AnonObjDecl(
                      List((leftLabel, Js.AnonElem(List())),
            ...
            $reduce(
              Js.AnonFunDecl(
                ...
                List(
                  Js.VarDef(
                    List(("result",
                          Js.AnonObjDecl(List((leftLabel, Js.AnonElem(List())),
                                              (rightLabel, Js.AnonElem(List()))))))),
                  Js.Call(
                    Js.Select(Js.Ident("values"), "forEach"),
                    List(Js.AnonFunDecl(
                      List("value"),
                      List(
                        Js.BinOp(
                          ...
                          Js.Call(Js.Select(Js.Select(Js.Ident("result"), leftLabel),
                                            "concat"),
                        ...
                        Js.BinOp(
                          ...
                          Js.Call(Js.Select(Js.Select(Js.Ident("result"), rightLabel),
                                            "concat"),
    ...
    def joinStructure(left: Workflow,
                      leftName: String,
                      leftBase: Fix[ExprOp],
                      right: Workflow,
                      leftKey: Reshape.Shape[ExprOp],
      ...
      Crystallize[WorkflowF].crystallize(
        joinStructure0(left, leftName, leftBase, right, leftKey, rightKey, fin, swapped))
        ...
        beWorkflow(
          joinStructure(
            ...
            chain[Workflow](
              ...
              $project(reshape("value" -> $field(JoinDir.Right.name, "city")), ExcludeId)
            ),
            false
          ).op)
    ...
    "plan simple join ($lookup)" in {
      plan("select zips2.city from zips join zips2 on zips.`_id` = zips2.`_id`") must
        beWorkflow(
          chain[Workflow](
            $read(collection("db", "zips")),
            $match(Selector.Doc(BsonField.Name("_id") -> Selector.Exists(true))),
            $project(reshape(JoinDir.Left.name        -> $$ROOT)),
            $lookup(CollectionName("zips2"),
    ...
    "plan simple join with sharded inputs" in {
      ...
      plan3_2(
        query,
        c =>
          Map(collection("db", "zips")  -> CollectionStatistics(10, 100, true),
    ...
    "plan simple join with sources in different DBs" in {
      // NB: cannot use $lookup, so fall back to the old approach
      val query =
        "select zips2.city from `/db1/zips` join `/db2/zips2` on zips.`_id` = zips2.`_id`"
            ...
            chain[Workflow](
              ...
              $project(
                reshape(
                  "__tmp11" ->
                    $cond(
                      $and($lte($literal(Bson.Doc()), $field(JoinDir.Right.name)),
                  ...
                  "__tmp12" ->
                    $cond(
                      ...
                      $cond(
                        $or(
                          $and($lt($literal(Bson.Null),
                                   $field(JoinDir.Right.name, "_id")),
                               $lt($field(JoinDir.Right.name, "_id"),
                                   $literal(Bson.Doc()))),
                          $and($lte($literal(Bson.Bool(false)),
                                    $field(JoinDir.Right.name, "_id")),
                               $lt($field(JoinDir.Right.name, "_id"),
                                   $literal(Bson.Regex("", ""))))
                        ),
                        $cond(
                          $and($lte($literal(Bson.Doc()), $field(JoinDir.Left.name)),
                               $lt($field(JoinDir.Left.name),
                                   $literal(Bson.Arr(List())))),
                          $cond(
                            $or(
                              $and($lt($literal(Bson.Null),
                                       $field(JoinDir.Left.name, "_id")),
                                   $lt($field(JoinDir.Left.name, "_id"),
                                       $literal(Bson.Doc()))),
                              $and($lte($literal(Bson.Bool(false)),
                                        $field(JoinDir.Left.name, "_id")),
                                   $lt($field(JoinDir.Left.name, "_id"),
              ...
              $match(
                Selector.Doc(BsonField.Name("__tmp12") -> Selector.Eq(Bson.Bool(true)))),
              $project(reshape("value"                 -> $field("__tmp11", "city")), ExcludeId)
            ),
            false
          ).op)
    }.pendingUntilFixed("#1940")
    ...
    "plan simple inner equi-join (map-reduce)" in {
      plan2_6("select foo.name, bar.address from foo join bar on foo.id = bar.foo_id") must
        beWorkflow(
          joinStructure(
            $read(collection("db", "foo")),
            "__tmp0",
    ...
    "plan simple inner equi-join ($lookup)" in {
      plan3_2("select foo.name, bar.address from foo join bar on foo.id = bar.foo_id",
              defaultStats,
              indexes(collection("db", "bar") -> BsonField.Name("foo_id"))) must
        beWorkflow(
          chain[Workflow](
            $read(collection("db", "foo")),
            $match(Selector.Doc(BsonField.Name("id") -> Selector.Exists(true))),
            $project(reshape(JoinDir.Left.name       -> $$ROOT)),
            $lookup(CollectionName("bar"),
        ...
        beWorkflow(
          chain[Workflow](
            ...
            $lookup(CollectionName("bar"),
                    ...
                    BsonField.Name("foo_id"),
                    JoinHandler.RightName),
            $project(reshape(JoinDir.Left.name  -> $field(JoinDir.Left.name),
          ...
          chain[Workflow](
            ...
            $match(
              Selector.And(
                ...
                Selector.Doc(BsonField.Name("rating") -> Selector.Gte(Bson.Int32(4))))),
            $project(reshape(JoinDir.Right.name -> $$ROOT, "__tmp2" -> $field("foo_id")),
                     ExcludeId),
            $lookup(CollectionName("foo"),
                    BsonField.Name("__tmp2"),
                    BsonField.Name("id"),
                    JoinHandler.LeftName),
            $project(reshape(JoinDir.Right.name -> $field(JoinDir.Right.name),
          ...
          joinStructure(
            ...
            chain[Workflow](
              ...
              $simpleMap(
                NonEmptyList(MapExpr(JsFn(
                  Name("x"),
                  Obj(ListMap(Name("__tmp7") ->
                    If(
                      BinOp(
                        jscore.And,
                        Call(ident("isObject"),
                             List(Select(ident("x"), JoinDir.Right.name))),
                        UnOp(jscore.Not,
                             Call(Select(ident("Array"), "isArray"),
                                  List(Select(ident("x"), JoinDir.Right.name))))
                      ),
                      If(
                        BinOp(
                          jscore.And,
                          Call(ident("isObject"),
                               List(Select(ident("x"), JoinDir.Left.name))),
                          UnOp(jscore.Not,
                               Call(Select(ident("Array"), "isArray"),
                                    List(Select(ident("x"), JoinDir.Left.name))))
                        ),
                        SpliceObjects(List(Select(ident("x"), JoinDir.Left.name),
      ...
      plan2_6(
        ...
        beWorkflow(
          joinStructure(
            ...
            Obj(ListMap(Name("0") -> Select(Select(ident("value"), JoinDir.Right.name),
                                            "id"))).right,
            chain[Workflow](
              _,
              $match(Selector.Doc(ListMap[BsonField, Selector.SelectorExpr](
                ...
                reshape(
                  "name" ->
                    $cond(
                      $and($lte($literal(Bson.Doc()), $field(JoinDir.Left.name)),
                           $lt($field(JoinDir.Left.name), $literal(Bson.Arr()))),
                      $cond(
                        $and($lte($literal(Bson.Doc()),
                                  $field(JoinDir.Left.name, JoinDir.Left.name)),
                             $lt($field(JoinDir.Left.name, JoinDir.Left.name),
                                 $literal(Bson.Arr()))),
                  ...
                  "address" ->
                    $cond(
                      ...
                      $cond(
                        $and($lte($literal(Bson.Doc()),
                                  $field(JoinDir.Left.name, JoinDir.Right.name)),
                             $lt($field(JoinDir.Left.name, JoinDir.Right.name),
                                 $literal(Bson.Arr()))),
    ...
    "plan join with multiple conditions" in {
      plan(
        "select l.sha as child, l.author.login as c_auth, r.sha as parent, r.author.login as p_auth from slamengine_commits as l join slamengine_commits as r on r.sha = l.parents[0].sha and l.author.login = r.author.login") must
        beWorkflow(
          joinStructure(
            chain[Workflow](
              $read(collection("db", "slamengine_commits")),
              $simpleMap(
                NonEmptyList(MapExpr(JsFn(
                  Name("x"),
                  obj(
                    "__tmp4" -> Select(Access(Select(ident("x"), "parents"),
                                              jscore.Literal(Js.Num(0, false))),
                                       "sha"),
                    "__tmp5" -> ident("x"),
            ...
            $field("__tmp5"),
            $read(collection("db", "slamengine_commits")),
            reshape("0" -> $field("__tmp4"), "1" -> $field("__tmp6")),
            obj("0" -> Select(ident("value"), "sha"),
                ...
                reshape(
                  ...
                  "c_auth" ->
                    $cond(
                      ...
                      $cond(
                        $and($lte($literal(Bson.Doc()),
                                  $field(JoinDir.Left.name, "author")),
                             $lt($field(JoinDir.Left.name, "author"),
                                 $literal(Bson.Arr()))),
                  ...
                  "p_auth" ->
                    $cond(
                      ...
                      $cond(
                        $and($lte($literal(Bson.Doc()),
                                  $field(JoinDir.Right.name, "author")),
                             $lt($field(JoinDir.Right.name, "author"),
                                 $literal(Bson.Arr()))),
    ...
    "plan join with non-JS-able condition" in {
      plan(
        "select z1.city as city1, z1.loc, z2.city as city2, z2.pop from zips as z1 join zips as z2 on z1.loc[*] = z2.loc[*]") must
        beWorkflow(
          joinStructure(
            chain[Workflow](
              $read(collection("db", "zips")),
              $project(reshape("__tmp0" -> $field("loc"), "__tmp1" -> $$ROOT), IgnoreId),
              $unwind(DocField(BsonField.Name("__tmp0")))),
            "__tmp2",
            $field("__tmp1"),
            chain[Workflow](
              $read(collection("db", "zips")),
              $project(reshape("__tmp3" -> $field("loc"), "__tmp4" -> $$ROOT), IgnoreId),
                    ...
                    $cond(
                      ...
                      $cond(
                        $or(
                          $and($lt($literal(Bson.Null),
                                   $field(JoinDir.Right.name, "pop")),
                               $lt($field(JoinDir.Right.name, "pop"),
                                   $literal(Bson.Doc()))),
                          $and($lte($literal(Bson.Bool(false)),
                                    $field(JoinDir.Right.name, "pop")),
                               $lt($field(JoinDir.Right.name, "pop"),
                        ...
                        $cond(
                          ...
                          $cond(
                            $or(
                              $and($lt($literal(Bson.Null),
                                       $field(JoinDir.Left.name, "pop")),
                                   $lt($field(JoinDir.Left.name, "pop"),
                                       $literal(Bson.Doc()))),
                              $and($lte($literal(Bson.Bool(false)),
                                        $field(JoinDir.Left.name, "pop")),
                                   $lt($field(JoinDir.Left.name, "pop"),
    ...
    def countOps(wf: Workflow,
                 p: PartialFunction[WorkflowF[Fix[WorkflowF]], Boolean]): Int = {
    ...
    def noConsecutiveSimpleMapOps(wf: Workflow) =
      countOps(
        wf, { case WC($SimpleMapF(Embed(WC($SimpleMapF(_, _, _))), _, _)) => true }) aka "the occurrences of consecutive $simpleMap ops:" must_== 0
        ...
        case IsSingleSource(op) =>
          simpleShape(op.src)
            .map { shape =>
              ...
              val missing = refs.collect {
                case v @ DocVar(_, Some(f)) if !shape.contains(f.flatten.head) => v
              }
              if (missing.isEmpty) Nil
              else
                List(
                  missing.map(_.bson).mkString(", ") + " missing in\n" + Fix[WorkflowF](
                    op.wf).render.shows)
            }
            .getOrElse(Nil)
        case _ => Nil
    ...
    def rootPushes(wf: Workflow) =
      wf.foldMap(_.unFix match {
        case WC(op @ $GroupF(src, Grouped(map), _))
            if map.values.toList.contains($push($$ROOT)) && simpleShape(src).isEmpty =>
          List(op)
        case _ => Nil
    ...
    "plan multiple reducing projections (all, distinct, orderBy)" >> Prop
      .forAll(
        select(distinct,
               maybeReducingExpr,
               Gen.option(filter),
               Gen.option(groupBySeveral),
               orderBySeveral)) { q =>
      ...
      .set(maxSize = 3)
      .pendingUntilFixed(notOnPar) // FIXME: with more then a few keys in the order by, the planner gets *very* slow (see SD-658)
    ...
    "plan multiple reducing projections (all, distinct)" >> Prop
      .forAll(
        select(distinct,
               maybeReducingExpr,
               Gen.option(filter),
    ...
    "plan multiple reducing projections (all)" >> Prop
      .forAll(
        select(notDistinct,
               maybeReducingExpr,
               Gen.option(filter),
    ...
    "plan multiple reducing projections (no filter)" >> Prop
      .forAll(
        select(notDistinct,
               maybeReducingExpr,
               noFilter,
  ...
  val groupBySeveral = Gen
    .nonEmptyListOf(Gen.oneOf(sql.IdentR("state"), sql.IdentR("territory")))
    .map(keys => GroupBy(keys.distinct, None))
  ...
  val filter = Gen.oneOf(
    ...
    for {
      ...
      sql.InvokeFunctionR(
        CIName("search"),
        List(x, sql.StringLiteralR("^BOULDER"), sql.BoolLiteralR(false))),
    Gen.const(sql.BinopR(sql.IdentR("p"), sql.IdentR("q"), quasar.sql.Eq))
  ) // Comparing two fields requires a $project before the $match
  ...
  def select(distinctGen: Gen[IsDistinct],
             exprGen: Gen[Fix[Sql]],
             filterGen: Gen[Option[Fix[Sql]]],
             groupByGen: Gen[Option[GroupBy[Fix[Sql]]]],
             orderByGen: Gen[Option[OrderBy[Fix[Sql]]]]): Gen[Query] =
    for {
      distinct <- distinctGen
      projs <- (genReduceInt ⊛ Gen.nonEmptyListOf(exprGen))(_ :: _)
  ...
  def genInnerInt =
    Gen.oneOf(
      ...
      sql.InvokeFunctionR(CIName("length"), List(sql.IdentR("city")))
    ) // requires JS
  def genReduceInt =
    genInnerInt.flatMap(
      x =>
  ...
  def genOuterInt =
    Gen.oneOf(
      ...
      genInnerInt.flatMap(x =>
        sql.BinopR(sql.IdentR("loc"), sql.ArrayLiteralR(List(x)), quasar.sql.Concat))
    )

  def genInnerStr =
    Gen.oneOf(sql.IdentR("city"),
  ...
  def genOuterStr =
    Gen.oneOf(
      ...
      genReduceStr,
      genReduceStr
        .flatMap(x => sql.InvokeFunctionR(CIName("lower"), List(x))), // an ExprOp
      genReduceStr.flatMap(x => sql.InvokeFunctionR(CIName("length"), List(x)))
    ) // requires JS

  implicit def shrinkQuery(implicit SS: Shrink[Fix[Sql]]): Shrink[Query] = Shrink { q =>
    fixParser
      .parseExpr(q)
      .fold(κ(Stream.empty), SS.shrink(_).map(sel => Query(pprint(sel))))
  }
  ...
  implicit def shrinkExpr: Shrink[Fix[Sql]] = {
    ...
    Shrink {
      case Embed(Select(d, projs, rel, filter, groupBy, orderBy)) =>
        val sDistinct =
          if (d == SelectDistinct)
            Stream(sql.SelectR(SelectAll, projs, rel, filter, groupBy, orderBy))
          else Stream.empty
        val sProjs =
          shortened(projs).map(ps => sql.SelectR(d, ps, rel, filter, groupBy, orderBy))
        val sGroupBy = groupBy
          .map {
            case GroupBy(keys, having) =>
              shortened(keys).map(ks =>
                sql.SelectR(d, projs, rel, filter, Some(GroupBy(ks, having)), orderBy))
          }
  ...
  "plan from LogicalPlan" should {
    ...
    "plan simple Sort" in {
      val lp =
        lpf.let(
          ...
          lpf.let(
            'tmp1,
            makeObj(
              "bar" -> lpf.invoke2(ObjectProject,
                                   lpf.free('tmp0),
                                   lpf.constant(Data.Str("bar")))),
            lpf.let('tmp2,
                    lpf.sort(lpf.free('tmp1),
      ...
      plan(lp) must beWorkflow(
        chain[Workflow]($read(collection("db", "foo")),
                        $project(reshape("bar"                   -> $field("bar")), IgnoreId),
                        $sort(NonEmptyList(BsonField.Name("bar") -> SortDir.Ascending))))
    }
    ...
    "plan Sort with expression" in {
      val lp =
        lpf.let(
          ...
          lpf.sort(
            lpf.free('tmp0),
            (math
               .Divide[Fix[LP]](lpf.invoke2(ObjectProject,
                                            lpf.free('tmp0),
                                            lpf.constant(Data.Str("bar"))),
      ...
      plan(lp) must beWorkflow(
        chain[Workflow](
          $read(collection("db", "foo")),
          $project(reshape("__tmp0" -> divide($field("bar"), $literal(Bson.Dec(10.0))),
                           "__tmp1" -> $$ROOT),
                   IgnoreId),
          $sort(NonEmptyList(BsonField.Name("__tmp0") -> SortDir.Ascending)),
          $project(reshape("value"                    -> $field("__tmp1")), ExcludeId)
    ...
    "plan Sort with expression and earlier pipeline op" in {
      ...
      plan(lp) must beWorkflow(
        chain[Workflow](
          $read(collection("db", "foo")),
          $match(Selector.Doc(BsonField.Name("baz") -> Selector.Eq(Bson.Int32(0)))),
          $sort(NonEmptyList(BsonField.Name("bar")  -> SortDir.Ascending))
        ))
    }
    ...
    "plan Sort expression (and extra project)" in {
      ...
      plan(lp) must beWorkflow(
        chain[Workflow](
          $read(collection("db", "foo")),
          $project(reshape("bar"    -> $field("bar"),
                           "__tmp0" -> divide($field("bar"), $literal(Bson.Dec(10.0)))),
                   IgnoreId),
          $sort(NonEmptyList(BsonField.Name("__tmp0") -> SortDir.Ascending)),
          $project(reshape("bar"                      -> $field("bar")), ExcludeId)
        ))
    }
    ...
    "plan with extra squash and flattening" in {
      ...
      plan(lp) must beWorkflow(
        chain[Workflow](
          $read(collection("db", "zips")),
          $project(
            reshape("__tmp12" -> $cond($and($lte($literal(Bson.Doc()), $$ROOT),
          ...
          $project(
            reshape(
              "__tmp14" -> $cond(
                $and($lte($literal(Bson.Arr(List())), $field("__tmp12", "loc")),
                     $lt($field("__tmp12", "loc"),
                         $literal(Bson.Binary.fromArray(Array[Byte]())))),
                $field("__tmp12", "loc"),
                $literal(Bson.Arr(List(Bson.Undefined)))
              ),
          ...
          $match(Selector.Doc(BsonField
            .Name("__tmp14") -> Selector.Regex("^.*MONT.*$", false, true, false, false))),
          $project(
            reshape(
              "__tmp16" -> $cond($and($lte($literal(Bson.Doc()), $field("__tmp15")),
                                      $lt($field("__tmp15"), $literal(Bson.Arr(List())))),
                                 $field("__tmp15"),
                                 $literal(Bson.Undefined))),
            IgnoreId
          ),
          $project(reshape("city" -> $field("__tmp16", "city")), IgnoreId),
          $group(grouped(), -\/(reshape("0" -> $field("city")))),
          $project(reshape("city" -> $field("_id", "0")), IgnoreId)
        ))
    }.pendingUntilFixed(notOnPar)
