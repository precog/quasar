...
trait MathLib extends Library {
  ...
  private val biReflexiveUnapply: Func.Untyper[nat._2] = partialUntyperV[nat._2] {
    case Type.Const(d) => success(Sized[IS](d.dataType, d.dataType))
    case t             => success(Sized[IS](t, t))
  }
  ...
  val Add = BinaryFunc(
    ...
    untyper[nat._2](t => Type.typecheck(Type.Timestamp ⨿ Type.Interval, t).fold(
      κ(t match {
        case Type.Const(d) => success(Sized[IS](d.dataType,  d.dataType))
        case Type.Int      => success(Sized[IS](Type.Int, Type.Int))
        case _             => success(Sized[IS](Type.Numeric, Type.Numeric))
      }),
      κ(success(Sized[IS](t, Type.Interval))))))
  ...
  val Power = BinaryFunc(
    ...
    Type.Numeric,
    Sized[IS](Type.Numeric, Type.Numeric),
    new Func.Simplifier {
      def apply[T[_[_]]: Recursive: Corecursive](orig: LogicalPlan[T[LogicalPlan]]) =
        orig match {
          case InvokeF(_, Sized(Embed(x), Embed(OneF()))) => x.some
  ...
  val Subtract = BinaryFunc(
    ...
    MathAbs,
    Sized[IS](MathAbs, MathAbs),
    new Func.Simplifier {
      def apply[T[_[_]]: Recursive: Corecursive](orig: LogicalPlan[T[LogicalPlan]]) =
        orig match {
          case InvokeF(_, Sized(Embed(x), Embed(ZeroF()))) => x.some
          case InvokeF(_, Sized(Embed(ZeroF()), x))        => Negate(Sized[IS](x)).some
          case _                                           => None
        }
    },
    (partialTyper[nat._2] {
    ...
    untyper[nat._2](t => Type.typecheck(Type.Temporal, t).fold(
      κ(Type.typecheck(Type.Interval, t).fold(
        ...
        κ(success(Sized[IS](Type.Temporal ⨿ Type.Interval, Type.Temporal ⨿ Type.Interval))))),
      κ(success(Sized[IS](t, Type.Interval))))))
  ...
  val Divide = BinaryFunc(
    ...
    untyper[nat._2](t => Type.typecheck(Type.Interval, t).fold(
      κ(t match {
        case Type.Const(d) => success(Sized[IS](d.dataType, d.dataType))
        case Type.Int      => success(Sized[IS](Type.Int, Type.Int))
        case _             => success(Sized[IS](MathRel, MathRel))
      }),
      κ(success(Sized[IS]((Type.Temporal ⨿ Type.Interval), MathRel))))))
  ...
  val Negate = UnaryFunc(
    ...
    untyper[nat._1] {
      case Type.Const(d) => success(Sized[IS](d.dataType))
      case t             => success(Sized[IS](t))
    })
