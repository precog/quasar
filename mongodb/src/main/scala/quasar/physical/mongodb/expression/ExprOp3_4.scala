...
object ExprOp3_4F {
  ...
  implicit val traverseBounds: Traverse[BoundsF] = new Traverse[BoundsF] {
    ...
    def traverseImpl[G[_], A, B](fa: BoundsF[A])(f: A => G[B])(
        implicit G: Applicative[G]): G[BoundsF[B]] =
      G.map(toList(fa).traverse(f))(unlist)
  }

  final case class $indexOfBytesF[A](string: A, substring: A, bounds: BoundsF[A])
      extends ExprOp3_4F[A]
  final case class $indexOfCPF[A](string: A, substring: A, bounds: BoundsF[A])
      extends ExprOp3_4F[A]
  ...
  implicit val equal: Delay[Equal, ExprOp3_4F] =
    new Delay[Equal, ExprOp3_4F] {
      def apply[A](eq: Equal[A]) = {
        ...
        Equal.equal {
          case ($indexOfBytesF(s1, t1, b1), $indexOfBytesF(s2, t2, b2)) =>
            s1 ≟ s2 && t1 ≟ t2 && b1 ≟ b2
          case ($indexOfCPF(s1, t1, b1), $indexOfCPF(s2, t2, b2)) =>
            s1 ≟ s2 && t1 ≟ t2 && b1 ≟ b2
          case ($splitF(s1, d1), $splitF(s2, d2)) => s1 ≟ s2 && d1 ≟ d2
          case ($strLenBytesF(s1), $strLenBytesF(s2)) => s1 ≟ s2
          case ($strLenCPF(s1), $strLenCPF(s2)) => s1 ≟ s2
          case ($substrBytesF(s1, i1, c1), $substrBytesF(s2, i2, c2)) =>
            s1 ≟ s2 && i1 ≟ i2 && c1 ≟ c2
  ...
  implicit val traverse: Traverse[ExprOp3_4F] = new Traverse[ExprOp3_4F] {
    def traverseImpl[G[_], A, B](fa: ExprOp3_4F[A])(f: A => G[B])(
        implicit G: Applicative[G]): G[ExprOp3_4F[B]] =
      fa match {
        case $indexOfBytesF(s, t, b) =>
          (f(s) |@| f(t) |@| Traverse[BoundsF].traverse(b)(f))($indexOfBytesF(_, _, _))
        case $indexOfCPF(s, t, b) =>
          (f(s) |@| f(t) |@| Traverse[BoundsF].traverse(b)(f))($indexOfCPF(_, _, _))
  ...
  implicit def ops[F[_]: Functor](implicit I: ExprOp3_4F :<: F): ExprOpOps.Aux[ExprOp3_4F, F] =
    new ExprOpOps[ExprOp3_4F] {
      type OUT[A] = F[A]
      ...
      def bson: Algebra[ExprOp3_4F, Bson] = {
        case $indexOfBytesF(s, t, b) =>
          Bson.Doc("$indexOfBytes" -> Bson.Arr(List(List(s, t), toList(b)).join))
        case $indexOfCPF(s, t, b) =>
          Bson.Doc("$indexOfCP" -> Bson.Arr(List(List(s, t), toList(b)).join))
  ...
  final class fixpoint[T, EX[_]: Functor](embed: EX[T] => T)(implicit I: ExprOp3_4F :<: EX) {
    ...
    def $substrBytes(string: T, start: T, index: T): T =
      convert($substrBytesF(string, start, index))
    def $substrCP(string: T, start: T, index: T): T = convert($substrCPF(string, start, index))
  }
}

object $indexOfBytes {
  def apply[EX[_], A](string: A, substring: A, bounds: Option[(A, Option[A])])(
      implicit I: ExprOp3_4F :<: EX): EX[A] =
