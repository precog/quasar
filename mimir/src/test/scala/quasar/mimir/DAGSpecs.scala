...
object DAGSpecs extends Specification with DAG with FNDummyModule {
  ...
  "dag decoration" should {
    ...
    "parse out reduce" in {
      ...
      result mustEqual Right(dag.Reduce(Reduction(Vector(), "count", 0x2000),
    ...
    "parse a single-level split" in {
      ...
      result must beLike {
        case Right(
            s @ dag.Split(dag.Group(2, Const(CTrue), UnfixedSolution(1, Const(CTrue))),
    ...
    "parse a bi-level split" in {
      ...
      result must beLike {
        case Right(
            s1 @ dag.Split(dag.Group(2, Const(CFalse), UnfixedSolution(1, Const(CTrue))),
    ...
    "accept split which reduces the stack" in {
      ...
      result must beLike {
        case Right(
            s @ dag.Split(dag.Group(2, Const(CNull), UnfixedSolution(1, Const(CTrue))),
    ...
    "continue processing beyond a filter" in {
      ...
      result mustEqual Right(
        Operate(Neg, Filter(IdentitySort, Const(CFalse)(line), Const(CTrue)(line))(line))(
          line))
    }
