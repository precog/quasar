...
trait JobManager[M[+ _]] { self =>
   ...
   * is returned.
   */
  def start(job: JobId, startedAt: LocalDateTime = LocalDateTime.now): M[Either[String, Job]]
   ...
   * the result.
   */
  def finish(job: JobId, finishedAt: LocalDateTime = LocalDateTime.now): M[Either[String, Job]]
