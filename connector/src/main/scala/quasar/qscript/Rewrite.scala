...
import scalaz.{:+: => _, Divide => _, _}, Inject.{reflexiveInjectInstance => _, _},
BijectionT._, Leibniz._, Scalaz._
...
class Rewrite[T[_[_]]: BirecursiveT: EqualT: ShowT: RenderTreeT] extends TTypes[T] {
  def rebuildArray[A: Show: RenderTree](funcs: List[FreeMapA[A]]): FreeMapA[A] = funcs match {
    ...
    case h :: t =>
      t.foldLeft(Free.roll(MFC(MakeArray[T, FreeMapA[A]](h))))((acc, e) =>
        Free.roll(MFC(ConcatArrays(acc, Free.roll(MFC(MakeArray(e)))))))
  }

  def rewriteShift(idStatus: IdStatus, repair: JoinFunc): Option[(IdStatus, JoinFunc)] =
    (idStatus ≟ IncludeId)
      .option[Option[(IdStatus, JoinFunc)]] {
  ...
  def elideNopQC[F[_]: Functor, G[_]: Functor](FtoG: F ~> G)(
      implicit QC: QScriptCore :<: F): QScriptCore[T[G]] => G[T[G]] = {
  ...
  class BranchUnification[F[_], A, B] private (
      val remap: Remap[A],
      val combine: Combine[F, A, B])

  object BranchUnification {
    def apply[F[_], A, B](remap: Remap[A])(
        combine: Combine[F, A, B]): BranchUnification[F, A, B] =
  ...
  def unifySimpleBranchesJoinSide[F[_], A](src: A, left: FreeQS, right: FreeQS)(
      rebase: FreeQS => A => Option[A])(
      implicit
      QC: QScriptCore :<: F,
      FI: Injectable.Aux[F, QScriptTotal]): BranchUnification[F, JoinSide, A] = {
    ...
    (left.resumeTwice, right.resumeTwice) match {
      ...
      case (-\/(l), -\/(r)) =>
        (l, r).uTraverse(m => FI.project(m) >>= QC.prj) collect {
  ...
  def unifySimpleBranchesHole[F[_], A](src: A, left: FreeQS, right: FreeQS)(
      rebase: FreeQS => A => Option[A])(
      implicit
      QC: QScriptCore :<: F,
      FI: Injectable.Aux[F, QScriptTotal]): BranchUnification[F, Hole, A] = {
    ...
    (left.resumeTwice, right.resumeTwice) match {
      ...
      case (\/-(SrcHole), \/-(SrcHole)) =>
        BranchUnification(jf => (jf.as(SrcHole): FreeMap).some)(func =>
          QC.inj(Map(src, func)).some)
      case (_, _) => NoneBranch
    }
  }

  def unifySimpleBranches[F[_], A](src: A, left: FreeQS, right: FreeQS, func: JoinFunc)(
      rebase: FreeQS => A => Option[A])(
      implicit
      QC: QScriptCore :<: F,
      FI: Injectable.Aux[F, QScriptTotal]): Option[F[A]] = {
  ...
  def unifySimpleBranchesCoEnv[F[_], A](src: A, l: FreeQS, r: FreeQS, combine: JoinFunc)(
      rebase: FreeQS => A => Option[A])(
      implicit
      QC: QScriptCore :<: F,
      FI: Injectable.Aux[F, QScriptTotal]): Option[CoEnv[Hole, F, A]] =
  ...
  def elideNopJoin[F[_], A](rebase: FreeQS => A => Option[A])(
      implicit QC: QScriptCore :<: F,
      FI: Injectable.Aux[F, QScriptTotal]): ThetaJoin[A] => Option[F[A]] = {
    case ThetaJoin(s, l, r, _, _, combine) =>
      unifySimpleBranches[F, A](s, l, r, combine)(rebase)(QC, FI)
    case _ => None
  }

  def compactLeftShift[F[_]: Functor, G[_]: Functor](FToG: PrismNT[G, F])(
      implicit QC: QScriptCore :<: F): QScriptCore[T[G]] => Option[F[T[G]]] = {
  ...
  private def applyNormalizations[F[_]: Traverse: Normalizable, G[_]: Traverse](
      ...
      QC: QScriptCore :<: F,
      TJ: ThetaJoin :<: F,
      FI: Injectable.Aux[F, QScriptTotal]): F[T[G]] => G[T[G]] =
  ...
  private def normalizeWithBijection[F[_]: Traverse: Normalizable, G[_]: Traverse, A](
      bij: Bijection[A, T[G]])(prism: PrismNT[G, F], rebase: FreeQS => T[G] => Option[T[G]])(
      implicit C: Coalesce.Aux[T, F, F],
      QC: QScriptCore :<: F,
      TJ: ThetaJoin :<: F,
      FI: Injectable.Aux[F, QScriptTotal]): F[A] => G[A] =
    fa => applyNormalizations[F, G](prism, rebase).apply(fa ∘ bij.toK.run) ∘ bij.fromK.run

  def normalize[F[_]: Traverse: Normalizable](
      implicit C: Coalesce.Aux[T, F, F],
      QC: QScriptCore :<: F,
      TJ: ThetaJoin :<: F,
      FI: Injectable.Aux[F, QScriptTotal]): F[T[F]] => F[T[F]] =
  ...
  def normalizeCoEnv[F[_]: Traverse: Normalizable](
      ...
      QC: QScriptCore :<: F,
      TJ: ThetaJoin :<: F,
      FI: Injectable.Aux[F, QScriptTotal]): F[Free[F, Hole]] => CoEnv[Hole, F, Free[F, Hole]] =
    normalizeWithBijection[F, CoEnv[Hole, F, ?], Free[F, Hole]](coenvBijection)(
      coenvPrism,
      rebaseTCo)
  ...
  def pathify[M[_]: Monad: MonadFsErr, IN[_]: Traverse, OUT[_]: Traverse](
      ...
      QC: QScriptCore :<: OUT,
      FI: Injectable.Aux[OUT, QScriptTotal]): T[IN] => M[T[OUT]] =
