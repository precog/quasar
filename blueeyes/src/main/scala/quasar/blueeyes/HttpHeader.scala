...
object HttpHeaderField {
  ...
  val ByName: Map[String, HttpHeaderField[_]] =
    All.map(v => (v.name.toLowerCase -> v)).toMap
...
object HttpHeader extends HttpHeaderImplicits {
  private val All = Map(
    HttpHeaderField.All.map(field => (field.name.toLowerCase, field)): _*)
...
object HttpHeaders {
  ...
  // Requests
  case class `Content-Length`(length: Long)
  ...
  implicit case object `Cache-Control` extends HttpHeaderField[`Cache-Control`] {
    override def parse(s: String) =
      Some(apply(CacheDirectives.parseCacheDirectives(s): _*))
  ...
  implicit case object Trailer extends HttpHeaderField[Trailer] {
    override def parse(s: String) =
      Some(apply(HttpHeaderField.parseAll(s, "trailer"): _*))
