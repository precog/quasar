...
final case class Grouped[EX[_]](value: ListMap[BsonField.Name, AccumOp[Fix[EX]]]) {
  ...
  def rewriteRefs(f: PartialFunction[DocVar, DocVar])(implicit ev0: ExprOpOps.Uni[EX],
                                                      ev1: Functor[EX]): Grouped[EX] =
...
object Grouped {
  ...
  implicit def GroupedRenderTree[EX[_]: Functor](
      implicit ev: ExprOpOps.Uni[EX]): RenderTree[Grouped[EX]] =
    new RenderTree[Grouped[EX]] {
      ...
      def render(grouped: Grouped[EX]) =
        NonTerminal(
          ...
          (grouped.value.map {
            case (name, expr) =>
              Terminal(
                "Name" :: GroupedNodeType,
                Some(name.bson.toJs.pprint(0) + " -> " + groupBson(expr).toJs.pprint(0)))
          }).toList
        )
    }
}
...
final case class Reshape[EX[_]](value: ListMap[BsonField.Name, Reshape.Shape[EX]]) {
  ...
  def toJs(implicit
    ...
    value
      .map {
        case (key, expr) =>
          key.asText -> expr.fold[PlannerError \/ JsFn](
            _.toJs,
            _.para(expression.toJs[Fix[EX], EX]))
      }
      .sequence
      .map { l =>
        JsFn(JsFn.defaultName, jscore.Obj(l.map {
          case (k, v) => jscore.Name(k) -> v(jscore.Ident(JsFn.defaultName))
        }))
      }

  @SuppressWarnings(Array("org.wartremover.warts.Recursion"))
  def bson(implicit ops: ExprOpOps.Uni[EX], ev: Functor[EX]): Bson.Doc =
    Bson.Doc(value.map {
  ...
  private def projectSeq(fs: NonEmptyList[BsonField.Name]): Option[Shape[EX]] =
    fs.foldLeftM[Option, Shape[EX]](-\/(this))(
      (rez, leaf) =>
        rez.fold(r =>
                   leaf match {
  ...
  @SuppressWarnings(Array("org.wartremover.warts.Recursion"))
  def rewriteRefs(applyVar: PartialFunction[DocVar, DocVar])(
      implicit
      ...
      value.transform((k, v) =>
        v.bimap(
          ...
          x =>
            (x match {
              case $include() =>
                (new ExprOpCoreF.fixpoint[Fix[EX], EX](_.embed)).$var(DocField(k))
              case _ => x
  ...
  def get(field: BsonField): Option[Shape[EX]] =
    field.flatten.foldLeftM[Option, Shape[EX]](-\/(this))((rez, elem) =>
      rez.fold(_.value.get(elem), κ(None)))

  @SuppressWarnings(Array("org.wartremover.warts.Recursion"))
  def find(expr: Fix[EX]): Option[BsonField] =
    value.foldLeft[Option[BsonField]](None) {
      case (acc, (field, value)) =>
        acc.orElse(
          value.fold(_.find(expr).map(field \ _),
  ...
  def set(field: BsonField, newv: Shape[EX]): Reshape[EX] = {
    def getOrDefault(o: Option[Shape[EX]]): Reshape[EX] =
      o.cata(_.fold(ι, κ(Reshape.emptyDoc[EX])), Reshape.emptyDoc[EX])

    @SuppressWarnings(
      Array("org.wartremover.warts.NonUnitStatements", "org.wartremover.warts.Recursion"))
...
object Reshape {
  ...
  def getAll[EX[_]](r: Reshape[EX]): List[(BsonField, Fix[EX])] = {
    def getAll0(f0: BsonField, e: Shape[EX]) =
      e.fold(r => getAll(r).map { case (f, e) => (f0 \ f) -> e }, e => (f0 -> e) :: Nil)
  ...
  def mergeMaps[A, B](lmap: ListMap[A, B], rmap: ListMap[A, B]): Option[ListMap[A, B]] =
  ...
  def merge[EX[_]](r1: Reshape[EX], r2: Reshape[EX]): Option[Reshape[EX]] = {
    ...
    if ((lmap.keySet & rmap.keySet).forall(k => lmap.get(k) == rmap.get(k)))
      Some(Reshape.setAll(r1, Reshape.getAll(r2).map(t => t._1 -> \/-(t._2))))
    else None
  }
  ...
  @SuppressWarnings(Array("org.wartremover.warts.Recursion"))
  private[mongodb] def renderReshape[EX[_]: Functor](shape: Reshape[EX])(
      implicit ops: ExprOpOps.Uni[EX]): List[RenderedTree] = {
    def renderField(field: BsonField, value: Shape[EX]) = {
      val (label, typ) = field.bson.toJs.pprint(0) -> "Name"
      value match {
        case -\/(shape) =>
          NonTerminal(typ :: ProjectNodeType, Some(label), renderReshape(shape))
        case \/-(exprOp) =>
          Terminal(typ :: ProjectNodeType,
                   Some(label + " -> " + exprOp.cata(ops.bson).toJs.pprint(0)))
      }
    }
