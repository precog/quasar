...
trait DefaultDecomposers {
  ...
  implicit val DoubleDecomposer: Decomposer[Double] = new Decomposer[Double] {
    def decompose(tvalue: Double): JValue =
      JNum(BigDecimal(tvalue, MathContext.UNLIMITED))
  }
  ...
  implicit val DateDecomposer: Decomposer[JDate] = new Decomposer[JDate] {
    def decompose(date: JDate): JValue =
      JNum(BigDecimal(date.getTime, MathContext.UNLIMITED))
  }

  implicit def OptionDecomposer[T](
      implicit decomposer: Decomposer[T]): Decomposer[Option[T]] =
  ...
  implicit def Tuple3Decomposer[T1, T2, T3](
      ...
      decomposer3: Decomposer[T3]): Decomposer[(T1, T2, T3)] =
    new Decomposer[(T1, T2, T3)] {
      def decompose(tvalue: (T1, T2, T3)) =
        JArray(
          decomposer1(tvalue._1) :: decomposer2(tvalue._2) :: decomposer3(tvalue._3) :: Nil)
    }
  ...
  implicit def SetDecomposer[T](
      implicit elementDecomposer: Decomposer[T]): Decomposer[Set[T]] =
    new Decomposer[Set[T]] {
      def decompose(tvalue: Set[T]): JValue =
        JArray(tvalue.toList.map(elementDecomposer.decompose _))
    }

  implicit def SeqDecomposer[T](
      implicit elementDecomposer: Decomposer[T]): Decomposer[Seq[T]] =
  ...
  implicit def MapDecomposer[K, V](
    ...
    def decompose(tvalue: Map[K, V]): JValue =
      SeqDecomposer(Tuple2Decomposer(keyDecomposer, valueDecomposer))
        .decompose(tvalue.toList)
  }
