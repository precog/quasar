...
class AggLibSpec extends mutable.Specification with ScalaCheck with ValidationMatchers {
  ...
  "Arbitrary" should {
    "type a nonempty constant set to the constant first value" ! prop { xs: NonEmptyList[Data] =>
      Arbitrary.tpe(Sized[IS](Type.Const(Data.Set(xs.list.toList)))) must beSuccessful(Type.Const(xs.head))
    }

    "error when applied to an empty constant set" >> {
      Arbitrary.tpe(Sized[IS](Type.Const(Data.Set(Nil)))) must beFailing
    }
  }
  ...
  "Avg" should {
    "type a constant Int set to the constant average of values" ! prop { n: BigInt =>
      ...
      Avg.tpe(Sized[IS](Type.Const(s))) must beSuccessful(Type.Const(Data.Dec(BigDecimal(n + 1))))
    }

    "type a constant Dec set to the constant average of values" ! prop { n0: Double =>
      val n = BigDecimal(n0)
      val s = Data.Set(List(Data.Dec(n), Data.Dec(n / 2.0)))
      Avg.tpe(Sized[IS](Type.Const(s))) must beSuccessful(Type.Const(Data.Dec(n * 0.75)))
    }
    ...
    "type a constant Interval set to the constant average of values" ! prop { n: Int =>
      ...
      Avg.tpe(Sized[IS](Type.Const(s))) must beSuccessful(Type.Const(Data.Dec(n.toLong + 1)))
    }

    "error when applied to an empty constant set" >> {
      Avg.tpe(Sized[IS](Type.Const(Data.Set(Nil)))) must beFailing
    }
  }

  "Count" should {
    "type a constant set as the constant length" ! prop { xs: List[Int] =>
      Count.tpe(Sized[IS](Type.Const(Data.Set(xs.map(Data.Int(_)))))) must beSuccessful(Type.Const(Data.Int(xs.length)))
    }
  }

  "Max" should {
    "type a constant set as the constant max value" ! prop { xs: NonEmptyList[Int] =>
      Max.tpe(Sized[IS](Type.Const(Data.Set(xs.list.toList.map(Data.Int(_)))))) must
        beSuccessful(Type.Const(Data.Int(xs.maximum1)))
    }

    "error when applied to an empty constant set" >> {
      Max.tpe(Sized[IS](Type.Const(Data.Set(Nil)))) must beFailing
    }
  }

  "Min" should {
    "type a constant set as the constant min value" ! prop { xs: NonEmptyList[Int] =>
      Min.tpe(Sized[IS](Type.Const(Data.Set(xs.list.toList.map(Data.Int(_)))))) must
        beSuccessful(Type.Const(Data.Int(xs.minimum1)))
    }

    "error when applied to an empty constant set" >> {
      Min.tpe(Sized[IS](Type.Const(Data.Set(Nil)))) must beFailing
    }
  }
  ...
  "Sum" should {
    "type a constant Int set to the constant Int sum of values" ! prop { xs: NonEmptyList[BigInt] =>
      ...
      Sum.tpe(Sized[IS](Type.Const(s))) must beSuccessful(Type.Const(Data.Int(xs.list.toList.sum)))
    }
    ...
    "type a constant Dec set to the constant Dec sum of values" ! prop { xs: NonEmptyList[Double] =>
      ...
      Sum.tpe(Sized[IS](Type.Const(s))) must beSuccessful(Type.Const(Data.Dec(ys.sum)))
    }
    ...
    "type a constant Interval set to the constant Interval sum of values" ! prop { xs: NonEmptyList[Int] =>
      ...
      Sum.tpe(Sized[IS](Type.Const(s))) must beSuccessful(Type.Const(Data.Interval(Duration.ofMillis(milliss.sum))))
    }

    "type an empty constant set to constant Int(0)" >> {
      Sum.tpe(Sized[IS](Type.Const(Data.Set(Nil)))) must beSuccessful(Type.Const(Data.Int(0)))
    }
  }
}
