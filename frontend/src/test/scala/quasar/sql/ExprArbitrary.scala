...
trait ExprArbitrary {
  ...
  implicit val exprShrink: Shrink[Fix[Sql]] = Shrink(expr =>
    (expr.project match {
  ...
  private def selectGen(depth: Int): Gen[Fix[Sql]] =
    for {
  ...
  private def projGen: Gen[Proj[Fix[Sql]]] =
    Gen.oneOf(
      ...
      exprGen(1) >>= (
          x =>
            Gen.oneOf(Gen.const(Proj(x, None)),
                      Gen.oneOf(Gen.alphaChar.map(_.toString),
    ...
    if (depth <= 0) simple
      ...
      Gen.frequency(
        ...
        1 -> (relationGen(depth - 1) ⊛ relationGen(depth - 1) ⊛
          ...
          exprGen(1))(JoinRelation(_, _, _, _))
      )
  }

  private def groupByGen(depth: Int): Gen[GroupBy[Fix[Sql]]] =
    (smallNonEmptyListOf(exprGen(depth)) ⊛ Gen.option(exprGen(depth)))(GroupBy(_, _))
  ...
  private def simpleExprGen: Gen[Fix[Sql]] =
    Gen.frequency(
      ...
      10 -> Gen.oneOf(Gen.alphaChar.map(_.toString),
                      Gen.const("name, address"),
                      Gen.const("q: \"a\"")) ∘
        (IdentR(_)),
      1 -> InvokeFunctionR(CIName("timestamp"),
                           List(StringLiteralR(Instant.now.toString))),
      1 -> Gen
        .choose(0L, 10000000000L)
        .map(millis =>
          InvokeFunctionR(CIName("interval"),
                          List(StringLiteralR(Duration.ofMillis(millis).toString)))),
  ...
  private def complexExprGen(depth: Int): Gen[Fix[Sql]] =
    Gen.frequency(
      ...
      1 -> exprGen(depth) ∘
        (arg =>
          InvokeFunctionR(CIName("like"),
                          List(arg, StringLiteralR("B%"), StringLiteralR("")))),
