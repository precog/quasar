...
private[mongodb] abstract class WorkflowExecutor[F[_]: Monad, C] {
  ...
  def evaluate(
    ...
    task(workflow) match {
      ...
      case PipelineTask(ReadTask(coll), pipeline) =>
        asM(evalPipeline(coll, pipeline)).map(_ bimap (List(_), WorkflowCursor(_, none)))
  ...
  private val asM: F ~> M =
    new (F ~> M) {
      def apply[A](fa: F[A]) =
        (fa.liftM[StateT[?[_], Long, ?]].liftM[ReaderT[?[_], CollectionName, ?]]: G[A])
  ...
  private def execute0(wt: WorkflowTask, out: Collection)(
      implicit ev: WorkflowOpCoreF :<: WorkflowF): N[Collection] = {
    ...
    wt match {
      case PureTask(doc @ Bson.Doc(_)) =>
        asM(insert(out, List(doc))).liftM[TempsT].as(out)
      ...
      case PipelineTask(source, pipeline) =>
        for {
          ...
          _ <- asM(
            aggregate(
              src,
              pipeline ::: List(PipelineOp($OutF((), out.collection).shapePreserving))))
            .liftM[TempsT]
        } yield out
      ...
      case MapReduceTask(source, mr, oa) =>
        for {
          ...
          _ <- asM(mapReduce(src, outputCollection(out, act), mr)).liftM[TempsT]
        } yield out

      case FoldLeftTask(rd @ ReadTask(_), _) =>
        invalidTask(rd, "FoldLeft from simple read").raiseError[M, Collection].liftM[TempsT]
      ...
      case FoldLeftTask(head, tail) =>
        for {
          ...
          _ <- tail.traverse_[N] {
            ...
            case other =>
              invalidTask(other, "un-mergable FoldLeft input").raiseError[M, Unit].liftM[TempsT]
          }
        } yield h
    }
  }
  ...
  private def evalPipeline(src: Collection, pipeline: Pipeline): F[Bson \/ C] = {
    ...
    pl match {
      ...
      case (
          List(
            PipelineOpCore($MatchF((), sel)),
            Projectable(bson),
            PipelineOpCore($SortF((), keys))),
          Nil) =>
  ...
  private def outputCollection(c: Collection, a: Action) =
    OutputCollection(c.collection, Some(ActionedOutput(a, Some(c.database), None)))
}
