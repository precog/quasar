...
/** A data-level transformation.
  */
@Lenses final case class Map[T[_[_]], A](src: A, f: FreeMap[T]) extends QScriptCore[T, A]
...
/** Flattens nested structure, converting each value into a data set, which are
  ...
  * original (`x`) dataset.
  */
@Lenses final case class LeftShift[T[_[_]], A](src: A,
                                               struct: FreeMap[T],
                                               idStatus: IdStatus,
                                               repair: JoinFunc[T])
...
@Lenses final case class Sort[T[_[_]], A](src: A,
                                          bucket: FreeMap[T],
...
/** Creates a new dataset that contains the elements from the datasets created
  ...
  */
@Lenses final case class Union[T[_[_]], A](src: A, lBranch: FreeQS[T], rBranch: FreeQS[T])
    extends QScriptCore[T, A]
...
@Lenses final case class Subset[T[_[_]], A](src: A,
                                            from: FreeQS[T],
                                            op: SelectionOp,
                                            count: FreeQS[T])
    extends QScriptCore[T, A]
...
/** A placeholder value that can appear in plans, but will never be referenced
  ...
  */
@Lenses final case class Unreferenced[T[_[_]], A]() extends QScriptCore[T, A]
...
object QScriptCore {
    ...
    new Delay[Equal, QScriptCore[T, ?]] {
      ...
      def apply[A](eq: Equal[A]) =
        Equal.equal {
          ...
          case (Subset(a1, f1, s1, c1), Subset(a2, f2, s2, c2)) =>
            eq.equal(a1, a2) && f1 ≟ f2 && s1 ≟ s2 && c1 ≟ c2
  ...
  implicit def traverse[T[_[_]]]: Traverse[QScriptCore[T, ?]] =
    new Traverse[QScriptCore[T, ?]] {
      def traverseImpl[G[_]: Applicative, A, B](fa: QScriptCore[T, A])(f: A => G[B]) =
        fa match {
      ...
      def apply[A](RA: RenderTree[A]): RenderTree[QScriptCore[T, A]] =
        new RenderTree[QScriptCore[T, A]] {
          ...
          def render(v: QScriptCore[T, A]) =
            v match {
              case Map(src, f) =>
                NonTerminal("Map" :: nt, None, RA.render(src) :: f.render :: Nil)
              case LeftShift(src, struct, id, repair) =>
                NonTerminal("LeftShift" :: nt,
                            None,
                            RA.render(src) ::
              ...
              case Sort(src, bucket, order) =>
                NonTerminal(
                  ...
                  RA.render(src) :: nested("Bucket", bucket) ::
                    NonTerminal("Order" :: "Sort" :: nt, None, order.map {
                    case (x, dir) =>
              ...
              case Union(src, lBranch, rBranch) =>
                NonTerminal("Union" :: nt,
                            None,
                            List(RA.render(src),
                                 nested("LeftBranch", lBranch),
                                 nested("RightBranch", rBranch)))
              case Filter(src, func) =>
                NonTerminal("Filter" :: nt, None, List(RA.render(src), func.render))
              case Subset(src, from, sel, count) =>
                NonTerminal(
                  "Subset" :: nt,
                  sel.shows.some,
                  List(RA.render(src), nested("From", from), nested("Count", count)))
              case Unreferenced() =>
      ...
      def mergeSrcs(left: FreeMap[T],
        ...
        (p1, p2) match {
          case (Unreferenced(), Unreferenced()) =>
            SrcMerge[QScriptCore[IT, ExternallyManaged], FreeMap[IT]](Unreferenced(),
                                                                      HoleF,
                                                                      HoleF).some
          case (Map(_, m1), Map(_, m2)) =>
            // TODO: optimize cases where one side is a subset of the other
            val (mf, lv, rv) = concat(m1 >> left, m2 >> right)
            SrcMerge[QScriptCore[IT, ExternallyManaged], FreeMap[IT]](Map(Extern, mf),
                                                                      lv,
                                                                      rv).some
          ...
          case (LeftShift(_, struct1, id1, repair1),
            ...
            val idAccess: IdStatus => JoinFunc[IT] = {
              case ExcludeId =>
                Free.roll(
                  MapFuncsCore.ProjectIndex[IT, JoinFunc[IT]](
                    RightSideF[IT],
            ...
            (lFunc ≟ rFunc).option {
              ...
              (id1 ≟ id2).fold(constructMerge(RightSideF, RightSideF),
          ...
          case (Sort(s1, b1, o1), Sort(_, b2, o2)) =>
            ...
            (lBucket ≟ rBucket && lOrder ≟ rOrder)
              .option(SrcMerge(Sort(s1, lBucket, lOrder), left, right))
          ...
          case (Subset(s1, f1, o1, c1), Subset(_, f2, o2, c2)) =>
            ...
            (from1 ≟ from2 && o1 ≟ o2 && count1 ≟ count2)
              .option(SrcMerge(Subset(s1, from1, o1, count1), HoleF, HoleF))
          ...
          case (Union(s1, l1, r1), Union(_, l2, r2)) =>
            ...
            (left1 ≟ left2 && right1 ≟ right2)
              .option(SrcMerge(Union(s1, left1, right1), HoleF, HoleF))
