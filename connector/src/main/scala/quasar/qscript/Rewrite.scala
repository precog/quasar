...
class Rewrite[T[_[_]]: BirecursiveT: EqualT] extends TTypes[T] {
  ...
  def shiftRead[F[_]: Functor, G[_]: Traverse](implicit QC: QScriptCore :<: G,
                                               TJ: ThetaJoin :<: G,
  ...
  def unifySimpleBranches[F[_], A](src: A, l: FreeQS, r: FreeQS, combine: JoinFunc)(
      ...
      case (-\/(m1), -\/(m2)) =>
        (FI.project(m1) >>= QC.prj, FI.project(m2) >>= QC.prj) match {
          ...
          case (Some(Map(\/-(SrcHole), mf1)),
                Some(LeftShift(-\/(values), struct, status, repair))) =>
          ...
          case (Some(LeftShift(-\/(values), struct, status, repair)),
                Some(Map(\/-(SrcHole), mf2))) =>
            FI.project(values) >>= QC.prj match {
  ...
  private def normalizeWithBijection[F[_]: Traverse: Normalizable, G[_]: Traverse, A](
      bij: Bijection[A, T[G]])(prism: PrismNT[G, F],
                               rebase: FreeQS => T[G] => Option[T[G]])(
      implicit C: Coalesce.Aux[T, F, F],
      QC: QScriptCore :<: F,
      TJ: ThetaJoin :<: F,
    ...
    fa =>
      applyNormalizations[F, G](prism, rebase)
  ...
  def normalizeCoEnv[F[_]: Traverse: Normalizable](implicit C: Coalesce.Aux[T, F, F],
                                                   QC: QScriptCore :<: F,
                                                   TJ: ThetaJoin :<: F,
