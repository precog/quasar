...
object SparkElastic extends SparkCore with ManagedWriteFile[AFile] with ChrootedInterpreter {
  ...
  def toLowerLevel[S[_]](sc: SparkContext, config: ElasticConfig)(
      implicit
      S0: Task :<: S,
      ...
      (genState, rddStates, readCursors, writeCursors) =>
        {
          ...
          val interpreter: Eff ~> S =
            (details.interpreter[Temp] andThen temp andThen injectNT[Task, S]) :+:
              (KeyValueStore.impl.fromTaskRef[WriteHandle, AFile](writeCursors) andThen injectNT[
              Task,
              S]) :+:
              (KeyValueStore.impl.fromTaskRef[ReadHandle, SparkCursor](readCursors) andThen injectNT[
              Task,
              S]) :+:
  ...
  object details {
    ...
    private def parseIndex(adir: ADir) =
      posixCodec.unsafePrintPath(adir).replace("/", "") // TODO_ES handle invalid paths

    private def fromFile(sc: SparkContext, file: AFile): Task[RDD[Data]] = Task.delay {
      sc.esJsonRDD(file2ES(file).shows)
        .map(_._2)
    ...
    def fileExists[S[_]](f: AFile)(
        implicit
        E: ElasticCall.Ops[S]): Free[S, Boolean] = E.typeExists(file2ES(f))

    def listContents[S[_]](adir: ADir)(
        implicit
        E: ElasticCall.Ops[S]): FileSystemErrT[Free[S, ?], Set[PathSegment]] = {
  ...
  object ElasticManageFileModule extends SparkCoreManageFileModule {
    ...
    def moveFile(sf: AFile, df: AFile): Free[Eff, Unit] =
      for {
    ...
    def moveDir(sd: ADir, dd: ADir): Free[Eff, Unit] = {
      ...
      def calculateDestinationIndex(index: String): String = {
        val destinationPath = posixCodec.unsafePrintPath(dd) ++ index.diff(
          posixCodec.unsafePrintPath(sd))
    ...
    def delete(path: APath): Backend[Unit] =
      refineType(path).fold(d => deleteDir(d), f => deleteFile(f)).liftB.unattempt
    ...
    private def deleteFile(file: AFile): Free[Eff, FileSystemError \/ Unit] =
      for {
        ...
        res <- if (exists) elasticCallOps.deleteType(indexType).map(_.right)
        else pathErr(pathNotFound(file)).left[Unit].point[Free[Eff, ?]]
      } yield res
    ...
    private def deleteDir(dir: ADir): Free[Eff, FileSystemError \/ Unit] =
      for {
        ...
        index = dir2Index(dir)
        result <- if (indices.isEmpty) pathErr(pathNotFound(dir)).left[Unit].point[Free[Eff, ?]]
        else {
          indices
            .filter(_.startsWith(index))
            .map(elasticCallOps.deleteIndex(_))
            .sequence
