...
object CoreMap extends Serializable {
  ...
  def changeFreeMap[T[_[_]]: BirecursiveT](f: FreeMap[T]): PlannerError \/ (Data => Data) =
    f.cataM(interpretM(κ(ι[Data].right[PlannerError]), change[T, Data]))

  def changeJoinFunc[T[_[_]]: BirecursiveT](
      f: JoinFunc[T]): PlannerError \/ ((Data, Data) => Data) =
    f.cataM(
        interpretM[PlannerError \/ ?, MapFunc[T, ?], JoinSide, ((Data, Data)) => Data](
          (js: JoinSide) =>
            (js match {
  ...
  def changeReduceFunc[T[_[_]]: BirecursiveT](
      f: Free[MapFunc[T, ?], ReduceIndex]): PlannerError \/ ((List[Data], List[Data]) => Data) =
    f.cataM(
        interpretM(
          _.idx.fold[((List[Data], List[Data])) => Data](i => _._1(i), i => _._2(i)).right,
          change[T, (List[Data], List[Data])]))
      .map(f => (l: List[Data], r: List[Data]) => f((l, r)))

  def change[T[_[_]]: BirecursiveT, A]: AlgebraM[PlannerError \/ ?, MapFunc[T, ?], A => Data] =
  ...
  private def substring(dStr: Data, dFrom: Data, dCount: Data): Data =
    (dStr, dFrom, dCount) match {
      case (Data.Str(str), Data.Int(from), Data.Int(count)) =>
        \/.fromTryCatchNonFatal(Data.Str(StringLib.safeSubstring(str, from.toInt, count.toInt)))
          .fold(κ(Data.NA), ι)
      case _ => undefined
    }
    ...
    (dStr, dDelim) match {
      case (Data.Str(str), Data.Str(delim)) =>
        \/.fromTryCatchNonFatal(
          Data.Arr(str.split(Regex.quote(delim), -1).toList.map(Data.Str(_))))
          .fold(κ(Data.NA), ι)
      case _ => undefined
    }
