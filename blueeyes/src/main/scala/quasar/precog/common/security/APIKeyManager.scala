...
trait APIKeyManager[M[+ _]] extends Logging { self =>
  ...
  def newStandardAPIKeyRecord(accountId: AccountId,
                              name: Option[String] = None,
  ...
  def findValidGrant(grantId: GrantId,
                     at: Option[LocalDateTime] = None): M[Option[Grant]] =
    ...
    validGrants(issuerKey, expiration).flatMap { grants =>
      if (!Grant.implies(grants, perms, expiration)) none[Grant].point[M]
        ...
        if (minimized.isEmpty) {
          ...
          createGrant(name, description, issuerKey, minimized, perms, expiration) map {
            some
          }
        }
      }
  ...
  def newAPIKeyWithGrants(name: Option[String],
                          description: Option[String],
                          issuerKey: APIKey,
  ...
  def hasCapability(apiKey: APIKey,
                    perms: Set[Permission],
                    at: Option[LocalDateTime] = None): M[Boolean] =
