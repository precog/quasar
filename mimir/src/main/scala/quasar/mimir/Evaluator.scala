  ...
  abstract class EvaluatorLike[N[+ _]](N0: Monad[N])(implicit mn: M ~> N, nm: N ~> M)
    ...
    def eval(graph: DepGraph, ctx: EvaluationContext, optimize: Boolean): N[Table] = {
        ...
        case IntersectBucketSpec(left, right) =>
          for {
            ...
            for {
              ...
              GroupingAlignment(keySpec, keySpec, leftRes, rightRes, GroupingSpec.Intersection)
          }
      ...
      def resolveLowLevelGroup(
          ...
          forest: BucketSpec,
          splits: Map[Identifier, Int => N[Table]]): StateT[N, EvaluatorState, GroupKeySpec] =
        forest match {
          case UnionBucketSpec(left, right) =>
            for {
      ...
      def prepareEval(
          graph: DepGraph,
          splits: Map[Identifier, Int => N[Table]]): StateT[N, EvaluatorState, PendingTable] = {
        ...
        def cross(graph: DepGraph, left: DepGraph, right: DepGraph, hint: Option[CrossOrder])(
          ...
          (prepareEval(left, splits) |@| prepareEval(right, splits)).tupled flatMap {
            case (ptLeft, ptRight) =>
              ...
              val resultM =
                Table.cross(lTable.compact(valueSpec), rTable.compact(valueSpec), hint)(
                  crossSpec)
        ...
        def evalNotTransSpecable(graph: DepGraph): StateT[N, EvaluatorState, PendingTable] =
          graph match {
                  ...
                  .fold(
                    {
                      ...
                      case ResourceError.PermissionsError(message) =>
                        report.warn(graph.loc, message) >> Table.empty.point[N]
                      case fatal =>
                        report.error(graph.loc, "Fatal error while loading dataset") >> report
            ...
            case dag.RelativeLoad(parent, jtpe) =>
              for {
                ...
                fullPrefix = prefixStr.replaceAll("([^/])$", "$1/") + midStr.replaceAll(
                  "([^/])$",
                  "$1/")
              ...
              val joined: StateT[N, EvaluatorState, (Morph1Apply, PendingTable)] =
                mor.alignment match {
                  ...
                  case MorphismAlignment.Match(morph1) if areJoinable(left, right) =>
                    ((transState liftM mn(morph1)) |@| join(graph, left, right, IdentitySort)(
                      spec)).tupled
                  ...
                  case MorphismAlignment.Match(morph1) =>
                    ...
                    ((transState liftM mn(morph1)) |@| cross(graph, left, right, Some(hint))(
                      spec)).tupled
            ...
            case r @ dag.Reduce(red, parent) =>
              for {
                ...
                result <- transState liftM mn(
                  red(
                    pendingTable.table.transform(DerefObjectStatic(liftedTrans, paths.Value))))
      ...
      resultTable map {
        _ paged yggConfig.maxSliceSize compact DerefObjectStatic(Leaf(Source), paths.Value)
      }
    }
    ...
    private def simpleJoin(
        left: Table,
        right: Table)(leftKey: TransSpec1, rightKey: TransSpec1, spec: TransSpec2): Table = {
      val emptySpec = trans.ConstLiteral(CEmptyArray, Leaf(Source))
      val result =
        left.cogroup(leftKey, rightKey, right)(emptySpec, emptySpec, trans.WrapArray(spec))
