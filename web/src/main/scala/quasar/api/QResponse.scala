...
@Lenses
final case class QResponse[S[_]](
    status: Status,
    headers: Headers,
    body: Process[Free[S, ?], ByteVector]) {
  import QResponse.{HttpResponseStreamFailureException, PROCESS_EFFECT_THRESHOLD_BYTES}
  ...
  def toHttpResponseF(i: Free[S, ?] ~> ResponseOr): Task[Response] = {
    val failTask: ResponseOr ~> Task = new (ResponseOr ~> Task) {
      def apply[A](ror: ResponseOr[A]) =
        ror
          .fold(resp => Task.fail(new HttpResponseStreamFailureException(resp)), _.point[Task])
          .join
    }
...
object QResponse {
  ...
  def json[A: EncodeJson, S[_]](status: Status, a: A): QResponse[S] =
    string[S](status, a.asJson.pretty(minspace))
      .modifyHeaders(_.put(`Content-Type`(MediaType.`application/json`, Some(Charset.`UTF-8`))))

  def response[S[_], A](status: Status, a: A)(
      implicit E: EntityEncoder[A],
  ...
  def streaming[S[_], A](
      p: Process[Free[S, ?], A])(implicit E: EntityEncoder[A], S0: Task :<: S): QResponse[S] =
    QResponse(
      Ok,
      E.headers,
