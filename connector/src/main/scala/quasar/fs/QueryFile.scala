...
object QueryFile {
  ...
  def convertAndNormalize[T[_[_]]: BirecursiveT: EqualT: ShowT,
                          QS[_]: Traverse: Normalizable](lp: T[LogicalPlan])(
      eval: QS[T[QS]] => QS[T[QS]])(implicit
    ...
    lp.transCata[T[LogicalPlan]](orOriginal(optimizer.elideLets))
      ...
      .map(
        target =>
          QC.inj((transform.reifyResult(target.ann, target.value)))
            .embed
            .transCata[T[QS]](eval))
  }

  def simplifyAndNormalize[T[_[_]]: BirecursiveT: EqualT: ShowT,
                           IQS[_]: Functor,
  ...
  private type QScriptInternal[T[_[_]], A] =
    (QScriptCore[T, ?] :\: ProjectBucket[T, ?] :\: ThetaJoin[T, ?] :/: Const[DeadEnd, ?])#M[
      A]
  ...
  /** This is a stop-gap function that QScript-based backends should use until
    ...
    */
  def convertToQScript[T[_[_]]: BirecursiveT: EqualT: RenderTreeT: ShowT,
                       QS[_]: Traverse: Normalizable](lp: T[LogicalPlan])(
      implicit
      CQ: Coalesce.Aux[T, QS, QS],
      PA: PruneArrays[QS],
  ...
  def convertToQScriptRead[T[_[_]]: BirecursiveT: EqualT: RenderTreeT: ShowT,
                           M[_]: Monad,
                           QS[_]: Traverse: Normalizable](
      listContents: DiscoverPath.ListContents[M])(lp: T[LogicalPlan])(
      implicit
    ...
    type InterimQS[A] =
      (QScriptCore[T, ?] :\: ProjectBucket[T, ?] :\: ThetaJoin[T, ?] :\: Const[
        Read[ADir],
        ?] :/: Const[Read[AFile], ?])#M[A]
    ...
    convertAndNormalize[T, QScriptInternal[T, ?]](lp)(rewrite.normalize)
      .fold(perr =>
              merr.raiseError(
                FileSystemError.planningFailed(lp.convertTo[Fix[LogicalPlan]], perr)),
            _.point[M])
    ...
    new RenderTree[QueryFile[A]] {
      def render(qf: QueryFile[A]) = qf match {
        ...
        case ListContents(dir) =>
          NonTerminal(List("ListContents"), None, List(dir.render))
