...
object data {
  ...
  def encodeXml[F[_]: MonadErrMsgs](data: Data): F[Elem] = {
    ...
    def ejsElem(name: QName, tpe: DataType, ns: NamespaceBinding, children: Seq[Node]): Elem =
      Elem(
        name.prefix.map(_.shows).orNull,
    ...
    def wrappedKey(unwrappedLabel: String): (QName, Option[Attribute]) = {
      (
        ejsonEncodedName,
        Attribute(ejsonNs.prefix.shows, ejsonEncodedAttr.localPart.shows, unwrappedLabel, Null).some)
    }
    ...
    encodeXml0(rootElem, inner)(ejsonEjson)(data).fold(_.raiseError[F, Elem], _.point[F])
  }
    ...
    val decodeObject: JsonObject => F[Data] = {
      ...
      case EJsonTyped(DT.Interval, ivl) =>
        ivl.string
          ...
          .cata(_.point[F], error(s"Expected an ISO-8601 formatted duration, found: $ivl"))

      case EJsonTyped(DT.Time, t) =>
        t.string
          .flatMap(isoLocalTime.getOption(_))
          .map(Data._time(_))
          .cata(_.point[F], error(s"Expected an ISO-8601 formatted local time, found: $t"))
      ...
      case other =>
        other.toList traverse { case (k, v) => decodeJson[F](v) strengthL k } map (Data.Obj(
          _: _*))
    }
  ...
  def decodeXml[F[_]: MonadErrMsgs](recover: Node => F[Data])(elem: Elem): F[Option[Data]] = {
    ...
    elem match {
      case DataNode(_, _) =>
        (decodeXml0(elem): F[V[Data]]) flatMap (_.fold(
