...
class Merge[T[_[_]]: BirecursiveT: EqualT: ShowT: RenderTreeT] extends TTypes[T] {
  ...
  private def delinearizeTargets[G[_]: Functor, A]: ElgotCoalgebra[Hole \/ ?, G, List[G[A]]] = {
  ...
  private def zipper[G[_]](implicit mergeable: Mergeable.Aux[T, G])
    : ElgotCoalgebra[ZipperAcc[G] \/ ?, ListF[G[EM], ?], (ZipperSides, ZipperTails[G])] = {
  ...
  private def makeZipper[S[_[_]], G[_]: Traverse, H[_]](left: S[H], right: S[H])(
    ...
    elgot(
      (
        ...
        ZipperTails(makeList[S, G, H](left), makeList[S, G, H](right))))(
      consZipped[G],
      zipper[G])
    ...
    (left ≟ right).fold(
      SrcMerge(left, HoleF[T], HoleF[T]).right[PlannerError], {
        ...
        val baseSrc: Option[CoEnvQS[FreeQS]] =
          rewrite.unifySimpleBranchesCoEnv[QScriptTotal, FreeQS](
            src,
            lBranch,
            rBranch,
            combine)(rebase0)
        ...
        baseSrc.cata(
          src => SrcMerge(src.embed, lacc, racc).right[PlannerError],
  ...
  def tryMergeBranches(rewrite: Rewrite[T])(
      left1: FreeQS,
      right1: FreeQS,
      left2: FreeQS,
      right2: FreeQS): PlannerError \/ (SrcMerge[FreeQS, FreeMap], SrcMerge[FreeQS, FreeMap]) =
