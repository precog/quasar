...
import quasar.api._, ApiErrorEntityDecoder._, MessageFormat.JsonContentType,
MessageFormatGen._
  ...
  "Data Service" should {
    "GET" >> {
      ...
      "respond with file data" >> {
        def isExpectedResponse(data: Vector[Data],
                               response: Response,
                               format: MessageFormat) = {
          val expectedBody: Process[Task, String] = format.encode(Process.emitAll(data))
          response
            .as[String]
            .unsafePerformSync must_=== expectedBody.runLog.unsafePerformSync.mkString("")
          response.status must_=== Status.Ok
          response.contentType must_=== Some(
            `Content-Type`(format.mediaType, Charset.`UTF-8`))
        }
        "in correct format" >> {
          "readable and line delimited json by default" >> prop {
            filesystem: SingleFileMemState =>
              val response = service(filesystem.state)(
                Request(uri = pathUri(filesystem.file))).unsafePerformSync
              isExpectedResponse(filesystem.contents, response, MessageFormat.Default)
          }
          "in any supported format if specified" >> {
            "in the content-type header" >> {
              def testProp(format: MessageFormat) = prop {
                filesystem: SingleFileMemState =>
              ...
              "csv" >> prop {
                (filesystem: SingleFileMemState, format: MessageFormat.Csv) =>
                  test(format, filesystem)
              }
              "or a more complicated proposition" >> prop {
                filesystem: SingleFileMemState =>
                  val request = Request(
                  ...
                  val response = service(filesystem.state)(request).unsafePerformSync
                  isExpectedResponse(filesystem.contents,
                                     response,
            ...
            "in the request-headers" >> prop { filesystem: SingleFileMemState =>
              ...
              val request = Request(
                uri = pathUri(filesystem.file).+?(
                  "request-headers",
                  s"""{"Accept": "application/ldjson; mode=precise" }"""))
              val response =
                HeaderParam(service(filesystem.state))(request).unsafePerformSync
              isExpectedResponse(filesystem.contents,
                                 response,
                                 JsonContentType(Precise, LineDelimited))
            }
        ...
        "with gziped encoding when specified" >> prop { filesystem: SingleFileMemState =>
          val request =
            Request(uri = pathUri(filesystem.file),
        ...
        "support disposition" >> prop { filesystem: SingleFileMemState =>
          val disposition =
            `Content-Disposition`("attachement", Map("filename" -> "data.json"))
          val request = Request(uri = pathUri(filesystem.file),
                                headers = Headers(
                                  Accept(jsonReadableLine.mediaType.withExtensions(
                                    Map("disposition" -> disposition.value)))))
          val response = service(filesystem.state)(request).unsafePerformSync
          isExpectedResponse(filesystem.contents, response, MessageFormat.Default)
          response.headers.get(`Content-Disposition`) must_=== Some(disposition)
        }
        "support disposition non-ascii filename" >> prop {
          filesystem: SingleFileMemState =>
            val disposition =
            ...
            (filesystem: SingleFileMemState,
             ...
             format: MessageFormat) =>
              val request = Request(uri = pathUri(filesystem.file)
                                    ...
                                    headers = Headers(Accept(format.mediaType)))
              val response = service(filesystem.state)(request).unsafePerformSync
              isExpectedResponse(filesystem.contents.drop(offset).take(limit),
                                 response,
                                 format)
          }
          "return 400 if provided with" >> {
              ...
              (path: AbsFile[Sandboxed], offset: Natural, limit: Int) =>
                (limit < 1) ==> {
                  val request = Request(uri =
                    pathUri(path).+?("offset", offset.shows).+?("limit", limit.shows))
            ...
            "if provided with both an invalid offset and limit" >> prop {
              (path: AbsFile[Sandboxed], limit: Int, offset: Long @@ Negative) =>
                (limit < 1) ==> {
                  val request = Request(uri =
                    pathUri(path).+?("limit", limit.shows).+?("offset", offset.shows))
                  val response = service(InMemState.empty)(request).unsafePerformSync
                  response.status must_=== Status.BadRequest
                  response.as[Json].unsafePerformSync must_=== Json(
        ...
        "support very large data set" >> {
          ...
          "plain text" >> {
            ...
            val response =
              service(fileSystemWithSampleFile(data))(request).unsafePerformSync
            isExpectedResponse(data, response, MessageFormat.Default)
          }
          "gziped" >> {
            val data = bigData
            val request =
              Request(uri = pathUri(sampleFile),
                      headers = Headers(`Accept-Encoding`(org.http4s.ContentCoding.gzip)))
            val response =
              service(fileSystemWithSampleFile(data))(request).unsafePerformSync
            isExpectedResponse(data, response, MessageFormat.Default)
      ...
      "download as zipped directory" >> prop { filesystem: NonEmptyDir =>
        val disposition =
          `Content-Disposition`("attachment", Map("filename" -> "foo.zip"))
        val requestMediaType =
          MediaType.`text/csv`.withExtensions(Map("disposition" -> disposition.value))
        val request = Request(uri = pathUri(filesystem.dir),
        ...
        .flakyTest(
          "scalacheck: Gave up after only 2 passed tests. 12 tests were discarded.") // NB: this test is slow because NonEmptyDir instances are still relatively large
    ...
    "POST and PUT" >> {
      ...
      testBoth { method =>
        "fail when media-type is" >> {
          "not supported" >> prop { file: Path[Abs, File, Sandboxed] =>
            ...
            response.as[ApiError].unsafePerformSync must beApiErrorLike[DecodeFailure](
              MediaTypeMismatch(MediaType.`text/plain`,
                                MessageFormat.supportedMediaTypes))
          }
        ...
        "accept valid data" >> {
          def accept[A: EntityEncoder](body: A,
                                       expected: List[Data],
            ...
            prop { (fileName: FileName, preExistingContent: Vector[Data]) =>
              ...
              val overrideContentType =
                mediaType.cata(mt => request.withContentType(`Content-Type`(mt).some),
                               request)
              val (service, ref) =
          ...
          "Json" >> {
            ...
            "when having multiple lines containing arrays" >> {
              ...
              "Readable" >> {
                accept(ReadableJson(
                         Json.array(replicate(Json.array(line1, readableLine2)): _*)),
                       replicate(Data.Arr(expectedData)))
              }
            }
          }
          "CSV" >> {
            ...
            "weird" >> {
              ...
              val weirdData = List(Data.Obj(ListMap("a" -> Data.Int(1))),
        ...
        "be 500 when error during writing" >> prop {
          (fileName: FileName, body: NonEmptyList[ReadableJson], failureMsg: String) =>
            ...
            val request = Request(uri = pathUri(destination), method = method)
              .withBody(body.list.toList)
            def service: HttpService =
              serviceErrs(InMemState.empty,
        ...
        (baseDir: ADir, files: Map[RFile, MessageFormat]) =>
          files.size > 1 ==> {
            ...
            val body = Zip.zipFiles[Task](files.map {
              case (p, fmt) =>
                p -> fmt
                  .encode[Task](Process.emitAll(content))
                  .flatMap(utf8Bytes)
    ...
    "MOVE" >> {
      ...
      def testMove[A: EntityDecoder, R: AsResult](from: APath,
                                                  to: APath,
                                                  state: InMemState,
                                                  body: A => R,
                                                  status: Status,
      ...
      "be 201 with dir" >> prop { (fs: NonEmptyDir, dir: ADir) =>
        (fs.dir ≠ dir) ==>
          testMove(
            ...
            newState = Changed(
              fs.filesInDir
                .map { case (relFile, data) => (dir </> relFile, data) }
                .list
                .toList
                .toMap)
          )
    ...
    "DELETE" >> {
      ...
      "be 204 with existing dir" >> prop { filesystem: NonEmptyDir =>
        ...
        .flakyTest(
          "scalacheck: 'Gave up after only 1 passed tests. 11 tests were discarded.'")
