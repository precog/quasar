...
object FileSystemError {
  ...
  final case class QScriptPlanningFailed private (err: PlannerError) extends FileSystemError
  ...
  final case class ReadFailed private (data: String, reason: String) extends FileSystemError
  final case class PartialWrite private (numFailed: Int) extends FileSystemError
  final case class WriteFailed private (data: Data, reason: String) extends FileSystemError

  val executionFailed = Prism
    .partial[FileSystemError, (Fix[LogicalPlan], String, JsonObject, Option[PhysicalError])] {
