  ...
  "Collection.fromFile" should {
    ...
    "never emit an invalid db name" >> prop { (db: SpecialStr, c: SpecialStr) =>
      ...
      notTooLong ==> {
        Collection
          .fromFile(f)
          .fold(err => scala.sys.error(err.toString), coll => {
            Result.foreach(" ./\\*<>:|?") { c =>
              coll.database.value.toList must not(contain(c))
            }
          })
      }
    }.set(maxSize = 5)
    ...
    "round-trip" >> prop { f: AbsFileOf[SpecialStr] =>
      ...
      (r.isRight) ==> {
        r.fold(err => scala.sys.error(err.toString),
    ...
    }.set(minSize = 2, maxSize = 120)
      .setGen(PathOf
        .absFileOfArbitrary[SpecialStr]
        .arbitrary
        .retryUntil(f => Collection.fromFile(f.path).isRight))
  }

  "Collection.prefixFromDir" should {
    "return a collection prefix" in {
      Collection.prefixFromDir(rootDir </> dir("foo") </> dir("bar")) must beRightDisjunction(
        CollectionName("bar"))
    }
