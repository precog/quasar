...
class WriteFileSpec
  ...
  "WriteFile" should {
    ...
    withDataWriters(("append", write.append), ("appendThese", write.appendThese)) {
      (n, wt) =>
    ...
    withDataWriters(("save", write.save), ("saveThese", write.saveThese)) { (n, wt) =>
      s"$n should replace existing file" >> prop {
        (f: AFile, xs: NonEmptyList[Data], ys: NonEmptyList[Data]) =>
          ...
          Mem
            .interpret(write)
            .run(before)
            .leftMap(_.contents) must_=== ((after.contents, Vector.empty.right.right))
      }
      ...
      s"$n should leave existing file untouched on failure" >> prop {
        (f: AFile, xs: NonEmptyList[Data], ys: NonEmptyList[Data]) =>
          ...
          Mem
            .interpretInjectingWriteErrors(write, List(ws))
            .run(before)
            .leftMap(_.contents) must_===
            ((before.contents, Vector(err).right.right))
      }

      s"$n should properly report errors when writing fails to create a file" >> prop {
        f: AFile =>
          val err    = writeFailed(Data.Str("bar"), "")
          val ws     = Vector(err)
    ...
    withDataWriters(("create", write.create), ("createThese", write.createThese)) {
      (n, wt) =>
        s"$n should fail if file exists" >> prop {
          (f: AFile, xs: NonEmptyList[Data], ys: NonEmptyList[Data]) =>
            ...
            Mem.interpret(doWrite).run(before) must_=== ((before,
                                                          pathErr(pathExists(f)).left))
        }
        ...
        s"$n should consume all input into a new file" >> prop {
          (f: AFile, xs: NonEmptyList[Data]) =>
            ...
            Mem
              .interpret(write)
              .run(emptyMem)
              .leftMap(_.contents) must_=== ((Map(f -> xs.toVector),
                                              Vector.empty.right.right))
        }
    }

    withDataWriters(("replace", write.replace),
                    ("replaceWithThese", write.replaceWithThese)) { (n, wt) =>
      s"$n should fail if the file does not exist" >> prop {
        (f: AFile, xs: Vector[Data]) =>
          Mem.interpretEmpty(wt(f, xs.toProcess).runLogCatch.run).toEither must beLeft(
