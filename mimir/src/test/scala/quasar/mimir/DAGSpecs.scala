...
object DAGSpecs extends Specification with DAG with FNDummyModule {
  ...
  "dag decoration" should {
    ...
    "parse out absolute_load" in {
      val result =
        decorate(Vector(Line(1, 1, ""), PushString("/foo"), instructions.AbsoluteLoad))
    ...
    "parse out relative_load" in {
      val result =
        decorate(Vector(Line(1, 1, ""), PushString("/foo"), instructions.RelativeLoad))
      result mustEqual Right(
    ...
    "parse out reduce" in {
      val result = decorate(
        Vector(
          Line(1, 1, ""),
          PushFalse,
          instructions.Reduce(BuiltInReduction(Reduction(Vector(), "count", 0x2000)))))
      result mustEqual Right(
        dag.Reduce(Reduction(Vector(), "count", 0x2000), Const(CFalse)(Line(1, 1, "")))(
          Line(1, 1, "")))
    }
      ...
      val expected = Join(
        ...
        Operate(
          ...
          dag.Reduce(
            ...
            Join(DerefObject, Cross(None), medals, Const(CString("Weight"))(line))(line))(
            line))(line),
        Operate(
          WrapArray,
          dag.Reduce(
            Reduction(Vector(), "max", 0x2001),
            Join(DerefObject, Cross(None), medals, Const(CString("HeightIncm"))(line))(line))(
            line))(line)
      )(line)
      ...
      result must beLike {
        case Right(
            s @ dag.Split(
              ...
              IUI(true, sg2: SplitGroup, IUI(true, sg1: SplitGroup, sp1: SplitParam)),
              id)) => {
    ...
    "determine a histogram of a composite key of revenue and campaign" in {
          ...
          dag.Group(
            ...
            UnfixedSolution(
              ...
              Join(
                DerefObject,
                Cross(None),
                dag.AbsoluteLoad(Const(CString("/organizations"))(line), JType.JUniverseT)(
                  line),
                Const(CString("revenue"))(line))(line))
          ),
          dag.Group(
      ...
      val expectedTarget = Join(
        JoinObject,
        Cross(None),
        Join(WrapObject, Cross(None), Const(CString("revenue"))(line), SplitParam(1, id)(line))(
          line),
        Join(
          WrapObject,
          Cross(None),
          ...
          dag.Reduce(
            ...
            SplitGroup(4, Identities.Specs(Vector(LoadIds("/campaigns"))), id)(line))(line)
        )(line)
      )(line)
    ...
    "continue processing beyond a filter" in {
      ...
      result mustEqual Right(
        Operate(Neg, Filter(IdentitySort, Const(CFalse)(line), Const(CTrue)(line))(line))(line))
    }
  ...
  "stream validation" should {
    ...
    "detect stack underflow" in {
      ...
      "map2_match" >> {
        ...
        {
          val instr = Map2Match(Add)
          decorate(Vector(Line(1, 1, ""), PushTrue, Map1(Comp), instr, Map2Match(Sub))) mustEqual Left(
            StackUnderflow(instr))
        }
      }
      ...
      "map2_cross" >> {
        ...
        {
          val instr = Map2Cross(Add)
          decorate(Vector(Line(1, 1, ""), PushTrue, Map1(Comp), instr, Map2Cross(Sub))) mustEqual Left(
            StackUnderflow(instr))
        }
      }
      ...
      "filter_match" >> {
        ...
        {
          val instr = FilterMatch
          decorate(Vector(Line(1, 1, ""), PushTrue, PushTrue, Map2Match(Add), instr)) mustEqual Left(
            StackUnderflow(instr))
        }
      }
    ...
    "reject multiple stack values at end" in {
      ...
      decorate(Vector(Line(1, 1, ""), PushTrue, PushFalse, PushNum("42"))) mustEqual Left(
        MultipleStackValuesAtEnd)
      decorate(Vector(Line(1, 1, ""), PushTrue, PushFalse, PushNum("42"), PushString("foo"))) mustEqual Left(
        MultipleStackValuesAtEnd)
    }
    ...
    "accept merge with reduced (but reordered) stack" in {
      ...
      val split = dag.Split(
        ...
        IUI(true, SplitGroup(2, Identities.Specs(Vector()), id)(line), Const(CTrue)(line))(
          line),
        id
      )(line)
  ...
  "mapDown" should {
    "rewrite a AbsoluteLoad shared across Split branches to the same object" in {
      ...
      val input =
        dag.Split(dag.Group(1, load, UnfixedSolution(0, load)), SplitParam(0, id)(line), id)(
          line)
