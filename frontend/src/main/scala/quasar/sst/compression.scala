...
object compression {
  import StructuralType.{isConst, typeTransform, ConstST, TagST, TypeST}, ExtractPrimary.ops._
  ...
  def coalesceKeys[J: Order, A: Order: Field: ConvertableTo](
      ...
      JR: Recursive.Aux[J, EJson]): SSTF[J, A, SST[J, A]] => SSTF[J, A, SST[J, A]] =
    typeTransform[J] {
      case EnvT((ts, TypeF.Map(kn, unk))) if kn.size > maxSize.value =>
        val grouped =
          kn.foldlWithKey(IMap.empty[PrimaryTag, J ==>> Unit])(
            (m, j, _) =>
              m.alter(
                primaryTagOf(j),
  ...
  def limitArrays[J: Order, A: Order](maxLength: Positive)(
    ...
    case EnvT((ts, TypeF.Arr(-\/(elts @ ICons(h, t))))) if elts.length > maxLength.value =>
      ...
      envT(
        ...
        TypeF.arr(\/-(NonEmptyList.nel(h, t).suml1)))
  }
  ...
  def narrowUnion[J: Order, A: Order: Field: ConvertableTo](
      ...
      implicit
      JC: Corecursive.Aux[J, EJson],
      JR: Recursive.Aux[J, EJson]): SSTF[J, A, SST[J, A]] => SSTF[J, A, SST[J, A]] = totally {
  ...
  private def compressMap[J: Order, A: Order: Field: ConvertableTo](
      ...
      implicit
      JC: Corecursive.Aux[J, EJson],
      JR: Recursive.Aux[J, EJson]): Option[(SST[J, A], SST[J, A])] =
