...
trait SetLib extends Library {
      ...
      def apply[T](orig: LP[T])(implicit TR: Recursive.Aux[T, LP], TC: Corecursive.Aux[T, LP]) =
        orig match {
          case InvokeUnapply(
              _,
              Sized(
                Embed(InvokeUnapply(Take, Sized(src, Embed(Constant(Data.Int(m)))))),
      ...
      def apply[T](orig: LP[T])(implicit TR: Recursive.Aux[T, LP], TC: Corecursive.Aux[T, LP]) =
        orig match {
          ...
          case InvokeUnapply(
              _,
              Sized(
                Embed(InvokeUnapply(Drop, Sized(src, Embed(Constant(Data.Int(m)))))),
                Embed(Constant(Data.Int(n))))) =>
  ...
  val FullOuterJoin = TernaryFunc(
    ...
    partialTyper[nat._3] {
      ...
      case Sized(s1, s2, _) =>
        Type.Obj(
          Map(JoinDir.Left.name -> (s1 ⨿ Type.Null), JoinDir.Right.name -> (s2 ⨿ Type.Null)),
          None)
    },
    untyper[nat._3](
      t =>
