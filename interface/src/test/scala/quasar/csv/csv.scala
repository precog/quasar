...
class CsvSpec extends quasar.Qspec {
  ...
  "testParse" should {
    "standard" in {
      ...
      testParse(p, Standard) must beSome(
        TestResult(3, List(3, 3, 3, 0, 0, 0, 0, 0, 0, 0)))
    }
  }

  "bestParse" should {
    def parse(text: String) =
      bestParse(CsvParser.AllParsers)(text).map(_.toList.sequence).join

    "parse standard format" in {
      parse(Standard) must_==
        \/-(
          List(Record(List("a", "b", "c")),
               Record(List("1", "2", "3")),
               Record(List("4", "5", "6")),
    ...
    "parse more standard format" in {
      ...
      parse("""Year,Make,Model,Length
        ...
        \/-(
          List(Record(List("Year", "Make", "Model", "Length")),
