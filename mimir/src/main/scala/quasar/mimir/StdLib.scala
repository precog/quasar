...
trait TableLibModule[M[+ _]] extends TableModule[M] with TransSpecModule {
  ...
  trait TableLib extends Library {
    ...
    object MorphismAlignment {
      ...
      case class Custom(alignment: IdentityPolicy, f: (Table, Table) => M[(Table, Morph1Apply)])
    ...
    def coalesce(
        reductions: List[(Reduction, Option[(JType => JType, ColumnRef => Option[ColumnRef])])])
      : Reduction
  }
}
...
trait ColumnarTableLibModule[M[+ _]] extends TableLibModule[M] with ColumnarTableModule[M] {
  trait ColumnarTableLib extends TableLib {
    class WrapArrayTableReduction(
        val r: Reduction,
        val jtypef: Option[(JType => JType, ColumnRef => Option[ColumnRef])])
    ...
    def coalesce(
      ...
      def rec(
          reductions: List[
            (Reduction, Option[(JType => JType, ColumnRef => Option[ColumnRef])])],
...
object StdLib {
  ...
  object StrFrom {
    ...
    class Dt(c: DateColumn, defined: ZonedDateTime => Boolean, f: ZonedDateTime => String)
        extends Map1Column(c)
        with StrColumn {
  ...
  object LongFrom {
    ...
    class D(c: DoubleColumn, defined: Long => Boolean, f: Long => Long)
        extends Map1Column(c)
        with LongColumn {
    ...
    class N(c: NumColumn, defined: Long => Boolean, f: Long => Long)
        extends Map1Column(c)
        with LongColumn {
    ...
    class Dt(c: DateColumn, defined: ZonedDateTime => Boolean, f: ZonedDateTime => Long)
        extends Map1Column(c)
        with LongColumn {
  ...
  object NumFrom {
    ...
    class D(c: DoubleColumn, defined: BigDecimal => Boolean, f: BigDecimal => BigDecimal)
        extends Map1Column(c)
        with NumColumn {
    ...
    // unsafe!  use only if you know what you're doing
    class L(c: LongColumn, defined: BigDecimal => Boolean, f: BigDecimal => BigDecimal)
        extends Map1Column(c)
        with NumColumn {
    ...
    class S(c: StrColumn, defined: String => Boolean, f: String => BigDecimal)
        extends Map1Column(c)
        with NumColumn {
  ...
  object BoolFrom {
    ...
    class Dt(c: DateColumn, defined: ZonedDateTime => Boolean, f: ZonedDateTime => Boolean)
        extends Map1Column(c)
        with BoolColumn {
    ...
    class DtDt(
        c1: DateColumn,
        c2: DateColumn,
        defined: (ZonedDateTime, ZonedDateTime) => Boolean,
        f: (ZonedDateTime, ZonedDateTime) => Boolean)
        extends Map2Column(c1, c2)
        with BoolColumn {
