...
class PlannerSpec
    ...
    with TreeMatchers
    with PendingWithActualTracking {
  ...
  private def toRFile(testName: String): RFile =
    resourcesDir </>
  ...
  def queryPlanner[M[_]: Monad: MonadFsErr: PhaseResultTell](
      ...
      execTime: Instant): M[Crystallized[WorkflowF]] = {
    for {
  ...
  def indexes(ps: (Collection, BsonField)*): Collection => Option[Set[Index]] = {
    val map: Map[Collection, Set[Index]] = ps.toList.foldMap {
      case (c, f) =>
        Map(c -> Set(Index(f.asText + "_", NonEmptyList(f -> IndexType.Ascending), false)))
    }
    c =>
  ...
  def planLP(logical: Fix[LP]): Either[FileSystemError, Crystallized[WorkflowF]] = {
    (for {
      ...
      phys <- MongoDb.doPlan(
        qs,
        fs.QueryContext(defaultStats, defaultIndexes, listContents),
        MongoQueryModel.`3.2`,
        Instant.now)
  ...
  "plan from query string" should {
    "plan simple select *" in {
      plan(sqlE"select * from foo") must beWorkflow($read[WorkflowF](collection("db", "foo")))
    }
    ...
    "plan count(*)" in {
      plan(sqlE"select count(*) from foo") must beWorkflow(
        chain[Workflow](
          ...
          $group(grouped("f0" -> $sum($literal(Bson.Int32(1)))), \/-($literal(Bson.Null))),
                ...
                "value" ->
                  $cond(
                    ...
                    $cond(
                      ...
                      $let(
                        ListMap(
                          DocVar.Name("a1") -> $field("bar"),
                          DocVar.Name("a2") -> $field("baz")),
                        $cond(
                          $and($isArray($field("$a1")), $isArray($field("$a2"))),
      ...
      plan(sqlE"select coalesce(bar, baz) from foo") must
        beWorkflow(
          chain[Workflow](
            ...
            $project(
              reshape("value" ->
                $cond($eq($field("bar"), $literal(Bson.Null)), $field("baz"), $field("bar"))),
              ExcludeId)))
    }
    ...
    "plan now() with a literal timestamp" in {
      ...
      planAt(time, sqlE"""select NOW(), bar from foo""") must
        beWorkflow(
          chain[Workflow](
            $read(collection("db", "foo")),
            $project(reshape("0" -> $literal(bsTime), "bar" -> $field("bar")))))
    }
                ...
                "value" ->
                  $cond(
                    $and(
                      ...
                      $lt($field("baz"), $literal(Bson.Regex("", "")))),
                    $cond(
                      $eq($dayOfWeek($field("baz")), $literal(Bson.Int32(1))),
      ...
      plan(sqlE"""select * from foo where date_part("year", ts) = 2016""") must
        beWorkflow0(
          chain[Workflow](
            ...
            $match(Selector.Doc(BsonField.Name("__tmp2") -> Selector.Eq(Bson.Bool(true)))),
                ...
                BinOp(
                  ...
                  BinOp(
                    ...
                    BinOp(
                      ...
                      Call(
                        ident("isNumber"),
                        List(Access(Select(ident("this"), "loc"), Literal(Js.Num(0, false))))),
                      BinOp(
                        jscore.Or,
                        BinOp(
                          Instance,
      ...
      plan3_2(sqlE"select * from zips where length(city) < 4") must
        beWorkflow0(
          chain[Workflow](
            ...
            $project(reshape("value" -> $field("src")), ExcludeId)
          ))
                      ...
                      BinOp(
                        ...
                        BinOp(
                          ...
                          Call(
                            ...
                            List(Select(Select(ident("x"), "city"), "length"))),
                          Literal(Js.Num(4, false))),
                        BinOp(Lt, Select(ident("x"), "pop"), Literal(Js.Num(20000, false)))
                      ),
                      ident(Js.Undefined.ident)
                    ),
                  "src" -> ident("x")
          ...
          chain[Workflow](
            ...
            $match(Selector.Where(If(
              ...
              BinOp(
                ...
                Call(
                  Select(Select(ident("this"), "loc"), "indexOf"),
        ...
        chain[Workflow](
          ...
          $match(Selector.And(
            ...
            Selector.Doc(BsonField
              .Name("city") -> Selector.Regex("^B[AEIOU]+LD.*", false, true, false, false))
          ))
        ))
    }
    ...
    "plan filter with alternative ~" in {
      plan(sqlE"""select * from a where "foo" ~ pattern or target ~ pattern""") must beWorkflow0(
        chain[Workflow](
              ...
              obj(
                "__tmp8" -> Call(
                  Select(
                    New(
                      Name("RegExp"),
                      List(Select(ident("x"), "pattern"), jscore.Literal(Js.Str("m")))),
                    "test"),
          ...
          chain[Workflow](
            ...
            $match(Selector.Where(If(
              ...
              BinOp(
                jscore.Eq,
                Select(
                  Access(Select(ident("this"), "parents"), jscore.Literal(Js.Num(0, false))),
                  "sha"),
            ...
            $group(grouped("count" -> $sum($literal(Bson.Int32(1)))), \/-($literal(Bson.Null)))
          ))
      ...
      plan(sqlE"select city from zips group by city having count(*) > 10") must
        beWorkflow0(
          chain[Workflow](
            $read(collection("db", "zips")),
            $group(
              grouped("__tmp1" -> $sum($literal(Bson.Int32(1)))),
      ...
      plan(sqlE"select * from zips where city <> state") must
        beWorkflow(
          chain[Workflow](
            $read(collection("db", "zips")),
            $project(
              reshape("0" -> $neq($field("city"), $field("state")), "src" -> $$ROOT),
              IgnoreId),
            $match(Selector.Doc(BsonField.Name("0") -> Selector.Eq(Bson.Bool(true)))),
            $project(reshape("value" -> $field("src")), ExcludeId)
          ))
      ...
      plan(sqlE"select * from zips where city <> state and pop < 10000") must
        beWorkflow0(
          chain[Workflow](
            ...
            $match(Selector.Doc(BsonField.Name("__tmp4") -> Selector.Eq(Bson.Bool(true)))),
          ...
          chain[Workflow](
            ...
            $simpleMap(
              NonEmptyList(
                MapExpr(JsFn(
                  Name("x"),
                  SpliceObjects(List(ident("x"), obj("pop" -> Select(ident("x"), "pop"))))))),
              ListMap())
          ))
    }
        ...
        beWorkflow0(
          chain[Workflow](
            ...
            $simpleMap(
              NonEmptyList(MapExpr(JsFn(
                Name("x"),
                obj("__tmp2" -> SpliceObjects(List(
                  obj("state2" -> Select(ident("x"), "state")),
                  ident("x"),
                  obj("city2" -> Select(ident("x"), "city")),
                  ident("x"),
                  obj("pop2" -> Select(ident("x"), "pop")))))
      ...
      plan(sqlE"select name from person order by height") must
        beWorkflow0(
          chain[Workflow](
            ...
            $project(reshape("name" -> $field("name"), "__tmp0" -> $field("height")), IgnoreId),
      ...
      plan(sqlE"SELECT city, COUNT(*) AS cnt FROM zips ORDER BY cnt DESC") must
        beWorkflow0 {
          chain[Workflow](
            $read(collection("db", "zips")),
            $group(
              grouped("city" -> $push($field("city")), "cnt" -> $sum($literal(Bson.Int32(1)))),
              \/-($literal(Bson.Null))),
      ...
      plan(sqlE"SELECT COUNT(*) as cnt, LENGTH(city) FROM zips") must
        beWorkflow0 {
          chain[Workflow](
            ...
            $group(
              grouped("cnt" -> $sum($literal(Bson.Int32(1))), "1" -> $push($field("1"))),
              \/-($literal(Bson.Null))),
            $unwind(DocField("1"))
          )
        }
      ...
      plan(sqlE"select count(*) as cnt, city from zips group by city") must
        beWorkflow0 {
          chain[Workflow](
            $read(collection("db", "zips")),
            $group(
              grouped("cnt" -> $sum($literal(Bson.Int32(1)))),
              -\/(reshape("0" -> $field("city")))),
    ...
    "collect unaggregated fields into single doc when grouping" in {
      ...
      notOnPar,
      testFile("collect unaggregated fields into single doc when grouping"))
    ...
    "plan unaggregated field when grouping, second case" in {
      ...
      notOnPar,
      testFile("plan unaggregated field when grouping, second case"))
      ...
      plan(sqlE"select sum(avg(pop)), min(city) from zips group by foo") must
        beWorkflow0 {
          chain[Workflow](
            ...
            $group(
              grouped("0" -> $sum($field("__tmp10")), "1" -> $push($field("1"))),
              \/-($literal(Bson.Null))),
            $unwind(DocField("1"))
          )
        }
    ...
    "plan sum of expression in expression with another projection when grouped" in {
      ...
      notOnPar,
      testFile("plan sum of expression in expression with another projection when grouped"))
      ...
      plan3_2(sqlE"select state, length(min(city)) as shortest from zips group by state") must
        beWorkflow0(
          chain[Workflow](
            ...
            $project(reshape("state" -> $field("_id", "0"), "__tmp6" -> $include()), IgnoreId),
            $simpleMap(
              NonEmptyList(
                MapExpr(
                  JsFn(
      ...
      plan3_2(sqlE"select length(city) as len, count(*) as cnt from zips group by length(city)") must
        beWorkflow(
          chain[Workflow](
            ...
            $group(
              grouped("len" -> $first($field("f0")), "cnt" -> $sum($literal(Bson.Int32(1)))),
              -\/(reshape("0" -> $field("b0"))))
          ))
    }
    ...
    "plan expressions with ~" in {
      plan(sqlE"""select foo ~ "bar.*", "abc" ~ "a|b", "baz" ~ regex, target ~ regex from a""") must beWorkflow(
        chain[Workflow](
          $read(collection("db", "a")),
          $simpleMap(
            NonEmptyList(MapExpr(JsFn(
                ...
                "0" -> If(
                  ...
                  Call(
                    Select(
                      New(
                        Name("RegExp"),
                        List(jscore.Literal(Js.Str("bar.*")), jscore.Literal(Js.Str("m")))),
                      "test"),
                ...
                "2" -> If(
                  ...
                  Call(
                    Select(
                      New(
                        Name("RegExp"),
                        List(Select(ident("x"), "regex"), jscore.Literal(Js.Str("m")))),
                      "test"),
          ...
          $project(
            reshape("0" -> $include(), "1" -> $include(), "2" -> $include(), "3" -> $include()),
            IgnoreId)
        ))
    }
    ...
    "plan array concat with filter" in {
      plan(sqlE"""select loc || [ pop ] from zips where city = "BOULDER" """) must
        beWorkflow0 {
          chain[Workflow](
            $read(collection("db", "zips")),
            $match(Selector.Doc(BsonField.Name("city") -> Selector.Eq(Bson.Text("BOULDER")))),
            $simpleMap(
              NonEmptyList(MapExpr(JsFn(
                Name("x"),
                obj("__tmp4" ->
      ...
      plan(sqlE"select `_id` as zip, loc[*] from zips order by loc[*]") must
        beWorkflow0(
          chain[Workflow](
            ...
            $unwind(DocField(BsonField.Name("__tmp2"))),
            $project(
              reshape("zip" -> $field("__tmp3", "_id"), "loc" -> $field("__tmp2")),
              IgnoreId),
      ...
      plan(
        ...
        beWorkflow0(
          chain[Workflow](
            ...
            $match(Selector.Or(
              Selector.And(
                Selector.Doc(BsonField.Name("__tmp19") \ BsonField.Name("__tmp14") \ BsonField
                  .Name("id") -> Selector.Type(BsonType.Text)),
                Selector.Doc(BsonField.Name("__tmp19") \ BsonField.Name("__tmp14") \ BsonField
                  .Name("id") -> Selector.Regex("^.*Dr.*$", false, true, false, false))
              ),
              Selector.Doc(BsonField
    ...
    "plan limit with offset" in {
      plan(sqlE"SELECT * FROM zips OFFSET 100 LIMIT 5") must
        beWorkflow(chain[Workflow]($read(collection("db", "zips")), $limit(105), $skip(100)))
    }
      ...
      plan(sqlE"SELECT city, SUM(pop) AS pop FROM zips GROUP BY city ORDER BY pop") must
        beWorkflow0 {
          chain[Workflow](
            ...
            $project(reshape("city" -> $field("_id", "0"), "pop" -> $include()), IgnoreId),
            $sort(NonEmptyList(BsonField.Name("pop") -> SortDir.Ascending))
          )
        }
      ...
      plan(sqlE"select foo is null from zips where foo is null") must
        beWorkflow(
          chain[Workflow](
            ...
            $project(reshape("value" -> $eq($field("foo"), $literal(Bson.Null))), ExcludeId)
          ))
    }
    ...
    "plan simple distinct" in {
      plan(sqlE"select distinct city, state from zips") must
        beWorkflow0(
          chain[Workflow](
            $read(collection("db", "zips")),
            $group(grouped(), -\/(reshape("0" -> $field("city"), "1" -> $field("state")))),
            $project(
              reshape("city" -> $field("_id", "0"), "state" -> $field("_id", "1")),
              IgnoreId)
          ))
                ...
                reshape("0" ->
                  $cond(
                    $and(
                      $lte($literal(Bson.Text("")), $field("city")),
                      $lt($field("city"), $literal(Bson.Doc()))),
                    $substr($field("city"), $literal(Bson.Int32(0)), $literal(Bson.Int32(1))),
                    $literal(Bson.Undefined)
                  )))
            ),
            $group(
            ...
            $simpleMap(
              NonEmptyList(
                MapExpr(JsFn(
                  Name("x"),
                  obj("__tmp1" ->
                    Call(ident("remove"), List(ident("x"), jscore.Literal(Js.Str("_id")))))))),
              ListMap()),
    ...
    "plan distinct with unrelated order by" in {
      plan(sqlE"select distinct city from zips order by pop desc") must
        beWorkflow0(
          chain[Workflow](
            $read(collection("db", "zips")),
            $project(reshape("__sd__0" -> $field("pop"), "city" -> $field("city")), IgnoreId),
            $sort(NonEmptyList(BsonField.Name("__sd__0") -> SortDir.Descending)),
            $group(grouped("__tmp2" -> $first($$ROOT)), -\/(reshape("city" -> $field("city")))),
            $project(
              reshape(
    ...
    "plan distinct as function with group" in {
      plan(sqlE"select state, count(distinct(city)) from zips group by state") must
        beWorkflow0(
          chain[Workflow](
            $read(collection("db", "zips")),
            $group(grouped("__tmp0" -> $first($$ROOT)), -\/(reshape("0" -> $field("city")))),
            $group(
              grouped(
    ...
    "plan combination of two distinct sets" in {
      plan(sqlE"SELECT (DISTINCT foo.bar) + (DISTINCT foo.baz) FROM foo") must
        beWorkflow0($read(collection("db", "zips")))
      ...
      plan(
        ...
        beWorkflow0(
          chain[Workflow](
            ...
            $match(Selector.Doc(BsonField.Name("__tmp6") -> Selector.Eq(Bson.Bool(true)))),
          ...
          chain[Workflow](
            ...
            $match(
              Selector.Or(
                ...
                Selector.And(
                  Selector.Doc(BsonField.Name("ts") -> Selector.Gte(date29)),
    ...
    "plan js and filter with id" in {
      Bson.ObjectId
        ...
        .fold[Result](failure("Couldn’t create ObjectId."))(oid =>
          plan3_2(
            sqlE"""select length(city), foo = oid("0123456789abcdef01234567") from days where `_id` = oid("0123456789abcdef01234567")""") must
          ...
          $simpleMap(
            NonEmptyList(MapExpr(JsFn(
              ...
              obj(
                "0" ->
                  If(
                    Call(ident("isString"), List(Select(ident("x"), "name"))),
                    Call(
                      ident("NumberLong"),
        ...
        beWorkflow0(
          joinStructure(
            ...
            chain[Workflow](
              ...
              $match(Selector.Doc(BsonField.Name("__tmp12") -> Selector.Eq(Bson.Bool(true)))),
    ...
    "plan simple inner equi-join with expression ($lookup)" in {
      plan3_4(
        ...
        beWorkflow0(
          chain[Workflow](
            $read(collection("db", "foo")),
            $project(
              reshape(JoinDir.Left.name -> $$ROOT, "__tmp0" -> $toLower($field("id"))),
              IgnoreId),
            $lookup(
              CollectionName("bar"),
      ...
      "#1560",
      testFile("plan simple inner equi-join with expression ($lookup)"))
    ...
    "plan simple inner equi-join with pre-filtering ($lookup)" in {
      plan3_4(
        ...
        beWorkflow0(
          chain[Workflow](
            ...
            $project(
              reshape(JoinDir.Right.name -> $$ROOT, "__tmp2" -> $field("foo_id")),
              ExcludeId),
            $lookup(
              CollectionName("foo"),
      ...
      "#1560",
      testFile("plan simple inner equi-join with pre-filtering ($lookup)"))
            ...
            chain[Workflow](
              ...
              $project(
                reshape(
                  JoinDir.Left.name ->
                    $cond(
                      $eq($size($field(JoinDir.Left.name)), $literal(Bson.Int32(0))),
                      $literal(Bson.Arr(List(Bson.Doc()))),
                      $field(JoinDir.Left.name)),
                  JoinDir.Right.name ->
                    $cond(
                      $eq($size($field(JoinDir.Right.name)), $literal(Bson.Int32(0))),
                      $literal(Bson.Arr(List(Bson.Doc()))),
                      $field(JoinDir.Right.name))
                ),
                IgnoreId
    ...
    "plan simple left equi-join (map-reduce)" in {
      plan(sqlE"select foo.name, bar.address from foo left join bar on foo.id = bar.foo_id") must
        beWorkflow0(
          joinStructure(
            $read(collection("db", "foo")),
            "__tmp0",
    ...
    "plan simple left equi-join ($lookup)" in {
      ...
      testFile("plan simple left equi-join ($lookup)"))
    ...
    "plan simple right equi-join ($lookup)" in {
      ...
      "TODO: left/right joins in $lookup",
      testFile("plan simple right equi-join ($lookup)"))
      ...
      plan3_4(
        ...
        beWorkflow0(
          chain[Workflow](
            ...
            $match(Selector.Doc(BsonField.Name("TestProgramId") -> Selector.Exists(true))),
            ...
            chain[Workflow](
              ...
              $simpleMap(
                NonEmptyList(MapExpr(JsFn(
                  ...
                  obj(
                    "__tmp4" -> Select(
                      Access(Select(ident("x"), "parents"), jscore.Literal(Js.Num(0, false))),
                      "sha"),
                    "__tmp5" -> ident("x"),
          ...
          case IsSingleSource(op) =>
            simpleShape(op.src).map { shape =>
              ...
              if (missing.isEmpty) Nil
                ...
                List(missing.map(_.bson).mkString(", ") + " missing in\n" + Fix[WorkflowF](
                  op.wf).render.shows)
            }.getOrElse(Nil)
          case _ => Nil
      ...
      val lp =
        lpf.let(
          ...
          lpf.let(
            ...
            makeObj(
              "bar" -> lpf
                .invoke2(ObjectProject, lpf.free('tmp0), lpf.constant(Data.Str("bar")))),
            lpf.let(
              'tmp2,
              lpf.sort(
                lpf.free('tmp1),
                (
                  lpf.invoke2(ObjectProject, lpf.free('tmp1), lpf.constant(Data.Str("bar"))),
                  SortDir.asc).wrapNel),
              lpf.free('tmp2))
          )
        )

      planLP(lp) must beWorkflow0(
        chain[Workflow](
          $read(collection("db", "foo")),
    ...
    "plan Sort with expression" in {
      ...
      planLP(lp) must beWorkflow(
        chain[Workflow](
          $read(collection("db", "foo")),
          $project(
            reshape("0" -> divide($field("bar"), $literal(Bson.Dec(10.0))), "src" -> $$ROOT),
            IgnoreId),
      ...
      planLP(lp) must beWorkflow0(
        chain[Workflow](
          ...
          $match(Selector.Doc(BsonField
            .Name("__tmp6") -> Selector.Regex("^.*MONT.*$", false, true, false, false))),
          $project(
            reshape(
              "__tmp8" -> $cond(
                $and(
