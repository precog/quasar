...
object MongoDb
  ...
  private def checkPathsExist[T[_[_]]: BirecursiveT](
      qs: T[MongoDb.QSM[T, ?]]): Backend[Unit] = {
    ...
    val e: MongoLogWFR[BsonCursor, Unit] =
      EitherT[MongoLogWF[BsonCursor, ?], FileSystemError, Unit](
  ...
  def doPlan[
      ...
      qs: T[QSM[T, ?]],
      ctx: fs.QueryContext[N],
      queryModel: MongoQueryModel,
      execTime: Instant): N[Repr] =
  ...
  def plan[T[_[_]]: BirecursiveT: EqualT: ShowT: RenderTreeT](qs: T[QSM[T, ?]]): Backend[Repr] =
    for {
      ...
      ctx <- toBackendP(
        fs.QueryContext.queryContext[T, Backend](qs, QueryFileModule.listContents))
  ...
  def toBackendP[C[_], A](c: C[(PhaseResults, FileSystemError \/ A)])(
      implicit inj: C :<: fs.Eff): Backend[A] =
  ...
  object QueryFileModule extends QueryFileModule {
    ...
    private def mkInterp[F[_]: Functor](
        implicit C: MonadReader_[F, Config]): F[fs.QueryFileInterpreter] =
  ...
  object ManagedReadFileModule extends ManagedReadFileModule {
    ...
    private def cursor(
        coll: Collection,
        offset: Natural,
        limit: Option[Positive]): MongoDbIO[BsonCursor] =
      for {
    ...
    def readCursor(f: AFile, offset: Natural, limit: Option[Positive]): Backend[BsonCursor] =
      Collection
        .fromFile(f)
        .fold(
  ...
  object ManagedWriteFileModule extends ManagedWriteFileModule {
    private def dataToDocument(v: BsonVersion, d: Data): FileSystemError \/ Bson.Doc =
      BsonCodec
        ...
        .flatMap {
          ...
          case otherwise =>
            FileSystemError.writeFailed(d, "MongoDB is only able to store documents").left
        }
    ...
    def writeChunk(c: Collection, chunk: Vector[Data]): Configured[Vector[FileSystemError]] =
      for {
        v <- config[Configured].map(cfg =>
          MongoQueryModel.toBsonVersion(MongoQueryModel(cfg.serverVersion)))
        r <- doWriteChunk(v, c, chunk)
      } yield r

    private def doWriteChunk(
        v: BsonVersion,
        c: Collection,
        chunk: Vector[Data]): Configured[Vector[FileSystemError]] = {
      val (errs, docs) = chunk foldMap { d =>
        dataToDocument(v, d).fold(e => (Vector(e), Vector()), d => (Vector(), Vector(d)))
      }
      val io = MongoDbIO
  ...
  object ManageFileModule extends ManageFileModule {
    ...
    def move(scenario: MoveScenario, semantics: MoveSemantics): Backend[Unit] = {
      val mm: MongoManage[FileSystemError \/ Unit] =
        scenario.fold(moveDir(_, _, semantics), moveFile(_, _, semantics)).run.liftM[ManageInT]
      toBackend(mm)
    }

    def delete(path: APath): Backend[Unit] = {
      val mm: MongoManage[FileSystemError \/ Unit] =
        pathy.Path.refineType(path).fold(deleteDir, deleteFile).run.liftM[ManageInT]
      toBackend(mm)
    }
    ...
    def tempFile(near: APath): Backend[AFile] = {
      val checkPath =
        EitherT
          .fromDisjunction[MongoManage](Collection.dbNameFromPath(near))
