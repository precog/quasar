...
object MetadataFixture {
  ...
  def runQueryWithMounts(mem: InMemState, mnts: Map[APath, MountConfig]): QueryFile ~> Task = {
    val run: Eff ~> Task = runQuery(mem) :+: runConstantMount[Task](mnts) :+: runConstantVCache[
      Task](Map.empty)
    val addViews = view.queryFile[Eff]
    val addModules = flatMapSNT(
      transformIn[QueryFile, Eff, Free[Eff, ?]](module.queryFile[Eff], liftFT))
  ...
  def service(mem: InMemState, mnts: Map[APath, MountConfig]): Service[Request, Response] = {
    metadata
      .service[Eff]
      .toHttpService(
        liftMT[Task, ResponseT] compose (runQueryWithMounts(mem, mnts) :+: runConstantMount[
          Task](mnts) :+: runConstantVCache[Task](Map.empty)))
      .orNotFound
  }
}
...
class MetadataServiceSpec extends quasar.Qspec with FileSystemFixture with Http4s {
  ...
  "Metadata Service" should {
    "respond with NotFound" >> {
      ...
      "if directory does not exist" >> prop { dir: ADir =>
        (dir != rootDir) ==> {
          val response =
            service(InMemState.empty, Map())(Request(uri = pathUri(dir))).unsafePerformSync
      ...
      "file does not exist" >> prop { file: AFile =>
        val response =
          service(InMemState.empty, Map())(Request(uri = pathUri(file))).unsafePerformSync
        response.as[ApiError].unsafePerformSync must beApiErrorLike(pathNotFound(file))
      }

      "if file with same name as existing directory (without trailing slash)" >> prop {
        s: SingleFileMemState =>
          depth(s.file) > 1 ==> {
            ...
            val response =
              service(s.state, Map())(Request(uri = pathUri(fileWithSameName))).unsafePerformSync
            response.as[ApiError].unsafePerformSync must beApiErrorLike(
              pathNotFound(fileWithSameName))
          }
      }
    }
    ...
    "respond with OK" >> {
      "and empty list for existing empty directory" >> {
        service(InMemState.empty, Map())(Request(uri = Uri(path = "/")))
          .as[Json]
          .unsafePerformSync must_== Json("children" := List[FsNode]())
      }
      ...
      "and list of children for existing nonempty directory" >> prop { s: NonEmptyDir =>
        ...
        service(s.state, Map())(Request(uri = pathUri(s.dir)))
          .as[Json]
          .unsafePerformSync must_== Json("children" := childNodes.toIList.sorted)
      ...
      "and mounts when any children happen to be mount points" >> prop {
        (
          ...
          fsMountName ≠ moduleName) ==> {
            val moduleConfig: List[Statement[Fix[Sql]]] =
            ...
            service(mem, mnts)(Request(uri = pathUri(parent)))
              .as[Json]
              .unsafePerformSync must_=== Json("children" := List(
      ...
      "and functions as files on a module mount with additional info about functions parameters" >> prop {
        dir: ADir =>
          val moduleConfig: List[Statement[Fix[Sql]]] = List(
            FunctionDecl(CIName("FOO"), List(CIName("BAR")), Fix(boolLiteral(true))),
            FunctionDecl(
              CIName("BAR"),
              List(CIName("BAR"), CIName("BAZ")),
              Fix(boolLiteral(false)))
          )
          val mnts = Map[APath, MountConfig]((dir, MountConfig.moduleConfig(moduleConfig)))
          val mem = InMemState.empty

          service(mem, mnts)(Request(uri = pathUri(dir)))
            .as[Json]
            .unsafePerformSync must_=== Json(
            "children" := List(
      ...
      "support offset and limit" >> prop {
        (dir: NonEmptyDir, offset: Int @@ NonNegative, limit: Int @@ RPositive) =>
          val childNodes = dir.ls.map(FsNode(_, None))

          service(dir.state, Map())(Request(
            uri = pathUri(dir.dir).+?("offset", offset.toString).+?("limit", limit.toString)))
            .as[Json]
            .unsafePerformSync must_== Json(
            "children" := childNodes.toIList.sorted.drop(offset).take(limit))
      }.set(minTestsOk = 10) // NB: this test is slow because NonEmptyDir instances are still relatively large

      "and empty object for existing file" >> prop { s: SingleFileMemState =>
        service(s.state, Map())(Request(uri = pathUri(s.file)))
          .as[Json]
          .unsafePerformSync must_=== Json.obj()
      }
    }
  }
}
