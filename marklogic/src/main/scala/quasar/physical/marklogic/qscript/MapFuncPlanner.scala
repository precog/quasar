...
private[qscript] final class MapFuncPlanner[
  ...
  val plan: AlgebraM[F, MapFuncCore[T, ?], XQuery] = {
    ...
    case JoinSideName(n) =>
      MonadPlanErr[F].raiseError(
        MarkLogicPlannerError.unreachable(s"JoinSideName(${Show[Symbol].shows(n)})"))
    ...
    case ToTimestamp(millis) =>
      SP.castIfNode(millis) >>= (lib.timestampToDateTime[F] apply _)
    case TypeOf(x) => lib.typeOf[F, FMT] apply x
    case Now()     => fn.currentDateTime.point[F]

    case ExtractCentury(time) =>
      asDateTime(time) map (dt => fn.ceiling(fn.yearFromDateTime(dt) div 100.xqy))
    case ExtractDayOfMonth(time) => asDateTime(time) map fn.dayFromDateTime
    case ExtractDecade(time) =>
      asDateTime(time) map (dt => fn.floor(fn.yearFromDateTime(dt) div 10.xqy))
    case ExtractDayOfWeek(time) =>
      asDate(time) map (d => mkSeq_(xdmp.weekdayFromDate(d) mod 7.xqy))
    case ExtractDayOfYear(time) => asDate(time) map (xdmp.yeardayFromDate)
    case ExtractEpoch(time) =>
      asDateTime(time) flatMap (lib.secondsSinceEpoch[F].apply(_))
    ...
    case ExtractIsoYear(time) =>
      asDateTime(time) flatMap (lib.isoyearFromDateTime[F].apply(_))
    case ExtractMicroseconds(time) =>
      asDateTime(time) map (dt => mkSeq_(fn.secondsFromDateTime(dt) * 1000000.xqy))
    case ExtractMillennium(time) =>
      asDateTime(time) map (dt => fn.ceiling(fn.yearFromDateTime(dt) div 1000.xqy))
    case ExtractMilliseconds(time) =>
      asDateTime(time) map (dt => mkSeq_(fn.secondsFromDateTime(dt) * 1000.xqy))
    ...
    case ExtractTimezone(time) =>
      asDateTime(time) flatMap (lib.timezoneOffsetSeconds[F].apply(_))
    case ExtractTimezoneHour(time) =>
      asDateTime(time) map (dt => fn.hoursFromDuration(fn.timezoneFromDateTime(dt)))
    case ExtractTimezoneMinute(time) =>
      asDateTime(time) map (dt => fn.minutesFromDuration(fn.timezoneFromDateTime(dt)))
    ...
    case MapFuncsCore.Eq(x, y) =>
      castedBinOp(x, y)((a, b) => handleWith(a eq b, fn.False))
    ...
    case Between(v1, v2, v3) =>
      castedTernOp(v1, v2, v3)((x1, x2, x3) => mkSeq_(x2 le x1) and mkSeq_(x1 le x3))
    ...
    case QSearch(in, ptn, ci) =>
      fn.matches(in, ptn, Some(if_(ci) then_ "im".xs else_ "m".xs)).point[F]
    ...
    case Within(x, arr) =>
      (freshName[F] |@| freshName[F] |@| SP.leftShift(arr))((a, b, bs) =>
  ...
  private def castedBinOpF(x: XQuery, y: XQuery)(
      op: (XQuery, XQuery) => F[XQuery]): F[XQuery] =
    binOpF(x, y)((vx, vy) => (SP.castIfNode(vx) |@| SP.castIfNode(vy))(op).join)

  private def castedBinOp(x: XQuery, y: XQuery)(
      op: (XQuery, XQuery) => XQuery): F[XQuery] =
    castedBinOpF(x, y)((a, b) => op(a, b).point[F])

  private def ternOpF(x: XQuery, y: XQuery, z: XQuery)(
      op: (XQuery, XQuery, XQuery) => F[XQuery]): F[XQuery] =
  ...
  private def castedTernOp(x: XQuery, y: XQuery, z: XQuery)(
      op: (XQuery, XQuery, XQuery) => XQuery): F[XQuery] =
    ternOpF(x, y, z)((vx, vy, vz) =>
      (SP.castIfNode(vx) |@| SP.castIfNode(vy) |@| SP.castIfNode(vz))(op))
}
