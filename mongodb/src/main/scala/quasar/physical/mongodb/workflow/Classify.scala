...
object Classify {
  implicit def coproductClassify[F[_]: Functor, G[_]: Functor, A](
      implicit CF: Classify[F],
      CG: Classify[G]): Classify[Coproduct[F, G, ?]] = new Classify[Coproduct[F, G, ?]] {
    def source[A](v: Coproduct[F, G, A]) =
      v.run.fold(CF.source(_).map(_.fmap(Coproduct.leftc[F, G, A](_))),
                 CG.source(_).map(_.fmap(Coproduct.rightc[F, G, A](_))))

    def singleSource[A](v: Coproduct[F, G, A]) =
      v.run.fold(CF.singleSource(_).map(_.fmap(ι, Inject[F, Coproduct[F, G, ?]])),
                 CG.singleSource(_).map(_.fmap(ι, Inject[G, Coproduct[F, G, ?]])))

    def pipeline[A](v: Coproduct[F, G, A]) =
      v.run.fold(CF.pipeline(_).map(_.fmap(ι, Inject[F, Coproduct[F, G, ?]])),
                 CG.pipeline(_).map(_.fmap(ι, Inject[G, Coproduct[F, G, ?]])))

    def shapePreserving[A](v: Coproduct[F, G, A]) =
      v.run.fold(CF.shapePreserving(_).map(_.fmap(ι, Inject[F, Coproduct[F, G, ?]])),
