...
object Bson {
  ...
  final case class Binary(value: ImmutableArray[Byte]) extends Bson {
    ...
    def toJs =
      Js.Call(Js.Ident("BinData"),
              List(Js.Num(0, false),
                   Js.Str(new sun.misc.BASE64Encoder().encode(value.toArray))))
  ...
  final case class Doc(value: ListMap[String, Bson])
    ...
    def repr: BsonDocument =
      new BsonDocument(
        value.toList.map { case (k, v) => new BsonElement(k, v.repr) }.asJava)
  ...
  object ObjectId {
    def fromArray(array: Array[Byte]): ObjectId =
      ObjectId(ImmutableArray.fromArray(array))
  ...
  final case class Date(millis: Long) extends Bson {
    ...
    def toJs =
      Js.Call(Js.Ident("ISODate"), List(Js.Str(Instant.ofEpochMilli(millis).toString)))
  }
  object Date {
