...
package object fs {
  ...
  def fileSystem[S[_]](
    ...
    S0: Task :<: S,
    S1: PhysErr :<: S): EnvErrT[Task, AnalyticalFileSystem ~> Free[S, ?]] = {
    val runM = Hoist[EnvErrT].hoist(MongoDbIO.runNT(client))

    (
      runM(WorkflowExecutor.mongoDb) |@|
        queryfile
          .run[BsonCursor, S](client, defDb)
          .liftM[EnvErrT] |@|
  ...
  def qscriptFileSystem[S[_]](
    ...
    (
      ...
      val fileSystemInterpreter =
        interpretFileSystem[Free[S, ?]](qfile compose queryfile.interpretQ(execMongo),
                                        rfile compose readfile.interpret,
                                        wfile compose writefile.interpret,
                                        mfile compose managefile.interpret)
  ...
  private[fs] def asyncClientDef[S[_]](
    ...
    util
      .createAsyncMongoClient[Eff](uri)
