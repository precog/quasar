...
class PipelineSpec extends quasar.Qspec with ArbBsonField {
  ...
  def genProject(size: Int): Gen[$ProjectF[Unit]] =
    for {
      fields <- Gen.nonEmptyListOf(for {
        ...
        value <- if (size <= 0) genExpr3_2.map(\/-(_))
          ...
          Gen.oneOf(genProject(size - 1).map(p => -\/(p.shape)), genExpr3_2.map(\/-(_)))
      } yield BsonField.Name(field) -> value)
      id <- Gen.oneOf(ExcludeId, IncludeId)
    } yield $ProjectF((), Reshape[ExprOp](ListMap(fields: _*)), id)

  implicit def arbProject =
    Arbitrary[$ProjectF[Unit]](Gen.resize(5, Gen.sized(genProject)))
  ...
  def genGroup =
    for {
      ...
      $GroupF(
        (),
        Grouped(ListMap(
          BsonField.Name("docsByAuthor" + i.toString) -> $sum($literal(Bson.Int32(1))))),
  ...
  implicit def arbitraryShapePreservingOp: Arbitrary[ShapePreservingPipelineOp] =
    Arbitrary {
      // Note: Gen.oneOf is overridden and this variant requires two explicit args
  ...
  def arbitraryShapePreservingOpGens = {
    def matchGen =
      for {
        ...
        ShapePreservingPipelineOp(
          PipelineOp(
            $MatchF((),
                    Selector.Doc(BsonField.Name(c.toString) -> Selector.Eq(
                      Bson.Int32(-1)))).shapePreserving))
    ...
    def sortGen =
      for {
        ...
        ShapePreservingPipelineOp(
          PipelineOp($SortF(
            (),
            NonEmptyList(BsonField.Name("name1") -> SortDir.Ascending)).shapePreserving))
  ...
  "Project.get" should {
    "retrieve whatever value it was set to" >> prop {
      (p: $ProjectF[Unit], f: BsonField) =>
  ...
  "Project.setAll" should {
    "actually set all" >> prop { (p: $ProjectF[Unit]) =>
      ...
      "result could have `_id -> _id` inserted without changing semantics")
  }
    ...
    "remove one un-nested field" in {
      val op = $SimpleMapF(
        ...
        NonEmptyList(
          MapExpr(
            JsFn(Name("x"),
                 obj("a" -> Select(ident("x"), "x"), "b" -> Select(ident("x"), "y"))))),
        ListMap()
      )
      val exp = $SimpleMapF(
        $read[WorkflowOpCoreF](collection("db", "foo")),
        NonEmptyList(MapExpr(JsFn(Name("x"), obj("a" -> Select(ident("x"), "x"))))),
        ListMap())
    ...
    "remove one nested field" in {
      val op = $SimpleMapF(
        ...
        NonEmptyList(
          MapExpr(
            JsFn(Name("x"),
                 obj("a" -> Select(ident("x"), "x"),
                     "b" -> obj("c" -> Select(ident("x"), "y"),
                                "d" -> Select(ident("x"), "z")))))),
        ListMap()
      )
      val exp = $SimpleMapF(
