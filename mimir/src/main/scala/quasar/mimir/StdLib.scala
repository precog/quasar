...
trait TableLibModule[M[+ _]] extends TableModule[M] with TransSpecModule {
  ...
  trait TableLib extends Library {
    ...
    trait Op2Array extends Op2 {
      def spec[A <: SourceType](left: TransSpec[A], right: TransSpec[A]): TransSpec[A] = {
        trans.MapWith(trans.InnerArrayConcat(trans.WrapArray(trans.Map1(left, prepare)),
...
trait ColumnarTableLibModule[M[+ _]]
  ...
  trait ColumnarTableLib extends TableLib {
    class WrapArrayTableReduction(val r: Reduction, val jtypef: Option[JType => JType])
        ...
        reductions match {
          case (x, jtypef) :: xs => {
            val impl = new Reduction(Vector(), "") {
              ...
              val tpe =
                UnaryOperationType(JUnionT(x.tpe.arg, acc.tpe.arg), JArrayUnfixedT)
            }
...
object StdLib {
  ...
  object StrFrom {
    ...
    class Dt(c: DateColumn, defined: LocalDateTime => Boolean, f: LocalDateTime => String)
        extends Map1Column(c)
        with StrColumn {
  ...
  object LongFrom {
    ...
    class Dt(c: DateColumn, defined: LocalDateTime => Boolean, f: LocalDateTime => Long)
        extends Map1Column(c)
        with LongColumn {
  ...
  object BoolFrom {
    ...
    class DtDt(c1: DateColumn,
               c2: DateColumn,
               defined: (LocalDateTime, LocalDateTime) => Boolean,
               f: (LocalDateTime, LocalDateTime) => Boolean)
        extends Map2Column(c1, c2)
        with BoolColumn {
