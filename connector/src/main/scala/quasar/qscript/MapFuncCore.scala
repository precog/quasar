...
object MapFuncCore {
  ...
  def rollMF[T[_[_]], A](
      mf: MapFuncCore[T, FreeMapA[T, A]]): CoEnv[A, MapFuncCore[T, ?], FreeMapA[T, A]] =
  ...
  object StaticArray {
    def unapply[T[_[_]]: BirecursiveT, A](
        mf: CoMapFuncR[T, A]): Option[List[FreeMapA[T, A]]] =
      mf match {
        case ConcatArraysN(as) =>
          as.foldRightM[Option, List[FreeMapA[T, A]]](Nil)((mf, acc) =>
            (mf.project.run.toOption collect {
  ...
  object StaticArrayPrefix {
    def unapply[T[_[_]]: BirecursiveT, A](
      ...
      mf match {
        case ConcatArraysN(as) =>
          as.foldLeftM[List[FreeMapA[T, A]] \/ ?, List[FreeMapA[T, A]]](Nil)((acc, mf) =>
              mf.project.run.fold(
                κ(acc.left),
                _ match {
  ...
  object StaticMap {
    def unapply[T[_[_]]: BirecursiveT, A](
        mf: CoMapFuncR[T, A]): Option[List[(T[EJson], FreeMapA[T, A])]] =
      mf match {
        case ConcatMapsN(as) =>
          as.foldRightM[Option, List[(T[EJson], FreeMapA[T, A])]](Nil)((mf, acc) =>
            (mf.project.run.toOption >>= {
  ...
  object ConcatArraysN {
    def apply[T[_[_]]: BirecursiveT, A](
        args: List[FreeMapA[T, A]]): CoEnv[A, MapFuncCore[T, ?], FreeMapA[T, A]] = {
      args.toList match {
        case h :: t =>
          t.foldLeft(h)((a, b) => rollMF[T, A](ConcatArrays(a, b)).embed).project
    ...
    def unapply[T[_[_]]: BirecursiveT, A](
        mf: CoEnv[A, MapFuncCore[T, ?], FreeMapA[T, A]]): Option[List[FreeMapA[T, A]]] =
      mf.run.fold(
        κ(None), {
  ...
  object ConcatMapsN {
    def apply[T[_[_]]: BirecursiveT, A](
      ...
      args.toList match {
        case h :: t =>
          t.foldLeft(h)((a, b) => rollMF[T, A](ConcatMaps(a, b)).embed).project
  ...
  def foldConstant[T[_[_]]: BirecursiveT, A]: CoMapFuncR[T, A] => Option[T[EJson]] = {
    object ConstEC {
  ...
  def replaceJoinSides[T[_[_]]: BirecursiveT](
      left: Symbol,
      right: Symbol): CoMapFuncR[T, JoinSide] => CoMapFuncR[T, JoinSide] =
    _.run match {
    ...
    _.run.fold(
      κ(None), {
        case Eq(Embed(CoEnv(\/-(Constant(v1)))), Embed(CoEnv(\/-(Constant(v2))))) =>
          rollMF[T, A](Constant(EJson.fromCommon(ejson.Bool[T[EJson]](v1 ≟ v2)))).some

        case ProjectIndex(Embed(StaticArrayPrefix(as)),
        ...
        case ProjectField(Embed(ConcatMapsN(as)), Embed(CoEnv(\/-(Constant(field))))) =>
          as.collectFirst {
            ...
            case Embed(
                CoEnv(\/-(MakeMap(Embed(CoEnv(\/-(Constant(src)))), Embed(value)))))
                if field ≟ src =>
              value.some
        ...
        case ConcatArrays(Embed(CoEnv(\/-(Constant(Embed(EC(ejson.Arr(Nil))))))),
                          Embed(CoEnv(\/-(rhs)))) =>
          rollMF[T, A](rhs).some

        // elide Nil array on the right
        case ConcatArrays(Embed(CoEnv(\/-(lhs))),
                          Embed(CoEnv(\/-(Constant(Embed(EC(ejson.Arr(Nil)))))))) =>
          rollMF[T, A](lhs).some

        // elide Nil map on the left
        case ConcatMaps(Embed(CoEnv(\/-(Constant(Embed(EX(ejson.Map(Nil))))))),
                        Embed(CoEnv(\/-(rhs)))) =>
          rollMF[T, A](rhs).some

        // elide Nil map on the right
        case ConcatMaps(Embed(CoEnv(\/-(lhs))),
  ...
  implicit def traverse[T[_[_]]]: Traverse[MapFuncCore[T, ?]] =
    new Traverse[MapFuncCore[T, ?]] {
      def traverseImpl[G[_], A, B](fa: MapFuncCore[T, A])(f: A => G[B])(
          implicit G: Applicative[G]): G[MapFuncCore[T, B]] = fa match {
        // nullary
  ...
  implicit def equal[T[_[_]]: EqualT, A]: Delay[Equal, MapFuncCore[T, ?]] =
    new Delay[Equal, MapFuncCore[T, ?]] {
      ...
      def apply[A](in: Equal[A]): Equal[MapFuncCore[T, A]] = Equal.equal {
        ...
        case (IfUndefined(a1, a2), IfUndefined(b1, b2)) =>
          in.equal(a1, b1) && in.equal(a2, b2)
        ...
        case (ConcatMaps(a1, a2), ConcatMaps(b1, b2)) =>
          in.equal(a1, b1) && in.equal(a2, b2)
        case (ProjectIndex(a1, a2), ProjectIndex(b1, b2)) =>
          in.equal(a1, b1) && in.equal(a2, b2)
        case (ProjectField(a1, a2), ProjectField(b1, b2)) =>
          in.equal(a1, b1) && in.equal(a2, b2)
        case (DeleteField(a1, a2), DeleteField(b1, b2)) =>
          in.equal(a1, b1) && in.equal(a2, b2)
        case (ConcatArrays(a1, a2), ConcatArrays(b1, b2)) =>
          in.equal(a1, b1) && in.equal(a2, b2)
        case (Range(a1, a2), Range(b1, b2)) => in.equal(a1, b1) && in.equal(a2, b2)

        //  ternary
        case (Between(a1, a2, a3), Between(b1, b2, b3)) =>
          in.equal(a1, b1) && in.equal(a2, b2) && in.equal(a3, b3)
        case (Cond(a1, a2, a3), Cond(b1, b2, b3)) =>
          in.equal(a1, b1) && in.equal(a2, b2) && in.equal(a3, b3)
        case (Search(a1, a2, a3), Search(b1, b2, b3)) =>
          in.equal(a1, b1) && in.equal(a2, b2) && in.equal(a3, b3)
        case (Substring(a1, a2, a3), Substring(b1, b2, b3)) =>
          in.equal(a1, b1) && in.equal(a2, b2) && in.equal(a3, b3)
        case (Guard(a1, atpe, a2, a3), Guard(b1, btpe, b2, b3)) =>
          atpe ≟ btpe && in.equal(a1, b1) && in.equal(a2, b2) && in.equal(a3, b3)
      ...
      def apply[A](sh: Show[A]): Show[MapFuncCore[T, A]] = {
        ...
        Show.show {
          ...
          case TemporalTrunc(a1, a2) =>
            Cord("TemporalTrunc(", a1.show, ", ", sh.show(a2), ")")
  ...
  def translateBinaryMapping[T[_[_]], A]: BinaryFunc => (A, A) => MapFuncCore[T, A] = {
    ...
    case string.Concat | structural.ArrayConcat | structural.ConcatOp =>
      ConcatArrays(_, _)
  }
...
object MapFuncsCore {
  ...
  @Lenses final case class TemporalTrunc[T[_[_]], A](part: TemporalPart, a1: A)
      extends Unary[T, A]
  ...
  @Lenses final case class Cond[T[_[_]], A](cond: A, then_ : A, else_ : A)
      extends Ternary[T, A] {
    def a1 = cond
    def a2 = then_
    def a3 = else_
  ...
  @Lenses final case class Substring[T[_[_]], A](string: A, from: A, count: A)
      extends Ternary[T, A] {
    def a1 = string
    def a2 = from
    def a3 = count
  ...
  @Lenses final case class ProjectIndex[T[_[_]], A](src: A, index: A)
      extends Binary[T, A] {
    def a1 = src
    def a2 = index
  }
  @Lenses final case class ProjectField[T[_[_]], A](src: A, field: A)
      extends Binary[T, A] {
    def a1 = src
    def a2 = field
  }
  @Lenses final case class DeleteField[T[_[_]], A](src: A, field: A)
      extends Binary[T, A] {
    def a1 = src
    def a2 = field
  }
  ...
  object BoolLit {
    ...
    def unapply[T[_[_]]: RecursiveT, A](mf: FreeMapA[T, A]): Option[Boolean] =
      mf.resume.fold({
        case Constant(ej) =>
          CommonEJson.prj(ej.project).flatMap {
  ...
  object IntLitMapFunc {
    def unapply[T[_[_]]: RecursiveT, A](mf: MapFuncCore[T, A]): Option[BigInt] =
      mf match {
        case Constant(ej) =>
