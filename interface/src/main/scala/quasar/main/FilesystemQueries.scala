...
import quasar.{queryPlan, Data, Variables}
...
class FilesystemQueries[S[_]](implicit val Q: QueryFile.Ops[S]) {
  ...
  def evaluateQuery(
      ...
      basePath: ADir,
      off: Natural,
      lim: Option[Positive]): Process[CompExecM, Data] =
   ...
   */
  def executeQuery(query: Fix[Sql], vars: Variables, basePath: ADir, out: AFile)(
      implicit W: WriteFile.Ops[S],
      MF: ManageFile.Ops[S]): CompExecM[Unit] =
    compToCompExec(queryPlan(query, vars, basePath, 0L, None)).flatMap(
      lp =>
        execToCompExec(
          lp.fold(
            d =>
              fsErrToExec(
                W.saveThese(out, d.toVector).flatMap(fse => (fse.headOption <\/ (())).liftT)),
            Q.execute(_, out))))
  ...
  def queryResults(
    ...
    compToCompExec(queryPlan(query, vars, basePath, off, lim)).flatMap(
      _.fold(ds => Process.emitAll(ds).point[CompExecM], lp => execToCompExec(Q.results(lp))))
}
