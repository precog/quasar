...
final class MarkLogicConfigSpec extends quasar.Qspec with MarkLogicConfigArbitrary {
  "fromUriString" >> {
    ...
    "builds config from a valid uri" >> {
      MarkLogicConfig.fromUriString[ErrorMessages \/ ?](validUri).toOption must_= Some(
        MarkLogicConfig(new URI("xcc://user:pass@ml.example.com:8007/somedb"),
    ...
    "root dir is root when only db specified" >> {
      MarkLogicConfig
        .fromUriString[ErrorMessages \/ ?](
          "xcc://ml.example.com:8007/adb"
        )
        .toOption
        .map(_.rootDir) must_= Some(rootDir)
    }
    ...
    "uses XML doc type when no format parameter given" >> {
      MarkLogicConfig
        ...
        .toOption
        .map(_.docType) must_= Some(DocType.xml)
    }
  ...
  "roundtrip from/asUriString" >> prop { cfg: MarkLogicConfig =>
    MarkLogicConfig
      .fromUriString[ErrorMessages \/ ?](cfg.asUriString)
      .toOption must_= Some(cfg)
  }
}
