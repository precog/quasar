...
trait JoinOptimizerSpecs[M[+ _]]
  ...
  "join optimization" should {
    ...
    "eliminate naive cartesian products in trivial array join cases" in {
      ...
      val expectedOpt =
        Join(
          JoinArray,
          ValueSort(0),
          Operate(WrapArray, Join(DerefObject, Cross(None), liftedLHS, height)(line))(
            line),
      ...
      def makeAddSortKey(key: DepGraph, value: DepGraph) =
        AddSortKey(
          Join(
            JoinObject,
            IdentitySort,
            Join(WrapObject, Cross(Some(CrossRight)), Const(CString("key"))(line), key)(
              line),
            Join(WrapObject,
                 Cross(Some(CrossRight)),
    ...
    "eliminate cartesian with a new inside an array join" in {
      ...
      val name = Const(CString("Name"))(line)
      val medals =
        dag.AbsoluteLoad(Const(CString("/summer_games/london_medals"))(line))(line)
      ...
      val input = Filter(
        ...
        Join(
          ...
          Join(
            JoinArray,
            Cross(None),
            Operate(WrapArray, Join(DerefObject, Cross(None), newMedals, name)(line))(
              line),
    ...
    "eliminate cartesian with a new and a filter inside an array join" in {
      ...
      val expectedOpt =
        Join(
          JoinArray,
          ValueSort(0),
          Operate(WrapArray, Join(DerefObject, Cross(None), liftedLHS, country)(line))(
            line),
          Operate(WrapArray, Join(DerefObject, Cross(None), liftedRHS, country)(line))(
            line)
        )(line)
