...
sealed abstract class TypeStat[A] {
  ...
  def +(other: TypeStat[A])(implicit O: Order[A], F: Field[A]): TypeStat[A] =
    (this, other) match {
      ...
      case (Byte(c1, min1, max1), Byte(c2, min2, max2)) =>
        byte(c1 + c2, mn(min1, min2), mx(max1, max2))
      case (Char(c1, min1, max1), Char(c2, min2, max2)) =>
        char(c1 + c2, mn(min1, min2), mx(max1, max2))
      case (Int(s1, min1, max1), Int(s2, min2, max2)) =>
        int(s1 + s2, mn(min1, min2), mx(max1, max2))
      case (Dec(s1, min1, max1), Dec(s2, min2, max2)) =>
        dec(s1 + s2, mn(min1, min2), mx(max1, max2))
      case (Coll(c1, min1, max1), Coll(c2, min2, max2)) =>
        coll(c1 + c2, mn(min1, min2), mx(max1, max2))
...
object TypeStat extends TypeStatInstances {
  ...
  final case class Str[A](cnt: A, minLen: A, maxLen: A, min: String, max: String)
      extends TypeStat[A]
  final case class Int[A](stats: SampleStats[A], min: BigInt, max: BigInt)
      extends TypeStat[A]
  final case class Dec[A](stats: SampleStats[A], min: BigDecimal, max: BigDecimal)
      extends TypeStat[A]
  final case class Coll[A](cnt: A, minSize: Option[A], maxSize: Option[A])
      extends TypeStat[A]
  final case class Count[A](cnt: A) extends TypeStat[A]

  def bool[A] =
    Prism.partial[TypeStat[A], (A, A)] {
      case Bool(t, f) => (t, f)
    }((Bool[A](_, _)).tupled)

  def byte[A] =
    Prism.partial[TypeStat[A], (A, SByte, SByte)] {
      case Byte(n, min, max) => (n, min, max)
    }((Byte[A](_, _, _)).tupled)

  def char[A] =
    Prism.partial[TypeStat[A], (A, SChar, SChar)] {
      case Char(n, min, max) => (n, min, max)
    }((Char[A](_, _, _)).tupled)

  def str[A] =
    Prism.partial[TypeStat[A], (A, A, A, String, String)] {
      case Str(n, minLen, maxLen, min, max) => (n, minLen, maxLen, min, max)
    }((Str[A](_, _, _, _, _)).tupled)

  def int[A] =
    Prism.partial[TypeStat[A], (SampleStats[A], BigInt, BigInt)] {
      case Int(ss, min, max) => (ss, min, max)
    }((Int[A](_, _, _)).tupled)

  def dec[A] =
    Prism.partial[TypeStat[A], (SampleStats[A], BigDecimal, BigDecimal)] {
      case Dec(ss, min, max) => (ss, min, max)
    }((Dec[A](_, _, _)).tupled)

  def coll[A] =
    Prism.partial[TypeStat[A], (A, Option[A], Option[A])] {
      case Coll(c, min, max) => (c, min, max)
    }((Coll[A](_, _, _)).tupled)

  def count[A] =
    Prism.partial[TypeStat[A], A] {
      case Count(n) => n
    }(Count(_))
  ...
  def fromFoldable[F[_]: Foldable, A](cnt: A, fa: F[_])(
      implicit A: ConvertableTo[A]): TypeStat[A] =
  ...
  private def maxOr[F[_]: Foldable, A: Order: AdditiveSemigroup](
      a: A,
      fa: F[Option[TypeStat[A]]]): A =
...
sealed abstract class TypeStatInstances {
  ...
  implicit def encodeEJson[A: EncodeEJson: Equal: Field: NRoot]
    : EncodeEJson[TypeStat[A]] =
  ...
  implicit def show[A: Show: Equal: Field: NRoot]: Show[TypeStat[A]] =
    Show.shows {
      ...
      case Str(c, minl, maxl, min, max) =>
        s"Str(${c.shows}, ${minl.shows}, ${maxl.shows}, ${min.shows}, ${max.shows})"
  ...
  private[sst] def encodeEJson0[A: EncodeEJson: Equal: Field: NRoot, J](
    ...
    def minmax[B: EncodeEJson](kind: String, c: A, mn: B, mx: B): J =
      kmap(kind, "count" -> c.asEJson[J], "min" -> mn.asEJson[J], "max" -> mx.asEJson[J])

    def sstats(ss: SampleStats[A]): J =
      emap(
        ("mean", ss.mean.asEJson[J]) ::
          optEntry("variance", isPopulation.fold(ss.variance, ss.populationVariance)) :::
          optEntry("skewness", isPopulation.fold(ss.skewness, ss.populationSkewness)) :::
          optEntry("kurtosis", isPopulation.fold(ss.kurtosis, ss.populationKurtosis)): _*)
    ...
    ts match {
      case Bool(t, f) =>
        kmap("boolean", "true" -> t.asEJson[J], "false" -> f.asEJson[J])
