...
final class LogicalPlanR[T](implicit TR: Recursive.Aux[T, LP], TC: Corecursive.Aux[T, LP]) {
   ...
   * • otherwise, we fail
   */
  private def unifyOrCheck(inf: Type, poss: Type, term: T): NameT[SemDisj, ConstrainedPlan[T]] =
      ...
      term match {
        ...
        case InvokeUnapply(structural.FlattenArray, Sized(arg)) =>
          for {
            ...
            consts <- emitName[SemDisj, Func.Input[T, nat._1]](
              Func.Input1(arg).traverse(ensureConstraint(_, constant(Data.Arr(List(Data.NA))))))
        ...
        case Let(name, value, in) =>
          unifyOrCheck(
            inf,
            in.inferred,
            let(name, appConst(value, constant(Data.NA)), appConst(in, constant(Data.NA))))
        ...
        case Sort(expr, ords) =>
          unifyOrCheck(
            ...
            expr.inferred,
            sort(
              appConst(expr, constant(Data.NA)),
