...
trait CogroupSpec[M[+ _]]
  ...
  implicit val cogroupData = Arbitrary(
    for {
      ...
      val (lschema, rschema) = Bifunctor[Tuple2].umap(cschema.splitAt(cschema.size / 2)) {
        _.map(_._1).toSet
      }
      val (l, r) = data map {
        case (ids, values) =>
  ...
  def testTrivialCogroup(f: Table => Table = identity[Table]) = {
    ...
    val result: Table = ltable.cogroup(SourceKey.Single, SourceKey.Single, rtable)(
      ...
      OuterObjectConcat(
        WrapObject(SourceKey.Left, "key"),
        WrapObject(OuterArrayConcat(SourceValue.Left, SourceValue.Right), "value"))
    )
  ...
  def testSimpleCogroup(f: Table => Table = identity[Table]) = {
    ...
    def recBoth(i: Int) =
      toRecord(
        Array(i),
        JObject(
          List(JField("left", JString(i.toString)), JField("right", JString(i.toString)))))

    val ltable = fromSample(
      SampleData(
        Stream(recl(0), recl(1), recl(3), recl(3), recl(5), recl(7), recl(8), recl(8))))
    val rtable = fromSample(SampleData(
      Stream(recr(0), recr(2), recr(3), recr(4), recr(5), recr(5), recr(6), recr(8), recr(8))))
    ...
    val result: Table = ltable.cogroup(SourceKey.Single, SourceKey.Single, rtable)(
      ...
      OuterObjectConcat(
        WrapObject(SourceKey.Left, "key"),
        WrapObject(OuterObjectConcat(SourceValue.Left, SourceValue.Right), "value"))
    )
  ...
  def testUnionCogroup = {
    ...
    val ltable = fromSample(
      SampleData(Stream(recl(0, 1), recl(1, 12), recl(3, 13), recl(4, 42), recl(5, 77))))
    val rtable = fromSample(
      SampleData(Stream(recr(6, -1), recr(7, 0), recr(8, 14), recr(9, 42), recr(10, 77))))
