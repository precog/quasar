...
import quasar.{queryPlan, resolveImports, Data, DataCodec, Variables}
...
import quasar.qscript.{qScriptReadToQscriptTotal, DiscoverPath, QScriptRead}
...
object Repl {
  ...
  final case class RunState(
    ...
    def targetDir(path: Option[XDir]): ADir =
      path match {
        ...
        case Some(\/-(a)) => a
        case Some(-\/(r)) =>
          (unsandbox(cwd) </> r).relativeTo(rootDir[Sandboxed]).cata(rootDir </> _, rootDir)
      }
  ...
  def command[S[_]](cmd: Command)(
    ...
    def write(
        f: (AFile, Vector[Data]) => W.M[Vector[FileSystemError]],
        dst: XFile,
    ...
    def compileQuery(
        expr: Fix[Sql],
        vars: Variables,
        basePath: ADir): (PhaseResults, SemanticErrors \/ (FileSystemError \/ Unit)) = {
      ...
      queryPlan(expr, vars, basePath, 0L, None)
        ...
        .flatMap(
          _.fold(
            ...
            lp =>
              QueryFile
                .convertToQScriptRead[Fix, M, QScriptRead[Fix, ?]](
                  DiscoverPath.ListContents.static[List, M](absFiles(lp)))(lp)
                .void))
        .run
        .run
        .run
    ...
    cmd match {
      ...
      case Ls(d) =>
        for {
          ...
          names <- files.toList.traverse[Free[S, ?], String](
            _.fold(
              d =>
                mountType[S](path </> dir1(d)).map(t => d.value + t.cata(t => s"@ ($t)", "/")),
              f =>
                mountType[S](path </> file1(f)).map(t => f.value + t.cata(t => s"@ ($t)", ""))))
        ...
        n.cata(
          name => {
            for {
              ...
              query = fsQ
                .executeQuery(block, Variables.fromMap(state.variables), state.cwd, out)
              _ <- runQuery(state, query)(
                κ(P.println("Wrote file: " + posixCodec.printPath(out))))
            } yield ()
          },
          for {
            state <- RS.get
