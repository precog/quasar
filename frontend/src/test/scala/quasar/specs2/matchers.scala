  ...
  def beEqualIfSuccess[E, A](expected: Validation[E, A]) =
    new Matcher[Validation[E, A]] {
      def apply[S <: Validation[E, A]](s: Expectable[S]) = {
        ...
        v.fold(
          κ(
            result(expected.fold(κ(true), κ(false)),
                   "both failed",
                   s"$v is not $expected",
                   s)),
          a =>
            expected.fold(κ(result(false, "", "expected failure", s)),
...
trait DisjunctionMatchers extends org.specs2.scalaz.DisjunctionMatchers {
  def beRightDisjOrDiff[A, B: Equal](expected: B)(
      implicit rb: RenderTree[B]): Matcher[A \/ B] = new Matcher[A \/ B] {
