...
trait BlockStoreColumnarTableModule[M[+ _]] extends ColumnarTableModule[M] {
  ...
  protected lazy val blockModuleLogger =
    LoggerFactory.getLogger("quasar.yggdrasil.table.BlockStoreColumnarTableModule")
  ...
  protected class MergeEngine[KeyType, BlockData <: BlockProjectionData[KeyType, Slice]] {
     ...
     * block of data.
     */
    case class Cell private[MergeEngine] (index: Int, maxKey: KeyType, slice0: Slice)(
        succf: KeyType => M[Option[BlockData]],
        remap: Array[Int],
        var position: Int) {
      ...
      def split: (Slice, CellState) = {
        ...
        (
          if (position == 0) finished else finished.sparsen(remap, remap(position - 1) + 1),
          nextState)
      }
    ...
    def mergeProjections(inputSortOrder: DesiredSortOrder, cellStates: Stream[CellState])(
      ...
      StreamT.unfoldM[M, Slice, Stream[CellState]](cellStates) { cellStates =>
        ...
        if (expired.isEmpty) {
          ...
          val successorStatesM =
            expired.map(_.succ).sequence.map(_.toStream.collect({ case Some(cs) => cs }))
  ...
  trait BlockStoreColumnarTableCompanion extends ColumnarTableCompanion {
    ...
    def align(
      ...
      case class RunLeft(rightRow: Int, rightKey: Slice, rightAuthority: Option[Slice])
          extends AlignState
      case class RunRight(leftRow: Int, leftKey: Slice, rightAuthority: Option[Slice])
          extends AlignState
      ...
      def buildRowComparator(lkey: Slice, rkey: Slice, rauth: Slice): RowComparator =
        new RowComparator {
          ...
          def compare(i1: Int, i2: Int) = {
            if (i2 < 0 && rauth != null) auxComparator.compare(i1, rauth.size + i2)
        ...
        def step(
                ...
                comparator.compare(lidx, ridx) match {
                  ...
                  case GT =>
                    if (span eq RightSpan) sys.error("Inputs to align are not correctly sorted")
                      ...
                      buildFilters(comparator, lidx, lsize, lacc, ridx + 1, rsize, racc, NoSpan)
                }
              } else if (lidx < lsize) {
          ...
          def continue(
            ...
            case MoreLeft(span, leq, ridx, req) =>
              def next(lbs: JDBMState, rbs: JDBMState): M[(JDBMState, JDBMState)] =
                ltail.uncons flatMap {
                  ...
                  case None =>
                    ...
                    val remission =
                      req.nonEmpty.option(rhead.mapColumns(cf.util.filter(0, rhead.size, req)))
                    (remission map { e =>
              ...
              val lemission =
                leq.nonEmpty.option(lhead.mapColumns(cf.util.filter(0, lhead.size, leq)))
              lemission map { e =>
                for {
    ...
    def writeTables(
      ...
      write0(
        reduceSlices(slices),
        WriteState(JDBMState.empty("writeSortedSpace"), valueTrans, identifiedKeyTrans.toList))
    }
      ...
      valueTrans.advance(slice) flatMap {
        case (valueTrans0, vslice) => {
          ...
          def storeTransformed(
            ...
            : M[(JDBMState, List[(SliceTransform1[_], String)])] = transforms match {
    ...
    protected def writeRawSlices(
      ...
      jdbmState.indices.get(indexMapKey) map {
        ...
        case (index, jdbmState) =>
          ...
          val newIndex =
            index.copy(count = index.count + (newInsertCount - jdbmState.insertCount))
        ...
        jdbmState
          .copy(indices = jdbmState.indices + (indexMapKey -> sortedSlice), insertCount = 0)
      }
    }
    ...
    def loadTable(
      ...
      def cellsMs: Stream[M[Option[CellState]]] = indices.values.toStream.zipWithIndex map {
        case (SortedSlice(name, kslice, vslice, _, _, _, _), index) =>
          ...
          M.point(
            Some(CellState(index, new Array[Byte](0), slice, (k: SortingKey) => M.point(None))))
  ...
  abstract class Table(slices: StreamT[M, Slice], size: TableSize)
     ...
     */
    def toInternalTable(
        limit: Int = yggConfig.maxSliceSize): EitherT[M, ExternalTable, InternalTable]
    ...
    protected def writeSorted(
      ...
      val (sourceTrans0, keyTrans0, valueTrans0) = if (!unique) {
        (
          ...
          groupKeys map { kt =>
            OuterObjectConcat(
              WrapObject(deepMap(kt) { case Leaf(_) => TransSpec1.DerefArray0 }, "0"),
