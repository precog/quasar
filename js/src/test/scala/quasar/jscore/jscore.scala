...
class JsCoreSpecs extends quasar.Qspec with TreeMatchers with ScalazMatchers {
  "toJs" should {
    ...
    "don't null-check method call on newly-constructed Array" in {
      val expr =
        Call(Select(Arr(List(Literal(Js.Num(0, false)), Literal(Js.Num(1, false)))),
                    "indexOf"),
             List(ident("x")))
    ...
    "splice obj constructor" in {
      val expr =
        SpliceObjects(List(Obj(ListMap(Name("foo") -> Select(ident("bar"), "baz")))))
  ...
  "simplify" should {
    ...
    "inline object components" in {
      ...
      x.simplify must_=== Arr(
        List(ident("y"),
             ident("y"),
             If(ident("r"), Select(ident("r"), "foo"), ident("bar"))))
    }
  }
  ...
  "JsFn" should {
    "substitute with shadowing Let" in {
      val fn = JsFn(Name("x"),
                    BinOp(Add,
                          ident("x"),
                          Let(Name("x"),
                              BinOp(Add, Literal(Js.Num(1, false)), ident("x")),
                              ident("x"))))
      val exp =
        BinOp(Add,
    ...
    "toString" should {
      "be the same as the equivalent JS" in {
        val js = JsFn(Name("val"),
                      Obj(
                        ListMap(Name("a") -> Select(ident("val"), "x"),
