...
trait JobManager[M[+ _]] { self =>
  ...
  /**
    ...
    */
  def start(job: JobId,
            startedAt: LocalDateTime = LocalDateTime.now): M[Either[String, Job]]
  ...
  def finish(job: JobId,
             finishedAt: LocalDateTime = LocalDateTime.now): M[Either[String, Job]]

  /**
    * Moves the job to the `Expired` terminal state.
    */
  def expire(job: JobId,
             expiredAt: LocalDateTime = LocalDateTime.now): M[Either[String, Job]]
...
trait JobStateManager[M[+ _]] { self: JobManager[M] =>
  ...
  def start(id: JobId, startTime: LocalDateTime): M[Either[String, Job]] =
    transition(id) {
