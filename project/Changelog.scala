...
object ChangeLog extends Plugin {
  ...
  lazy val changelogCommand = Command.command("changelog") { state =>
    ...
    val byVersion = sources
      .map(f =>
        f.getName match {
          case Version(maj, min, rev) =>
            (Some(maj.toInt), Some(min.toInt), Some(rev.toInt)) -> f
      ...
      .sortBy {
        case ((maj, min, rev), _) =>
          (maj.getOrElse(1000), min.getOrElse(1000), rev.getOrElse(1000))
      }
      .reverse

    val lines = "# Change Log" :: "" :: byVersion.flatMap {
      case (ver, f) =>
        val head = ver match {
