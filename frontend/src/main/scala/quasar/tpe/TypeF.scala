...
object TypeF extends TypeFInstances {
  ...
  def leaf[J, A]: Prism[TypeF[J, A], SimpleType \/ J] =
    Prism[TypeF[J, A], SimpleType \/ J](tf =>
      simple[J, A].getOption(tf).map(_.left) orElse const[J, A].getOption(tf).map(_.right))(
...
private[quasar] sealed abstract class TypeFInstances {
  ...
  implicit def encodeEJsonK[A](implicit A: EncodeEJson[A]): EncodeEJsonK[TypeF[A, ?]] =
    new EncodeEJsonK[TypeF[A, ?]] {
      def encodeK[J](
          ...
          JR: Recursive.Aux[J, EJson]): Algebra[TypeF[A, ?], J] = {
          ...
          val other = unk map {
            case (k, v) =>
              (
                ...
                map1(C(ejson.str[J]("keys")).embed -> k, C(ejson.str[J]("values")).embed -> v)
              )
          }
  ...
  implicit def traverse[J]: Traverse[TypeF[J, ?]] =
    new Traverse[TypeF[J, ?]] {
      def traverseImpl[G[_]: Applicative, A, B](tf: TypeF[J, A])(f: A => G[B]): G[TypeF[J, B]] =
        tf match {
