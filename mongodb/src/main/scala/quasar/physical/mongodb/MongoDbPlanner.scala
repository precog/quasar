...
import quasar.common.{PhaseResult, PhaseResultTell, PhaseResults, SortDir}
...
import quasar.physical.mongodb.planner.{
  FuncHandler,
  JoinHandler,
  JoinSource,
  JsFuncHandler,
...
final case class There[T[_[_]]](index: Int, next: InputFinder[T]) extends InputFinder[T] {
...
object MongoDbPlanner {
  ...
  def generateTypeCheck[In, Out](or: (Out, Out) => Out)(
      f: PartialFunction[Type, In => Out]): Type => Option[In => Out] =
    typ =>
      f.lift(typ)
        .fold(typ match {
          case Type.Interval => generateTypeCheck(or)(f)(Type.Dec)
          case Type.Arr(_) => generateTypeCheck(or)(f)(Type.AnyArray)
          case Type.Timestamp | Type.Timestamp ⨿ Type.Date |
          ...
          case a ⨿ b =>
            (generateTypeCheck(or)(f)(a) ⊛ generateTypeCheck(or)(f)(b))((a, b) =>
              ((expr: In) => or(a(expr), b(expr))))
          case _ => None
        })(Some(_))
  ...
  def processMapFuncExpr[
      ...
      EX[_]: Traverse,
      A](funcHandler: MapFunc[T, ?] ~> OptionFree[EX, ?])(fm: FreeMapA[T, A])(
      recovery: A => Fix[ExprOp])(implicit inj: EX :<: ExprOp): M[Fix[ExprOp]] =
    fm.cataM(
      interpretM[M, MapFunc[T, ?], A, Fix[ExprOp]](
        recovery(_).point[M],
        expression(funcHandler)))

  def getSelector[T[_[_]]: BirecursiveT: ShowT, M[_]: Monad: MonadFsErr, EX[_]: Traverse](
      v: BsonVersion)(fm: FreeMap[T])(
      implicit inj: EX :<: ExprOp): OutputM[PartialSelector[T]] =
    fm.zygo(
  ...
  def processMapFunc[T[_[_]]: BirecursiveT: ShowT, M[_]: Monad: MonadFsErr: ExecTimeR, A](
      fm: FreeMapA[T, A])(recovery: A => JsCore): M[JsCore] =
  ...
  def ejsonToExpression[M[_]: Applicative: MonadFsErr, EJ](v: BsonVersion)(ej: EJ)(
      implicit EJ: Recursive.Aux[EJ, EJson]): M[Fix[ExprOp]] =
    ej.cataM(BsonCodec.fromEJson(v))
      .fold(pe => raiseErr(qscriptPlanningFailed(pe)), $literal(_).point[M])

  // TODO: Use `JsonCodec.encode` and avoid failing.
  def ejsonToJs[M[_]: Applicative: MonadFsErr, EJ: Show](ej: EJ)(
      implicit EJ: Recursive.Aux[EJ, EJson]): M[JsCore] =
    ej.cata(Data.fromEJson)
      .toJs
      .fold(raiseErr[M, JsCore](qscriptPlanningFailed(NonRepresentableEJson(ej.shows))))(
        _.point[M])
  ...
  def expression[
    ...
    def execTime(implicit ev: ExecTimeR[M]): M[Bson.Date] =
      OptionT[M, Bson.Date](ev.ask.map(Bson.Date.fromInstant(_))).getOrElseF(raiseErr(
      ...
      case Guard(expr, typ, cont, fallback) =>
        ...
        def exprCheck: Type => Option[Fix[ExprOp] => Fix[ExprOp]] =
          generateTypeCheck[Fix[ExprOp], Fix[ExprOp]]($or(_, _)) {
            ...
            case Type.Int | Type.Dec | Type.Int ⨿ Type.Dec |
            ...
            case Type.Obj(map, _) =>
              ((expr: Fix[ExprOp]) => {
                ...
                expr match {
                  case $var(dv) =>
                    map.foldLeft(basic)((acc, pair) =>
                      exprCheck(pair._2).fold(acc)(e =>
                        $and(acc, e($var(dv \ BsonField.Name(pair._1))))))
    ...
    mf =>
      handleCommon(mf).cata(_.point[M], handleSpecial(mf))
  }
  ...
  def javascript[T[_[_]]: BirecursiveT: ShowT, M[_]: Applicative: MonadFsErr: ExecTimeR]
    ...
    def trunc(expr: JsCore): JsCore =
      Let(
        Name("x"),
        expr,
        BinOp(jscore.Sub, ident("x"), BinOp(jscore.Mod, ident("x"), Literal(Js.Num(1, false)))))
    ...
    val handleSpecialCore: MapFuncCore[T, JsCore] => M[JsCore] = {
      ...
      case Date(a1) =>
        If(
          Call(
            Select(
              Call(ident("RegExp"), List(Literal(Js.Str("^" + string.dateRegex + "$")))),
              "test"),
            List(a1)),
      ...
      case Time(a1) =>
        If(
          Call(
            Select(
              Call(ident("RegExp"), List(Literal(Js.Str("^" + string.timeRegex + "$")))),
              "test"),
            List(a1)),
          a1,
      ...
      case Timestamp(a1) =>
        If(
          Call(
            Select(
              Call(ident("RegExp"), List(Literal(Js.Str("^" + string.timestampRegex + "$")))),
              "test"),
            List(a1)),
          Call(ident("ISODate"), List(a1)),
          ident("undefined")
      ...
      case ExtractMonth(date) =>
        BinOp(jscore.Add, Call(Select(date, "getUTCMonth"), Nil), Literal(Js.Num(1, false)))
          .point[M]
      ...
      case Between(a1, a2, a3) =>
        Call(
          ident("&&"),
          List(Call(ident("<="), List(a2, a1)), Call(ident("<="), List(a1, a3)))).point[M]
      case Cond(a1, a2, a3) => If(a1, a2, a3).point[M]

      case Within(a1, a2) =>
        BinOp(jscore.Neq, Literal(Js.Num(-1, false)), Call(Select(a2, "indexOf"), List(a1)))
          .point[M]
      ...
      case Bool(a1) =>
        If(
          BinOp(jscore.Eq, a1, Literal(Js.Str("true"))),
          Literal(Js.Bool(true)),
          If(
            BinOp(jscore.Eq, a1, Literal(Js.Str("false"))),
      ...
      case Integer(a1) =>
        If(
          Call(
            Select(
              Call(ident("RegExp"), List(Literal(Js.Str("^" + string.intRegex + "$")))),
              "test"),
            List(a1)),
      ...
      case Decimal(a1) =>
        If(
          Call(
            Select(
              Call(ident("RegExp"), List(Literal(Js.Str("^" + string.floatRegex + "$")))),
              "test"),
            List(a1)),
      ...
      case Null(a1) =>
        If(BinOp(jscore.Eq, a1, Literal(Js.Str("null"))), Literal(Js.Null), ident("undefined"))
          .point[M]
      case ToString(a1) =>
        If(
          isInt(a1),
          ...
          //     losing precision.
          Call(
            Select(Call(ident("String"), List(a1)), "replace"),
            List(
              Call(ident("RegExp"), List(Literal(Js.Str("[^-0-9]+")), Literal(Js.Str("g")))),
              Literal(Js.Str("")))
          ),
          If(
            binop(jscore.Or, isTimestamp(a1), isDate(a1)),
            Call(Select(a1, "toISOString"), Nil),
            Call(ident("String"), List(a1)))
        ).point[M]
      case Search(a1, a2, a3) =>
        Call(
          Select(
            New(Name("RegExp"), List(a2, If(a3, Literal(Js.Str("im")), Literal(Js.Str("m"))))),
            "test"),
          List(a1)).point[M]
      ...
      case Guard(expr, typ, cont, fallback) =>
        val jsCheck: Type => Option[JsCore => JsCore] =
          generateTypeCheck[JsCore, JsCore](BinOp(jscore.Or, _, _)) {
            ...
            case Type.Dec => isDec
            case Type.Int | Type.Int ⨿ Type.Dec | Type.Int ⨿ Type.Dec ⨿ Type.Interval =>
              isAnyNumber
            case Type.Str => isString
            case Type.Obj(_, _) ⨿ Type.FlexArr(_, _, _) => isObjectOrArray
        ...
        jsCheck(typ).fold[M[JsCore]](
          raiseErr(qscriptPlanningFailed(InternalError.fromMsg("uncheckable type"))))(f =>
          If(f(expr), cont, fallback).point[M])
        ...
        Call(
          Select(
            Call(
              ...
              List(Literal(Js.Null), Call(ident("Array"), List(BinOp(Sub, end, start))))),
            "map"),
          List(
            Fun(List(Name("element"), Name("index")), BinOp(jscore.Add, ident("index"), start)))
        ).point[M]
    }
  ...
  def defaultSelector[T[_[_]]]: PartialSelector[T] =
    ({
      case List(field) =>
        Selector.Doc(ListMap(field -> Selector.Expr(Selector.Eq(Bson.Bool(true)))))
    }, List(Here[T]()))
  ...
  def selector[T[_[_]]: RecursiveT: ShowT](v: BsonVersion)
    ...
    def invoke(func: MapFunc[T, (T[MapFunc[T, ?]], Output)]): Output = {
       ...
       */
      def relop(x: (T[MapFunc[T, ?]], Output), y: (T[MapFunc[T, ?]], Output))(
          f: Bson => Selector.Condition,
          r: Bson => Selector.Condition): Output =
        (x, y) match {
          case (_, IsBson(v2)) =>
            \/-(
              (
                { case List(f1) => Selector.Doc(ListMap(f1 -> Selector.Expr(f(v2)))) },
                List(There(0, Here[T]()))))
          case (IsBson(v1), _) =>
            \/-(
              (
                { case List(f2) => Selector.Doc(ListMap(f2 -> Selector.Expr(r(v1)))) },
                List(There(1, Here[T]()))))
      ...
      def relDateOp1(
          f: Bson.Date => Selector.Condition,
          date: Data.Date,
          g: Data.Date => Data.Timestamp,
          index: Int): Output =
        Bson.Date
          .fromInstant(g(date).value)
          .fold[Output](-\/(NonRepresentableData(g(date))))(d =>
            \/-(({ case x :: Nil => Selector.Doc(x -> f(d)) }, List(There(index, Here[T]())))))

      def relDateOp2(
          conj: (Selector, Selector) => Selector,
          f1: Bson.Date => Selector.Condition,
          f2: Bson.Date => Selector.Condition,
          date: Data.Date,
          g1: Data.Date => Data.Timestamp,
          g2: Data.Date => Data.Timestamp,
          index: Int): Output =
        ((Bson.Date.fromInstant(g1(date).value) \/> NonRepresentableData(g1(date))) ⊛
          (Bson.Date.fromInstant(g2(date).value) \/> NonRepresentableData(g2(date))))(
          (d1, d2) =>
            ({
              case x :: Nil =>
                conj(Selector.Doc(x -> f1(d1)), Selector.Doc(x -> f2(d2)))
            }, List(There(index, Here[T]()))))

      def invoke2Nel(x: Output, y: Output)(f: (Selector, Selector) => Selector): Output =
        (x ⊛ y) {
          case ((f1, p1), (f2, p2)) =>
            ({
              case list =>
      ...
      def reversibleRelop(x: (T[MapFunc[T, ?]], Output), y: (T[MapFunc[T, ?]], Output))(
          f: MapFunc[T, _]): Output =
        (relFunc(f) ⊛ flip(f).flatMap(relFunc))(relop(x, y)(_, _))
          .getOrElse(-\/(InternalError fromMsg "couldn’t decipher operation"))
      ...
      func match {
        ...
        case MFC(Search(_, IsText(patt), IsBool(b))) =>
          \/-(({
            case List(f1) =>
              Selector.Doc(
                ListMap(f1 -> Selector.Expr(Selector.Regex(patt, b, true, false, false))))
          }, List(There(0, Here[T]()))))
        ...
        case MFC(Guard(_, typ, cont, _)) =>
          def selCheck: Type => Option[BsonField => Selector] =
            generateTypeCheck[BsonField, Selector](Selector.Or(_, _)) {
              case Type.Null =>
                ((f: BsonField) => Selector.Doc(f -> Selector.Type(BsonType.Null)))
              case Type.Dec =>
                ((f: BsonField) => Selector.Doc(f -> Selector.Type(BsonType.Dec)))
              case Type.Int =>
                ((f: BsonField) =>
                  Selector.Or(
              ...
              case Type.Str =>
                ((f: BsonField) => Selector.Doc(f -> Selector.Type(BsonType.Text)))
              case Type.Obj(_, _) =>
          ...
          selCheck(typ).fold[OutputM[PartialSelector[T]]](
            -\/(InternalError.fromMsg(node.map(_._1).shows)))(f =>
  ...
  def liftM[M[_]: Monad: MonadFsErr, A](meh: WBM[A]): M[A] =
    meh.fold(e => raiseErr(qscriptPlanningFailed(e)), _.point[M])
  ...
  trait Planner[F[_]] {
    ...
    def plan[
        ...
        ev3: EX :<: ExprOp): AlgebraM[M, F, WorkflowBuilder[WF]]
  }
  ...
  object Planner {
              ...
              .fold(
                ...
                coll => {
                  ...
                  qs.getConst.idStatus match {
                    case IdOnly =>
                      getExprBuilder[T, M, WF, EX](funcHandler)(
                        dataset,
                        Free.roll(MFC(MapFuncsCore
                          .ProjectField[T, FreeMap[T]](HoleF[T], MapFuncsCore.StrLit("_id")))))
                    case IncludeId =>
                      getExprBuilder[T, M, WF, EX](funcHandler)(
                        dataset,
                        MapFuncCore.StaticArray(
                          List(
                            Free.roll(
                              MFC(MapFuncsCore.ProjectField[T, FreeMap[T]](
                                HoleF[T],
                                MapFuncsCore.StrLit("_id")))),
                            HoleF)))
        ...
        def plan[
            ...
            if (repair.contains(LeftSideF))
              (handleFreeMap[T, M, EX](funcHandler, struct) ⊛
                getJsMerge[T, M](
                  ...
                  jscore.Select(jscore.Ident(JsFn.defaultName), "f")))(
                (expr, j) =>
                  ExprBuilder(
                    FlatteningBuilder(
                      DocBuilder(
              ...
              getExprBuilder[T, M, WF, EX](funcHandler)(src, struct) >>= (builder =>
                getExprBuilder[T, M, WF, EX](funcHandler)(
                  FlatteningBuilder(builder, Set(StructureType.Object(DocVar.ROOT(), id))),
                  repair.as(SrcHole)))
          case Reduce(src, bucket, reducers, repair) =>
            (bucket.traverse(handleFreeMap[T, M, EX](funcHandler, _)) ⊛
              reducers.traverse(_.traverse(handleFreeMap[T, M, EX](funcHandler, _))))(
              (b, red) => {
                getReduceBuilder[T, M, WF, EX](funcHandler)(
                  ...
                  semiAlignExpr[λ[α => List[ReduceFunc[α]]]](red)(Traverse[List].compose).fold(
                    WB.groupBy(
                      DocBuilder(
                        ...
                        red.unite.zipWithIndex
                          .map(_.map(i => BsonField.Name(createFieldName("f", i))).swap)
                          .toListMap ++
                          b.zipWithIndex
                            .map(_.map(i => BsonField.Name(createFieldName("b", i))).swap)
                            .toListMap
                      ),
                      b.zipWithIndex.map(p =>
                        docVarToExpr(DocField(BsonField.Name(createFieldName("b", p._2))))),
                      red.zipWithIndex
                        .map(ai =>
                          (
                            BsonField.Name(createFieldName("f", ai._2)),
                            accumulator(ai._1.as($field(createFieldName("f", ai._2))))))
                        .toListMap
                    ))(
                    exprs =>
                      WB.groupBy(
                        ...
                        exprs.zipWithIndex
                          .map(ai =>
                            (BsonField.Name(createFieldName("f", ai._2)), accumulator(ai._1)))
                          .toListMap)),
                  repair
                )
              }).join
          ...
          case Filter(src, cond) =>
            getSelector[T, M, EX](v)(cond).fold(
              _ =>
                ...
                case (sel, inputs) =>
                  inputs
                    .traverse(f => handleFreeMap[T, M, EX](funcHandler, f(cond)))
          ...
          case Subset(src, from, sel, count) =>
            (rebaseWB[T, M, WF, EX](joinHandler, funcHandler, v, from, src) ⊛
              (rebaseWB[T, M, WF, EX](joinHandler, funcHandler, v, count, src) >>= (HasInt[
                M,
                WF](_))))(sel match {
              case Drop => WB.skip
              case Take => WB.limit
            ...
            (rebaseWB[T, M, WF, EX](joinHandler, funcHandler, v, qs.lBranch, qs.src) ⊛
              ...
              (lb, rb) => {
                ...
                (lKey.traverse(handleFreeMap[T, M, EX](funcHandler, _)) ⊛
                  ...
                  (lk, rk) =>
                    liftM[M, WorkflowBuilder[WF]](
                      joinHandler.run(qs.f, JoinSource(lb, lk), JoinSource(rb, rk))) >>=
                      (
                        getExprBuilder[T, M, WF, EX](funcHandler)(
                          _,
                          qs.combine >>= {
                            case LeftSide =>
                              Free.roll(MFC(
                                MapFuncsCore.ProjectField(HoleF, MapFuncsCore.StrLit("left"))))
                            case RightSide =>
                              Free.roll(MFC(
                                MapFuncsCore.ProjectField(HoleF, MapFuncsCore.StrLit("right"))))
                          }
                        )
                    )).join
              }).join
    ...
    implicit def coproduct[T[_[_]], F[_], G[_]](
        ...
        G: Planner.Aux[T, G]): Planner.Aux[T, Coproduct[F, G, ?]] =
    ...
    def default[T[_[_]], F[_]](label: String): Planner.Aux[T, F] =
      new Planner[F] {
        ...
        def plan[
          ...
          κ(
            raiseErr(
              qscriptPlanningFailed(InternalError.fromMsg(s"should not be reached: $label"))))
      }
  ...
  def getExpr[
      ...
      M[_]: Monad: ExecTimeR: MonadFsErr,
      EX[_]: Traverse: Inject[?[_], ExprOp]](funcHandler: MapFunc[T, ?] ~> OptionFree[EX, ?])(
      fm: FreeMap[T]): M[Fix[ExprOp]] =
  ...
  def getBuilder[
      T[_[_]]: BirecursiveT: ShowT,
      M[_]: Monad: MonadFsErr,
      WF[_],
      EX[_]: Traverse,
      A](handler: FreeMapA[T, A] => M[Expr])(src: WorkflowBuilder[WF], fm: FreeMapA[T, A])(
  ...
  def getExprBuilder[
      ...
      fm: FreeMap[T])(implicit ev: EX :<: ExprOp): M[WorkflowBuilder[WF]] =
  ...
  def getReduceBuilder[
      ...
      EX[_]: Traverse](funcHandler: MapFunc[T, ?] ~> OptionFree[EX, ?])(
      src: WorkflowBuilder[WF],
      fm: FreeMapA[T, ReduceIndex])(implicit ev: EX :<: ExprOp): M[WorkflowBuilder[WF]] =
  ...
  def getJsMerge[T[_[_]]: BirecursiveT: ShowT, M[_]: Monad: MonadFsErr: ExecTimeR](
      ...
      a1: JsCore,
      a2: JsCore): M[JsFn] =
    processMapFunc[T, M, JoinSide](jf) {
      case LeftSide => a1
      case RightSide => a2
    } ∘ (JsFn(JsFn.defaultName, _))

  def exprOrJs[M[_]: Applicative: MonadFsErr, A](
      a: A)(exf: A => M[Fix[ExprOp]], jsf: A => M[JsFn]): M[Expr] = {
    // TODO: Return _both_ errors
    val js = jsf(a)
    val expr = exf(a)
    handleErr[M, Expr]((js ⊛ expr)(\&/.Both(_, _)))(_ =>
      handleErr[M, Expr](js.map(-\&/))(_ => expr.map(\&/-)))
  }

  def handleFreeMap[
      T[_[_]]: BirecursiveT: ShowT,
      M[_]: Monad: ExecTimeR: MonadFsErr,
      EX[_]: Traverse](funcHandler: MapFunc[T, ?] ~> OptionFree[EX, ?], fm: FreeMap[T])(
      implicit ev: EX :<: ExprOp): M[Expr] =
  ...
  def handleRedRepair[
      ...
      EX[_]: Traverse](
      funcHandler: MapFunc[T, ?] ~> OptionFree[EX, ?],
      jr: FreeMapA[T, ReduceIndex])(implicit ev: EX :<: ExprOp): M[Expr] =
  ...
  def getExprRed[
      ...
      M[_]: Monad: ExecTimeR: MonadFsErr,
      EX[_]: Traverse](funcHandler: MapFunc[T, ?] ~> OptionFree[EX, ?])(
      jr: FreeMapA[T, ReduceIndex])(implicit ev: EX :<: ExprOp): M[Fix[ExprOp]] =
    processMapFuncExpr[T, M, EX, ReduceIndex](funcHandler)(jr)(
      _.idx.fold(i => $field("_id", i.toString), i => $field(createFieldName("f", i))))

  def getJsRed[T[_[_]]: BirecursiveT: ShowT, M[_]: Monad: MonadFsErr: ExecTimeR](
      jr: Free[MapFunc[T, ?], ReduceIndex]): M[JsFn] =
    processMapFunc[T, M, ReduceIndex](jr)(
      _.idx.fold(
  ...
  def rebaseWB[
      ...
      ev1: RenderTree[WorkflowBuilder[WF]],
      ev2: WorkflowBuilder.Ops[WF],
      ev3: EX :<: ExprOp): M[WorkflowBuilder[WF]] =
    free.cataM(
      interpretM[M, QScriptTotal[T, ?], qscript.Hole, WorkflowBuilder[WF]](
        κ(src.point[M]),
        F.plan(joinHandler, funcHandler, v)))

  // TODO: Need `Delay[Show, WorkflowBuilder]`
  @SuppressWarnings(Array("org.wartremover.warts.ToString"))
  def HasLiteral[M[_]: Applicative: MonadFsErr, WF[_]](wb: WorkflowBuilder[WF])(
      implicit ev0: WorkflowOpCoreF :<: WF): M[Bson] =
    asLiteral(wb).fold(
      raiseErr[M, Bson](qscriptPlanningFailed(NonRepresentableEJson(wb.toString))))(_.point[M])

  @SuppressWarnings(Array("org.wartremover.warts.ToString"))
  def HasInt[M[_]: Monad: MonadFsErr, WF[_]](wb: WorkflowBuilder[WF])(
      implicit ev0: WorkflowOpCoreF :<: WF): M[Long] =
    HasLiteral[M, WF](wb) >>= {
  ...
  def findFirst[T[_[_]]: RecursiveT, F[_]: Functor: Foldable, A](
      f: PartialFunction[T[F], A]): CoalgebraM[A \/ ?, F, T[F]] =
    tf => (f.lift(tf) \/> tf.project).swap

  def elideMoreGeneralGuards[M[_]: Applicative: MonadFsErr, T[_[_]]: RecursiveT](
      subType: Type): CoEnvMap[T, FreeMap[T]] => M[CoEnvMap[T, FreeMap[T]]] = {
    def f: CoEnvMap[T, FreeMap[T]] => M[CoEnvMap[T, FreeMap[T]]] = {
      case free @ CoEnv(
            \/-(MFC(MapFuncsCore.Guard(Embed(CoEnv(-\/(SrcHole))), typ, cont, fb)))) =>
            ...
            raiseErr(
              qscriptPlanningFailed(
                InternalError.fromMsg(
                  s"can only contain ${subType.shows}, but a(n) ${typ.shows} is expected")))
          else {
            CoEnv[Hole, MapFunc[T, ?], FreeMap[T]](
              MFC(MapFuncsCore.Guard[T, FreeMap[T]](HoleF[T], union, cont, fb)).right).point[M]
          }
        }
      case x => x.point[M]
    }
          ...
          GtoF.get(src) >>= QC.prj match {
            case Some(Sort(innerSrc, bucket, order)) =>
              val innerMap =
                GtoF
                  .reverseGet(
                    QC.inj(Map(innerSrc, MapFuncCore.StaticArray(List(fm, HoleF[T])))))
                  .embed
              QC.inj(
                Map(
                  GtoF
                    .reverseGet(QC.inj(Sort(
                      innerMap,
                      bucket.map(_ >> Free.roll[MapFunc[T, ?], Hole](
                        MFC(MF.ProjectIndex(HoleF[T], MF.IntLit(1))))),
                      order.map {
                        case (fm, dir) =>
                          (
                            fm >> Free.roll[MapFunc[T, ?], Hole](
                              MFC(MF.ProjectIndex(HoleF[T], MF.IntLit(1)))),
                            dir)
                      }
                    )))
  ...
  def assumeReadType[
      ...
      F[_]: Functor](typ: Type)(
      implicit
      QC: QScriptCore[T, ?] :<: F,
      SR: Const[ShiftedRead[AFile], ?] :<: F): QScriptCore[T, T[F]] => M[F[T[F]]] = {
    case f @ Filter(src, cond) =>
      src.project match {
        case QC(Filter(Embed(SR(Const(ShiftedRead(_, ExcludeId)))), _)) | QC(
              Sort(Embed(SR(Const(ShiftedRead(_, ExcludeId)))), _, _)) | QC(
              Sort(Embed(QC(Filter(Embed(SR(Const(ShiftedRead(_, ExcludeId)))), _))), _, _)) |
            ...
            .map(_.toList.filter {
              ...
              case h :: t =>
                QC(
                  Filter(
                    src,
                    t.foldLeft[FreeMap[T]](h)((acc, e) =>
                      Free.roll(MFC(MapFuncsCore.And(acc, e))))))
            })
  ...
  def log[M[_]: Monad, A: RenderTree](label: String, ma: M[A])(
      implicit mtell: MonadTell_[M, PhaseResults]): M[A] =
  ...
  def toMongoQScript[
    ...
    for {
      ...
      optimized <- rewrite
        ...
        .point[M]
      mongoQs <- optimized.transCataM(
        liftFGM(assumeReadType[M, T, fs.MongoQScript[T, ?]](Type.AnyObject)))
  ...
  def plan0[
      ...
      v: BsonVersion)(qs: T[fs.MongoQScript[T, ?]])(
      implicit
      ev0: WorkflowOpCoreF :<: WF,
      ev1: WorkflowBuilder.Ops[WF],
      ev2: EX :<: ExprOp,
      ev3: RenderTree[Fix[WF]]): M[Crystallized[WF]] = {
    ...
    for {
      ...
      wb <- log(
        ...
        opt.cataM[M, WorkflowBuilder[WF]](
          Planner[T, fs.MongoQScript[T, ?]]
            .plan[M, WF, EX](joinHandler, funcHandler(v), v)
            .apply(_) ∘
  ...
  def planExecTime[
      ...
      qs: T[fs.MongoQScript[T, ?]],
      queryContext: fs.QueryContext[M],
  ...
  def plan[
      ...
      qs: T[fs.MongoQScript[T, ?]],
      queryContext: fs.QueryContext[M],
      queryModel: MongoQueryModel): M[Crystallized[WorkflowF]] = {
    ...
    queryModel match {
      case `3.4` =>
        ...
        plan0[T, M, Workflow3_2F, Expr3_4](
          queryContext.listContents,
          joinHandler,
          FuncHandler.handle3_4[MapFunc[T, ?]],
          bsonVersion)(qs)
      ...
      case `3.2` =>
        val joinHandler =
          JoinHandler.fallback[Workflow3_2F, WBM](
            JoinHandler.pipeline(queryContext.statistics, queryContext.indexes),
            JoinHandler.mapReduce)
        plan0[T, M, Workflow3_2F, Expr3_2](
          queryContext.listContents,
          joinHandler,
          FuncHandler.handle3_2[MapFunc[T, ?]],
          bsonVersion)(qs)

      case `3.0` =>
        val joinHandler = JoinHandler.mapReduce[WBM, Workflow2_6F]
        plan0[T, M, Workflow2_6F, Expr3_0](
          queryContext.listContents,
          joinHandler,
          FuncHandler.handle3_0[MapFunc[T, ?]],
          bsonVersion)(qs).map(_.inject[WorkflowF])

      case _ =>
        val joinHandler = JoinHandler.mapReduce[WBM, Workflow2_6F]
        plan0[T, M, Workflow2_6F, Expr2_6](
          queryContext.listContents,
          joinHandler,
          FuncHandler.handle2_6[MapFunc[T, ?]],
          bsonVersion)(qs).map(_.inject[WorkflowF])
    }
  }
}
