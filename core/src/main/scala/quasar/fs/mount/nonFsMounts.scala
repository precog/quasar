...
object nonFsMounts {
    ...
    def mountMove(scenario: MoveScenario, semantics: MoveSemantics): manage.M[Unit] = {
      ...
      def cacheMove(s: AFile) =
        refineType(scenario.dst)
          .leftMap(p =>
            pathErr(PathError.invalidPath(p, "view mount destination must be a file")))
          .traverse(d => VC.move(s, d))

      val move =
        (mount.exists(scenario.src) |@| mount.exists(scenario.dst) |@| destinationNonEmpty).tupled.flatMap {
          case (srcMountExists, dstMountExists, dstNonEmpty) =>
            (semantics match {
              ...
              case _ if srcMountExists && dstNonEmpty =>
                ...
                (manage.delete(scenario.dst).run *> overwriteMount(scenario.src, scenario.dst))
      ...
      EitherT(vcacheGet(scenario.src).fold(cacheMove, move).join)
    }
  ...
  private def vcacheGet[S[_]](p: APath)(
      implicit VC: VCache.Ops[S]): OptionT[Free[S, ?], AFile] =
