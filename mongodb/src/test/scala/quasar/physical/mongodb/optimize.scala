  ...
  "simplifyGroup" should {
    "elide useless reduction" in {
      val op =
        chain[Workflow](
          ...
          $group(
            Grouped(ListMap(BsonField.Name("city") -> $last($field("city")))),
            $field("city").right))
      ...
      val exp =
        chain[Workflow](
          ...
          $project(Reshape(ListMap(BsonField.Name("city") -> $field("_id").right)), IgnoreId)
        )
          ...
          $group(
            ...
            Reshape(
              ListMap(
                ...
                BsonField.Name("1") -> $field("state").right)).left),
          $project(
            Reshape(ListMap(BsonField.Name("city") -> $field("_id", "0").right)),
            IgnoreId)
        )
    ...
    "preserve useless-but-array-creating reduction" in {
      val op = chain[Workflow](
        $read(collection("db", "zips")),
        $group(
          Grouped(ListMap(BsonField.Name("city") -> $push($field("city")))),
  ...
  "reorder" should {
    ...
    "push $match before $project" in {
      ...
      val exp = chain[Workflow](
        $read(collection("db", "zips")),
        $match(
          Selector.Doc((BsonField.Name("address") \ BsonField.Name("city")) -> Selector.Eq(
            Bson.Text("BOULDER")))),
        $project(
          Reshape(
            ListMap(BsonField.Name("city") -> \/-(
    ...
    "not push $match before $project with dependency" in {
      val op = chain[Workflow](
        ...
        $match(Selector.Doc(BsonField.Name("__tmp0") -> Selector.Eq(Bson.Text("boulder"))))
      )
      ...
      val op = chain[Workflow](
        ...
        $simpleMap(
          NonEmptyList(MapExpr(
            JsFn(Name("x"), obj("__tmp0" -> ident("x"), "pop" -> Select(ident("x"), "pop"))))),
          ListMap()),
        $match(
          Selector.Doc(BsonField.Name("__tmp0") \ BsonField.Name("city") -> Selector.Eq(
            Bson.Text("BOULDER"))))
        ...
        $simpleMap(
          NonEmptyList(
            MapExpr(
              JsFn(
                Name("x"),
                SpliceObjects(List(ident("x"), obj("city" -> Select(ident("x"), "city"))))))),
          ListMap()),
        $match(Selector.Doc(BsonField.Name("city") -> Selector.Eq(Bson.Text("BOULDER"))))
      )
      val exp = chain[Workflow](
    ...
    "not push $match before $simpleMap with dependency" in {
      ...
      val op = chain[Workflow](
        ...
        $match(Selector.Doc(BsonField.Name("__sd_tmp_0") -> Selector.Lt(Bson.Int32(1000))))
      )
    ...
    "not push $match before flattening $simpleMap" in {
      ...
      val op = chain[Workflow](
        ...
        $simpleMap(
          NonEmptyList(
            MapExpr(
              JsFn(Name("x"), obj("city" -> Select(Select(ident("x"), "__tmp0"), "city")))),
            FlatExpr(JsFn(Name("x"), Select(ident("x"), "loc")))),
          ListMap()
        ),
        $match(Selector.Doc(BsonField.Name("city") -> Selector.Eq(Bson.Text("BOULDER"))))
  ...
  "inline" should {
    "inline simple project on group" in {
      val inlined = inlineProjectGroup(
        Reshape(ListMap(BsonField.Name("foo") -> \/-($var(DocField(BsonField.Name("value")))))),
    ...
    "inline project on group with nesting" in {
      val inlined = inlineProjectGroup(
        ...
        Grouped(
          ListMap(BsonField.Name("value") -> $push($var(DocField(BsonField.Name("foo"))))))
      )
