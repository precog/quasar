...
sealed abstract class PlannerInstances extends PlannerInstances0 {
  implicit def coproduct[M[_], FMT, F[_], G[_], J](
      implicit F: Lazy[Planner[M, FMT, F, J]],
      G: Lazy[Planner[M, FMT, G, J]]
...
sealed abstract class PlannerInstances0 extends PlannerInstances1 {
  ...
  implicit def constReadDir[F[_]: MonadPlanErr, FMT, J]
    : Planner[F, FMT, Const[Read[ADir], ?], J] =
    new UnreachablePlanner[F, FMT, Const[Read[ADir], ?], J]("Read[ADir]")

  implicit def constShiftedReadFile[F[_]: MonadPlanErr, FMT, J]
    : Planner[F, FMT, Const[ShiftedRead[AFile], ?], J] =
    new UnreachablePlanner[F, FMT, Const[ShiftedRead[AFile], ?], J]("ShiftedRead[AFile]")

  implicit def equiJoin[F[_]: MonadPlanErr, FMT, T[_[_]], J]
    : Planner[F, FMT, EquiJoin[T, ?], J] =
    new UnreachablePlanner[F, FMT, EquiJoin[T, ?], J]("EquiJoin")

  implicit def projectBucket[F[_]: MonadPlanErr, FMT, T[_[_]], J]
    : Planner[F, FMT, ProjectBucket[T, ?], J] =
    new UnreachablePlanner[F, FMT, ProjectBucket[T, ?], J]("ProjectBucket")
}

sealed abstract class PlannerInstances1 {
  implicit def eitherTPlanner[M[_]: Functor, FMT, F[_], E, J](
      implicit P: Planner[M, FMT, F, J]): Planner[EitherT[M, E, ?], FMT, F, J] =
    new Planner[EitherT[M, E, ?], FMT, F, J] {
      def plan[Q](implicit Q: Birecursive.Aux[Q, Query[J, ?]]) =
        fx => EitherT(P.plan.apply(fx) map (_.right[E]))
    }

  implicit def writerTPlanner[M[_]: Functor, FMT, F[_], W: Monoid, J](
      implicit P: Planner[M, FMT, F, J]): Planner[WriterT[M, W, ?], FMT, F, J] =
