...
object xdmitem {
  ...
  private def parseTemporal[F[_]: MonadErrMsgs, A](name: String, parse: String => Option[A])(
      str: String): F[A] =
  ...
  private def xmlToData[F[_]: MonadErrMsgs](xmlString: String): F[Data] = {
    val elem =
      SecureXML.loadString(xmlString).fold(_.toString.wrapNel.raiseError[F, Elem], _.point[F])
