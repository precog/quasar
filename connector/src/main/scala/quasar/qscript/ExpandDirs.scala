...
trait ExpandDirs[IN[_]] {
  ...
  def expandDirs[M[_]: Monad: MonadFsErr, F[_]: Functor](
      OutToF: OUT ~> F,
      g: DiscoverPath.ListContents[M]): IN[IT[F]] => M[F[IT[F]]]
}
...
abstract class ExpandDirsInstances {
  ...
  implicit def readDir[T[_[_]]: BirecursiveT, F[_]: Functor](
      ...
      QC: QScriptCore[T, ?] :<: F,
      FI: Injectable.Aux[F, QScriptTotal[T, ?]]): ExpandDirs.Aux[T, Const[Read[ADir], ?], F] =
  ...
  implicit def qscriptCore[T[_[_]]: BirecursiveT, F[_]](
      implicit QC: QScriptCore[T, ?] :<: F): ExpandDirs.Aux[T, QScriptCore[T, ?], F] =
    expandDirsBranch[T].qscriptCore[F]

  implicit def thetaJoin[T[_[_]]: BirecursiveT, F[_]](
      implicit TJ: ThetaJoin[T, ?] :<: F): ExpandDirs.Aux[T, ThetaJoin[T, ?], F] =
    expandDirsBranch[T].thetaJoin[F]

  implicit def equiJoin[T[_[_]]: BirecursiveT, F[_]](
      implicit EJ: EquiJoin[T, ?] :<: F): ExpandDirs.Aux[T, EquiJoin[T, ?], F] =
    expandDirsBranch[T].equiJoin[F]

  implicit def coproduct[T[_[_]], F[_], G[_], H[_]](
      implicit F: ExpandDirs.Aux[T, F, H],
      G: ExpandDirs.Aux[T, G, H]): ExpandDirs.Aux[T, Coproduct[F, G, ?], H] =
  ...
  def default[T[_[_]], F[_], G[_]](implicit F: F :<: G): ExpandDirs.Aux[T, F, G] =
    new ExpandDirs[F] {
      type IT[F[_]] = T[F]
      type OUT[A] = G[A]
  ...
  implicit def deadEnd[T[_[_]], F[_]](
      implicit DE: Const[DeadEnd, ?] :<: F): ExpandDirs.Aux[T, Const[DeadEnd, ?], F] =
    default

  implicit def readFile[T[_[_]], F[_]](
      implicit R: Const[Read[AFile], ?] :<: F): ExpandDirs.Aux[T, Const[Read[AFile], ?], F] =
    default

  implicit def shiftedReadFile[T[_[_]], F[_]](implicit SR: Const[ShiftedRead[AFile], ?] :<: F)
    : ExpandDirs.Aux[T, Const[ShiftedRead[AFile], ?], F] =
    default

  implicit def projectBucket[T[_[_]], F[_]](
      implicit PB: ProjectBucket[T, ?] :<: F): ExpandDirs.Aux[T, ProjectBucket[T, ?], F] =
    default
}

private[qscript] final class ExpandDirsPath[T[_[_]]: BirecursiveT, O[_]: Functor](
    implicit FI: Injectable.Aux[O, QScriptTotal[T, ?]],
    QC: QScriptCore[T, ?] :<: O
) extends TTypes[T] {
  def union(elems: NonEmptyList[O[T[O]]]): O[T[O]] =
    elems.foldRight1(
      (elem, acc) =>
        QC.inj(Union(
          QC.inj(Unreferenced[T, T[O]]()).embed,
  ...
  def allDescendents[M[_]: Monad: MonadFsErr](
      ...
      wrapFile: AFile => O[T[O]]): ADir => M[List[O[T[O]]]] =
    dir =>
      (listContents(dir) >>=
        (ps =>
          ISet
            ...
            .traverseM(_.fold(
              d =>
                allDescendents[M](listContents, wrapFile)
                  .apply(dir </> dir1(d)) ∘ (_ ∘ (wrapDir(d.value, _))),
  ...
  def unionDirs[M[_]: Monad: MonadFsErr](
      g: DiscoverPath.ListContents[M],
      wrapFile: AFile => O[T[O]]): ADir => M[Option[NonEmptyList[O[T[O]]]]] =
  ...
  def unionAll[M[_]: Monad: MonadFsErr, F[_]: Functor](
      OutToF: O ~> F,
      g: DiscoverPath.ListContents[M],
      wrapFile: AFile => O[T[O]]): ADir => M[F[T[F]]] =
    dir =>
      unionDirs[M](g, wrapFile).apply(dir) >>= (_.fold[M[F[T[F]]]](
        MonadError_[M, FileSystemError].raiseError(
          FileSystemError.qscriptPlanningFailed(NoFilesFound(List(dir)))))(nel =>
        OutToF(union(nel) ∘ (_.transAna[T[F]](OutToF))).point[M]))

  def readDir(
      implicit R: Const[Read[AFile], ?] :<: O): ExpandDirs.Aux[T, Const[Read[ADir], ?], O] =
  ...
  def shiftedReadDir(implicit SR: Const[ShiftedRead[AFile], ?] :<: O)
    : ExpandDirs.Aux[T, Const[ShiftedRead[ADir], ?], O] =
...
private[qscript] final class ExpandDirsBranch[T[_[_]]: BirecursiveT] extends TTypes[T] {
  ...
  def applyToBranch[M[_]: Monad: MonadFsErr](
      listContents: DiscoverPath.ListContents[M],
      branch: FreeQS): M[FreeQS] =
    branch.transCataM[M, T[CoEnvQS], CoEnvQS](
      liftCoM[T, M, QScriptTotal, Hole, T[CoEnvQS]](
        ExpandDirsTotal.expandDirs(coenvPrism[QScriptTotal, Hole].reverseGet, listContents))
    ) ∘ (_.convertTo[FreeQS])

  def qscriptCore[O[_]](implicit QC: QScriptCore :<: O): ExpandDirs.Aux[T, QScriptCore, O] =
  ...
  def thetaJoin[O[_]](implicit TJ: ThetaJoin :<: O): ExpandDirs.Aux[T, ThetaJoin, O] =
  ...
  def equiJoin[O[_]](implicit EJ: EquiJoin :<: O): ExpandDirs.Aux[T, EquiJoin, O] =
