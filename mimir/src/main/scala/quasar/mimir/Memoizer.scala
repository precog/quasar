...
trait Memoizer extends DAG {
    ...
    def memoized(node: DepGraph): DepGraph = {
      def inner(target: DepGraph): DepGraph = target match {
        ...
        case node @ dag.Join(op, joinSort, left, right) => {
          if (numRefs(node) > MemoThreshold)
            Memoize(dag.Join(op, joinSort, memoized(left), memoized(right))(node.loc),
                    scaleMemoPriority(numRefs(node)))
          else
        ...
        case node @ dag.Filter(joinSort, left, right) => {
          if (numRefs(node) > MemoThreshold)
            Memoize(dag.Filter(joinSort, memoized(left), memoized(right))(node.loc),
                    scaleMemoPriority(numRefs(node)))
          else
  ...
  private def findForcingRefsInSpec(spec: BucketSpec,
    ...
    spec match {
      ...
      case Group(id, target, forest) =>
        findForcingRefs(target, OpSide.Center(split)) |+| findForcingRefsInSpec(forest,
                                                                                split)
  ...
  private def findForcingRefs(graph: DepGraph,
    ...
    case Join(_, Cross(_), left, right)
      ...
      findForcingRefs(left, OpSide.Left(graph)) |+| findForcingRefs(right,
                                                                    OpSide.Right(graph))
    ...
    case Filter(Cross(_), target, boolean)
      ...
      findForcingRefs(target, OpSide.Left(graph)) |+| findForcingRefs(boolean,
                                                                      OpSide.Right(graph))
    }
