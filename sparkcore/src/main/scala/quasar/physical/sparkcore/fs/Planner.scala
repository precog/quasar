...
object Planner {
  ...
  implicit def shiftedReadPath[S[_]]: Planner[Const[ShiftedRead[ADir], ?], S] =
    unreachable("shifted read of a dir")
  implicit def projectBucket[T[_[_]], S[_]]: Planner[ProjectBucket[T, ?], S] =
    unreachable("projectBucket")
  implicit def thetaJoin[T[_[_]], S[_]]: Planner[ThetaJoin[T, ?], S] = unreachable("thetajoin")
  implicit def shiftedread[S[_]]: Planner[Const[ShiftedRead[AFile], ?], S] =
    new ShiftedReadPlanner[S]
  implicit def qscriptCore[T[_[_]]: BirecursiveT: ShowT, S[_]]: Planner[QScriptCore[T, ?], S] =
    new QScriptCorePlanner[T, S]
  implicit def equiJoin[T[_[_]]: BirecursiveT: ShowT, S[_]]: Planner[EquiJoin[T, ?], S] =
    new EquiJoinPlanner[T, S]
  ...
  implicit def coproduct[F[_], G[_], S[_]](
    ...
    new Planner[Coproduct[F, G, ?], S] {
      def plan(fromFile: AFile => Free[S, RDD[Data]], first: RDD[Data] => Free[S, Data])
        : AlgebraM[SparkState[S, ?], Coproduct[F, G, ?], RDD[Data]] =
        _.run.fold(F.plan(fromFile, first), G.plan(fromFile, first))
    }
  ...
  private def unreachable[F[_], S[_]](what: String): Planner[F, S] =
    new Planner[F, S] {
      def plan(
          fromFile: AFile => Free[S, RDD[Data]],
          first: RDD[Data] => Free[S, Data]): AlgebraM[SparkState[S, ?], F, RDD[Data]] =
        _ =>
