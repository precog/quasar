...
import quasar.jscore, jscore.{JsCoreF, Name}
...
object JsFuncHandler {
  ...
  implicit def mapFuncCore[T[_[_]]: BirecursiveT, J, E]: JsFuncHandler[MapFuncCore[T, ?]] =
          ...
          def apply[A](mfc: MapFuncCore[T, A]): OptionFree[JsCoreF, A] = {
            ...
            def trunc(expr: JS): JS =
              Let(
                Name("x"),
                expr,
                BinOp(jscore.Sub, ident("x"), BinOp(jscore.Mod, ident("x"), litNum(1))))
            ...
            def quarter(date: JS): JS =
              BinOp(
                ...
                Call(
                  ...
                  List(BinOp(jscore.Div, Call(select(date, "getUTCMonth"), Nil), litNum(3)))),
                litNum(1))
            ...
            def century(date: JS): JS =
              Call(
                ...
                List(BinOp(jscore.Div, year(date), litNum(100))))

            def millennium(date: JS): JS =
              Call(
                select(ident("Math"), "ceil"),
                List(BinOp(jscore.Div, year(date), litNum(1000))))
            ...
            def partial(mfc: MapFuncCore[T, A]): OptionFree[JsCoreF, A] = mfc.some collect {
              ...
              case Between(value, min, max) =>
                BinOp(jscore.And, BinOp(jscore.Lte, min, value), BinOp(jscore.Lte, value, max))
              ...
              case Substring(field, start, len) =>
                If(
                  BinOp(jscore.Lt, start, litNum(0)),
                  litStr(""),
                  If(
                    BinOp(jscore.Lt, len, litNum(0)),
              ...
              case Search(field, pattern, insen) =>
                Call(
                  select(
                    New(Name("RegExp"), List(pattern, If(insen, litStr("im"), litStr("m")))),
                    "test"),
                  List(field))
              case Null(str) =>
                If(BinOp(jscore.Eq, str, litStr("null")), Literal(Js.Null), ident("undefined"))
              case Bool(str) =>
                If(
              ...
              case Integer(str) =>
                If(
                  Call(
                    select(
                      Call(ident("RegExp"), List(litStr("^" + string.intRegex + "$"))),
                      "test"),
                    List(str)),
              ...
              case Decimal(str) =>
                If(
                  Call(
                    select(
                      Call(ident("RegExp"), List(litStr("^" + string.floatRegex + "$"))),
                      "test"),
                    List(str)),
              ...
              case Date(str) =>
                If(
                  Call(
                    select(
                      Call(ident("RegExp"), List(litStr("^" + string.dateRegex + "$"))),
                      "test"),
                    List(str)),
              ...
              case Time(str) =>
                If(
                  Call(
                    select(
                      Call(ident("RegExp"), List(litStr("^" + string.timeRegex + "$"))),
                      "test"),
                    List(str)),
                  str,
                  ident("undefined"))
              case Timestamp(str) =>
                If(
                  Call(
                    select(
                      Call(ident("RegExp"), List(litStr("^" + string.timestampRegex + "$"))),
                      "test"),
                    List(str)),
                  Call(ident("ISODate"), List(str)),
                  ident("undefined"))
              ...
              case ToString(value) =>
                If(
                  ...
                  //     losing precision.
                  Call(
                    select(Call(ident("String"), List(value)), "replace"),
                    List(
                      Call(ident("RegExp"), List(litStr("[^-0-9]+"), litStr("g"))),
                      litStr(""))),
                  If(
                    BinOp(jscore.Or, isTimestamp(value), isDate(value)),
              ...
              case ExtractEpoch(date) =>
                BinOp(jscore.Div, Call(select(date, "valueOf"), Nil), litNum(1000))
              case ExtractHour(date) => hour(date)
              case ExtractIsoDayOfWeek(date) =>
                Let(
                  Name("x"),
                  dayOfWeek(date),
                  If(BinOp(jscore.Eq, ident("x"), litNum(0)), litNum(7), ident("x")))
              ...
              case ExtractMicroseconds(date) =>
                BinOp(
                  ...
                  BinOp(
                    jscore.Add,
                    millisecond(date),
                    BinOp(jscore.Mult, second(date), litNum(1000))),
                  litNum(1000))
              ...
              case TemporalTrunc(Quarter, date) =>
                dateZ(
                  ...
                  BinOp(
                    jscore.Add,
                    BinOp(jscore.Mult, BinOp(jscore.Sub, quarter(date), litNum(1)), litNum(3)),
                    litNum(1)),
                  litNum(1),
                  litNum(0),
                  litNum(0),
              ...
              case TypeOf(v) =>
                Let(
                  Name("typ"),
                  UnOp(jscore.TypeOf, v),
                  If(
                    BinOp(jscore.Eq, ident("typ"), Literal(Js.Str("object"))),
                    If(
                      BinOp(jscore.Eq, v, Literal(Js.Null)),
                      Literal(Js.Str("null")),
                      If(
                        Call(select(ident("Array"), "isArray"), List(v)),
            ...
            partial(mfc) orElse (mfc match {
              case Constant(v1) =>
                v1.cata(Data.fromEJson)
                  .toJs
                  .map(_.transCata[Free[JsCoreF, A]](js => CoEnv(js.right[A])))
              case _ => None
            })
          }
        }
  ...
  def mapFuncDerived[T[_[_]]]: JsFuncHandler[MapFuncDerived[T, ?]] =
  ...
  implicit def mapFuncDerivedUnhandled[T[_[_]]: CorecursiveT](
      implicit core: JsFuncHandler[MapFuncCore[T, ?]]): JsFuncHandler[MapFuncDerived[T, ?]] =
    new JsFuncHandler[MapFuncDerived[T, ?]] {
  ...
  implicit def mapFuncCoproduct[F[_], G[_]](
      implicit F: JsFuncHandler[F],
      G: JsFuncHandler[G]): JsFuncHandler[Coproduct[F, G, ?]] =
    new JsFuncHandler[Coproduct[F, G, ?]] {
      def handle: Coproduct[F, G, ?] ~> OptionFree[JsCoreF, ?] =
        λ[Coproduct[F, G, ?] ~> OptionFree[JsCoreF, ?]](
          _.run.fold(
            F.handle.apply,
            G.handle.apply
          ))
