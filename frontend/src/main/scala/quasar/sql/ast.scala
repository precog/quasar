      ...
      def apply[A](ra: RenderTree[A]): RenderTree[Sql[A]] = new RenderTree[Sql[A]] {
        ...
        def render(n: Sql[A]) = n match {
          ...
          case InvokeFunction(name, args) =>
            NonTerminal("InvokeFunction" :: astType, Some(name.value), args.map(ra.render))
          ...
          case Match(expr, cases, None) =>
            NonTerminal("Match" :: astType, None, ra.render(expr) :: cases.map(renderCase))

          case Switch(cases, Some(default)) =>
            NonTerminal("Switch" :: astType, None, cases.map(renderCase) :+ ra.render(default))
      ...
      fa match {
        case Select(dist, proj, rel, filter, group, order) =>
          (proj.traverse(p => f(p.expr).map(Proj(_, p.alias))) ⊛
            ...
            filter.traverse(f) ⊛
            group.traverse(g => (g.keys.traverse(f) ⊛ g.having.traverse(f))(GroupBy(_, _))) ⊛
              ...
              case Ident(name) =>
                posixCodec
                  ...
                  .cata(
                    ...
                    SemanticError
                      .GenericError(s"bad path: $name (note: absolute file path required)")
                      .left) // FIXME
...
@Lenses final case class InvokeFunction[A] private[sql] (name: CIName, args: List[A])
    extends Sql[A]
@Lenses final case class Match[A] private[sql] (
    expr: A,
    cases: List[Case[A]],
    default: Option[A])
    extends Sql[A]
