  ...
  def readRenamed(src: AFile, from: String, to: String): Fix[LP] =
    lpr.invoke1(
      ...
      lpr.invoke2(
        ...
        lpr.invoke2(structural.ObjectProject, lpr.read(src), lpr.constant(Data._str(from))))
    )
  ...
  fileSystemShould { (fs, _) =>
    "Querying Files" should {
      ...
      "listing directory returns immediate child nodes" >> {
        ...
        runT(fs.testInterpM)(p).runEither must beRight(containTheSameElementsAs(expectedNodes))
      }
      ...
      "listing results should not contain deleted files" >> {
        ...
        runT(fs.testInterpM)(p).runEither must beRight(containTheSameElementsAs(preDelete))

        runT(fs.setupInterpM)(manage.delete(f1)).runVoid

        runT(fs.testInterpM)(p).runEither must beRight(containTheSameElementsAs(preDelete.tail))
      }
