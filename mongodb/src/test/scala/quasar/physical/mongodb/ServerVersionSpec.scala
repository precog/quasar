...
class ServerVersionSpec
  ...
  "ServerVersion" should {
    ...
    "parse truncated version" >> {
      ServerVersion.fromString("2.6") must beRightDisjunction(
    ...
    "compare with same major/minor" >> prop {
      (maj: Int, min: Int, rev: Int, extra: String) =>
    ...
    "compare with different major" >> prop {
      (maj: Int, min: Int, rev: Int, extra: String) =>
        maj < Int.MaxValue ==> {
          ServerVersion(maj, min, Some(rev), extra) must
    ...
    "compare with different minor" >> prop {
      (maj: Int, min: Int, rev: Int, extra: String) =>
        min < Int.MaxValue ==> {
          ServerVersion(maj, min, Some(rev), extra) must
