...
object MongoDbIO {
      ...
      .onFailure {
        case t: MongoCommandException =>
          credentials
            .liftM[Process]
            .flatMap(ys =>
              Process.emitAll(ys.map(y => DatabaseName(y.getSource)).distinct))
  ...
  /** Attempts to insert as many of the given documents into the collection as
    ...
    */
  def insertAny[F[_]: Foldable](coll: Collection,
  ...
  def rename(src: Collection,
             dst: Collection,
             semantics: RenameSemantics): MongoDbIO[Unit] = {
        ...
        .flatMap(
          c =>
            async[java.lang.Void](
              c.renameCollection(dst.asNamespace,
  ...
  def serverVersion: MongoDbIO[ServerVersion] = {
    ...
    val finalize
      : ((Vector[String], Vector[ServerVersion])) => MongoDbIO[ServerVersion] = {
      case (errs, vers) =>
  ...
  def collectionStatistics(coll: Collection): MongoDbIO[CollectionStatistics] = {
    ...
    runCommand(coll.database, cmd)
      ...
      .flatMap(
        _.fold(err =>
                 fail(new MongoException("could not read collection statistics: " + err)),
               _.point[MongoDbIO]))
  }
  ...
  private def collect[A](iter: MongoIterable[A]): MongoDbIO[List[A]] = {
    ...
    async[java.util.ArrayList[A]](cb =>
      iter.into[java.util.ArrayList[A]](new java.util.ArrayList[A], cb))
  ...
  private[mongodb] def collection(
      c: Collection): MongoDbIO[MongoCollection[BsonDocument]] =
  ...
  private def iterableToProcess[A](it: MongoIterable[A]): Process[MongoDbIO, A] = {
    ...
    def go(c: AsyncBatchCursor[A]): Process[MongoDbIO, A] =
      Process
        .eval(async(c.next))
        .flatMap(r =>
          Option(r).cata(as => Process.emitAll(as.asScala.toVector) ++ go(c),
                         Process.halt))
