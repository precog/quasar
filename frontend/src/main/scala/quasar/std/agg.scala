...
import scalaz._, Validation.{failureNel, success}
...
trait AggLib extends Library {
  ...
  val Sum = UnaryFunc(
    ...
    partialTyperV[nat._1] {
      ...
      case Sized(Type.Const(s @ Data.Set(xs))) if s.dataType == Type.Int =>
        intSet(xs).map(ys => Type.Const(Data.Int(ys.sum))).validationNel

      case Sized(Type.Const(s @ Data.Set(xs))) if s.dataType == Type.Dec =>
        decSet(xs).map(ys => Type.Const(Data.Dec(ys.sum))).validationNel
  ...
  val Min = UnaryFunc(
    ...
    partialTyperV[nat._1] {
      case Sized(Type.Const(Data.Set(xs))) =>
        reduceComparableSet(Data.Comparable.min)(xs).map(c => Type.Const(c.value))
  ...
  val Max = UnaryFunc(
    ...
    partialTyperV[nat._1] {
      case Sized(Type.Const(Data.Set(xs))) =>
        reduceComparableSet(Data.Comparable.max)(xs).map(c => Type.Const(c.value))
  ...
  val Avg = UnaryFunc(
    ...
    partialTyperV[nat._1] {
      ...
      case Sized(Type.Const(Data.Set(xs))) =>
        numSet(xs).map(ns => Type.Const(Data.Dec(ns.sum / ns.length))).validationNel
    ...
    _.toNel.fold(expectedNonEmptySet[Data.Comparable])(
      xs =>
        xs.traverse(Data.Comparable(_))
          ...
          .toSuccessNel(SemanticError
            .DomainError(Data.Set(xs.list.toList), some("Expected Set of comparable values"))))
  ...
  private val numSet: List[Data] => SemanticError \/ List[BigDecimal] =
    set =>
      errSetF
        ...
        .orElse(decSet(set))
        .leftAs(
          SemanticError.DomainError(Data.Set(set), some("Expected Set of numeric values")))
  ...
  private def homogenizedPF[A](
      f: PartialFunction[Data, A],
      err: String): List[Data] => SemanticError \/ List[A] =
    homogenized(f.lift, err)

  private def homogenized[A](
      f: Data => Option[A],
      err: String): List[Data] => SemanticError \/ List[A] =
