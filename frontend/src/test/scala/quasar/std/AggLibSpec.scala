...
class AggLibSpec extends quasar.Qspec {
  ...
  "Arbitrary" should {
    "type a nonempty constant set to the constant first value" >> prop { xs: NonEmptyList[Data] =>
      Arbitrary.tpe(Func.Input1(Type.Const(Data.Set(xs.list.toList)))) must beSuccess(Type.Const(xs.head))
    }

    "error when applied to an empty constant set" >> {
      Arbitrary.tpe(Func.Input1(Type.Const(Data.Set(Nil)))) must beFailure
    }
  }
  ...
  "Avg" should {
    "type a constant Int set to the constant average of values" >> prop { n: BigInt =>
      ...
      Avg.tpe(Func.Input1(Type.Const(s))) must beSuccess(Type.Const(Data.Dec(BigDecimal(n + 1))))
    }

    "type a constant Dec set to the constant average of values" >> prop { n0: Double =>
      val n = BigDecimal(n0)
      val s = Data.Set(List(Data.Dec(n), Data.Dec(n / 2.0)))
      Avg.tpe(Func.Input1(Type.Const(s))) must beSuccess(Type.Const(Data.Dec(n * 0.75)))
    }
    ...
    "type a constant Interval set to the constant average of values" >> prop { n: Int =>
      ...
      Avg.tpe(Func.Input1(Type.Const(s))) must beSuccess(Type.Const(Data.Dec(n.toLong + 1)))
    }

    "error when applied to an empty constant set" >> {
      Avg.tpe(Func.Input1(Type.Const(Data.Set(Nil)))) must beFailure
    }
  }

  "Count" should {
    "type a constant set as the constant length" >> prop { xs: List[Int] =>
      Count.tpe(Func.Input1(Type.Const(Data.Set(xs.map(Data.Int(_)))))) must beSuccess(Type.Const(Data.Int(xs.length)))
    }
  }
  ...
  "Max" should {
    "type a constant set as the constant max value" >> prop { xs: NonEmptyList[Int] =>
      Max.tpe(Func.Input1(Type.Const(Data.Set(xs.list.toList.map(Data.Int(_)))))) must
        beSuccess(Type.Const(Data.Int(xs.maximum1)))
    }

    "error when applied to an empty constant set" >> {
      Max.tpe(Func.Input1(Type.Const(Data.Set(Nil)))) must beFailure
    }
  }
  ...
  "Min" should {
    "type a constant set as the constant min value" >> prop { xs: NonEmptyList[Int] =>
      Min.tpe(Func.Input1(Type.Const(Data.Set(xs.list.toList.map(Data.Int(_)))))) must
        beSuccess(Type.Const(Data.Int(xs.minimum1)))
    }

    "error when applied to an empty constant set" >> {
      Min.tpe(Func.Input1(Type.Const(Data.Set(Nil)))) must beFailure
    }
  }
  ...
  "Sum" should {
    "type a constant Int set to the constant Int sum of values" >> prop { xs: NonEmptyList[BigInt] =>
      ...
      Sum.tpe(Func.Input1(Type.Const(s))) must beSuccess(Type.Const(Data.Int(xs.list.toList.sum)))
    }
    ...
    "type a constant Dec set to the constant Dec sum of values" >> prop { xs: NonEmptyList[Double] =>
      ...
      Sum.tpe(Func.Input1(Type.Const(s))) must beSuccess(Type.Const(Data.Dec(ys.sum)))
    }
    ...
    "type a constant Interval set to the constant Interval sum of values" >> prop { xs: NonEmptyList[Int] =>
      ...
      Sum.tpe(Func.Input1(Type.Const(s))) must beSuccess(Type.Const(Data.Interval(Duration.ofMillis(milliss.sum))))
    }

    "type an empty constant set to constant Int(0)" >> {
      Sum.tpe(Func.Input1(Type.Const(Data.Set(Nil)))) must beSuccess(Type.Const(Data.Int(0)))
    }
  }
}
