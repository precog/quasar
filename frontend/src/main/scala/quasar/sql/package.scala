...
package object sql {
  ...
  def setLiteral[A] =
    Prism.partial[Sql[A], List[A]] { case SetLiteral(exprs) => exprs }(SetLiteral(_))
  def arrayLiteral[A] =
    Prism.partial[Sql[A], List[A]] { case ArrayLiteral(exprs) => exprs }(ArrayLiteral(_))
  def mapLiteral[A] =
    Prism.partial[Sql[A], List[(A, A)]] { case MapLiteral(exprs) => exprs }(MapLiteral(_))
  def splice[A] =
    Prism.partial[Sql[A], Option[A]] { case Splice(exprs) => exprs }(Splice(_))
  def binop[A] =
    Prism.partial[Sql[A], (A, A, BinaryOperator)] { case Binop(l, r, op) => (l, r, op) }(
      (Binop[A] _).tupled)
  def unop[A] =
    Prism.partial[Sql[A], (A, UnaryOperator)] { case Unop(a, op) => (a, op) }(
      (Unop[A] _).tupled)
  def ident[A] = Prism.partial[Sql[A], String] { case Ident(name) => name }(Ident(_))
  def invokeFunction[A] =
    Prism.partial[Sql[A], (CIName, List[A])] {
      case InvokeFunction(name, args) => (name, args)
    }((InvokeFunction[A] _).tupled)
  def matc[A] =
    Prism.partial[Sql[A], (A, List[Case[A]], Option[A])] {
      case Match(expr, cases, default) => (expr, cases, default)
    }((Match[A] _).tupled)
  def switch[A] =
    Prism.partial[Sql[A], (List[Case[A]], Option[A])] {
      case Switch(cases, default) => (cases, default)
    }((Switch[A] _).tupled)
  def let[A] =
    Prism.partial[Sql[A], (CIName, A, A)] { case Let(n, f, b) => (n, f, b) }(
      (Let[A](_, _, _)).tupled)
  def intLiteral[A] =
    Prism.partial[Sql[A], Long] { case IntLiteral(v) => v }(IntLiteral(_))
  def floatLiteral[A] =
    Prism.partial[Sql[A], Double] { case FloatLiteral(v) => v }(FloatLiteral(_))
  def stringLiteral[A] =
    Prism.partial[Sql[A], String] { case StringLiteral(v) => v }(StringLiteral(_))
  def nullLiteral[A] =
    Prism.partial[Sql[A], Unit] { case NullLiteral() => () }(_ => NullLiteral())
  def boolLiteral[A] =
    Prism.partial[Sql[A], Boolean] { case BoolLiteral(v) => v }(BoolLiteral(_))
  ...
  def CrossRelation[T](left: SqlRelation[T], right: SqlRelation[T])(
      implicit T: Corecursive.Aux[T, Sql]) =
    JoinRelation(left, right, JoinType.Inner, boolLiteral[T](true).embed)

  def projectionNames[T](projections: List[Proj[T]], relName: Option[String])(
      implicit T: Recursive.Aux[T, Sql]): SemanticError \/ List[(String, T)] = {
    ...
    (aliases diff aliases.distinct).headOption.cata[SemanticError \/ List[(String, T)]](
      ...
      projections.zipWithIndex
        .mapAccumLeftM(aliases.toSet) {
          case (used, (Proj(expr, alias), index)) =>
            alias.cata(
              a => (used, a -> expr).right, {
                val tentativeName = extractName(expr) getOrElse index.toString
                val alternatives =
                  Stream.from(0).map(suffix => tentativeName + suffix.toString)
                (tentativeName #:: alternatives).dropWhile(used.contains).headOption.map {
                  name =>
                    // WartRemover seems to be confused by the `+` method on `Set`
                    @SuppressWarnings(Array("org.wartremover.warts.StringPlusAny"))
                    val newUsed = used + name
                    (newUsed, name -> expr)
                } \/> SemanticError.GenericError(
                  "Could not generate alias for a relation") // unlikely since we know it's an quasi-infinite stream
              }
            )
        }
        .map(_._2)
  ...
  implicit class ExprOps[T[_[_]]: BirecursiveT](q: T[Sql]) {
    ...
    def makeTables(bindings: List[String]): T[Sql] = q.project match {
      case sel @ Select(_, _, _, _, _, _) => {
        ...
        val sel2 =
          Functor[Sql].map(sel)(_.makeTables(bindings)).asInstanceOf[Select[T[Sql]]]

        def mkRel[A](rel: SqlRelation[A]): SqlRelation[A] = rel match {
          case ir @ IdentRelationAST(name, alias) if !bindings.contains(name) => {
            val filePath =
              posixCodec.parsePath(Some(_), Some(_), _ => None, _ => None)(name)
        ...
        Select.relation
          .modify((r: Option[SqlRelation[T[Sql]]]) => r.map(mkRel(_)))(sel2)
          .embed
      }
      ...
      q.cataM[EitherT[M, SemanticError, ?], T[Sql]] {
        case invoke @ InvokeFunction(name, args) =>
          EitherT(scope(name, args.size).flatMap {
            ...
            case ambiguous =>
              SemanticError
                .ambiguousFunctionInvoke(name, ambiguous.map {
                  case (func, from) => (func.name, from)
                })
                .left
                .point[M]
          })
  ...
  def resolveImportsImpl[M[_]: Monad, T[_[_]]: BirecursiveT](
      scopedExpr: ScopedExpr[T[Sql]],
      baseDir: ADir,
      retrieve: ADir => M[List[Statement[T[Sql]]]]): EitherT[M, SemanticError, T[Sql]] = {

    def absImport(i: Import[T[Sql]], from: ADir): SemanticError \/ ADir =
      refineTypeAbs(i.path)
        .fold(sandboxCurrent(_), r => sandboxCurrent(unsandbox(from) </> r))
        .toRightDisjunction {
          val invalidPathString = posixCodec.unsafePrintPath(i.path)
          val fromString        = posixCodec.printPath(from)
          SemanticError.GenericError(
            s"$invalidPathString is invalid because it is located at $fromString")
        }

    @SuppressWarnings(Array("org.wartremover.warts.Recursion"))
    def scopeFromHere(imports: List[Import[T[Sql]]],
                      funcsHere: List[FunctionDecl[T[Sql]]],
                      here: ADir)
      : (CIName, Int) => EitherT[M, SemanticError, List[(FunctionDecl[T[Sql]], ADir)]] = {
      case (name, arity) =>
        imports
          .traverse(absImport(_, here))
          .fold(
            ...
            absImportPaths => {
              ...
              val funcsFromImports = absImportPaths.traverse(d =>
                retrieve(d).map(stats => (stats.decls, stats.imports, d)))
              // All functions in "this" scope along with their own imports
              val allFuncs = funcsFromImports.map((funcsHere, imports, here) :: _)
              EitherT
                .right(allFuncs)
                .flatMap(_.traverse {
                  case (funcs, imports, from) =>
                    def matchesSignature(func: FunctionDecl[T[Sql]]) =
                      func.name === name && arity === func.args.size
                    funcs.filter(matchesSignature).traverse { decl =>
                      ...
                      decl
                        .traverse(_.inlineInvokes(currentScope))
                        .flattenLeft
                        .strengthR(from)
                    }
                })
    ...
    scopedExpr.expr
      .inlineInvokes(scopeFromHere(scopedExpr.imports, scopedExpr.defs, baseDir))
      .flattenLeft
  }
  ...
  private def _q(s: String): String =
    "\"" + s.replace("\\", "\\\\").replace("\"", "\\\"") + "\""

  private def _qq(delimiter: String, s: String): String = s match {
    case SimpleNamePattern() => s
    case _ =>
      delimiter + s.replace("\\", "\\\\").replace(delimiter, "\\" + delimiter) + delimiter
  }

  @SuppressWarnings(Array("org.wartremover.warts.Recursion"))
  private def pprintRelationƒ[T](r: SqlRelation[(T, String)])(
      implicit T: Recursive.Aux[T, Sql]): String = {
    ...
    (r match {
      ...
      case JoinRelation(left, right, tpe, clause) =>
        (tpe, clause._1) match {
          ...
          case (_, _) =>
            List("(",
                 pprintRelationƒ(left),
                 ppJoinType(tpe),
                 pprintRelationƒ(right),
                 "on",
                 clause._2,
                 ")")
        }
  ...
  def pprintƒ[T](implicit T: Recursive.Aux[T, Sql]): Sql[(T, String)] => String = {
...

    {
      case Select(isDistinct, projections, relations, filter, groupBy, orderBy) =>
        "(" +
          List(
            ...
            Some(
              projections
                .map(p => p.alias.foldLeft(p.expr._2)(_ + " as " + _qq("`", _)))
                .mkString(", ")),
            ...
            orderBy.map(
              o =>
                List("order by",
                     o.keys.map(x => x._2._2 + " " + x._1.shows) intercalate (", "))
                  .mkString(" "))
          ).foldMap(_.toList).mkString(" ") +
          ")"
      ...
      case InvokeFunction(name, args) =>
        (name, args) match {
          case (CIName("like"),
                (_, value) :: (_, pattern) :: (Embed(StringLiteral("\\")), _) :: Nil) =>
            "(" + value + ") like (" + pattern + ")"
  ...
  def normalizeƒ[T](implicit T: Corecursive.Aux[T, Sql]): Sql[T] => Option[Sql[T]] = {
  ...
  def traverseRelation[G[_], A, B](r: SqlRelation[A], f: A => G[B])(
      implicit G: Applicative[G]): G[SqlRelation[B]] =
    r match {
      case IdentRelationAST(name, alias) =>
