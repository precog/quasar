...
import quasar.{Data, Func, NullaryFunc, UnaryFunc, Mapping, Type, SemanticError},
SemanticError._
...
trait DateLib extends Library with Serializable {
    ...
    \/.fromTryCatchNonFatal(Duration.parse(str)).bimap(
      κ(
        DateFormatError(
          ...
          str,
          Some(
            "expected, e.g. P3DT12H30M15.0S; note: year/month not currently supported"))),
      Data.Interval.apply)
  }
  ...
  def truncZonedDateTime(part: TemporalPart,
  ...
  def truncTimestamp(part: TemporalPart,
                     ts: Data.Timestamp): SemanticError \/ Data.Timestamp =
