...
object LogicalPlan {
      ...
      def apply[α](ra: RenderTree[α]): RenderTree[LogicalPlan[α]] =
        new RenderTree[LogicalPlan[α]] {
          ...
          def render(v: LogicalPlan[α]) = v match {
            ...
            case InvokeFUnapply(func, args)      => NonTerminal("Invoke" :: nodeType, Some(func.name), args.unsized.toList.map(ra.render))
    ...
    new EqualF[LogicalPlan] {
      def equal[A: Equal](v1: LogicalPlan[A], v2: LogicalPlan[A]): Boolean =
        (v1, v2) match {
          ...
          case (InvokeFUnapply(f1, v1), InvokeFUnapply(f2, v2)) => f1 == f2 && v1.unsized.toList ≟ v2.unsized.toList
  ...
  val normalizeLetsƒ: LogicalPlan[Fix[LogicalPlan]] => Option[LogicalPlan[Fix[LogicalPlan]]] = {
      ...
      case InvokeFUnapply(func @ UnaryFunc(_, _, _, _, _, _, _, _), Sized(a1)) => a1 match {
        case Fix(LetF(a, x1, x2)) => LetF(a, x1, Invoke[nat._1](func, Sized[IS](x2))).some
        case _ => None
      }

      case InvokeFUnapply(func @ BinaryFunc(_, _, _, _, _, _, _, _), Sized(a1, a2)) => (a1, a2) match {
        case (Fix(LetF(a, x1, x2)), a2) => LetF(a, x1, Invoke[nat._2](func, Sized[IS](x2, a2))).some
        case (a1, Fix(LetF(a, x1, x2))) => LetF(a, x1, Invoke[nat._2](func, Sized[IS](a1, x2))).some
        case _ => None
      }

      case InvokeFUnapply(func @ TernaryFunc(_, _, _, _, _, _, _, _), Sized(a1, a2, a3)) => (a1, a2, a3) match {
        case (Fix(LetF(a, x1, x2)), a2, a3) => LetF(a, x1, Invoke[nat._3](func, Sized[IS](x2, a2, a3))).some
        case (a1, Fix(LetF(a, x1, x2)), a3) => LetF(a, x1, Invoke[nat._3](func, Sized[IS](a1, x2, a3))).some
        case (a1, a2, Fix(LetF(a, x1, x2))) => LetF(a, x1, Invoke[nat._3](func, Sized[IS](a1, a2, x2))).some
        case _ => None
      }
  ...
  // Sized[(Type, List[NC], Fix[LP])]
  // (Sized[Type], Sized[List[NC]], Sized[Fix[LP]])
  ...
  def inferTypes(typ: Type, term: Fix[LogicalPlan]):
    ...
    (term.unFix match {
      ...
      case InvokeF(func, args) => for {
        ...
        args0 <- types.zip(args).traverseU {
    ...
    case (inf, term) =>
      ...
      term match {
        ...
        case InvokeFUnapply(MakeObject, Sized(name, value)) =>
          lift(MakeObject.tpe(Sized[IS](name, value).map(_.inferred)).disjunction).flatMap(
            applyConstraints(_, value)(x => Fix(MakeObject(Sized[IS](name.plan, x)))))
        case InvokeFUnapply(MakeArray, Sized(value)) =>
          lift(MakeArray.tpe(Sized[IS](value).map(_.inferred)).disjunction).flatMap(
            applyConstraints(_, value)(x => Fix(MakeArray(Sized[IS](x)))))
        // TODO: Move this case to the Mongo planner once type information is
        //       available there.
        case InvokeFUnapply(ConcatOp, Sized(left, right)) =>
          val (types, constraints0, terms) = Sized[IS](left, right).map {
            case ConstrainedPlan(in, con, pl) => (in, (con, pl))
          }.unzip3[Type, List[NamedConstraint], Fix[LogicalPlan]]

          val constraints = constraints0.unsized.toList.flatten
        ...
        case InvokeFUnapply(relations.Or, Sized(left, right)) =>
          lift(relations.Or.tpe(Sized[IS](left, right).map(_.inferred)).disjunction).flatMap(unifyOrCheck(inf, _, Invoke(relations.Or, Sized[IS](left, right).map(appConst(_, Constant(Data.NA))))))
        case InvokeFUnapply(structural.FlattenArray, Sized(arg)) =>
          for {
            types <- lift(structural.FlattenArray.tpe(Sized[IS](arg).map(_.inferred)).disjunction)
            consts <- emitName[SemDisj, Func.Input[Fix[LogicalPlan], nat._1]](Sized[IS](arg).map(ensureConstraint(_, Constant(Data.Arr(List(Data.NA))))).sequenceU)
        ...
        case InvokeFUnapply(structural.FlattenMap, Sized(arg)) => for {
          types <- lift(structural.FlattenMap.tpe(Sized[IS](arg).map(_.inferred)).disjunction)
          consts <- emitName[SemDisj, Func.Input[Fix[LogicalPlan], nat._1]](Sized[IS](arg).map(ensureConstraint(_, Constant(Data.Obj(ListMap("" -> Data.NA))))).sequenceU)
        ...
        case InvokeFUnapply(func @ UnaryFunc(_, _, _, _, _, _, _, _), Sized(a1)) =>
          handleGenericInvoke(inf, InvokeF(func, Sized[IS](a1)))
        case InvokeFUnapply(func @ BinaryFunc(_, _, _, _, _, _, _, _), Sized(a1, a2)) =>
          handleGenericInvoke(inf, InvokeF(func, Sized[IS](a1, a2)))
        case InvokeFUnapply(func @ TernaryFunc(_, _, _, _, _, _, _, _), Sized(a1, a2, a3)) =>
          handleGenericInvoke(inf, InvokeF(func, Sized[IS](a1, a2, a3)))
        case TypecheckF(expr, typ, cont, fallback) =>
  ...
  private def handleGenericInvoke[N <: Nat](inf: Type, invoke: InvokeF[ConstrainedPlan, N]): NameT[SemDisj, ConstrainedPlan] = {
    ...
    func.effect match {
      case Mapping =>
        ...
        lift(func.tpe(types).disjunction).flatMap(unifyOrCheck(inf, _, Invoke(func, terms))).map(cp =>
          cp.copy(constraints = cp.constraints ++ constraints))

      case _ =>
        lift(func.tpe(args.map(_.inferred)).disjunction).flatMap(unifyOrCheck(inf, _, Invoke(func, args.map(appConst(_, Constant(Data.NA))))))
    }
  }
