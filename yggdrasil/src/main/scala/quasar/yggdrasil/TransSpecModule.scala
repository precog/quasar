...
trait TransSpecModule extends FNModule {
  ...
  object trans {
    ...
    case class Filter[+A <: SourceType](source: TransSpec[A], predicate: TransSpec[A])
        extends TransSpec[A]
    ...
    case class Map2[+A <: SourceType](left: TransSpec[A], right: TransSpec[A], f: F2)
        extends TransSpec[A]
    ...
    case class ObjectDelete[+A <: SourceType](source: TransSpec[A], fields: Set[CPathField])
        extends TransSpec[A]
    ...
    case class WrapObject[+A <: SourceType](source: TransSpec[A], field: String)
        extends ObjectSpec[A]

    case class WrapObjectDynamic[+A <: SourceType](left: TransSpec[A], right: TransSpec[A])
        extends TransSpec[A]

    case class WrapArray[+A <: SourceType](source: TransSpec[A]) extends ArraySpec[A]

    case class DerefObjectStatic[+A <: SourceType](source: TransSpec[A], field: CPathField)
        extends TransSpec[A]

    case class DerefMetadataStatic[+A <: SourceType](source: TransSpec[A], field: CPathMeta)
        extends TransSpec[A]

    case class DerefObjectDynamic[+A <: SourceType](left: TransSpec[A], right: TransSpec[A])
        extends TransSpec[A]

    case class DerefArrayStatic[+A <: SourceType](source: TransSpec[A], element: CPathIndex)
        extends TransSpec[A]

    case class DerefArrayDynamic[+A <: SourceType](left: TransSpec[A], right: TransSpec[A])
        extends TransSpec[A]
    ...
    case class Within[+A <: SourceType](item: TransSpec[A], in: TransSpec[A])
        extends TransSpec[A]
    ...
    object TransSpec {
        ...
        def createSpecs(trees: Seq[CPathTree[Int]]): Seq[TransSpec[A]] = trees.map { child =>
          child match {
            ...
            case node @ IndexNode(CPathIndex(_), _) =>
              trans
                .WrapArray(concatChildren(node, leaf)) //assuming that indices received in order
      ...
      def mapSources[A <: SourceType, B <: SourceType](spec: TransSpec[A])(
        ...
        spec match {
          ...
          case trans.Cond(pred, left, right) =>
            trans.Cond(mapSources(pred)(f), mapSources(left)(f), mapSources(right)(f))
        }
      }

      def deepMap[A <: SourceType](spec: TransSpec[A])(
          f: PartialFunction[TransSpec[A], TransSpec[A]]): TransSpec[A] = spec match {
      ...
      def normalize[A <: SourceType](ts: TransSpec[A], undef: TransSpec[A]): TransSpec[A] = {
        ...
        def flattenConcats: Option[TransSpec[A]] = {
          def flattenOuterArrayConcats[A <: SourceType](
              proj: TransSpec[A]): Option[List[TransSpec[A]]] = proj match {
            case OuterArrayConcat(ls @ _*) =>
              Some(ls.toList.flatMap(a => flattenOuterArrayConcats(a).getOrElse(a :: Nil)))
            case _ => None
          }

          def flattenOuterObjectConcats[A <: SourceType](
              proj: TransSpec[A]): Option[List[TransSpec[A]]] = proj match {
          ...
          def flattenInnerArrayConcats[A <: SourceType](
              proj: TransSpec[A]): Option[List[TransSpec[A]]] = proj match {
            case InnerArrayConcat(ls @ _*) =>
              Some(ls.toList.flatMap(a => flattenInnerArrayConcats(a).getOrElse(a :: Nil)))
            case _ => None
          }
          def flattenInnerObjectConcats[A <: SourceType](
              proj: TransSpec[A]): Option[List[TransSpec[A]]] = proj match {
            case InnerObjectConcat(ls @ _*) =>
              Some(ls.toList.flatMap(a => flattenInnerObjectConcats(a).getOrElse(a :: Nil)))
            case _ => None
          }
          flattenOuterArrayConcats(ts)
            .map(ks => OuterArrayConcat(ks.map(normalize(_, undef)): _*))
            .orElse(flattenOuterObjectConcats(ts).map(ks =>
              OuterObjectConcat(ks.map(normalize(_, undef)): _*)))
            .orElse(flattenInnerArrayConcats(ts).map(ks =>
              InnerArrayConcat(ks.map(normalize(_, undef)): _*)))
            .orElse(flattenInnerObjectConcats(ts).map(ks =>
              InnerObjectConcat(ks.map(normalize(_, undef)): _*)))
        }
        ...
        flattenConcats.getOrElse {
          ts match {
                ...
                case n @ OuterArrayConcat(ks @ _*) =>
                  if (ks.length < (i + 1) && ks.forall(_.isInstanceOf[WrapArray[_]])) {
                    ...
                    ks.foldLeft((0.some, none[TransSpec[A]])) {
                      ...
                      ._2
                      .getOrElse(DerefArrayStatic(n, f))
                  }
                case WrapArray(k) =>
                  if (i == 0) {
                    k
            ...
            case Cond(p, l, r) =>
              Cond(normalize(p, undef), normalize(l, undef), normalize(r, undef))
            case Filter(s, t) => Filter(normalize(s, undef), normalize(t, undef))
            case FilterDefined(s, df, t) =>
              FilterDefined(normalize(s, undef), normalize(df, undef), t)
      ...
      def rephrase[A <: SourceType](
        ...
        final case class PeelState(
            delta: Option[Int],
            keys: Option[Set[String]],
            substitutions: Map[TransSpec1, TransSpec1])
        ...
        def peelInvert(
            ...
            currentIndex: Int,
            keys: Set[String],
            inverseLayers: TransSpec1 => TransSpec1): PeelState = {
          ...
          def arrayConcat(rs: Seq[TransSpec[A]]) = {
            val (delta, substsOut) = rs.foldLeft((0.some, Map.empty[TransSpec1, TransSpec1])) {
              case ((Some(i), substs), ts) =>
                val PeelState(newDelta, _, newSubsts) = peelInvert(
                  ts,
          ...
          def objectConcat(rs: Seq[TransSpec[A]]) = {
            val (resultKeys, out) =
              rs.foldRight((Set.empty[String].some, Map.empty[TransSpec1, TransSpec1])) {
                case (ts, (Some(ks), substs)) =>
                  val PeelState(_, newKeys, newSubsts) =
                    peelInvert(ts, currentIndex = 0, keys = keys ++ ks, inverseLayers)
          ...
          projection match {
              ...
              if (keys(k)) {
                ...
                val PeelState(_, _, nestedSubstitutions) =
                  peelInvert(
                    ...
                    currentIndex = 0,
                    Set.empty,
                    inverseLayers andThen (DerefObjectStatic(_, CPathField(k))))
                PeelState(0.some, Set(k).some, nestedSubstitutions)
              }
            case ObjectDelete(s, k) =>
              val PeelState(_, nestedKeys, nestedSubstitutions) =
                peelInvert(s, currentIndex = 0, keys, inverseLayers)
            ...
            case WrapArray(s) =>
              val PeelState(_, _, nestedSubstitutions) = peelInvert(
                s,
                currentIndex = 0,
                Set.empty,
                inverseLayers andThen (DerefArrayStatic(_, CPathIndex(currentIndex))))
            ...
            case ArraySwap(s, i) =>
              peelInvert(
                ...
                currentIndex = 0,
                Set.empty,
                inverseLayers andThen (ArraySwap(_, i)))
            ...
            case rootSubtree if allRootPaths(rootSubtree) =>
              ...
              PeelState(
                0.some,
                Set.empty[String].some,
                Map(rootSubtreeAsTransSpec1 -> inverseLayers(Leaf(Source))))
        ...
        val peelSubstitutions =
          peelInvert(
            ...
            currentIndex = 0,
            keys = Set.empty,
            inverseLayers = identity[TransSpec1]).substitutions
      ...
      def dnf(keySpec: GroupKeySpec): GroupKeySpec = {
        keySpec match {
          ...
          case GroupKeySpecAnd(GroupKeySpecOr(ol, or), right) =>
            GroupKeySpecOr(dnf(GroupKeySpecAnd(ol, right)), dnf(GroupKeySpecAnd(or, right)))
          case GroupKeySpecAnd(left, GroupKeySpecOr(ol, or)) =>
            GroupKeySpecOr(dnf(GroupKeySpecAnd(left, ol)), dnf(GroupKeySpecAnd(left, or)))
  ...
  def makeTableTrans(tableTrans: TableTransSpec1): TransSpec1 = {
    ...
    wrapped.foldLeft[TransSpec1](ObjectDelete(Leaf(Source), Set(tableTrans.keys.toSeq: _*))) {
      (acc, ts) =>
