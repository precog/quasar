  ...
  "SQLParser" should {
      ...
      parse("""select case from when where then and end""") should
        beRightDisjOrDiff(
          SelectR(
            ...
            List(
              Proj(
                MatchR(IdentR("from"), List(Case(IdentR("where"), IdentR("and"))), None),
                None)),
            None,
            None,
            None,
      ...
      parse("insert into zips ('a', 'b') values (1, 2), (3, 4)") should
        beRightDisjOrDiff(
          Distinct(UnionAll(
            SetLiteralR(List(
              MapLiteralR(List(
                StringLiteralR("a") -> IntLiteralR(1),
                StringLiteralR("b") -> IntLiteralR(2))),
              MapLiteralR(List(
                StringLiteralR("a") -> IntLiteralR(3),
    ...
    "parse array constructor and concat op" in {
      parse("select loc || [ pop ] from zips") must beRightDisjunction(
        SelectR(
          SelectAll,
          List(Proj(BinopR(IdentR("loc"), ArrayLiteralR(List(IdentR("pop"))), Concat), None)),
          Some(TableRelationAST(file("zips"), None)),
          None,
          None,
          None))
    ...
    "not parse basic let with digit as the identifier" in {
      parse("""8 := 5; 8""") must
        beLeftDisjunction(GenericParsingError("keyword 'except' expected; but found `:='"))
    }
      ...
      parse("""select foo from (bar := 12; baz) as quag""") must
        beRightDisjunction(
          SelectR(
            SelectAll,
            List(Proj(IdentR("foo"), None)),
            Some(ExprRelationAST(LetR(CIName("bar"), IntLiteralR(12), IdentR("baz")), "quag")),
            None,
            None,
            None))
    }
