...
package object expression {
  ...
  def translate[T: Equal, EX[_]: Traverse](
      ...
      I: ExprOpCoreF :<: EX,
      ev1: ExprOpOps.Uni[EX]): PartialFunction[T, PlannerError \/ JsFn] = {
    val mjs = quasar.physical.mongodb.javascript[JsCore](_.embed)
    import mjs._

    def app(x: T, tc: JsCore => JsCore): PlannerError \/ JsFn =
      x.para(toJs[T, EX])
        .map(f => JsFn(JsFn.defaultName, tc(f(jscore.Ident(JsFn.defaultName)))))
      ...
      case Check(expr, typ) =>
        typ match {
          ...
          case Type.Date ⨿ Type.Timestamp =>
            app(expr, x => jscore.BinOp(jscore.Or, isDate(x), isTimestamp(x)))
          ...
          case Type.Bool ⨿ Type.Date ⨿ Type.Timestamp =>
            app(expr,
                x => jscore.binop(jscore.Or, isDate(x), isTimestamp(x), isBoolean(x)))
          case Type.Int ⨿ Type.Dec ⨿ Type.Str =>
            app(expr, x => jscore.binop(jscore.Or, isAnyNumber(x), isString(x)))
  ...
  def toJs[T: Equal, EX[_]: Traverse](
      ...
      TC: Corecursive.Aux[T, EX],
      ev0: ExprOpCoreF :<: EX,
      ops: ExprOpOps.Uni[EX]): GAlgebra[(T, ?), EX, PlannerError \/ JsFn] = { t =>
  ...
  implicit def exprOpRenderTree[T[_[_]]: RecursiveT, EX[_]: Functor](
    ...
    new RenderTree[T[EX]] {
      def render(v: T[EX]) =
        Terminal(List("ExprOp"), v.cata(ops.bson).toJs.pprint(0).some)
    }
}
