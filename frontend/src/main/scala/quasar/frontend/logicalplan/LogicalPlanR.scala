...
final case class ConstrainedPlan[T](inferred: Type,
...
final class LogicalPlanR[T](implicit TR: Recursive.Aux[T, LP], TC: Corecursive.Aux[T, LP]) {
  ...
  //     isn't exposed there.
  def renameƒ[M[_]: Monad](
      f: Symbol => M[Symbol]): CoalgebraM[M, LP, (Map[Symbol, Symbol], T)] = {
    case (bound, t) =>
      t.project match {
        case Let(sym, expr, body) =>
          f(sym).map(sym1 => lp.let(sym1, (bound, expr), (bound + (sym -> sym1), body)))
        case Free(sym) =>
  ...
  val normalizeLetsƒ: LP[T] => Option[LP[T]] = {
    ...
    case InvokeUnapply(relations.Cond, Sized(a1, a2, a3)) =>
      (a1, a2, a3) match {
  ...
  def inferTypes(typ: Type, term: T): SemValidation[Typed[LP]] = {
        ...
        LP.funcFromJoinType(joinType).untpe(typ).flatMap { types =>
          inferTypes(types(2), cond).flatMap { cond0 =>
            ...
            val rTyp = cond0
              ...
              .concatenate(Type.TypeGlbMonoid)
            (inferTypes(Type.glb(lTyp, types(0)), l) |@| inferTypes(
              Type.glb(rTyp, types(1)),
              r))(lp.join(_, _, joinType, JoinCondition(lName, rName, cond0)))
          }
        }
  ...
  /** This function compares the inferred (required) type with the possible type
    ...
    * • otherwise, we fail
    */
  private def unifyOrCheck(inf: Type,
                           poss: Type,
                           term: T): NameT[SemDisj, ConstrainedPlan[T]] =
    if (inf.contains(poss)) {
      ...
      lift(
        (SemanticError
          .genericError(
            s"You provided a ${poss.shows} where we expected a ${inf.shows} in $term"))
          .wrapNel
          .left)

  private def appConst(constraints: ConstrainedPlan[T], fallback: T) =
    constraints.constraints.foldLeft(constraints.plan)((acc, con) =>
      let(con.name, con.term, typecheck(free(con.name), con.inferred, acc, fallback)))
  ...
  /** This inserts a constraint on a node that might not strictly require a type
    ...
    * shape.
    */
  private def ensureConstraint(constraints: ConstrainedPlan[T],
                               fallback: T): State[NameGen, T] = {
    ...
    case (inf, term) =>
      ...
      term match {
        ...
        case InvokeUnapply(MakeObject, Sized(name, value)) =>
          lift(MakeObject.tpe(Func.Input2(name, value).map(_.inferred)).disjunction)
            .flatMap(applyConstraints(_, value)(MakeObject(name.plan, _).embed))
        case InvokeUnapply(MakeArray, Sized(value)) =>
          lift(MakeArray.tpe(Func.Input1(value).map(_.inferred)).disjunction)
        ...
        case InvokeUnapply(ConcatOp, Sized(left, right)) =>
          val (types, constraints0, terms) = Func
            .Input2(left, right)
            .map {
          ...
          lift(ConcatOp.tpe(types).disjunction)
            .flatMap[NameGen, ConstrainedPlan[T]](poss =>
              poss match {
                case t if Type.Str.contains(t) =>
                  unifyOrCheck(inf, poss, invoke(string.Concat, terms))
                case t if t.arrayLike =>
                  unifyOrCheck(inf, poss, invoke(ArrayConcat, terms))
                case _ =>
                  lift(
                    -\/(NonEmptyList(
                      SemanticError.GenericError("can't concat mixed/unknown types"))))
            })
            .map(cp => cp.copy(constraints = cp.constraints ++ constraints))
        case InvokeUnapply(relations.Or, Sized(left, right)) =>
          lift(relations.Or.tpe(Func.Input2(left, right).map(_.inferred)).disjunction)
            .flatMap(
              unifyOrCheck(
                inf,
                _,
                relations.Or(left, right).map(appConst(_, constant(Data.NA))).embed))
        case InvokeUnapply(structural.FlattenArray, Sized(arg)) =>
          for {
            types <- lift(
              structural.FlattenArray.tpe(Func.Input1(arg).map(_.inferred)).disjunction)
            consts <- emitName[SemDisj, Func.Input[T, nat._1]](
              Func
                .Input1(arg)
                .traverse(ensureConstraint(_, constant(Data.Arr(List(Data.NA))))))
            plan <- unifyOrCheck(inf,
                                 types,
                                 invoke[nat._1](structural.FlattenArray, consts))
          } yield plan
        case InvokeUnapply(structural.FlattenMap, Sized(arg)) =>
          for {
            types <- lift(
              structural.FlattenMap.tpe(Func.Input1(arg).map(_.inferred)).disjunction)
            consts <- emitName[SemDisj, Func.Input[T, nat._1]](Func
              .Input1(arg)
              .traverse(ensureConstraint(_, constant(Data.Obj(ListMap("" -> Data.NA))))))
        ...
        case Typecheck(expr, typ, cont, fallback) =>
          ...
          val (constrs, plan): (List[NamedConstraint[T]], T) =
            expr.constraints.foldLeftM(expr.plan) {
              case (acc, constr) =>
        ...
        case Sort(expr, ords) =>
          unifyOrCheck(inf,
                       expr.inferred,
                       sort(appConst(expr, constant(Data.NA)),
                            ords map (_ leftMap (appConst(_, constant(Data.NA))))))
        case TemporalTrunc(part, src) =>
          val typer: Func.Typer[Nat._1] = StdLib.partialTyperV[nat._1] {
            case Sized(Type.Const(d @ Data.Date(_))) =>
              truncDate(part, d).validationNel ∘ (Type.Const(_))
            case Sized(Type.Const(t @ Data.Time(_))) =>
              truncTime(part, t).validationNel ∘ (Type.Const(_))
            case Sized(Type.Const(t @ Data.Timestamp(_))) =>
              truncTimestamp(part, t).validationNel ∘ (Type.Const(_))
            case Sized(t) => t.success
          }

          val constructLPNode: Func.Input[T, Nat._1] => T = {
            case Sized(i) => temporalTrunc(part, i)
          }
  ...
  private def checkInvoke[N <: Nat](
    ...
    val (types, constraints0, terms) = args
      .map {
        case ConstrainedPlan(in, con, pl) => (in, (con, pl))
      }
      .unzip3[Type, List[NamedConstraint[T]], T]
    ...
    lift(typer(types).disjunction)
      .flatMap(unifyOrCheck(inf, _, constructLPNode(terms)))
      .map(cp => cp.copy(constraints = cp.constraints ++ constraints))
  }
  ...
  private def checkJoin(inf: Type,
                        func: GenericFunc[nat._3],
                        args: Func.Input[ConstrainedPlan[T], nat._3],
                        lName: Symbol,
                        rName: Symbol,
                        tpe: JoinType): NameT[SemDisj, ConstrainedPlan[T]] = {
    val const = args.map(appConst(_, constant(Data.NA)))
    lift(func.typer0(args.map(_.inferred)).disjunction).flatMap(
      unifyOrCheck(inf,
                   _,
                   join(const(0), const(1), tpe, JoinCondition(lName, rName, const(2)))))
  }
  ...
  def ensureCorrectTypes(term: T): ValidationNel[SemanticError, T] = {
    ...
    inferTypes(Type.Top, term).flatMap(
      _.cataM(liftTM(checkTypesƒ))
        .map(appConst(_, constant(Data.NA)))
        .evalZero
        .validation)
  }
  ...
  def lpParaZygoHistoM[M[_]: Monad, A, B](
      t: T)(f: LP[(T, B)] => B, g: LP[Cofree[LP, (B, A)]] => M[A]): M[A] = {
    @SuppressWarnings(Array("org.wartremover.warts.Recursion"))
    def loop(t: T, bind: Map[Symbol, Cofree[LP, (B, A)]]): M[Cofree[LP, (B, A)]] = {
      lazy val default: M[Cofree[LP, (B, A)]] = for {
        lp <- t.project.traverse(x =>
          for {
            co <- loop(x, bind)
