...
package object json {
  ...
  implicit class JPathOps(private val x: JPath) {
    ...
    def expand(jvalue: JValue): List[JPath] = {
      def expand0(current: List[JPathNode], right: List[JPathNode], d: JValue): List[JPath] =
        right match {
  ...
  implicit class JValueOps(private val self: JValue) {
    ...
    def minimize: Option[JValue] = {
      self match {
        case JObjectFields(fields) =>
          Some(JObject(fields flatMap { case JField(k, v) => v.minimize.map(JField(k, _)) }))
