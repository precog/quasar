...
import SemanticError.{MissingField, MissingIndex, TypeError}

import scala.Any

import argonaut._, Argonaut._, ArgonautScalaz._
import scalaz._, Scalaz._, NonEmptyList.nels, Validation.{failureNel, success}
...
sealed abstract class Type extends Product with Serializable { self =>
  ...
  final def arrayMinLength: Option[Int] = this match {
    ...
    case x @ Coproduct(_, _) =>
      x.flatten.toList.foldLeft[Option[Int]](None)((a, n) => (a |@| n.arrayMinLength)(_ min _))
    case _ => None
  }
  ...
  final def objectField(field: Type): SemanticResult[Type] = {
    if (Type.lub(field, Str) ≠ Str) failureNel(TypeError(Str, field, None))
      ...
      (field, this) match {
        ...
        case (Str, t) =>
          t.objectType
            .fold[SemanticResult[Type]](failureNel(TypeError(AnyObject, this, None)))(success)
        ...
        case (Const(Data.Str(field)), Obj(map, uk)) =>
          map
            .get(field)
            .fold(uk.fold[SemanticResult[Type]](failureNel(MissingField(field)))(success))(
              success)
  ...
  final def arrayElem(index: Type): SemanticResult[Type] = {
    if (Type.lub(index, Int) ≠ Int) failureNel(TypeError(Int, index, None))
      ...
      (index, this) match {
        ...
        case (Int, _) =>
          this.arrayType
            .fold[SemanticResult[Type]](failureNel(TypeError(AnyArray, this, None)))(success)
...
object Type extends TypeInstances {
    ...
    (superType, subType) match {
      ...
      case (Obj(supMap, supUk), Obj(subMap, subUk)) =>
        supMap.toList.foldMap {
          case (k, v) =>
            subMap.get(k).fold[SemanticResult[Unit]](fail(superType, subType))(typecheck(v, _))
        } +++
          supUk.fold(
            subUk.fold[SemanticResult[Unit]](if ((subMap -- supMap.keySet).isEmpty) succeed(())
  ...
  implicit val equal: Equal[Type] = Equal.equal((a, b) =>
    (a, b) match {
      case (Top, Top) | (Bottom, Bottom) | (Null, Null) | (Str, Str) | (Int, Int) | (Dec, Dec) |
          (Bool, Bool) | (Binary, Binary) | (Timestamp, Timestamp) | (Date, Date) |
          (Time, Time) | (Interval, Interval) | (Id, Id) =>
        true
