...
class MountServiceSpec extends quasar.Qspec with Http4s {
  ...
  def beMountNotFoundError(path: APath) =
    beApiErrorWithMessage(Status.NotFound withReason "Mount point not found.",
                          "path" := path)

  def beInvalidConfigError(rsn: String) =
    equal(
      ApiError.apiError(Status.BadRequest withReason "Invalid mount configuration.",
                        "reasons" := List(rsn)))
  ...
  "Mount Service" should {
    "GET" should {
      ...
      "succeed with correct view path" >> prop { f: AFile =>
        runTest { service =>
          val cfg = MountConfig.viewConfig0(sqlB"select * from zips where pop > :cutoff",
                                            "cutoff" -> "1000")
            ...
            runTest { service =>
              ...
              for {
                ...
                (body must_== s"moved ${printPath(src)} to ${printPath(dst)}") and
                  ...
                  (mntd must_== Set(MR.mountFileSystem(dst, StubFs, fooUri),
    ...
    "Common" >> {
      ...
      trait RequestBuilder {
        def apply[B](parent: ADir,
                     mount: RPath,
                     body: B)(implicit B: EntityEncoder[B]): Free[Eff, Request]
      }
      ...
      def testBoth(test: RequestBuilder => Fragment) = {
        "POST" should {
          test(new RequestBuilder {
            def apply[B](parent: ADir,
                         mount: RPath,
                         body: B)(implicit B: EntityEncoder[B]) =
              lift(
                Request(method = POST,
                        uri = pathUri(parent),
        ...
        "PUT" should {
          test(new RequestBuilder {
            def apply[B](parent: ADir,
                         ...
                         body: B)(implicit B: EntityEncoder[B]) =
              lift(
                Request(method = PUT, uri = pathUri(parent </> mount))
                  .withBody(body)).into[Eff]
          })
        }
      }
      ...
      testBoth { reqBuilder =>
        "succeed with filesystem path" >> prop { (parent: ADir, fsDir: RDir) =>
          runTest { service =>
            for {
              req <- reqBuilder(parent,
                                fsDir,
                                """{"stub": { "connectionUri": "foo" } }""")
              r <- service(req)
              (res, mntd) = r
              body <- lift(res.as[String]).into[Eff]
              dst = parent </> fsDir
        ...
        "succeed with view under existing fs path" >> prop {
          (fs: ADir, viewSuffix: RFile) =>
            runTest { service =>
              val scopedExpr = sqlB"select * from zips where pop < :cutoff"
              val vars       = Variables(Map(VarName("cutoff") -> VarValue("1000")))
              val cfg        = MountConfig.viewConfig(scopedExpr, vars)
        ...
        "succeed with module path" >> prop { (parent: ADir, d: RDir) =>
          runTest { service =>
            val cfgStr = EncodeJson
              .of[MountConfig]
              .encode(MountConfig.moduleConfig(sampleStatements))
        ...
        "be 409 with fs above existing fs path" >> prop {
          (d: ADir, fs: RDir, fsSuffix: RDir) =>
            (!identicalPath(fsSuffix, currentDir)) ==> {
              runTest { service =>
                val cfgStr = EncodeJson
                  .of[MountConfig]
                  .encode(MountConfig.fileSystemConfig(StubFs, fooUri))
        ...
        "be 400 with view config and dir path in X-File-Name header" >> prop {
          (parent: ADir, viewDir: RDir) =>
            runTest { service =>
              val cfg =
                MountConfig.viewConfig0(sqlB"select * from zips where pop < :cutoff",
                                        "cutoff" -> "1000")
              val cfgStr = EncodeJson.of[MountConfig].encode(cfg)
