...
import quasar.blueeyes.json.{JArray, JBool, JNull, JNum, JObject, JString, JUndefined, JValue}
...
trait DefaultExtractors {
  ...
  implicit def Tuple2Extractor[A, B](
      ...
      eb: Extractor[B]): Extractor[(A, B)] = new Extractor[(A, B)] {
    def validated(jvalue: JValue) = jvalue match {
  ...
  implicit def Tuple3Extractor[A, B, C](
      implicit ea: Extractor[A],
      eb: Extractor[B],
      ec: Extractor[C]): Extractor[(A, B, C)] = new Extractor[(A, B, C)] {
    def validated(jvalue: JValue) = jvalue match {
  ...
  implicit def Tuple4Extractor[A, B, C, D](
      implicit ea: Extractor[A],
      eb: Extractor[B],
      ec: Extractor[C],
  ...
  implicit def ArrayExtractor[T](
      implicit m: CTag[T],
      elementExtractor: Extractor[T]): Extractor[Array[T]] = new Extractor[Array[T]] {
    def validated(jvalue: JValue) = jvalue match {
      case JArray(values) =>
  ...
  implicit def SetExtractor[T](implicit elementExtractor: Extractor[T]): Extractor[Set[T]] =
    new Extractor[Set[T]] {
      def validated(jvalue: JValue) = jvalue match {
        case JArray(values) =>
  ...
  implicit def ListExtractor[T](implicit elementExtractor: Extractor[T]): Extractor[List[T]] =
  ...
  implicit def MapExtractor[K, V](
      implicit keyExtractor: Extractor[K],
      valueExtractor: Extractor[V]): Extractor[Map[K, V]] = new Extractor[Map[K, V]] {
