...
object TestConfig {
  ...
  val MONGO_READ_ONLY = ExternalBackendRef(
    BackendRef(BackendName("mongodb_read_only"), ISet singleton BackendCapability.query()),
    FileSystemType("mongodb"))
  val SPARK_HDFS = ExternalBackendRef(
    BackendRef(BackendName("spark_hdfs"), BackendCapability.All),
    FileSystemType("spark-hdfs"))
  val SPARK_LOCAL = ExternalBackendRef(
    BackendRef(BackendName("spark_local"), BackendCapability.All),
    FileSystemType("spark-local"))
  val SPARK_ELASTIC = ExternalBackendRef(
    BackendRef(BackendName("spark_elastic"), BackendCapability.All),
    FileSystemType("spark-elastic"))
  val SPARK_CASSANDRA = ExternalBackendRef(
    BackendRef(BackendName("spark_cassandra"), BackendCapability.All),
    FileSystemType("spark-cassandra"))
  val POSTGRES = ExternalBackendRef(
    BackendRef(BackendName("postgres"), BackendCapability.All),
    FileSystemType("postgres"))
  ...
  def externalFileSystems[S[_]](
    ...
    def lookupFileSystem(r: ExternalBackendRef, p: ADir): OptionT[Task, FileSystemUT[S]] = {
      def rsrc(
          connect: Task[(S ~> Task, Task[Unit])]): Task[TaskResource[(S ~> Task, Task[Unit])]] =
    ...
    TestConfig.testDataPrefix flatMap { prefix =>
      TestConfig.backendRefs.toIList.traverse(r =>
        lookupFileSystem(r, prefix).run.map(SupportedFs(r.ref, _)))
    }
  }
  ...
  def loadConnectionUriPair(
    ...
    OptionT((loadConnectionUri(insertConfName(name)).run |@| loadConnectionUri(
