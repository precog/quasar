...
package object workflow {
  ...
  def coalesceAll[F[_]: Functor](implicit I: WorkflowOpCoreF :<: F): Coalesce[F] =
    new Coalesce[F] {
      def coalesceƒ: F[Fix[F]] => Option[F[Fix[F]]] = {
        ...
        case I(p @ $ProjectF(src, shape, id)) =>
          src.project match {
            case I($ProjectF(src0, shape0, id0)) =>
              inlineProject(p, List(shape0)).map(sh =>
                I.inj($ProjectF(src0, sh, id0 |+| id)))
            ...
            case I($UnwindF(Embed(I($GroupF(src, grouped, by))), unwound))
                if id != ExcludeId =>
              inlineProjectUnwindGroup(shape, unwound, grouped).map {
                case (unwound, grouped) =>
        ...
        case I(op0 @ $GroupF(_, _, _)) =>
          inlineGroupProjects(op0).map {
            case (src, gr, by) => I.inj($GroupF(src, gr, by))
          }
        case I($GeoNearF(src, _, _, _, _, _, _, _, _, _)) =>
          src.project match {
            // FIXME: merge the params
            case I($GeoNearF(_, _, _, _, _, _, _, _, _, _)) => None
            case _                                          => None
          }
        case I($MapF(src, fn, scope)) =>
          src.project match {
            case I($MapF(src0, fn0, scope0)) =>
              Reshape
                .mergeMaps(scope0, scope)
                .map(sc => I.inj($MapF(src0, $MapF.compose(fn, fn0), sc)))
            case I($FlatMapF(src0, fn0, scope0)) =>
              Reshape
                .mergeMaps(scope0, scope)
                .map(sc => I.inj($FlatMapF(src0, $FlatMapF.mapCompose(fn, fn0), sc)))
            case _ => None
          }
        case I($FlatMapF(src, fn, scope)) =>
          src.project match {
            case I($MapF(src0, fn0, scope0)) =>
              Reshape
                .mergeMaps(scope0, scope)
                .map(sc => I.inj($FlatMapF(src0, $MapF.compose(fn, fn0), sc)))
            case I($FlatMapF(src0, fn0, scope0)) =>
              Reshape
                .mergeMaps(scope0, scope)
                .map(sc => I.inj($FlatMapF(src0, $FlatMapF.kleisliCompose(fn, fn0), sc)))
            case _ => None
          }
        case I(sm @ $SimpleMapF(src, _, _)) =>
          src.project match {
    ...
    PipelineOp(
      I.inj(op.wf.void)
        ...
        .fold(
          op =>
            (op match {
              case op @ $LookupF(_, _, _, _, _) =>
                rewriteRefs3_2(prefix).apply(op).pipeline
          ...
          op =>
            (op match {
              ...
              case op @ $GeoNearF(_, _, _, _, _, _, _, _, _, _) =>
                rewriteRefs2_6(prefix).apply(op).pipeline
  ...
  abstract class RewriteRefs[F[_]](val applyVar0: PartialFunction[DocVar, DocVar]) {
    ...
    def applyFieldName(name: BsonField): BsonField = {
      applyVar(DocField(name)).deref
        .getOrElse(name) // TODO: Delete field if it's transformed away to nothing???
    }

    def applySelector(s: Selector): Selector =
      s.mapUpFields(PartialFunction(applyFieldName _))

    def applyNel[A](m: NonEmptyList[(BsonField, A)]): NonEmptyList[(BsonField, A)] =
      m.map(t => applyFieldName(t._1) -> t._2)
  ...
  private[workflow] def rewriteRefs2_6(f: PartialFunction[DocVar, DocVar])(
    ...
    def apply[A <: WorkflowOpCoreF[_]](op: A) = {
      (op match {
        ...
        case g: $GeoNearF[_] =>
          g.copy(distanceField = applyFieldName(g.distanceField),
  ...
  private[workflow] def rewriteRefs3_2(f: PartialFunction[DocVar, DocVar]) =
    new RewriteRefs[WorkflowOp3_2F](f) {
  ...
  def simpleShape[F[_]](op: Fix[F])(
      implicit I: F :<: Workflow3_2F): Option[List[BsonField.Name]] = {
    I.inj(op.unFix)
      .run
      .fold[Option[List[BsonField.Name]]](
        {
  ...
  abstract class SingleSourceF[F[_], A] { self =>
    ...
    def reparentW[T](newSrc: T)(implicit T: Corecursive.Aux[T, F], F: Functor[F]): T =
  ...
  object IsShapePreserving {
    def unapply[F[_], A](op: F[A])(
        implicit F: Classify[F]): Option[ShapePreservingF[F, A]] =
  ...
  implicit def workflowFCrush(
      implicit I: WorkflowOpCoreF :<: WorkflowF): Crush[WorkflowF] =
    new Crush[WorkflowF] {
      @SuppressWarnings(Array("org.wartremover.warts.Recursion"))
      def crush[T[_[_]]: BirecursiveT](
            ...
            (ExprVar,
             MapReduceTask(
               ...
               MapReduce(
                 ...
                 selection = Some(
                   rewriteRefs2_6(prefixBase(base))
                     .apply(Functor[WorkflowOpCoreF]
                       .void(op)
                       .asInstanceOf[$MatchF[T[WorkflowOpCoreF]]])
                     .selector)
               ),
               None
             ))
        ...
        case I(
            op @ $MapF((_,
                        (base,
                         src1 @ MapReduceTask(
                           src0,
                           mr @ MapReduce(m, r, sel, sort, limit, None, scope0, _, _),
                           oa))),
                       fn,
                       scope)) if m == $MapF.mapNOP && r == $ReduceF.reduceNOP =>
          Reshape
            .mergeMaps(scope0, scope)
            .fold(op.newMR(base, src1, sel, sort, limit))(
              s =>
                base -> MapReduceTask(src0,
        ...
        case I(
            op @ $SimpleMapF((_,
                              (base,
                               src1 @ MapReduceTask(
                                 src0,
                                 mr @ MapReduce(_, _, _, _, _, None, scope0, _, _),
                                 oa))),
                             NonEmptyList(MapExpr(expr), INil()),
                             scope)) =>
          Reshape
            .mergeMaps(scope0, scope)
            .fold(op.newMR(base, src1, None, None, None))(
              s =>
                base -> MapReduceTask(
                  src0,
        ...
        case I(
            op @ $ReduceF((_,
                           (base,
                            src1 @ MapReduceTask(
                              src0,
                              mr @ MapReduce(_, reduceNOP, _, _, _, None, scope0, _, _),
                              oa))),
                          fn,
                          scope)) =>
          Reshape
        ...
        case I(op: MapReduceF[_]) =>
          op.singleSource.src match {
            ...
            case (_,
                  (base, PipelineTask(src0, List(PipelineOpCore($LimitF(_, count)))))) =>
              op.newMR(base, src0, None, None, Some(count))
            case (_,
                  (base,
                   PipelineTask(src0,
                                List(PipelineOpCore($MatchF(_, sel)),
                                     PipelineOpCore($SortF(_, sort)))))) =>
              op.newMR(base, src0, Some(sel), Some(sort), None)
            case (_,
                  (base,
                   PipelineTask(src0,
                                List(PipelineOpCore($MatchF(_, sel)),
                                     PipelineOpCore($LimitF(_, count)))))) =>
              op.newMR(base, src0, Some(sel), None, Some(count))
            case (_,
                  (base,
                   PipelineTask(src0,
                                List(PipelineOpCore($SortF(_, sort)),
                                     PipelineOpCore($LimitF(_, count)))))) =>
      ...
      @SuppressWarnings(Array("org.wartremover.warts.Recursion"))
      def pipeline[T[_[_]]: BirecursiveT](op: PipelineF[WorkflowF, T[WorkflowF]])
            ...
            if (pipelinable(selector)) {
              ...
              src.project match {
                case IsPipeline(p) =>
                  pipeline(p).cata({
                    case (base, up, prev) =>
                      Some((base, up, prev :+ toPipelineOp(op, base)))
      ...
      def alwaysPipePipe[T[_[_]]: BirecursiveT](op: PipelineF[WorkflowF, T[WorkflowF]])
        ...
        (op.src.project match {
        ...
        }).cata(
          {
            case (base, up, prev) =>
              ...
              (repairBase(nb), task, prev :+ toPipelineOp(op, nb))
          },
          (repairBase(base), crushed, List(toPipelineOp(op, base)))
        )
      }
    }
  ...
  implicit def crystallizeWorkflowF[F[_]: Functor: Classify: Coalesce: Refs](
      ...
      ev1: F :<: WorkflowF,
      ev2: ExprOpOps.Uni[ExprOp]): Crystallize[F] =
    new Crystallize[F] {
      ...
      def crystallize(op: Fix[F]) = {
        ...
        val crystallizeƒ: F[Fix[F]] => F[Fix[F]] = {
          case I(mr: MapReduceF[Fix[F]]) =>
            mr.singleSource.src.project match {
          ...
          case I($FoldLeftF(head, tail)) =>
            I.inj(
              $FoldLeftF[Fix[F]](
                chain(head,
                      $project[F](Reshape(ListMap(ExprName -> \/-($$ROOT))), IncludeId)),
                tail.map(x =>
                  x.project match {
        ...
        def fixShape(wf: Fix[F]) =
          simpleShape(wf).fold(finished)(
            n =>
              $project[F](Reshape(n.map(_ -> \/-($include())).toListMap), IgnoreId)
                .apply(finished))
  ...
  implicit def workflowRenderTree[T[_[_]]: RecursiveT, F[_]: Traverse: Classify](
      implicit ev0: WorkflowOpCoreF :<: F,
      ev1: RenderTree[F[Unit]]): RenderTree[T[F]] =
    new RenderTree[T[F]] {
