...
final class OperationsSpec extends quasar.Qspec {
  def markLogicOpsShould(f: Op ~> Id): Unit = {
    "Appending to files consisting of a single Map" >> {
      ...
      def appendArrayToMapTest[T: SearchOptions: AsContent[?, Data]: StructuralPlanner[Op, ?]](
          loc: AFile): MatchResult[_] =
        appendToMapTest(loc, Data._arr(toAppend.values.toList))(verifyAppended)

      def appendMapToMapTest[T: SearchOptions: AsContent[?, Data]: StructuralPlanner[Op, ?]](
          loc: AFile): MatchResult[_] =
  ...
  def fileContents[T: SearchOptions](path: AFile): Op[Data] =
    Xcc[Op]
      ...
      .map(
        _.headOption flatMap (xdmitem.toData[ErrorMessages \/ ?](_).toOption) getOrElse Data.NA)
