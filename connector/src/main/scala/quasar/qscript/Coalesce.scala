...
trait Coalesce[IN[_]] {
  ...
  protected[qscript] def coalesceQC[F[_]: Functor](FToOut: PrismNT[F, OUT])(
      implicit QC: QScriptCore[IT, ?] :<: OUT): IN[IT[F]] => Option[IN[IT[F]]]

  def coalesceQCNormalize[F[_]: Functor](FToOut: PrismNT[F, OUT])(
      implicit QC: QScriptCore[IT, ?] :<: OUT,
      N: Normalizable[IN]): IN[IT[F]] => Option[IN[IT[F]]] =
    applyTransforms(coalesceQC[F](FToOut), N.normalizeF(_: IN[IT[F]]))

  /** Coalesce for types containing ShiftedRead. */
  protected[qscript] def coalesceSR[F[_]: Functor, A](FToOut: PrismNT[F, OUT])(
      implicit QC: QScriptCore[IT, ?] :<: OUT,
      SR: Const[ShiftedRead[A], ?] :<: OUT): IN[IT[F]] => Option[IN[IT[F]]]

  def coalesceSRNormalize[F[_]: Functor, A](FToOut: PrismNT[F, OUT])(
      implicit QC: QScriptCore[IT, ?] :<: OUT,
      SR: Const[ShiftedRead[A], ?] :<: OUT,
      N: Normalizable[IN]): IN[IT[F]] => Option[IN[IT[F]]] =
    applyTransforms(coalesceSR[F, A](FToOut), N.normalizeF(_: IN[IT[F]]))

  /** Coalesce for types containing EquiJoin. */
  protected[qscript] def coalesceEJ[F[_]: Functor](FToOut: F ~> λ[α => Option[OUT[α]]])(
      implicit EJ: EquiJoin[IT, ?] :<: OUT): IN[IT[F]] => Option[OUT[IT[F]]]

  def coalesceEJNormalize[F[_]: Functor](FToOut: F ~> λ[α => Option[OUT[α]]])(
      implicit EJ: EquiJoin[IT, ?] :<: OUT,
      N: Normalizable[OUT]): IN[IT[F]] => Option[OUT[IT[F]]] = in => {
    coalesceEJ[F](FToOut).apply(in).flatMap(N.normalizeF(_: OUT[IT[F]]))
  }

  /** Coalesce for types containing ThetaJoin. */
  protected[qscript] def coalesceTJ[F[_]: Functor](FToOut: F ~> λ[α => Option[OUT[α]]])(
      implicit TJ: ThetaJoin[IT, ?] :<: OUT): IN[IT[F]] => Option[OUT[IT[F]]]

  def coalesceTJNormalize[F[_]: Functor](FToOut: F ~> λ[α => Option[OUT[α]]])(
...
trait CoalesceInstances {
  ...
  implicit def qscriptCore[T[_[_]]: BirecursiveT: EqualT: ShowT: RenderTreeT, G[_]](
      implicit QC: QScriptCore[T, ?] :<: G): Coalesce.Aux[T, QScriptCore[T, ?], G] =
  ...
  implicit def coproduct[T[_[_]], G[_], H[_], I[_]](
    ...
    new Coalesce[Coproduct[G, H, ?]] {
      ...
      def coalesceEJ[F[_]: Functor](FToOut: F ~> λ[α => Option[OUT[α]]])(
          implicit EJ: EquiJoin[IT, ?] :<: OUT) =
        _.run.fold(G.coalesceEJ(FToOut), H.coalesceEJ(FToOut))

      def coalesceTJ[F[_]: Functor](FToOut: F ~> λ[α => Option[OUT[α]]])(
          implicit TJ: ThetaJoin[IT, ?] :<: OUT) =
...
class CoalesceT[T[_[_]]: BirecursiveT: EqualT: ShowT: RenderTreeT] extends TTypes[T] {
  ...
  private def freeTotal(branch: FreeQS)(
      coalesce: QScriptTotal[T[CoEnvQS]] => Option[QScriptTotal[T[CoEnvQS]]]): FreeQS = {
    val modify: T[CoEnvQS] => T[CoEnvQS] =
      _.cata(
        (co: CoEnvQS[T[CoEnvQS]]) =>
          co.run
            .fold(
              κ(co),
              in => CoEnv[Hole, QScriptTotal, T[CoEnvQS]](repeatedly(coalesce)(in).right))
            .embed)
  ...
  private def freeSR(branch: FreeQS): FreeQS = {
    def freeSR0[A](b: FreeQS)(implicit SR: Const[ShiftedRead[A], ?] :<: QScriptTotal): FreeQS =
      freeTotal(b)(
        CoalesceTotal.coalesceSRNormalize[CoEnvQS, A](coenvPrism[QScriptTotal, Hole]))
  ...
  private def makeBranched[A, B](lOrig: A, rOrig: A)(op: A => Option[A])(
      f: (A, A) => B): Option[B] =
  ...
  private def eliminateRightSideProj(elem: FreeMap): Option[FreeMap] = {
    ...
    (elem.elgotPara(count(oneRef)) ≟ rightCount)
      .option(elem.transApoT(substitute(oneRef, HoleF)))
  }
  ...
  def qscriptCore[G[_]](implicit QC: QScriptCore :<: G): Coalesce.Aux[T, QScriptCore, G] =
    new Coalesce[QScriptCore] {
      ...
      private def extractFilterFromQC[F[_]: Functor](FToOut: OUT ~> F)(
          implicit QC: QScriptCore :<: OUT)
        : QScriptCore[IT[F]] => Option[QScriptCore[IT[F]]] = {
        case LeftShift(src, struct, id, repair) =>
          MapFuncCore.extractFilter(struct)(_.some) ∘ {
            case (f, m, _) =>
              LeftShift(FToOut(QC.inj(Filter(src, f))).embed, m, id, repair)
          }
        case Map(src, mf) =>
          MapFuncCore.extractFilter(mf)(_.some) ∘ {
            case (f, m, _) =>
      ...
      def coalesceQC[F[_]: Functor](FToOut: PrismNT[F, OUT])(implicit QC: QScriptCore :<: OUT) = {
        case Map(Embed(src), mf) =>
          FToOut.get(src) >>= QC.prj >>= (s =>
            if (mf.length ≟ 0 && (s match { case Unreferenced() => false; case _ => true }))
              Map(FToOut.reverseGet(QC.inj(Unreferenced[T, T[F]]())).embed, mf).some
            else
              s match {
                ...
                case Subset(innerSrc, lb, sel, rb) =>
                  Subset(
                    ...
                    sel,
                    rb).some
                case Filter(Embed(innerSrc), cond) =>
                  FToOut.get(innerSrc) >>= QC.prj >>= {
                ...
                case Sort(Embed(innerSrc), buckets, ordering) =>
                  FToOut.get(innerSrc) >>= QC.prj >>= {
                    case Map(doubleInner, mfInner) =>
                      Map(
                        FToOut
          ...
          FToOut.get(src) >>= QC.prj >>= {
            case LeftShift(innerSrc, innerStruct, innerId, innerRepair)
              ...
              LeftShift(
                FToOut
                  .reverseGet(
                    QC.inj(LeftShift(innerSrc, innerStruct, innerId, struct >> innerRepair)))
                  .embed,
                HoleF,
                id,
                shiftRepair).some
        ...
        case Reduce(Embed(src), bucket, reducers, redRepair) =>
          FToOut.get(src) >>= QC.prj >>= {
            case LeftShift(innerSrc, struct, id, shiftRepair) if shiftRepair =/= RightSideF =>
              (bucket.traverse(b => rightOnly(HoleF)(nm.freeMF(b >> shiftRepair))) ⊛
                reducers.traverse(
                  _.traverse(mf => rightOnly(HoleF)(nm.freeMF(mf >> shiftRepair)))))((sb, sr) =>
                Reduce(
            ...
            case LeftShift(innerSrc, struct, id, shiftRepair) =>
              (bucket
                .traverse(b => rewrite.rewriteShift(id, nm.freeMF(b >> shiftRepair)))
                .flatMap(sequenceBucket[IdStatus, JoinFunc]) ⊛
                reducers
                  .traverse(
                    _.traverse(mf => rewrite.rewriteShift(id, nm.freeMF(mf >> shiftRepair))))
                  .flatMap(_.traverse(sequenceReduce[IdStatus, JoinFunc]) >>= sequenceBucket[
                    IdStatus,
                    ReduceFunc[JoinFunc]])) {
                case ((bId, bucket), (rId, reducers)) =>
                  val newId = bId.fold(rId.getOrElse(ExcludeId).some)(b =>
                    rId.fold(b.some)(r => (b ≟ r).option(b)))
                  newId strengthR ((bucket, reducers))
              }.join >>= {
                case (newId, (bucket, reducers)) =>
                  (bucket.traverse(rightOnly(HoleF)) ⊛
                    (reducers.traverse(_.traverse(rightOnly(HoleF)))))(
                    (sb, sr) =>
                      Reduce(
                        FToOut
                          .reverseGet(QC.inj(LeftShift(innerSrc, struct, newId, RightSideF)))
                          .embed,
                        sb,
                        sr,
                        redRepair))
            ...
            case Map(innerSrc, mf) =>
              Reduce(innerSrc, bucket ∘ (_ >> mf), reducers.map(_.map(_ >> mf)), redRepair).some
            case Sort(innerSrc, _, _) if !reducers.exists(ReduceFunc.isOrderDependent) =>
      ...
      def coalesceSR[F[_]: Functor, A](FToOut: PrismNT[F, OUT])(
              ...
              (FToOut.get(innerSrc) >>= SR.prj) match {
                case Some(sr) =>
                  Map(
                    FToOut
                      .reverseGet(
                        QC.inj(Filter(FToOut.reverseGet(SR.inj(sr)).embed, mf >> innermf)))
                      .embed,
                    innermf).some
                case _ => None
              }
        ...
        case Map(Embed(src), mf) =>
          ((FToOut.get(src) >>= SR.prj) ⊛ eliminateRightSideProj(mf))(
            (const, newMF) =>
              Map(
                FToOut
                  .reverseGet(SR.inj(
                    Const[ShiftedRead[A], T[F]](ShiftedRead(const.getConst.path, ExcludeId))))
                  .embed,
                newMF)) <+>
            (((FToOut.get(src) >>= QC.prj) match {
              case Some(Filter(Embed(innerSrc), cond)) =>
                ((FToOut.get(innerSrc) >>= SR.prj) ⊛ eliminateRightSideProj(cond))(
                  (const, newCond) =>
                    Filter(
                      FToOut
                        .reverseGet(SR.inj(Const[ShiftedRead[A], T[F]](
                          ShiftedRead(const.getConst.path, ExcludeId))))
                        .embed,
                      newCond))
              case Some(Sort(Embed(innerSrc), bucket, ord)) =>
                ((FToOut.get(innerSrc) >>= SR.prj) ⊛
                  ...
                  ord.traverse(
                    Bitraverse[(?, ?)].leftTraverse.traverse(_)(eliminateRightSideProj)))(
              ...
              Map(FToOut.reverseGet(QC.inj(newFilter)).embed, newMF))
        case Reduce(Embed(src), bucket, reducers, repair) =>
          ((FToOut.get(src) >>= SR.prj) ⊛ bucket.traverse(eliminateRightSideProj) ⊛ reducers
            .traverse(_.traverse(eliminateRightSideProj)))(
      ...
      def coalesceEJ[F[_]: Functor](FToOut: F ~> λ[α => Option[OUT[α]]])(
        ...
        case Map(Embed(src), mf) =>
          (FToOut(src) >>= EJ.prj).map(ej =>
            EJ.inj(EquiJoin.combine.modify(mf >> (_: JoinFunc))(ej)))
        case Subset(src, from, sel, count) =>
      ...
      def coalesceTJ[F[_]: Functor](FToOut: F ~> λ[α => Option[OUT[α]]])(
        ...
        case Map(Embed(src), mf) =>
          (FToOut(src) >>= TJ.prj).map(tj =>
            TJ.inj(ThetaJoin.combine.modify(mf >> (_: JoinFunc))(tj)))
        case Filter(Embed(src), cond) =>
          (FToOut(src) >>= TJ.prj).map(
            tj =>
              TJ.inj(
                ThetaJoin
                  .on[IT, IT[F]]
                  .modify(on => Free.roll(MFC(And(on, cond >> tj.combine))))(tj)))
        case Subset(src, from, sel, count) =>
  ...
  def projectBucket[F[_]]: Coalesce.Aux[T, ProjectBucket, F] =
    new Coalesce[ProjectBucket] {
      ...
      def coalesceQC[F[_]: Functor](FToOut: PrismNT[F, OUT])(implicit QC: QScriptCore :<: OUT) = {
        case BucketField(Embed(src), value, field) =>
          FToOut.get(src) >>= QC.prj >>= {
            case Map(srcInner, mf) =>
              BucketField(srcInner, value >> mf, field >> mf).some
            case _ => None
          }
        case BucketIndex(Embed(src), value, index) =>
          FToOut.get(src) >>= QC.prj >>= {
            case Map(srcInner, mf) =>
  ...
  def thetaJoin[G[_]](implicit TJ: ThetaJoin :<: G): Coalesce.Aux[T, ThetaJoin, G] =
    new Coalesce[ThetaJoin] {
      ...
      def coalesceQC[F[_]: Functor](FToOut: PrismNT[F, OUT])(implicit QC: QScriptCore :<: OUT) =
        tj =>
          makeBranched(tj.lBranch, tj.rBranch)(ifNeq(freeQC))(
      ...
      def coalesceSR[F[_]: Functor, A](FToOut: PrismNT[F, OUT])(
          ...
          SR: Const[ShiftedRead[A], ?] :<: OUT) =
        tj =>
          makeBranched(tj.lBranch, tj.rBranch)(ifNeq(freeSR))(
            ThetaJoin(tj.src, _, _, tj.on, tj.f, tj.combine))

      def coalesceEJ[F[_]: Functor](FToOut: F ~> λ[α => Option[OUT[α]]])(
          implicit EJ: EquiJoin :<: OUT) =
        tj =>
          makeBranched(tj.lBranch, tj.rBranch)(ifNeq(freeEJ))((l, r) =>
            TJ.inj(ThetaJoin(tj.src, l, r, tj.on, tj.f, tj.combine)))

      def coalesceTJ[F[_]: Functor](FToOut: F ~> λ[α => Option[OUT[α]]])(
          implicit TJ: ThetaJoin :<: OUT) =
        tj =>
          makeBranched(tj.lBranch, tj.rBranch)(ifNeq(freeTJ))((l, r) =>
            TJ.inj(ThetaJoin(tj.src, l, r, tj.on, tj.f, tj.combine)))
    }
  ...
  def equiJoin[G[_]](implicit EJ: EquiJoin :<: G): Coalesce.Aux[T, EquiJoin, G] =
    new Coalesce[EquiJoin] {
      ...
      def coalesceQC[F[_]: Functor](FToOut: PrismNT[F, OUT])(implicit QC: QScriptCore :<: OUT) =
        (ej: EquiJoin[IT[F]]) => {
          val branched: Option[EquiJoin[T[F]]] = makeBranched(ej.lBranch, ej.rBranch)(
            ifNeq(freeQC))(EquiJoin(ej.src, _, _, ej.key, ej.f, ej.combine))
          ...
          def coalesceBranchMaps(ej: EquiJoin[T[F]]): Option[EquiJoin[T[F]]] = {
            def coalesceSide(
                branch: FreeQS,
                key: List[FreeMap],
                side: JoinSide): Option[(FreeQS, List[FreeMap], JoinFunc)] =
              branch.project.run.map(qct.prj) match {
                case \/-(Some(Map(innerSrc, mf))) =>
                  (innerSrc, key ∘ (_ >> mf), mf.as(side)).some
                case _ => none
              }

            (coalesceSide(ej.lBranch, ej.key ∘ (_._1), LeftSide) |@| coalesceSide(
              ej.rBranch,
              ej.key ∘ (_._2),
              RightSide)) {
      ...
      def coalesceSR[F[_]: Functor, A](FToOut: PrismNT[F, OUT])(
          ...
          SR: Const[ShiftedRead[A], ?] :<: OUT) =
        ej =>
          makeBranched(ej.lBranch, ej.rBranch)(ifNeq(freeSR))(
            EquiJoin(ej.src, _, _, ej.key, ej.f, ej.combine))

      def coalesceEJ[F[_]: Functor](FToOut: F ~> λ[α => Option[OUT[α]]])(
          implicit EJ: EquiJoin :<: OUT) =
        ej =>
          makeBranched(ej.lBranch, ej.rBranch)(ifNeq(freeEJ))((l, r) =>
            EJ.inj(EquiJoin(ej.src, l, r, ej.key, ej.f, ej.combine)))

      def coalesceTJ[F[_]: Functor](FToOut: F ~> λ[α => Option[OUT[α]]])(
          implicit TJ: ThetaJoin :<: OUT) =
        ej =>
          makeBranched(ej.lBranch, ej.rBranch)(ifNeq(freeTJ))((l, r) =>
            EJ.inj(EquiJoin(ej.src, l, r, ej.key, ej.f, ej.combine)))
    }
}
