...
object Query extends QueryInstances {
  ...
  final case class Directory[V, A](uris: IList[Uri], depth: MatchDepth)
      extends Query[V, A]
  ...
  def toXQuery[V, F[_]: Monad](f: V => F[XQuery]): AlgebraM[F, Query[V, ?], XQuery] = {
    ...
    case ElementAttributeRange(elements, attributes, op, values) =>
      values
        ...
        .map(mkSeq(_))
        .map(
          cts.elementAttributeRangeQuery(qnameSeq(elements),
                                         qnameSeq(attributes),
                                         ComparisonOp toXQuery op,
                                         _))
    case ElementAttributeValue(elements, attributes, values) =>
      cts
        .elementAttributeValueQuery(qnameSeq(elements),
                                    qnameSeq(attributes),
                                    strSeq(values))
        .point[F]
    case ElementAttributeWord(elements, attributes, words) =>
      cts
        .elementAttributeWordQuery(qnameSeq(elements),
                                   qnameSeq(attributes),
                                   strSeq(words))
        .point[F]
...
sealed abstract class QueryInstances {
  ...
  implicit def traverse[V]: Traverse[Query[V, ?]] =
    new Traverse[Query[V, ?]] {
      def traverseImpl[F[_]: Applicative, A, B](qa: Query[V, A])(
          f: A => F[B]): F[Query[V, B]] = {
        ...
        qa match {
          ...
          case ElementAttributeRange(elements, attributes, op, values) =>
            F(ElementAttributeRange(elements, attributes, op, values))
          case ElementAttributeValue(elements, attributes, values) =>
            F(ElementAttributeValue(elements, attributes, values))
          case ElementAttributeWord(elements, attributes, words) =>
            F(ElementAttributeWord(elements, attributes, words))
          ...
          case False()                            => F(False())
          case JsonPropertyRange(properties, op, values) =>
            F(JsonPropertyRange(properties, op, values))
          case JsonPropertyScope(properties, query) =>
            f(query) map (JsonPropertyScope(properties, _))
          case JsonPropertyValue(properties, values) =>
            F(JsonPropertyValue(properties, values))
          case JsonPropertyWord(properties, words) =>
            F(JsonPropertyWord(properties, words))
      ...
      def apply[A](eql: Equal[A]): Equal[Query[V, A]] = {
        ...
        Equal.equal {
          ...
          case (Document(u1), Document(u2)) =>
            u1 === u2
          case (ElementAttributeRange(e1, a1, o1, v1),
                ElementAttributeRange(e2, a2, o2, v2)) =>
    ...
    new Delay[RenderTree, Query[V, ?]] {
      def apply[A](rt: RenderTree[A]) = {
        ...
        RenderTree.make[Query[V, A]] {
          ...
          case ElementAttributeRange(elements, attributes, op, values) =>
            node("cts:element-attribute-range-query",
                 es(elements),
                 as(attributes),
                 op.render,
                 vs(values))
          case ElementAttributeValue(elements, attributes, values) =>
            node("cts:element-attribute-value-query",
                 es(elements),
                 as(attributes),
                 nodeL("Values", values))
          case ElementAttributeWord(elements, attributes, words) =>
            node("cts:element-attribute-word-query",
                 es(elements),
                 as(attributes),
                 ws(words))
          case Element(elements, query) =>
          ...
          case ElementRange(elements, op, values) =>
            node("cts:element-range-query", es(elements), op.render, vs(values))
          case ElementValue(elements, values) =>
            node("cts:element-value-query", es(elements), nodeL("Values", values))
          case ElementWord(elements, words) =>
            node("cts:element-word-query", es(elements), ws(words))
          case False() =>
            leaf("cts:false-query", none)
          case JsonPropertyRange(properties, op, values) =>
            node("cts:json-property-range-query", ps(properties), op.render, vs(values))
          case JsonPropertyScope(properties, query) =>
            node("cts:json-property-scope-query", ps(properties), query.render)
          case JsonPropertyValue(properties, values) =>
            node("cts:json-property-value-query", ps(properties), vs(values))
          case JsonPropertyWord(properties, words) =>
            node("cts:json-property-word-query", ps(properties), ws(words))
          case Near(queries, weight) =>
            nodeL("cts:near-query",
          ...
          case PathRange(paths, op, values) =>
            node("cts:path-range-query", nodeL("Paths", paths), op.render, vs(values))
          case True() =>
