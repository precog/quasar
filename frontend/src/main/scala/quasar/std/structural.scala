...
trait StructuralLib extends Library {
  ...
  val MakeObject = BinaryFunc(
    ...
    partialTyper[nat._2] {
      ...
      case Sized(Const(Data.Str(name)), Const(data)) =>
        Const(Data.Obj(ListMap(name -> data)))
    ...
    partialUntyperV[nat._2] {
      case Const(Data.Obj(map)) =>
        map.headOption match {
          case Some((key, value)) =>
            success(Func.Input2(Const(Data.Str(key)), Const(value)))
          case None =>
            failure(
              NonEmptyList(GenericError("MAKE_OBJECT can’t result in an empty object")))
        }
      case Obj(map, uk) =>
        map.headOption.fold(uk.fold[Func.VDomain[nat._2]](failure(
          NonEmptyList(GenericError("MAKE_OBJECT can’t result in an empty object"))))(t =>
          success(Func.Input2(Str, t)))) {
  ...
  val ObjectConcat: BinaryFunc = BinaryFunc(
    ...
    partialTyperV[nat._2] {
      case Sized(Const(Data.Obj(map1)), Const(Data.Obj(map2))) =>
        success(
          Const(Data.Obj(if (map1.isEmpty) map2
          else if (map2.isEmpty) map1
          else map1 ++ map2)))

      case Sized(Const(o1 @ Data.Obj(_)), o2) =>
        ObjectConcat.tpe(Func.Input2(o1.dataType, o2))
      case Sized(o1, Const(o2 @ Data.Obj(_))) =>
        ObjectConcat.tpe(Func.Input2(o1, o2.dataType))

      case Sized(Obj(map1, uk1), Obj(map2, None)) => success(Obj(map1 ++ map2, uk1))
      case Sized(Obj(map1, uk1), Obj(map2, Some(uk2))) =>
        success(Obj(map1 ∘ (_ ⨿ uk2) ++ map2, Some(uk1.fold(uk2)(_ ⨿ uk2))))
    },
  ...
  val ArrayConcat: BinaryFunc = BinaryFunc(
    ...
    noSimplification,
    partialTyperV[nat._2] {
      case Sized(Const(Data.Arr(els1)), Const(Data.Arr(els2))) =>
      ...
      case Sized(Const(a1 @ Data.Arr(_)), a2) =>
        ArrayConcat.tpe(Func.Input2(a1.dataType, a2))
      case Sized(a1, Const(a2 @ Data.Arr(_))) =>
        ArrayConcat.tpe(Func.Input2(a1, a2.dataType))

      case Sized(a1, FlexArr(min2, max2, elem2)) =>
        (a1.arrayMinLength |@| a1.arrayType)(
          (min1, typ1) =>
            success(
              FlexArr(min1 + min2,
      ...
      case Sized(FlexArr(min1, max1, elem1), a2) =>
        (a2.arrayMinLength |@| a2.arrayType)(
          (min2, typ2) =>
            success(
              FlexArr(min1 + min2,
    ...
    partialUntyperV[nat._2] {
      case x if x.arrayLike =>
        x.arrayType.fold[Func.VDomain[nat._2]](failure(NonEmptyList(GenericError(
          "internal error: " + x.shows + " is arrayLike, but no arrayType")))) { typ =>
  ...
  val ConcatOp = BinaryFunc(
    ...
    noSimplification,
    partialTyperV[nat._2] {
      case Sized(t1, t2) if t1.arrayLike && t2.contains(AnyArray ⨿ Str) =>
        ArrayConcat.tpe(Func.Input2(t1, FlexArr(0, None, Top)))
      case Sized(t1, t2) if t1.contains(AnyArray ⨿ Str) && t2.arrayLike =>
        ArrayConcat.tpe(Func.Input2(FlexArr(0, None, Top), t2))
      case Sized(t1, t2) if t1.arrayLike && t2.arrayLike =>
        ArrayConcat.tpe(Func.Input2(t1, t2))

      case Sized(Const(Data.Str(str1)), Const(Data.Str(str2))) =>
        success(Const(Data.Str(str1 ++ str2)))
      case Sized(t1, t2) if Str.contains(t1) && t2.contains(AnyArray ⨿ Str) =>
        success(Type.Str)
      case Sized(t1, t2) if t1.contains(AnyArray ⨿ Str) && Str.contains(t2) =>
        success(Type.Str)
      case Sized(t1, t2) if Str.contains(t1) && Str.contains(t2) =>
        StringLib.Concat.tpe(Func.Input2(t1, t2))

      case Sized(t1, t2) if t1 == t2 => success(t1)

      case Sized(Const(Data.Str(s)), Const(Data.Arr(ys))) =>
        success(Const(Data.Arr(s.map(c => Data._str(c.toString)).toList ::: ys)))
      case Sized(Const(Data.Arr(xs)), Const(Data.Str(s))) =>
        success(Const(Data.Arr(xs ::: s.map(c => Data._str(c.toString)).toList)))
      case Sized(t1, t2) if Str.contains(t1) && t2.arrayLike =>
        ArrayConcat.tpe(Func.Input2(FlexArr(0, None, Str), t2))
      case Sized(t1, t2) if t1.arrayLike && Str.contains(t2) =>
        ArrayConcat.tpe(Func.Input2(t1, FlexArr(0, None, Str)))
    },
    partialUntyperV[nat._2] {
      case x if x.contains(AnyArray ⨿ Str) =>
        success(Func.Input2(AnyArray ⨿ Str, AnyArray ⨿ Str))
  ...
  val FlattenMap = UnaryFunc(
    ...
    partialTyperV[nat._1] {
      ...
      case Sized(x) if x.objectLike =>
        x.objectType.fold[Func.VCodomain](failure(
          NonEmptyList(GenericError("internal error: objectLike, but no objectType"))))(
          success)
    },
  ...
  val FlattenArray = UnaryFunc(
    ...
    partialTyperV[nat._1] {
      ...
      case Sized(x) if x.arrayLike =>
        x.arrayType.fold[Func.VCodomain](failure(
          NonEmptyList(GenericError("internal error: arrayLike, but no arrayType"))))(
          success)
    },
  ...
  val ShiftMap = UnaryFunc(
    ...
    partialTyperV[nat._1] {
      case Sized(x) if x.objectLike =>
        x.objectType.fold[ValidationNel[SemanticError, Type]](failure(
          NonEmptyList(GenericError("internal error: objectLike, but no objectType"))))(
          success)
    },
    untyper[nat._1](tpe => success(Func.Input1(Obj(Map(), Some(tpe)))))
  ...
  val ShiftArray = UnaryFunc(
    ...
    new Func.Simplifier {
      def apply[T](orig: LP[T])(implicit TR: Recursive.Aux[T, LP],
                                TC: Corecursive.Aux[T, LP]) =
        orig match {
          case InvokeUnapply(_,
                             Sized(
                               Embed(InvokeUnapply(UnshiftArray, Sized(Embed(set)))))) =>
            set.some
          case _ => None
        }
    },
    partialTyperV[nat._1] {
      case Sized(Const(Data.Arr(elems))) => success(Const(Data.Set(elems)))
      case Sized(x) if x.arrayLike =>
        x.arrayType.fold[ValidationNel[SemanticError, Type]](failure(
          NonEmptyList(GenericError("internal error: arrayLike, but no arrayType"))))(
          success)
    },
    untyper[nat._1](tpe => success(Func.Input1(FlexArr(0, None, tpe))))
  ...
  val UnshiftMap: BinaryFunc = BinaryFunc(
    ...
    partialUntyperV[nat._2] {
      case tpe if tpe.objectLike =>
        tpe.objectType.fold[Func.VDomain[nat._2]](failure(
          NonEmptyList(GenericError("internal error: objectLike, but no objectType"))))(
  ...
  val UnshiftArray: UnaryFunc = UnaryFunc(
    ...
    new Func.Simplifier {
      def apply[T](orig: LP[T])(implicit TR: Recursive.Aux[T, LP],
        ...
        orig match {
          case InvokeUnapply(_,
                             Sized(
                               Embed(InvokeUnapply(ShiftArray, Sized(Embed(array)))))) =>
            array.some
          case InvokeUnapply(
              _,
              Sized(
                Embed(
                  InvokeUnapply(ShiftArrayIndices,
                                Sized(Embed(Constant(Data.Arr(array)))))))) =>
            Constant(Data.Arr((0 until array.length).toList ∘ (Data.Int(_)))).some
          case InvokeUnapply(
              _,
              Sized(
                Embed(InvokeUnapply(ShiftMap, Sized(Embed(Constant(Data.Obj(map)))))))) =>
            Constant(Data.Arr(map.values.toList)).some
          case InvokeUnapply(
              _,
              Sized(
                Embed(
                  InvokeUnapply(ShiftMapKeys, Sized(Embed(Constant(Data.Obj(map)))))))) =>
    ...
    partialUntyperV[nat._1] {
      case tpe if tpe.arrayLike =>
        tpe.arrayType.fold[Func.VDomain[nat._1]](failure(
          NonEmptyList(GenericError("internal error: arrayLike, but no arrayType"))))(x =>
          success(Func.Input1(x)))
    }
  )
  ...
  object MakeObjectN {
    ...
    def unapply[T](t: LP[T])(implicit T: Recursive.Aux[T, LP]): Option[List[(T, T)]] =
      t match {
        case InvokeUnapply(MakeObject, Sized(name, expr)) => Some(List((name, expr)))
        case InvokeUnapply(ObjectConcat, Sized(a, b)) =>
          (unapply(a.project) ⊛ unapply(b.project))(_ ::: _)
        case _ => None
      }
  }
  ...
  object MakeArrayN {
    ...
    object Attr {
      ...
      def unapply[A](t: Cofree[LP, A]): Option[List[Cofree[LP, A]]] =
        t.tail match {
          case InvokeUnapply(MakeArray, Sized(x)) => Some(x :: Nil)
          case InvokeUnapply(ArrayConcat, Sized(a, b)) =>
