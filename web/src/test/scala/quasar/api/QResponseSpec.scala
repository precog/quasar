  ...
  "toHttpResponse" should {
    ...
    "failed evaluation" >> {
      ...
      "has alternate response status" >> {
        failStream.toHttpResponse(evalStr("one")).unsafePerformSync.status must_== BadRequest
      }
      ...
      "has alternate response body" >> {
        failStream.toHttpResponse(evalStr("one")).as[String].unsafePerformSync must_== "FAIL"
      }
