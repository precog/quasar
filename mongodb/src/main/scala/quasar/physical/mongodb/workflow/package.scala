...
package object workflow {
      ...
      def coalesceƒ: F[Fix[F]] => Option[F[Fix[F]]] = {
        ...
        case I(p @ $ProjectF(src, shape, id)) =>
          src.project match {
            ...
            case I($UnwindF(Embed(I($GroupF(src, grouped, by))), unwound)) if id != ExcludeId =>
  ...
  implicit def workflowFCrush(implicit I: WorkflowOpCoreF :<: WorkflowF): Crush[WorkflowF] =
    new Crush[WorkflowF] {
      @SuppressWarnings(Array("org.wartremover.warts.Recursion"))
      def crush[T[_[_]]: BirecursiveT](op: WorkflowF[(T[WorkflowF], (DocVar, WorkflowTask))]) =
        op match {
      ...
      def alwaysPipePipe[T[_[_]]: BirecursiveT](
          op: PipelineF[WorkflowF, T[WorkflowF]]): (DocVar, WorkflowTask, List[PipelineOp]) = {
      ...
      def crystallize(op: Fix[F]) = {
        ...
        val crystallizeƒ: F[Fix[F]] => F[Fix[F]] = {
          ...
          case I($FoldLeftF(head, tail)) =>
            I.inj(
              $FoldLeftF[Fix[F]](
                chain(head, $project[F](Reshape(ListMap(ExprName -> \/-($$ROOT))), IncludeId)),
                tail.map(x =>
                  x.project match {
