...
trait BlockAlignSpec extends SpecificationLike with ScalaCheck {
  ...
  def alignAcrossBoundaries = {
    ...
    val sample = SampleData(elements.toStream.map(RValue.unsafeFromJValueRaw), Some((3,List((JPath(".xb5hs2ckjajs0k44x"),CDouble), (JPath(".zzTqxfzwzacakwjqeGFcnhpkzd5akfobsg2nxump"),CEmptyArray), (JPath(".sp7hpv"),CEmptyObject)))))
    testAlign(sample.sortBy(_ \ "key"))
  }
