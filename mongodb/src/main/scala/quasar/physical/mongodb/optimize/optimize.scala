...
package object optimize {
  object pipeline {
    ...
    private def deleteUnusedFields0[F[_]: Functor: Refs](
      ...
      def getRefs[A](op: F[_], prev: Option[Set[DocVar]]): Option[Set[DocVar]] = op match {
    ...
    private def simplifyGroupƒ[F[_]: Coalesce: Functor](
      ...
      case ev($GroupF(src, Grouped(cont), id)) =>
        val (newCont, proj) =
          cont.foldLeft[(
              ListMap[BsonField.Name, AccumOp[Fix[ExprOp]]],
              ListMap[BsonField.Name, Reshape.Shape[ExprOp]])]((ListMap(), ListMap())) {
        ...
        if (newCont == cont)
          ...
          chain(src, $group[F](Grouped(newCont), id), $project[F](Reshape(proj), IgnoreId)).project.some
      case _ => None
    }
    ...
    private def reorderOpsƒ[F[_]: Coalesce: Functor](
...

      {
        ...
        case I(
            $SkipF(
              Embed(I($SimpleMapF(src0, fn @ NonEmptyList(MapExpr(_), INil()), scope))),
              count)) =>
        ...
        case I(
            $MatchF(
              Embed(I($SimpleMapF(src0, exprs @ NonEmptyList(MapExpr(jsFn), INil()), scope))),
              sel)) => {
