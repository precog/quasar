...
object Sql {
    ...
    new Delay[RenderTree, Sql] {
      def apply[A](ra: RenderTree[A]): RenderTree[Sql[A]] = new RenderTree[Sql[A]] {
        ...
        def renderCase(c: Case[A]): RenderedTree =
          NonTerminal("Case" :: astType,
                      None,
                      ra.render(c.cond) :: ra.render(c.expr) :: Nil)
        ...
        def render(n: Sql[A]) = n match {
          case Select(isDistinct, projections, relations, filter, groupBy, orderBy) =>
            ...
            NonTerminal(
              nt,
              isDistinct match {
                case `SelectDistinct` => Some("distinct"); case _ => None
              },
              projections.map { p =>
                ...
                (relations.map(SqlRelation.renderTree(ra).render) ::
                  ...
                  groupBy.map {
                  case GroupBy(keys, Some(having)) =>
                    NonTerminal("GroupBy" :: astType,
                                None,
                                keys.map(ra.render) :+ ra.render(having))
                  case GroupBy(keys, None) =>
                    NonTerminal("GroupBy" :: astType, None, keys.map(ra.render))
                } ::
                  orderBy.map {
                  case OrderBy(keys) =>
                    val nt = "OrderBy" :: astType
                    NonTerminal(nt, None, keys.map {
                      case (t, x) =>
                        NonTerminal("OrderType" :: nt, Some(t.shows), ra.render(x) :: Nil)
                    }.toList)
                } ::
          ...
          case SetLiteral(exprs) =>
            NonTerminal("Set" :: astType, None, exprs.map(ra.render))
          case ArrayLiteral(exprs) =>
            NonTerminal("Array" :: astType, None, exprs.map(ra.render))
          case MapLiteral(exprs) =>
            NonTerminal("Map" :: astType, None, exprs.map(_.render))

          case InvokeFunction(name, args) =>
            NonTerminal("InvokeFunction" :: astType,
                        Some(name.value),
                        args.map(ra.render))

          case Match(expr, cases, Some(default)) =>
            NonTerminal("Match" :: astType,
                        None,
                        ra.render(expr) :: (cases.map(renderCase) :+ ra.render(default)))
          case Match(expr, cases, None) =>
            NonTerminal("Match" :: astType,
                        None,
                        ra.render(expr) :: cases.map(renderCase))

          case Switch(cases, Some(default)) =>
            NonTerminal("Switch" :: astType,
                        None,
                        cases.map(renderCase) :+ ra.render(default))
          case Switch(cases, None) =>
            NonTerminal("Switch" :: astType, None, cases.map(renderCase))

          case Binop(lhs, rhs, op) =>
            NonTerminal("Binop" :: astType,
                        Some(op.sql),
                        ra.render(lhs) :: ra.render(rhs) :: Nil)

          case Unop(expr, op) =>
            NonTerminal("Unop" :: astType, Some(op.sql), ra.render(expr) :: Nil)

          case Splice(expr) =>
            NonTerminal("Splice" :: astType, None, expr.toList.map(ra.render))
          ...
          case Let(name, form, body) =>
            NonTerminal("Let" :: astType,
                        Some(name.value),
                        ra.render(form) :: ra.render(body) :: Nil)
  ...
  implicit val traverse: Traverse[Sql] = new Traverse[Sql] {
    def traverseImpl[G[_], A, B](fa: Sql[A])(f: A => G[B])(
        implicit G: Applicative[G]): G[Sql[B]] = {
      ...
      fa match {
        ...
        case Match(expr, cases, default) =>
          (f(expr) ⊛ cases.traverse(traverseCase) ⊛ default.traverse(f))(matc(_, _, _))
        case Switch(cases, default) =>
          (cases.traverse(traverseCase) ⊛ default.traverse(f))(switch(_, _))
...
@Lenses final case class Select[A] private[sql] (isDistinct: IsDistinct,
    ...
    extends Sql[A] {
  def substituteRelationVariable[M[_]: Monad, T](mapping: Vari[A] => M[A])(
      implicit
      T0: Recursive.Aux[T, Sql],
      T1: Corecursive.Aux[T, Sql],
      ev: A <~< T): M[SemanticError \/ Select[A]] = {
    val newRelation = relation.traverse(
      _.transformM[EitherT[M, SemanticError, ?], A](
        {
          case VariRelationAST(vari, alias) =>
            EitherT(mapping(vari).map(ev(_).project match {
              case Ident(name) =>
                posixCodec
                  .parsePath(Some(_), Some(_), κ(None), κ(None))(name)
                  .cata(TableRelationAST(_, alias).right,
                        SemanticError
                          .GenericError(
                            s"bad path: $name (note: absolute file path required)")
                          .left) // FIXME
              ...
              case x =>
                SemanticError
                  .GenericError(s"not a valid table name: ${pprint(x.embed)}")
                  .left // FIXME
            }))
...
@Lenses final case class InvokeFunction[A] private[sql] (name: CIName, args: List[A])
    extends Sql[A]
@Lenses final case class Match[A] private[sql] (expr: A,
                                                cases: List[Case[A]],
                                                default: Option[A])
    extends Sql[A]
