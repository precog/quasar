...
class StructuralSpecs extends quasar.Qspec {
  ...
  "FlattenArrayIndices" should {
    ...
    "only untype from ints" >> prop { (nonInt: Type) =>
      FlattenArrayIndices.untpe(nonInt).map(_.unsized) must beFailure
    }.setArbitrary(arbStrType)
  }
  ...
  "ShiftArrayIndices" should {
    ...
    "only untype from ints" >> prop { (nonInt: Type) =>
      ShiftArrayIndices.untpe(nonInt).map(_.unsized) must beFailure
    }.setArbitrary(arbStrType)
  }
