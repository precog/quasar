...
class MathSpec extends quasar.Qspec with TypeArbitrary {
  ...
  "MathLib" should {
    ...
    "add timestamp and interval" in {
      val expr = Add.tpe(
        Func.Input2(Type.Const(Timestamp(Instant.parse("2015-01-21T00:00:00Z"))),
                    Type.Const(Interval(Duration.ofHours(9)))))
      expr should beSuccessful(
        Type.Const(Timestamp(Instant.parse("2015-01-21T09:00:00Z"))))
    }

    def permute(f: quasar.Func.Input[Type, nat._2] => ValidationNel[SemanticError, Type],
                t1: Const,
