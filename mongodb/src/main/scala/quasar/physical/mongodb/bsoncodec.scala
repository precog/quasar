...
object BsonCodec {
  ...
  object EJsonTypeSize {
    def apply(typ: String, size: Int): Bson =
      Bson.Doc(ListMap("_ejson.type" -> Bson.Text(typ), "_ejson.size" -> Bson.Int32(size)))
  ...
  def fromCommon(v: BsonVersion): Algebra[ejson.Common, Bson] = {
    ...
    case ejson.Dec(value) if v lt BsonVersion.`1.1` => Bson.Dec(value.toDouble)
    case ejson.Dec(value) if value.isDecimalDouble => Bson.Dec(value.toDouble)
      ...
      (meta, value) match {
        ...
        case (EJsonType("_ejson.date"), Bson.Doc(map)) =>
          (extract(map.get("year"), Bson._int32) ⊛
            extract(map.get("day_of_year"), Bson._int32))((y, d) => LocalDate.ofYearDay(y, d))
            .orElse(
              (extract(map.get("year"), Bson._int32) ⊛
  ...
  def toEJson[F[_]](
    ...
    case Bson.Dec(value) if (!value.isNaN && !value.isInfinity) => C.inj(ejson.Dec(value)).right
  ...
  def fromData(v: BsonVersion, data: Data): PlannerError \/ Bson =
    ...
    }, fromEJson(v)), Data.toEJson[EJson].apply(_).right)
