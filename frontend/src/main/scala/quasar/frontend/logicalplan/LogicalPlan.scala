...
object InvokeUnapply {
  def unapply[N <: Nat, A](in: Invoke[N, A]): Some[(GenericFunc[N], Func.Input[A, N])] =
...
final case class Sort[A](src: A, order: NonEmptyList[(A, SortDir)]) extends LogicalPlan[A]
...
object LogicalPlan {
  ...
  implicit val traverse: Traverse[LogicalPlan] =
    new Traverse[LogicalPlan] {
      def traverseImpl[G[_], A, B](fa: LogicalPlan[A])(f: A => G[B])(
          implicit G: Applicative[G]): G[LogicalPlan[B]] =
        fa match {
          ...
          case Join(l, r, tpe, JoinCondition(lName, rName, cond)) =>
            (f(l) ⊛ f(r) ⊛ f(cond))((v1, v2, v3) =>
              Join(v1, v2, tpe, JoinCondition(lName, rName, v3)))
      ...
      override def foldMap[A, B](fa: LogicalPlan[A])(f: A => B)(
      ...
      override def foldRight[A, B](fa: LogicalPlan[A], z: => B)(f: (A, => B) => B): B =
        fa match {
          ...
          case Sort(src, ords) =>
            f(src, ords.foldRight(z) { case ((a, _), b) => f(a, b) })
      ...
      def apply[A](sa: Show[A]): Show[LogicalPlan[A]] = {
        ...
        Show.show {
          ...
          case Invoke(func, values) =>
            ...
            func.show ++ Cord("(") ++
              values.foldLeft(Cord("")) {
                case (acc, v) => acc ++ sa.show(v) ++ Cord(", ")
              } ++ Cord(")")
          case JoinSideName(n) =>
      ...
      def apply[A](ra: RenderTree[A]): RenderTree[LogicalPlan[A]] =
        new RenderTree[LogicalPlan[A]] {
          ...
          def render(v: LogicalPlan[A]) = v match {
            ...
            case Constant(Data.Str(str)) =>
              Terminal("Str" :: "Constant" :: nodeType, Some(str.shows))
            case InvokeUnapply(func @ structural.ObjectProject, Sized(expr, name)) =>
              (ra.render(expr), ra.render(name)) match {
                case (exprR @ RenderedTree(_, Some(_), Nil),
                      RenderedTree(_, Some(n), Nil)) =>
                  Terminal("ObjectProject" :: nodeType, Some(exprR.shows + "{" + n + "}"))
                case (x, n) =>
                  NonTerminal("Invoke" :: nodeType, Some(func.shows), x :: n :: Nil)
              }

            case Read(file) =>
              Terminal("Read" :: nodeType, Some(posixCodec.printPath(file)))
            case Constant(data) => Terminal("Constant" :: nodeType, Some(data.shows))
            case InvokeUnapply(func, args) =>
              NonTerminal("Invoke" :: nodeType,
                          Some(func.shows),
                          args.unsized.map(ra.render))
            case JoinSideName(name) =>
              Terminal("JoinSideName" :: nodeType, Some(name.toString))
            ...
            case Let(ident, form, body) =>
              NonTerminal("Let" :: nodeType,
                          Some(ident.toString),
                          List(ra.render(form), ra.render(body)))
            case Sort(src, ords) =>
              NonTerminal("Sort" :: nodeType, None, (ra.render(src) :: ords.list.flatMap {
                case (a, d) => IList(ra.render(a), d.render)
              }).toList)
            case TemporalTrunc(part, src) =>
              NonTerminal("TemporalTrunc" :: nodeType,
                          Some(part.shows),
                          List(ra.render(src)))
            case Typecheck(expr, typ, cont, fallback) =>
              NonTerminal("Typecheck" :: nodeType,
                          Some(typ.shows),
    ...
    new Delay[Equal, LogicalPlan] {
      def apply[A](fa: Equal[A]) = {
        ...
        Equal.equal {
          ...
          case (InvokeUnapply(f1, v1), InvokeUnapply(f2, v2)) =>
            f1 == f2 && v1.unsized ≟ v2.unsized // TODO impl `scalaz.Equal` for `GenericFunc`
          case (JoinSideName(n1), JoinSideName(n2)) => n1 ≟ n2
          case (Join(l1, r1, t1, JoinCondition(lName1, rName1, c1)),
                Join(l2, r2, t2, JoinCondition(lName2, rName2, c2))) =>
  ...
  implicit val binder: Binder[LogicalPlan] = new Binder[LogicalPlan] {
    ...
    def bindings[T, A](t: LogicalPlan[T], b: G[A])(f: LogicalPlan[T] => A)(
        implicit T: Recursive.Aux[T, LogicalPlan]): G[A] =
      t match {
        case Let(ident, form, _) => b + (ident -> f(form.project))
        case _                   => b
      }

    def subst[T, A](t: LogicalPlan[T], b: G[A])(
        implicit T: Recursive.Aux[T, LogicalPlan]): Option[A] =
      t match {
