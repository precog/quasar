...
sealed abstract class NameGeneratorInstances extends NameGeneratorInstances0 {
  implicit def sequenceNameGenerator[F[_]](
      implicit F: MonadState[F, Long]): NameGenerator[F] =
    new NameGenerator[F] {
...
sealed abstract class NameGeneratorInstances0 {
  ...
  implicit def readerTNameGenerator[F[_]: NameGenerator, A]
    : NameGenerator[ReaderT[F, A, ?]] =
