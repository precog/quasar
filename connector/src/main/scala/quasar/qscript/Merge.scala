...
class Merge[T[_[_]]: BirecursiveT: EqualT: ShowT] extends TTypes[T] {
  case class ZipperSides(lSide: FreeMap, rSide: FreeMap)

  case class ZipperTails[G[_]](lTail: List[G[EM]], rTail: List[G[EM]])

  case class ZipperAcc[G[_]](acc: List[G[EM]], sides: ZipperSides, tails: ZipperTails[G])

  // TODO: Convert to NEL
  private def linearize[G[_]: Traverse]: Algebra[G, List[G[EM]]] =
    fl => fl.as[EM](Extern) :: fl.fold

  private def delinearizeInner[G[_]: Functor, H[_], A](HtoG: H ~> G)(
      implicit DE: Const[DeadEnd, ?] :<: H): Coalgebra[G, List[G[A]]] = {
  ...
  private def delinearizeTargetsCoEnv[A]: Coalgebra[CoEnvQS, List[CoEnvQS[A]]] = {
  ...
  private def zipper[G[_]](
      implicit mergeable: Mergeable.Aux[T, G]): ElgotCoalgebra[ZipperAcc[G] \/ ?,
                                                               ListF[G[EM], ?],
                                                               (ZipperSides,
                                                                ZipperTails[G])] = {
  ...
  private def makeList[S[_[_]], G[_]: Traverse, H[_]](input: S[H])(
      implicit R: Recursive.Aux[S[H], G]): List[G[EM]] =
    R.cata(input)(linearize[G]).reverse

  private def makeZipper[S[_[_]], G[_]: Traverse, H[_]](left: S[H], right: S[H])(
      implicit
      mergeable: Mergeable.Aux[T, G],
      R: Recursive.Aux[S[H], G]): ZipperAcc[G] =
    elgot(
      (ZipperSides(HoleF[T], HoleF[T]),
       ZipperTails(makeList[S, G, H](left), makeList[S, G, H](right))))(consZipped[G],
                                                                        zipper[G])

  def mergePair[S[_[_]], G[_]: Traverse, H[_]](left: S[H], right: S[H])(
      HtoG: H ~> G,
      unify: List[G[EM]] => FreeQS)(
      implicit
      mergeable: Mergeable.Aux[T, G],
      C: Corecursive.Aux[S[H], G],
      R: Recursive.Aux[S[H], G],
      DE: Const[DeadEnd, ?] :<: H): SrcMerge[S[H], FreeQS] = {

    val ZipperAcc(common, ZipperSides(lMap, rMap), ZipperTails(lTail, rTail)) =
      makeZipper[S, G, H](left, right)

    SrcMerge[S[H], FreeQS](common.reverse.ana[S[H]](delinearizeInner[G, H, EM](HtoG)),
  ...
  def mergeT[G[_]: Traverse](left: T[G], right: T[G])(
      ...
      DE: Const[DeadEnd, ?] :<: G,
      QC: QScriptCore :<: G,
      FI: Injectable.Aux[G, QScriptTotal]): SrcMerge[T[G], FreeQS] = {
    ...
    def unify: List[G[EM]] => FreeQS =
      _.reverse
        .ana[Free[G, Hole]](delinearizeTargets[G, EM] >>> (CoEnv(_)))
        .mapSuspension(FI.inject)
  ...
  private def mergeBranches(rewrite: Rewrite[T])(
      left: FreeQS,
      right: FreeQS): PlannerError \/ SrcMerge[FreeQS, FreeMap] =
    (left ≟ right).fold(
      SrcMerge(left, HoleF[T], HoleF[T]).right[PlannerError], {
        ...
        val baseSrc: Option[CoEnvQS[FreeQS]] =
          rewrite.unifySimpleBranchesCoEnv[QScriptTotal, FreeQS](src,
                                                                 lBranch,
                                                                 rBranch,
                                                                 combine)(rebase0)

        baseSrc.cata(src => SrcMerge(src.embed, lacc, racc).right[PlannerError],
                     InternalError
                       .fromMsg(s"failed to merge branches")
                       .left[SrcMerge[FreeQS, FreeMap]])
      }
    )

  // FIXME: We shouldn't have to guess which side is which.
  // Fixing #1556 should address this issue.
  def tryMergeBranches(
      rewrite: Rewrite[T])(left1: FreeQS, right1: FreeQS, left2: FreeQS, right2: FreeQS)
