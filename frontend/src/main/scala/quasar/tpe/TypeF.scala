...
object TypeF extends TypeFInstances {
  ...
  final case class Map[J, A](known: IMap[J, A], unknown: Option[(A, A)])
      extends TypeF[J, A]
  ...
  def leaf[J, A]: Prism[TypeF[J, A], SimpleType \/ J] =
    Prism[TypeF[J, A], SimpleType \/ J](
      tf =>
        simple[J, A].getOption(tf).map(_.left) orElse const[J, A]
          .getOption(tf)
          .map(_.right))(_.fold(simple[J, A](_), const[J, A](_)))
  ...
  object glb {
    ...
    final class PartiallyApplied[J] {
      def apply[T](x: T, y: T)(
        ...
        (x, y).ghylo[Id, T \/ ?](distCata[TypeF[J, ?]],
                                 distApo[T, TypeF[J, ?]],
  ...
  def glbƒ[J: Order, T](
    ...
    def mapGlb(xs: IMap[J, T],
               ux: Option[(T, T)],
               ys: IMap[J, T],
               uy: Option[(T, T)]): TypeF[J, LR] =
      map[J, LR](
        xs.alignWith(ys)(
          _.fold(
            x => (x, uy ? ⊥ | T),
  ...
  object identical {
    def apply[J] = new PartiallyApplied[J]
    final class PartiallyApplied[J] {
      def apply[T](a: T, b: T)(implicit J: Equal[J],
                               T: Recursive.Aux[T, TypeF[J, ?]]): Boolean =
...
private[quasar] sealed abstract class TypeFInstances {
          ...
          val other = unk map {
            case (k, v) =>
              (
                ...
                map1(C(ejson.str[J]("keys")).embed   -> k,
  ...
  implicit def traverse[J]: Traverse[TypeF[J, ?]] =
    new Traverse[TypeF[J, ?]] {
      def traverseImpl[G[_]: Applicative, A, B](tf: TypeF[J, A])(
          f: A => G[B]): G[TypeF[J, B]] = tf match {
        ...
        case Map(kn, unk) =>
          (kn.traverse(f) |@| UT.traverse(unk)(f))(TypeF.map[J, B](_, _))
