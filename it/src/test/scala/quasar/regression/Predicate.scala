...
object Predicate {
  ...
  def matchJson(expected: Option[Json]): Matcher[Option[Json]] =
    new Matcher[Option[Json]] {
      def apply[S <: Option[Json]](s: Expectable[S]) = {
        (expected, s.value) match {
          case (Some(expected), Some(actual)) =>
            ...
            } getOrElse result(actual == expected,
                               s"matches $expected",
                               s"$actual does not match $expected",
                               s)
    ...
    def apply[F[_]: Catchable: Monad](
      ...
      case OrderPreserved =>
        ...
        (actual tee expected)(tee.zipAll(None, None))
          ...
          .runLog
          .map(_.foldMap()(Result.ResultMonoid))
      case OrderIgnored =>
        actual0
          .scan((expected0.toSet, Set.empty[Json])) {
  ...
  final case object Exactly extends Predicate {
    def apply[F[_]: Catchable: Monad](
      ...
      case OrderIgnored =>
        actual0
          .scan((expected0.toSet, Set.empty[Json], None: Option[Json])) {
    ...
    def apply[F[_]: Catchable: Monad](
      ...
      if (expected.isEmpty)
        ...
        actual
          ...
          .map(unseen =>
            expected
              .filterNot(unseen contains _) aka "prohibited values" must beEmpty: Result)
          .runLastOr(success)
    }
  }
  ...
  implicit val PredicateDecodeJson: DecodeJson[Predicate] =
    DecodeJson(c =>
      c.as[String].flatMap {
        ...
        case str =>
          jfail(
            "Expected one of: atLeast, exactly, doesNotContain, initial, but found: " + str,
            c.history)
    })
}
