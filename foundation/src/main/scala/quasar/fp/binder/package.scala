...
package object binder {
   ...
   */
  def boundAttribute[T, F[_]: Functor, A](t: T)(f: T => A)(
      implicit TR: Recursive.Aux[T, F],
      TC: Corecursive.Aux[T, F],
      B: Binder[F]): Cofree[F, A] = {
  ...
  def boundCata[T, F[_]: Functor, A](t: T)(
      f: F[A] => A)(implicit T: Recursive.Aux[T, F], B: Binder[F]): A = {
    @SuppressWarnings(Array("org.wartremover.warts.Recursion"))
  ...
  def boundParaM[T, M[_]: Monad, F[_]: Traverse, A](t: T)(
      f: F[(T, A)] => M[A])(implicit T: Recursive.Aux[T, F], B: Binder[F]): M[A] = {
    @SuppressWarnings(Array("org.wartremover.warts.Recursion"))
    def loop(t: F[T], b: B.G[A]): M[A] = {
      Applicative[M]
        .sequence(B.bindings[T, M[A]](t, B.G.map(b)(_.point[M]))(s => loop(s, b)))(B.G)
        .flatMap { newB =>
          B.subst(t, newB)
            .cata[M[A]](
              _.point[M],
  ...
  def boundParaS[T, F[_]: Traverse, S, A](t: T)(
      f: F[(T, A)] => State[S, A])(implicit T: Recursive.Aux[T, F], B: Binder[F]): State[S, A] =
    boundParaM[T, State[S, ?], F, A](t)(f)

  def boundPara[T, F[_]: Functor, A](t: T)(
      f: F[(T, A)] => A)(implicit T: Recursive.Aux[T, F], B: Binder[F]): A = {
    @SuppressWarnings(Array("org.wartremover.warts.Recursion"))
    def loop(t: F[T], b: B.G[A]): A = {
      val newB = B.bindings(t, b)(loop(_, b))
