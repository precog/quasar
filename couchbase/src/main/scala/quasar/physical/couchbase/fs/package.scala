    ...
    for {
      ...
      cluster <- EitherT(
        Task
          .delay(
            CouchbaseCluster
              .fromConnectionString(ev, uri.copy(path = "", query = Query.empty).renderString)
              .right
          )
          .handle {
      ...
      idxExst <- Task
        ...
        .liftM[DefErrT]
      _ <- idxExst
        .unlessM(Task.delay(bktMgr
          .createN1qlIndex(s"quasar_${params.docTypeKey}_idx", true, false, params.docTypeKey)))
        .liftM[DefErrT]
