...
final class CompressionSpec
  ...
  "coalesceKeys" >> {
    def test(kind: String, f: Char => J) =
      s"compresses largest group of keys having same primary ${kind}" >> prop {
        (cs: ISet[Char], n: BigInt, b: Byte, unk0: Option[(LeafEjs, LeafEjs)]) =>
          (cs.size > 1) ==> {
  ...
  "coalescePrimary" >> {
    "combines consts with their primary type in unions" >> prop {
      (st0: SimpleType, sjs: ISet[LeafEjs]) =>
        ...
        val (matching, nonmatching) =
          ssts.partition(s => sstConst(s).exists(j => simpleTypeOf(j) exists (_ ≟ st)))
        val simplified =
          matching.map(x => envTType.set(TypeST(TypeF.simple(st)))(x.project).embed)
        val coalesced = NonEmptyList.nel(simpleSst, simplified).suml1

        val compressed =
          NonEmptyList.nel(simpleSst, ssts).suml1.transCata[S](compression.coalescePrimary)
    ...
    "no effect when a const's primary type not in the union" >> prop {
      ljs: NonEmptyList[LeafEjs] =>
  ...
  "coalesceWithUnknown" >> {
    def testUnk(
        kind: String,
        f: Char => J,
        g: (TypeStat[Real], SimpleType) => SSTF[J, Real, S]) =
      s"merges known map entry with unknown entry when same primary $kind appears in unknown" >> prop {
        (head: (Char, LeafEjs), xs0: NonEmptyList[(Char, LeafEjs)], kv: (BigInt, LeafEjs)) =>
          ...
          val u2 = head.bimap(
            c =>
              g(TypeStat.fromEJson(Real(1), E(ejson.char[J](c)).embed), SimpleType.Char).embed,
            _.toSST)
    ...
    def testUnkUnion(
      ...
      s"merges known map entry with unknown when primary $kind appears in unknown union" >> prop {
        (head: (Char, LeafEjs), xs0: NonEmptyList[(Char, LeafEjs)], kv: (BigInt, LeafEjs)) =>
          ...
          val u1u =
            u1.leftMap(s => envT(cnt1, TypeST(TypeF.union[J, S](tp, s, IList(st)))).embed)
          val u2u =
            u2.leftMap(s => envT(cnt1, TypeST(TypeF.union[J, S](s, st, IList(tp)))).embed)
          val kv1 = kv.bimap(i => E(ejson.int[J](i)).embed, _.toSST)
          val cs = xs.toList.map(_.bimap(f, _.toSST))
          val m = IMap.fromFoldable(kv1 :: cs)
    ...
    test(
      "tag",
      c =>
        E(ejson.Meta(E(ejson.char[J](c)).embed, ejson.Type[J](ejson.TypeTag("codepoint")))).embed,
      (ts, st) =>
        envT(
          TypeStat.count(ts.size),
          TagST[J](
            Tagged(ejson.TypeTag("codepoint"), envT(ts, TypeST(TypeF.simple[J, S](st))).embed)))
    )
    ...
    "has no effect on maps when primary type not in unknown" >> prop {
      xs: IList[(LeafEjs, LeafEjs)] =>
        val m = IMap.fromFoldable(xs.map(_.bimap(_.ejs, _.toSST)))
    ...
    "has no effect on maps when primary tag not in unknown" >> prop {
      xs: IList[(LeafEjs, LeafEjs)] =>
        val foo = ejson.TypeTag("foo")
        val bar = ejson.TypeTag("bar")
        val m = IMap.fromFoldable(
          xs.map(_.bimap(l => E(ejson.Meta(l.ejs, ejson.Type[J](foo))).embed, _.toSST)))
        val T =
          envT(cnt1, TagST[J](Tagged(bar, envT(cnt1, TypeST(TypeF.top[J, S]())).embed))).embed
        val sst = envT(cnt1, TypeST(TypeF.map[J, S](m, Some((T, T))))).embed
  ...
  "limitStrings" >> {
    "compresses strings longer than maxLen" >> prop { s: String =>
      (s.length > 1) ==> {
  ...
  "narrowUnion" >> {
    "reduces the largest group of values having the same primary type" >> prop {
      (bs: ISet[Byte], c1: Char, c2: Char, c3: Char, d1: BigDecimal) =>
        ((bs.size > 3) && (ISet.fromFoldable(IList(c1, c2, c3)).size ≟ 3)) ==> {
          ...
          val chars =
            IList(c1, c2, c3).map(c => SST.fromEJson(Real(1), E(ejson.char[J](c)).embed))
          ...
          val union1 =
            envT(union0.copoint, TypeST(TypeF.union[J, S](compByte, dec, chars))).embed
    ...
    "no effect on unions smaller or equal to maxSize" >> prop {
      (x: LeafEjs, y: LeafEjs, xs: IList[LeafEjs]) =>
        val union =
          envT(cnt1, TypeST(TypeF.union[J, S](x.toSST, y.toSST, xs map (_.toSST)))).embed
