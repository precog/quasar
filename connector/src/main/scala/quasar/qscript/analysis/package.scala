...
package object analysis {
  ...
  object Cardinality {
    ...
    implicit def read[A]: Cardinality[Const[Read[A], ?]] =
      new Cardinality[Const[Read[A], ?]] {
        def calculate[M[_]: Monad](
            pathCard: APath => M[Int]): AlgebraM[M, Const[Read[A], ?], Int] =
    ...
    implicit def shiftedRead[A <: APath]: Cardinality[Const[ShiftedRead[A], ?]] =
      new Cardinality[Const[ShiftedRead[A], ?]] {
        def calculate[M[_]: Monad](
            pathCard: APath => M[Int]): AlgebraM[M, Const[ShiftedRead[A], ?], Int] =
    ...
    implicit def qscriptCore[T[_[_]]: RecursiveT: ShowT]: Cardinality[QScriptCore[T, ?]] =
      new Cardinality[QScriptCore[T, ?]] {
        ...
        def calculate[M[_]: Monad](
            pathCard: APath => M[Int]): AlgebraM[M, QScriptCore[T, ?], Int] = {
          ...
          case Union(card, lBranch, rBranch) => {
            val compile = Cardinality[QScriptTotal[T, ?]].calculate(pathCard)
            (lBranch.cataM(interpretM(κ(card.point[M]), compile)) |@| rBranch.cataM(
              interpretM(κ(card.point[M]), compile))) { _ + _ }
          }
          case Unreferenced() => 1.point[M]
        }
      }
    implicit def projectBucket[T[_[_]]: RecursiveT: ShowT]
      : Cardinality[ProjectBucket[T, ?]] =
      new Cardinality[ProjectBucket[T, ?]] {
        def calculate[M[_]: Monad](
            pathCard: APath => M[Int]): AlgebraM[M, ProjectBucket[T, ?], Int] = {
    ...
    implicit def equiJoin[T[_[_]]: RecursiveT: ShowT]: Cardinality[EquiJoin[T, ?]] =
      new Cardinality[EquiJoin[T, ?]] {
        @SuppressWarnings(Array("org.wartremover.warts.Recursion"))
        def calculate[M[_]: Monad](
            pathCard: APath => M[Int]): AlgebraM[M, EquiJoin[T, ?], Int] = {
    ...
    implicit def thetaJoin[T[_[_]]: RecursiveT: ShowT]: Cardinality[ThetaJoin[T, ?]] =
      new Cardinality[ThetaJoin[T, ?]] {
        @SuppressWarnings(Array("org.wartremover.warts.Recursion"))
        def calculate[M[_]: Monad](
            pathCard: APath => M[Int]): AlgebraM[M, ThetaJoin[T, ?], Int] = {
    ...
    implicit def deadEnd: Cardinality[Const[DeadEnd, ?]] =
      new Cardinality[Const[DeadEnd, ?]] {
        def calculate[M[_]: Monad](
            pathCard: APath => M[Int]): AlgebraM[M, Const[DeadEnd, ?], Int] =
          κ(1.point[M])
      }

    implicit def coproduct[F[_], G[_]](
        implicit F: Cardinality[F],
        G: Cardinality[G]): Cardinality[Coproduct[F, G, ?]] =
      new Cardinality[Coproduct[F, G, ?]] {
        def calculate[M[_]: Monad](
            pathCard: APath => M[Int]): AlgebraM[M, Coproduct[F, G, ?], Int] =
  ...
  object Cost {
    implicit def deadEnd: Cost[Const[DeadEnd, ?]] = new Cost[Const[DeadEnd, ?]] {
      def evaluate[M[_]: Monad](
          pathCard: APath => M[Int]): GAlgebraM[(Int, ?), M, Const[DeadEnd, ?], Int] =
    ...
    implicit def read[A]: Cost[Const[Read[A], ?]] =
      new Cost[Const[Read[A], ?]] {
        def evaluate[M[_]: Monad](
            pathCard: APath => M[Int]): GAlgebraM[(Int, ?), M, Const[Read[A], ?], Int] =
          (qs: Const[Read[A], (Int, Int)]) => 1.point[M]
      }
    implicit def shiftedRead[A]: Cost[Const[ShiftedRead[A], ?]] =
      new Cost[Const[ShiftedRead[A], ?]] {
        def evaluate[M[_]: Monad](pathCard: APath => M[Int])
          : GAlgebraM[(Int, ?), M, Const[ShiftedRead[A], ?], Int] =
    ...
    implicit def qscriptCore[T[_[_]]: RecursiveT: ShowT]: Cost[QScriptCore[T, ?]] =
      new Cost[QScriptCore[T, ?]] {
        @SuppressWarnings(Array("org.wartremover.warts.Recursion"))
        def evaluate[M[_]: Monad](
            pathCard: APath => M[Int]): GAlgebraM[(Int, ?), M, QScriptCore[T, ?], Int] = {
          ...
          case Union((card, cost), lBranch, rBranch) => {
            ...
            val left = lBranch.zygoM(interpretM(κ(card.point[M]), compileCardinality),
                                     ginterpretM(κ(cost.point[M]), compileCost))
            val right = lBranch.zygoM(interpretM(κ(card.point[M]), compileCardinality),
                                      ginterpretM(κ(cost.point[M]), compileCost))
    ...
    implicit def projectBucket[T[_[_]]]: Cost[ProjectBucket[T, ?]] =
      new Cost[ProjectBucket[T, ?]] {
        def evaluate[M[_]: Monad](pathCard: APath => M[Int])
          : GAlgebraM[(Int, ?), M, ProjectBucket[T, ?], Int] = {
    ...
    implicit def equiJoin[T[_[_]]: RecursiveT: ShowT]: Cost[EquiJoin[T, ?]] =
      new Cost[EquiJoin[T, ?]] {
        @SuppressWarnings(Array("org.wartremover.warts.Recursion"))
        def evaluate[M[_]: Monad](
            pathCard: APath => M[Int]): GAlgebraM[(Int, ?), M, EquiJoin[T, ?], Int] = {
          case EquiJoin((card, cost), lBranch, rBranch, lKey, rKey, jt, combine) =>
            val compileCardinality = Cardinality[QScriptTotal[T, ?]].calculate(pathCard)
            val compileCost        = Cost[QScriptTotal[T, ?]].evaluate(pathCard)
            (lBranch.zygoM(interpretM(κ(card.point[M]), compileCardinality),
                           ginterpretM(κ(cost.point[M]), compileCost)) |@|
              rBranch.zygoM(interpretM(κ(card.point[M]), compileCardinality),
                            ginterpretM(κ(cost.point[M]), compileCost))) { _ + _ }
        }
      }
    implicit def thetaJoin[T[_[_]]: RecursiveT: ShowT]: Cost[ThetaJoin[T, ?]] =
      new Cost[ThetaJoin[T, ?]] {
        @SuppressWarnings(Array("org.wartremover.warts.Recursion"))
        def evaluate[M[_]: Monad](
            pathCard: APath => M[Int]): GAlgebraM[(Int, ?), M, ThetaJoin[T, ?], Int] = {
    ...
    implicit def coproduct[F[_], G[_]](implicit F: Cost[F],
                                       G: Cost[G]): Cost[Coproduct[F, G, ?]] =
      new Cost[Coproduct[F, G, ?]] {
        def evaluate[M[_]: Monad](
            pathCard: APath => M[Int]): GAlgebraM[(Int, ?), M, Coproduct[F, G, ?], Int] =
