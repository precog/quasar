...
final case class MetaStore private (
    connectionInfo: DbConnectionConfig,
    trans: StatefulTransactor,
    schemas: List[Schema[Int]]) {
...
object MetaStore {
  ...
  def connect(
      ...
      initializeOrUpdate: Boolean,
      schemas: List[Schema[Int]]): EitherT[Task, MetastoreFailure, MetaStore] = {
    for {
      tx <- poolingTransactor(DbConnectionConfig.connectionInfo(dbConfig), DefaultConfig)
        .leftMap(f => f: MetastoreFailure)
      _ <- onFailOrLeft(
        initializeOrUpdate.whenM(
          schemas.traverse(this.initializeOrUpdate(_, tx.transactor, None))) >>
          schemas.traverse(verifySchema(_, tx.transactor)) >>
          stdout(s"Using metastore: ${DbConnectionConfig.connectionInfo(dbConfig).url}")
            .liftM[EitherT[?[_], MetastoreFailure, ?]])(tx.shutdown)
  ...
  def initializeOrUpdate[A](
    ...
    val op: ConnectionIO[Option[Json]] =
      ...
      } yield ver.isEmpty.fold(jCfg ∘ (_.withObject(_ - mntsFieldName)), jCfg)

    EitherT(transactor.trans.apply(op).attempt ∘ (_.leftMap(t =>
      UnknownError(t, "While initializing or updating"))))
  }

  def verifySchema[A: Show](
      schema: Schema[A],
      transactor: Transactor[Task]): EitherT[Task, MetastoreFailure, Unit] = {
    EitherT(
      verifyMetaStoreSchema(schema).run
        .transact(transactor)
        .attempt
        .map(_.valueOr(t => UnknownError(t, "while verifying schema").left)))
  }
