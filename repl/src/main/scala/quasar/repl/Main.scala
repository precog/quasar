...
object Main {
  ...
  private def driver(f: Command => Free[DriverEff, Unit]): Task[Unit] = {
    ...
    runConsole.flatMap(c =>
      Task.delay(c.getShell.out.println("Exiting...")).onFinish(_ => Task.delay(c.stop)))
  }
  ...
  def repl[S[_]](
    ...
    TaskRef(Repl.RunState(rootDir, DebugLevel.Normal, 10, OutputFormat.Table, Map())).map {
      ref =>
