...
class RelationsSpec extends Specification with ScalaCheck with TypeArbitrary with ValidationMatchers with PendingWithAccurateCoverage {
  ...
  "RelationsLib" should {
    ...
    "fold integer eq" in {
      val expr = Eq.tpe(Sized[IS](Const(Int(1)), Const(Int(1))))
      expr should beSuccessful(Const(Bool(true)))
    }

    "fold eq with mixed numeric type" in {
      val expr = Eq.tpe(Sized[IS](Const(Int(1)), Const(Dec(1.0))))
      expr should beSuccessful(Const(Bool(true)))
    }

    "fold eq with mixed type" in {
      val expr = Eq.tpe(Sized[IS](Const(Int(1)), Const(Str("a"))))
      expr should beSuccessful(Const(Bool(false)))
    }

    "type Eq with Top" ! prop { (t : Type) =>
      Eq.tpe(Sized[IS](Type.Top, t)) should beSuccessful(Type.Bool)
      Eq.tpe(Sized[IS](t, Type.Top)) should beSuccessful(Type.Bool)
    }

    "type Neq with Top" ! prop { (t : Type) =>
      Neq.tpe(Sized[IS](Type.Top, t)) should beSuccessful(Type.Bool)
      Neq.tpe(Sized[IS](t, Type.Top)) should beSuccessful(Type.Bool)
    }

    "fold neq with mixed type" in {
      val expr = Neq.tpe(Sized[IS](Const(Int(1)), Const(Str("a"))))
      expr should beSuccessful(Const(Bool(true)))
    }
    ...
    "fold coalesce with left null type" ! prop { (t2 : Type) =>
      val expr = Coalesce.tpe(Sized[IS](Type.Null, t2))
      expr must beSuccessful(t2)
    }

    "fold coalesce with right null value" ! prop { (t1 : Type) =>
      val expr = Coalesce.tpe(Sized[IS](t1, Const(Null)))
      expr must beSuccessful(t1 match {
    ...
    "fold coalesce with left null value" ! prop { (t2 : Type) =>
      val expr = Coalesce.tpe(Sized[IS](Const(Null), t2))
      expr must beSuccessful(t2)
    }

    "fold coalesce with left value" ! prop { (t2 : Type) =>
      val expr = Coalesce.tpe(Sized[IS](Const(Int(3)), t2))
      expr must beSuccessful(Const(Int(3)))
    }

    "find lub for coalesce with int" in {
      val expr = Coalesce.tpe(Sized[IS](Type.Int, Type.Int))
      expr must beSuccessful(Type.Int)
    }

    "find lub for coalesce with arbitrary args" ! prop { (t1: Type, t2: Type) =>
      val expr = Coalesce.tpe(Sized[IS](t1, t2))
    ...
    "flip comparison ops" !
      Prop.forAll(comparisonOps, arbitrary[BigInt], arbitrary[BigInt]) {
        case (func, left, right) =>
          flip(func).map(
            _.tpe(Sized[IS](Type.Const(Int(right)), Type.Const(Int(left))))) must
            beSome(func.tpe(Sized[IS](Type.Const(Int(left)), Type.Const(Int(right)))))
    }
    ...
    "flip boolean ops" !
      Prop.forAll(Gen.oneOf(And, Or), arbitrary[Boolean], arbitrary[Boolean]) {
        case (func, left, right) =>
          flip(func).map(
            _.tpe(Sized[IS](Type.Const(Bool(right)), Type.Const(Bool(left))))) must
            beSome(func.tpe(Sized[IS](Type.Const(Bool(left)), Type.Const(Bool(right)))))
    }
      ...
      Prop.forAll(comparisonOps, arbitrary[BigInt], arbitrary[BigInt]) {
        case (func, left, right) =>
          RelationsLib.negate(func).map(
            _.tpe(Sized[IS](Type.Const(Int(left)), Type.Const(Int(right))))) must
          beSome(func.tpe(Sized[IS](Type.Const(Int(left)), Type.Const(Int(right)))).flatMap(x => Not.tpe(Sized[IS](x))))
    }
