      ...
      (data != Data.NA && !data.isInstanceOf[Data.Set]) ==> {
        data match {
          ...
          case Data.Dec(x) if v === BsonVersion.`1.1` =>
            (bson.toJs must_== jscore
              .Call(jscore.ident("NumberDecimal"), List(jscore.Literal(Js.Str(x.shows))))
              .toJs) or
...
object BsonGen {
  ...
  implicit def arbBson(v: BsonVersion): Arbitrary[Bson] =
    Arbitrary(Gen.oneOf(simpleGen(v), resize(5, objGen(v)), resize(5, arrGen(v))))
