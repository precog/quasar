...
object Build {
  ...
  val defaultArgSet = Seq("-Ypartial-unification")
  ...
  implicit class ProjectOps(val p: sbt.Project) {
    ...
    def scalacPlugins(ms: ModuleID*): Project =
      also(ms.toList map (m => addCompilerPlugin(m)))

    def setup: Project =
      (serialTests scalacPlugins (kindProjector) scalacArgs (defaultArgSet: _*) also (organization := "org.quasar-analytics",
      scalaVersion := "2.11.8",
