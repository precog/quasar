...
package object quasar {
  ...
  def precompile[T: Equal: RenderTree](query: Fix[Sql], vars: Variables, basePath: ADir)(
    ...
    for {
      ...
      normed <- phase("Normalized Projections",
                      normalizeProjections[Fix[Sql]](absAst).right)
