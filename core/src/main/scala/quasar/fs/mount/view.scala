...
object view {
  ...
  def readFile[S[_]](
    ...
    λ[ReadFile ~> Free[S, ?]] {
      ...
      case Close(handle) =>
        (viewState.get(handle) <* viewState.delete(handle).liftM[OptionT])
          .flatMapF {
  ...
  def writeFile[S[_]](
    ...
    nonFsMounts.failSomeWrites(
      on = file =>
        mount.lookupType(file).run.map(_.filter(_ === MountType.ViewMount).isDefined),
  ...
  def analyze[S[_]](implicit
                    M: Mounting.Ops[S],
                    A: Analyze.Ops[S]): Analyze ~> Free[S, ?] =
    new (Analyze ~> Free[S, ?]) {
  ...
  def resolveViewRefs[S[_]](plan: Fix[LP])(
      implicit M: Mounting.Ops[S]): FileSystemErrT[Free[S, ?], Fix[LP]] = {
    ...
    def compiledView(
      ...
      for {
        ...
        block.map(
          _.leftMap(_.wrapNel)
            .flatMap(precompile[Fix[LP]](_, viewConfig.vars, fileParent(loc)).run.value))
      ...
      (Set[FPath](), cleaned).anaM[Fix[LP]] {
        case (e, i @ Embed(lp.Read(p))) if !(e contains p) =>
          refineTypeAbs(p).swap
            .map(f =>
              EitherT(EitherT(compiledView(f) getOrElse i.right.right))
