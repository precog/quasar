...
object managefile {
    ...
    new (ManageFile ~> Free[S, ?]) {
      def apply[A](mf: ManageFile[A]): Free[S, A] = mf match {
        case Move(FileToFile(sf, df), semantics) =>
          (ensureMoveSemantics[Free[S, ?]](sf, df, pathExists(fileSystem) _, semantics)
            .toLeft(()) *>
            moveFile(sf, df, fileSystem).liftM[FileSystemErrT]).run
        case Move(DirToDir(sd, dd), semantics) =>
          (ensureMoveSemantics[Free[S, ?]](sd, dd, pathExists(fileSystem) _, semantics)
            .toLeft(()) *>
  ...
  private def moveFile[S[_]](src: AFile, dst: AFile, fileSystem: Task[FileSystem])(
      implicit
      s0: Task :<: S,
  ...
  private def moveDir[S[_]](src: ADir, dst: ADir, fileSystem: Task[FileSystem])(
      implicit
      s0: Task :<: S,
      s1: PhysErr :<: S): Free[S, Unit] = {
    val move: Task[PhysicalError \/ Unit] = (for {
  ...
  private def delete[S[_]](apath: APath, fileSystem: Task[FileSystem])(
      implicit
      s0: Task :<: S,
  ...
  private def tempFile[S[_]](near: APath)(
      implicit
      s0: Task :<: S): Free[S, FileSystemError \/ AFile] =
    lift(Task.delay {
