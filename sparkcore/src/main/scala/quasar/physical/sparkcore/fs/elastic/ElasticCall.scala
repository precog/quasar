...
final case class IndexInto(indexType: IndexType, data: List[(String, String)])
    extends ElasticCall[Unit]

object ElasticCall {

  class Ops[S[_]](implicit S: ElasticCall :<: S) {
    def createIndex(index: String): Free[S, Unit] = lift(CreateIndex(index)).into[S]
    def copyType(src: IndexType, dst: IndexType): Free[S, Unit] =
      lift(CopyType(src, dst)).into[S]
    ...
    def indexInto(indexType: IndexType, data: List[(String, String)]): Free[S, Unit] =
      lift(IndexInto(indexType, data)).into[S]
  ...
  implicit def interpreter(host: String, port: Int): ElasticCall ~> Task =
    new (ElasticCall ~> Task) {
      ...
      def _createIndex(index: String)(implicit client: HttpClient): Task[Unit] =
        for {
          indices <- _listIndices
          _ <- if (indices.contains(index)) ().point[Task]
          else client.execute { createIndex(index) }.toTask
        } yield ()

      def _deleteIndex(index: String)(implicit client: HttpClient): Task[Unit] =
        for {
          indices <- _listIndices
          _ <- if (!indices.contains(index)) ().point[Task]
          else client.execute { deleteIndex(index) }.toTask
        } yield ()

      def _indexInto(indexType: IndexType, data: List[(String, String)])(
          implicit
          client: HttpClient): Task[Unit] =
        client.execute { indexInto(indexType.index / indexType.typ) fields (data: _*) }.toTask
          .as(())
      ...
      def _listTypes(index: String)(implicit client: HttpClient): Task[List[String]] =
        for {
          indices <- _listIndices
          types <- if (!indices.contains(index)) List.empty.point[Task]
          else
            client.execute { getMapping(index) }.toTask
              .map(_.toList.flatMap(_.mappings.keys.toList))
        } yield types

      def _deleteType(index: String, typ: String)(implicit client: HttpClient): Task[Unit] =
        for {
          ...
          _ <- types
            .filter(_ =/= typ)
            .map(t => _copyType(IndexType(temp, t), IndexType(index, t)))
            .sequence
        } yield ()

      def _copyType(src: IndexType, dst: IndexType): Task[Unit] =
        for {
          httpClient <- Task.delay(PooledHttp1Client())
          result <- httpClient.expect[String](
            Request(
              method = POST,
              uri = Uri.unsafeFromString(s"http://${host}:${port}/_reindex?refresh=true"))
              .withBody(s"""{
      ...
      def _copyIndex(src: String, dst: String): Task[Unit] =
        for {
          httpClient <- Task.delay(PooledHttp1Client())
          result <- httpClient.expect[String](
            Request(
      ...
      def run[A](t: HttpClient => Task[A]): Task[A] =
        for {
      ...
      def apply[A](from: ElasticCall[A]) = from match {
        ...
        case IndexInto(indexType: IndexType, data: List[(String, String)]) =>
          run(c => _indexInto(indexType, data)(c))
