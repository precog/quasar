...
object xdmitem {
  ...
  def toData[F[_]: MonadErrMsgs](xdm: XdmItem): F[Data] = xdm match {
    ...
    case item: XdmComment =>
      Data.singletonObj("xdm:comment", Data._str(item.asString)).point[F]
    ...
    case item: XdmProcessingInstruction =>
      Data.singletonObj("xdm:processingInstruction", Data._str(item.asString)).point[F]
    ...
    case item: XSDateTime =>
      Data._timestamp(ZonedDateTime.parse(item.asString).toInstant).point[F]
    ...
    case item: XSGMonth =>
      Data.singletonObj("xs:gMonth", Data._str(item.asString)).point[F]
    case item: XSGMonthDay =>
      Data.singletonObj("xs:gMonthDay", Data._str(item.asString)).point[F]
    case item: XSGYear => Data.singletonObj("xs:gYear", Data._str(item.asString)).point[F]
    case item: XSGYearMonth =>
      Data.singletonObj("xs:gYearMonth", Data._str(item.asString)).point[F]
  ...
  private def parseTemporal[F[_]: MonadErrMsgs, A](
      name: String,
      parse: String => Option[A])(str: String): F[A] =
    parse(str).fold(s"Failed to parse '$str' as a $name.".wrapNel.raiseError[F, A])(
      _.point[F])

  private def xmlToData[F[_]: MonadErrMsgs](xmlString: String): F[Data] = {
    val elem = SecureXML
      .loadString(xmlString)
      .fold(_.toString.wrapNel.raiseError[F, Elem], _.point[F])
