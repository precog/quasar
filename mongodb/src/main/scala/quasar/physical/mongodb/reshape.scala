...
object Grouped {
  ...
  implicit def GroupedRenderTree[EX[_]: Functor](
      implicit ev: ExprOpOps.Uni[EX]): RenderTree[Grouped[EX]] = new RenderTree[Grouped[EX]] {
  ...
  def find(expr: Fix[EX]): Option[BsonField] =
    value.foldLeft[Option[BsonField]](None) {
      case (acc, (field, value)) =>
        acc.orElse(
          value.fold(_.find(expr).map(field \ _), ex => if (ex == expr) field.some else None))
    }
