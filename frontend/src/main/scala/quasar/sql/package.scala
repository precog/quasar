...
package object sql {
  ...
  def invokeFunction[A] =
    Prism.partial[Sql[A], (CIName, List[A])] { case InvokeFunction(name, args) => (name, args) }(
  ...
  def projectionName[T](
      ...
      implicit
      T: Recursive.Aux[T, Sql]): Option[String] = {
            ...
            alias.cata(
              a => (used, a -> expr).right, {
                ...
                (tentativeName #:: alternatives).dropWhile(used.contains).headOption.map {
                  ...
                  .GenericError("Could not generate alias for a relation") // unlikely since we know it's an quasi-infinite stream
              }
            )
        }
        .map(_._2)
                ...
                .flatMap(_.traverse {
                  case (funcs, imports, from) =>
                    ...
                    funcs.filter(matchesSignature).traverse { decl =>
                      ...
                      decl
                        .traverse(_.mkPathsAbsolute(from).inlineInvokes(currentScope))
                        .flattenLeft
                        .strengthR(from)
                    }
                })
      ...
      case Select(isDistinct, projections, relations, filter, groupBy, orderBy) =>
        "(" +
          List(
            ...
            orderBy.map(o =>
              List("order by", o.keys.map(x => x._2._2 + " " + x._1.shows) intercalate (", "))
                .mkString(" "))
