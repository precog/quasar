    ...
    new Traverse[ListMap[K, ?]] with IsEmpty[ListMap[K, ?]] {
      ...
      def traverseImpl[G[_], A, B](m: ListMap[K, A])(f: A => G[B])(
        ...
        scalaz.std.list.listInstance
          .traverseImpl(m.toList)({ case (k, v) => f(v) map (k -> _) }) map (_.toListMap)
      }
    }
...
package object fp
  ...
  implicit def constEqual[A: Equal]: Delay[Equal, Const[A, ?]] =
    Delay.fromNT(λ[Equal ~> DelayedA[A]#Equal](_ => Equal equal (_.getConst ≟ _.getConst)))
