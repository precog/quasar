...
class PlannerSpec
  ...
  def queryPlanner(expr: Fix[Sql],
    ...
    queryPlan(expr, Variables.empty, basePath, 0L, None)
      ...
      .flatMap(_.fold(
        ...
        MongoDbPlanner
          .plan(_, fs.QueryContext(model, stats, indexes, listContents))
          .leftMap(CPlannerError(_))
      ))

  def plan0(query: Fix[Sql],
            model: MongoQueryModel,
            stats: Collection => Option[CollectionStatistics],
            indexes: Collection => Option[Set[Index]])
  ...
  def planLP(logical: Fix[LP]): Either[PlannerError, Crystallized[WorkflowF]] = {
    (for {
      ...
      simplified <- emit(Vector.empty, \/-(optimizer.simplify(logical))): EitherWriter[
        PlannerError,
        Fix[LP]]
  ...
  "plan from query string" should {
    ...
    "plan filter with ~" in {
      plan(sqlE"""select * from zips where city ~ "^B[AEIOU]+LD.*" """) must beWorkflow(
        chain[Workflow](
          $read(collection("db", "zips")),
          $match(Selector.And(
    ...
    "plan filter with alternative ~" in {
      plan(sqlE"""select * from a where "foo" ~ pattern or target ~ pattern""") must beWorkflow(
        chain[Workflow](
    ...
    "plan complex filter" in {
      plan(
        sqlE"""select * from foo where bar > 10 and (baz = "quux" or foop = "zebra")""") must
        beWorkflow(
          chain[Workflow](
            $read(collection("db", "foo")),
            $match(Selector.And(
    ...
    "plan filter with both index and field projections" in {
      plan(
        sqlE"""select count(parents[0].sha) as count from slamengine_commits where parents[0].sha = "56d1caf5d082d1a6840090986e277d36d03f1859" """) must
        beWorkflow(
          chain[Workflow](
    ...
    "plan select with multiple wildcards and fields" in {
      plan(
        sqlE"select state as state2, *, city as city2, *, pop as pop2 from zips where pop < 1000") must
        beWorkflow(
          chain[Workflow](
            $read(collection("db", "zips")),
            $match(
    ...
    "plan useful group by" in {
      plan(
        sqlE"""select city || ", " || state, sum(pop) from zips group by city, state""") must
        beWorkflow(
          chain[Workflow](
            $read(collection("db", "zips")),
            $group(
    ...
    "plan group by month" in {
      plan(
        sqlE"""select avg(score) as a, DATE_PART("month", `date`) as m from caloriesBurnedData group by DATE_PART("month", `date`)""") must
        beWorkflow(
          chain[Workflow](
    ...
    "plan expr3 with grouping" in {
      plan(
        sqlE"select case when pop > 1000 then city else lower(city) end, count(*) from zips group by city") must
        beRight
    }
    ...
    "plan js expr grouped by js expr" in {
      plan(
        sqlE"select length(city) as len, count(*) as cnt from zips group by length(city)") must
        beWorkflow(
          chain[Workflow](
            $read(collection("db", "zips")),
            $simpleMap(
    ...
    "plan expressions with ~" in {
      plan(
        sqlE"""select foo ~ "bar.*", "abc" ~ "a|b", "baz" ~ regex, target ~ regex from a""") must beWorkflow(
        chain[Workflow](
          $read(collection("db", "a")),
          $simpleMap(
            NonEmptyList(MapExpr(JsFn(
    ...
    "group by flattened field" in {
      plan(
        sqlE"select substring(parents[*].sha, 0, 1), count(*) from slamengine_commits group by substring(parents[*].sha, 0, 1)") must
        beWorkflow(
          chain[Workflow](
            $read(collection("db", "slamengine_commits")),
            $project(
    ...
    "unify flattened with double-flattened" in {
      plan(
        sqlE"""select * from user_comments where (comments[*].id LIKE "%Dr%" OR comments[*].replyTo[*] LIKE "%Dr%")""") must
        beWorkflow(
          chain[Workflow](
    ...
    "plan distinct with sum and group" in {
      plan(
        sqlE"SELECT DISTINCT SUM(pop) AS totalPop, city, state FROM zips GROUP BY city") must
        beWorkflow(
          chain[Workflow](
            $read(collection("db", "zips")),
            $group(
    ...
    "plan distinct with sum, group, and orderBy" in {
      plan(
        sqlE"SELECT DISTINCT SUM(pop) AS totalPop, city, state FROM zips GROUP BY city ORDER BY totalPop DESC") must
        beWorkflow(
          chain[Workflow](
            $read(collection("db", "zips")),
            $group(
    ...
    "plan filter with timestamp and interval" in {
      ...
      plan(
        sqlE"""select * from days where date < timestamp("2014-11-17T22:00:00Z") and date - interval("PT12H") > timestamp("2014-11-17T00:00:00Z")""") must
        beWorkflow(
          chain[Workflow](
    ...
    "plan filter on date" in {
      ...
      plan(
        sqlE"""select * from logs where ((ts > date("2015-01-22") and ts <= date("2015-01-27")) and ts != date("2015-01-25")) or ts = date("2015-01-29")""") must
        beWorkflow(
          chain[Workflow](
    ...
    "plan js and filter with id" in {
      Bson.ObjectId
        ...
        .fold[Result](failure("Couldn’t create ObjectId."))(oid =>
          plan(sqlE"""select length(city), foo = oid("0123456789abcdef01234567") from days where `_id` = oid("0123456789abcdef01234567")""") must
    ...
    "plan simple join with sources in different DBs" in {
      // NB: cannot use $lookup, so fall back to the old approach
      val query =
        sqlE"select zips2.city from `/db1/zips` join `/db2/zips2` on zips.`_id` = zips2.`_id`"
        ...
        beWorkflow(
          joinStructure(
            ...
            chain[Workflow](
              ...
              $match(
                Selector.Doc(BsonField.Name("__tmp4") -> Selector.Eq(Bson.Bool(true)))),
              $project(reshape("value"                -> $field("__tmp3", "city")), ExcludeId)
            ),
            false
          ).op)
    }
    ...
    "plan simple inner equi-join (map-reduce)" in {
      plan2_6(sqlE"select foo.name, bar.address from foo join bar on foo.id = bar.foo_id") must
        beWorkflow(
          joinStructure(
            $read(collection("db", "foo")),
            "__tmp0",
    ...
    "plan simple inner equi-join ($lookup)" in {
      plan3_2(sqlE"select foo.name, bar.address from foo join bar on foo.id = bar.foo_id",
              defaultStats,
      ...
      plan3_2(
        ...
        beWorkflow(
          chain[Workflow](
            ...
            $match(
              Selector.Doc(BsonField.Name("TestProgramId") -> Selector.Exists(true))),
            $project(reshape("right"                       -> $$ROOT), IgnoreId),
            $lookup(CollectionName("zips"),
            ...
            $project(reshape("_id" -> $field("_id", "0"), "1" -> $include), IgnoreId)
          ))
    }

    "plan join with multiple conditions" in {
      plan(
        sqlE"select l.sha as child, l.author.login as c_auth, r.sha as parent, r.author.login as p_auth from slamengine_commits as l join slamengine_commits as r on r.sha = l.parents[0].sha and l.author.login = r.author.login") must
        beWorkflow(
          joinStructure(
            chain[Workflow](
              $read(collection("db", "slamengine_commits")),
    ...
    "plan join with non-JS-able condition" in {
      plan(
        sqlE"select z1.city as city1, z1.loc, z2.city as city2, z2.pop from zips as z1 join zips as z2 on z1.loc[*] = z2.loc[*]") must
        beWorkflow(
          joinStructure(
            chain[Workflow](
              $read(collection("db", "zips")),
    ...
    "SD-1263 specific case of plan multiple reducing projections (all, distinct, orderBy)" in {
      val q =
        sqlE"select distinct loc || [pop - 1] as p1, pop - 1 as p2 from zips group by territory order by p2".project
          .asInstanceOf[Select[Fix[Sql]]]
  ...
  val groupBySeveral = Gen
    .nonEmptyListOf(Gen.oneOf(sqlF.IdentR("state"), sqlF.IdentR("territory")))
  ...
  val filter = Gen.oneOf(
    ...
    for {
      ...
      sqlF.InvokeFunctionR(
        CIName("search"),
        List(x, sqlF.StringLiteralR("^BOULDER"), sqlF.BoolLiteralR(false))),
    Gen.const(sqlF.BinopR(sqlF.IdentR("p"), sqlF.IdentR("q"), sql.Eq))
  ...
  def select(distinctGen: Gen[IsDistinct],
             ...
             orderByGen: Gen[Option[OrderBy[Fix[Sql]]]]): Gen[Select[Fix[Sql]]] =
    for {
  ...
  def genInnerInt =
    Gen.oneOf(
      ...
      sqlF.InvokeFunctionR(CIName("length"), List(sqlF.IdentR("city")))
    ) // requires JS
  def genReduceInt =
    genInnerInt.flatMap(
      x =>
  ...
  def genOuterInt =
    Gen.oneOf(
      ...
      genInnerInt.flatMap(x =>
        sqlF.BinopR(sqlF.IdentR("loc"), sqlF.ArrayLiteralR(List(x)), Concat))
    )

  def genInnerStr =
    Gen.oneOf(sqlF.IdentR("city"),
  ...
  def genOuterStr =
    Gen.oneOf(
      ...
      genReduceStr,
      genReduceStr
        .flatMap(x => sqlF.InvokeFunctionR(CIName("lower"), List(x))), // an ExprOp
      genReduceStr.flatMap(x => sqlF.InvokeFunctionR(CIName("length"), List(x)))
    ) // requires JS
  ...
  implicit def shrinkExpr: Shrink[Fix[Sql]] = {
    ...
    Shrink {
      case Embed(Select(d, projs, rel, filter, groupBy, orderBy)) =>
        val sDistinct =
          if (d == SelectDistinct)
            Stream(sqlF.SelectR(SelectAll, projs, rel, filter, groupBy, orderBy))
          else Stream.empty
        val sProjs =
          shortened(projs).map(ps => sqlF.SelectR(d, ps, rel, filter, groupBy, orderBy))
        val sGroupBy = groupBy
          .map {
            case GroupBy(keys, having) =>
              shortened(keys).map(ks =>
                sqlF.SelectR(d, projs, rel, filter, Some(GroupBy(ks, having)), orderBy))
          }
    ...
    "include all phases when successful" in {
      planLog(sqlE"select city from zips").map(_.name) must_===
        Vector(
          ...
          "Logical Plan (remove typecheck filters)",
