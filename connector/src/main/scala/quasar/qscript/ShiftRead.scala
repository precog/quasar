...
object ShiftRead {
  ...
  implicit def read[T[_[_]]: BirecursiveT, F[_], A](
      ...
      QC: QScriptCore[T, ?] :<: F): ShiftRead.Aux[T, Const[Read[A], ?], F] =
    new ShiftRead[Const[Read[A], ?]] {
      type G[A] = F[A]
      def shiftRead[H[_]](GtoH: G ~> H) =
        λ[Const[Read[A], ?] ~> FixFreeH[H, ?]](
          read =>
            GtoH(
              QC.inj(Reduce(
                Free.liftF(
                  GtoH(SR.inj(Const(ShiftedRead(read.getConst.path, IncludeId))))),
                NullLit(),
                List(ReduceFuncs.UnshiftMap(Free.roll(ProjectIndex(HoleF, IntLit(0))),
  ...
  implicit def qscriptCore[T[_[_]]: BirecursiveT, F[_]](
      implicit QC: QScriptCore[T, ?] :<: F): ShiftRead.Aux[T, QScriptCore[T, ?], F] =
    new ShiftRead[QScriptCore[T, ?]] {
      type G[A] = F[A]
      def shiftRead[H[_]](GtoH: G ~> H) =
        λ[QScriptCore[T, ?] ~> FixFreeH[H, ?]](qc =>
  ...
  implicit def thetaJoin[T[_[_]]: BirecursiveT, F[_]](
      implicit TJ: ThetaJoin[T, ?] :<: F): ShiftRead.Aux[T, ThetaJoin[T, ?], F] =
    new ShiftRead[ThetaJoin[T, ?]] {
      type G[A] = F[A]
      def shiftRead[H[_]](GtoH: G ~> H) =
        λ[ThetaJoin[T, ?] ~> FixFreeH[H, ?]](
          tj =>
            GtoH(
              TJ.inj(
                ThetaJoin(Free.point(tj.src),
  ...
  implicit def equiJoin[T[_[_]]: BirecursiveT, F[_]](
      implicit EJ: EquiJoin[T, ?] :<: F): ShiftRead.Aux[T, EquiJoin[T, ?], F] =
    new ShiftRead[EquiJoin[T, ?]] {
      type G[A] = F[A]
      def shiftRead[H[_]](GtoH: G ~> H) =
        λ[EquiJoin[T, ?] ~> FixFreeH[H, ?]](
          ej =>
            GtoH(
              EJ.inj(
                EquiJoin(Free.point(ej.src),
  ...
  implicit def coproduct[T[_[_]], F[_], I[_], J[_]](
      implicit I: ShiftRead.Aux[T, I, F],
      J: ShiftRead.Aux[T, J, F]): ShiftRead.Aux[T, Coproduct[I, J, ?], F] =
  ...
  def default[T[_[_]], F[_]: Functor, I[_]](implicit F: F :<: I): ShiftRead.Aux[T, F, I] =
    new ShiftRead[F] {
      type G[A] = I[A]
      def shiftRead[H[_]](GtoH: G ~> H) =
        λ[F ~> FixFreeH[H, ?]](fa => GtoH(F inj (fa map Free.point)))
    }

  implicit def deadEnd[T[_[_]], F[_]](
      implicit DE: Const[DeadEnd, ?] :<: F): ShiftRead.Aux[T, Const[DeadEnd, ?], F] =
    default
  ...
  implicit def projectBucket[T[_[_]], F[_]](
      implicit PB: ProjectBucket[T, ?] :<: F): ShiftRead.Aux[T, ProjectBucket[T, ?], F] =
    default
}
