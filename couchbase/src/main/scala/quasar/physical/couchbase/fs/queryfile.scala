...
abstract class queryfile {
  ...
  def executePlan(repr: Repr, out: AFile): Backend[Unit] =
    for {
      ...
      _ <- lift(
        docs.nonEmpty.whenM(
          Task.delay(
            Observable.from(docs).flatMap(ctx.bucket.async.insert(_).asScala).toBlocking.last
          ))).into[Eff].liftB
    } yield ()
  ...
  def explain(repr: Repr): Backend[String] =
    ME.unattempt(
      RenderQuery.compact(repr).leftMap(FileSystemError.qscriptPlanningFailed(_)).η[Backend])
