...
package object fs {
  ...
  def parseConfig(connectionUri: ConnectionUri): DefErrT[Task, Config] = {
    final case class ConnUriParams(bucket: String,
                                   pass: String,
                                   docTypeKey: String,
                                   socketConnectTimeout: Duration,
                                   queryTimeout: Duration)

    def env(p: ConnUriParams): Task[CouchbaseEnvironment] =
      Task.delay(
    ...
    implicit class LiftDT[A](v: => NonEmptyList[String] \/ A) {
      def liftDT: DefErrT[Task, A] =
        EitherT(Task.delay(v.leftMap(_.left[EnvironmentError])))
    }
    ...
    for {
      ...
      cluster <- EitherT(
        Task
          .delay(
            CouchbaseCluster
              .fromConnectionString(ev,
                                    uri.copy(path = "", query = Query.empty).renderString)
              .right
          )
          .handle {
        ...
        Task
          .delay(
            (cm.info.getMinVersion.compareTo(minimumRequiredVersion) >= 0).unlessM(
              s"Couchbase Server must be ${minimumRequiredVersion}+".wrapNel
      ...
      bkt <- EitherT(
        Task
          .delay(cm.hasBucket(params.bucket).booleanValue)
          .ifM(Task.delay(
                 cluster.openBucket(params.bucket, params.pass).right[DefinitionError]),
               Task.now(s"Bucket ${params.bucket} not found".wrapNel.left.left)))
      bktMgr = bkt.bucketManager
      idxExst <- Task
        .delay(bktMgr.listN1qlIndexes.asScala.find { i =>
      ...
      _ <- idxExst
        .unlessM(
          Task.delay(
            bktMgr.createN1qlIndex(s"quasar_${params.docTypeKey}_idx",
                                   true,
                                   false,
                                   params.docTypeKey)))
        .liftM[DefErrT]
