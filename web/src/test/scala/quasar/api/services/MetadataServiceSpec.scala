...
object MetadataFixture {
  ...
  def runQueryWithMounts(mem: InMemState,
                         mnts: Map[APath, MountConfig]): QueryFile ~> Task = {
      ...
      "if file with same name as existing directory (without trailing slash)" >> prop {
        s: SingleFileMemState =>
          depth(s.file) > 1 ==> {
            ...
            val fileWithSameName = parentDir(parent).get </> file(
              dirName(parent).get.value)
            val response = service(s.state, Map())(
              Request(uri = pathUri(fileWithSameName))).unsafePerformSync
    ...
    "respond with OK" >> {
      ...
      "and list of children for existing nonempty directory" >> prop { s: NonEmptyDir =>
        ...
        service(s.state, Map())(Request(uri = pathUri(s.dir)))
          .as[Json]
          .unsafePerformSync must_== Json("children" := childNodes.sorted)
      ...
      "and functions as files on a module mount with additionnal info about functions parameters" >> prop {
        dir: ADir =>
          val moduleConfig: List[Statement[Fix[Sql]]] = List(
            FunctionDecl(CIName("FOO"), List(CIName("BAR")), Fix(boolLiteral(true))),
            FunctionDecl(CIName("BAR"),
                         List(CIName("BAR"), CIName("BAZ")),
                         Fix(boolLiteral(false)))
          )
          val mnts =
            Map[APath, MountConfig]((dir, MountConfig.moduleConfig(moduleConfig)))
