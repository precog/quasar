...
object Prettify {
  ...
  def flatten(data: Data): ListMap[Path, Data] = {
    ...
    def loop(data: Data): Data \/ List[(Path, Data)] = {
      ...
      data match {
        case Data.Arr(value) =>
          \/-(value.zipWithIndex.flatMap { case (c, i) => prepend(IndexSeg(i), c) })
        case Data.Obj(value) =>
          \/-(value.toList.flatMap { case (f, c) => prepend(FieldSeg(f), c) })
        case _ => -\/(data)
      }
    }
  ...
  def unflatten(values: ListMap[Path, Data]): Data = {
    ...
    def append(v: Data, p: Path, d: Data): Data = (v, p) match {
      ...
      case (Data.Arr(values), Path(IndexSeg(x) :: rest)) if values.size ≟ (x + 1) =>
        Data.Arr(
          values.dropRight(1) :+ append(values.lastOption.getOrElse(init), Path(rest), d))
  ...
  def renderTable(rows: List[Data]): List[String] =
    if (rows.isEmpty) Nil
      ...
      val columns: List[(Path, List[Aligned[String]])] =
        columnNames(rows).map(n =>
          n -> flat.map(m => m.get(n).fold[Aligned[String]](Aligned.Left(""))(render)))

      val widths: List[((Path, List[Aligned[String]]), Int)] =
        columns.map {
          case (path, vals) =>
            (path, vals) -> (path.label.length :: vals.map(_.value).map(_.length + 1)).max
        }
  ...
  /**
    ...
    */
  def renderStream[F[_]](src: Process[F, Data],
                         n: Int Refined Positive): Process[F, List[String]] = {
    ...
    def sampleMap[A, B, C](src: Process[F, A],
                           n: Int,
                           sample: IndexedSeq[A] => B,
                           prefix: B => C,
                           f: (A, B) => C): Process[F, C] = {
      (src.chunk(n) ++ Process.emit(Vector.empty) ++ Process.emit(Vector.empty))
        .zipWithState[Option[B]](None) {
          case (as, optB) =>
        ...
        .flatMap {
          ...
          case (Some((as, None)), (_, Some(b))) =>
            Process.emit(prefix(b)) ++
              Process.emitAll(as.map(a => f(a, b)))
          case (Some((as, Some(_))), (_, Some(b))) =>
            Process.emitAll(as.map(a => f(a, b)))
...
//  }
  ...
  private def dataAsColumns[F[_]: Monad: Plus](preview: List[Data],
                                               all: F[Data]): F[List[String]] = {
    val cols   = columnNames(preview)
    val header = cols.map(_.label)
    val columnarData =
      all.map(data => cols.map(c => flatten(data).get(c).fold("")(render(_).value)))
