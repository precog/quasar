...
private final case class SimplifiedJoinCondition[T[_[_]]](keys: List[EquiJoinKey[T]],
          ...
          def separateConditions(fm: JoinFunc[T]): SimplifiedJoinCondition[T] =
            fm.resume match {
              case -\/(And(a, b)) =>
                ...
                SimplifiedJoinCondition(fir.keys ++ sec.keys,
                                        fir.filter.fold(sec.filter)(f =>
                                          sec.filter.fold(f.some)(s =>
                                            Free.roll(And[T, JoinFunc[T]](f, s)).some)))
              case -\/(Eq(l, r)) =>
          ...
          GtoH(
            QC.inj(Map(
              filter.foldLeft(GtoH(EJ.inj(EquiJoin(
                ...
                tj.f,
                Free.roll(ConcatArrays(Free.roll(MakeArray(Free.point(LeftSide))),
