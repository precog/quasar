  ...
  "merge" should {
          ...
          GroupingSource(
            ...
            Some(
              InnerObjectConcat(
                ...
                WrapObject(DerefObjectStatic(DerefObjectStatic(Leaf(Source), valueField),
                                             cField),
                           "value"))),
            0,
            GroupKeySpecOr(
    ...
    "execute the medals query without a cross" in {
      val medalsJson =
        ...
        | """.stripMargin
      val medals =
        fromJson(JParser.parseManyFromString(medalsJson).valueOr(throw _).toStream)
