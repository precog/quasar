...
import quasar.contrib.pathy.{refineTypeAbs, FPath}
        ...
        new RenderTree[LogicalPlan[A]] {
          ...
          def render(v: LogicalPlan[A]) = v match {
            ...
            case InvokeUnapply(func, args) =>
              NonTerminal("Invoke" :: nodeType, Some(func.shows), args.unsized.map(ra.render))
    ...
    new Delay[Equal, LogicalPlan] {
      def apply[A](fa: Equal[A]) = {
        ...
        Equal.equal {
          ...
          case (
              Join(l1, r1, t1, JoinCondition(lName1, rName1, c1)),
