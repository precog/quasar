...
class ProvenanceT[T[_[_]]: CorecursiveT: EqualT](implicit J: Equal[T[EJson]])
    extends TTypes[T] {
  ...
  def genComparison(lp: Provenance, rp: Provenance): Option[JoinFunc] =
    (lp, rp) match {
      case (Value(v1), Value(v2)) =>
        Free.roll(MapFuncsCore.Eq[T, JoinFunc](v1.as(LeftSide), v2.as(RightSide))).some
      case (Value(v1), Proj(d2)) =>
        Free
          .roll(MapFuncsCore.Eq[T, JoinFunc](v1.as(LeftSide), Free.roll(Constant(d2))))
          .some
      case (Proj(d1), Value(v2)) =>
        Free
          .roll(MapFuncsCore.Eq[T, JoinFunc](Free.roll(Constant(d1)), v2.as(RightSide)))
          .some
      case (Both(l1, r1), Both(l2, r2)) =>
        genComparison(l1, l2).fold(genComparison(r1, r2))(lc =>
          genComparison(r1, r2).fold(lc)(rc => Free.roll(And[T, JoinFunc](lc, rc))).some)
      case (OneOf(l1, r1), OneOf(l2, r2)) =>
        genComparison(l1, l2).fold(genComparison(r1, r2))(lc =>
          genComparison(r1, r2).fold(lc)(rc => Free.roll(And[T, JoinFunc](lc, rc))).some)
  ...
  def genBuckets(ps: List[Provenance]): Option[(List[Provenance], FreeMap)] =
    ps.traverse(genBucket)
      ...
      .traverse(_.join match {
        case Nil => None
        case h :: t =>
          t.foldLeft(Free.roll(MakeArray[T, FreeMap](h)))((a, e) =>
              Free.roll(ConcatArrays(a, Free.roll(MakeArray(e)))))
            .some
      })
  ...
  def joinProvenances(leftBuckets: List[Provenance],
  ...
  def unionProvenances(leftBuckets: List[Provenance],
                       rightBuckets: List[Provenance]): List[Provenance] =
    leftBuckets.reverse
      .alignWith(rightBuckets.reverse) {
