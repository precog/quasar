...
object Query extends QueryInstances {
  ...
  def toXQuery[V, F[_]: Monad](f: V => F[XQuery]): AlgebraM[F, Query[V, ?], XQuery] = {
    ...
    case ElementAttributeValue(elements, attributes, values) =>
      cts
        .elementAttributeValueQuery(qnameSeq(elements), qnameSeq(attributes), strSeq(values))
        .point[F]
    case ElementAttributeWord(elements, attributes, words) =>
      cts
        .elementAttributeWordQuery(qnameSeq(elements), qnameSeq(attributes), strSeq(words))
        .point[F]
    ...
    case JsonPropertyValue(properties, values) =>
      values.traverse(f).map(mkSeq(_)).map(cts.jsonPropertyValueQuery(strSeq(properties), _))
        ...
        RenderTree.make[Query[V, A]] {
          ...
          case ElementAttributeValue(elements, attributes, values) =>
            node(
              ...
              nodeL("Values", values))
          case ElementAttributeWord(elements, attributes, words) =>
            node("cts:element-attribute-word-query", es(elements), as(attributes), ws(words))
          case Element(elements, query) =>
