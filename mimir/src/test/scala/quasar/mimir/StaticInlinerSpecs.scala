...
trait StaticInlinerSpecs[M[+ _]] extends EvaluatorSpecification[M] {
  ...
  "static inlining of Root computation" should {
    "detect and resolve addition" in {
      ...
      val input = Join(Add, Cross(None), Const(CLong(42))(line), Const(CNum(3.14))(line))(line)
    ...
    "detect and resolve operations at depth" in {
      ...
      val input = Join(
        ...
        Cross(None),
        Const(CLong(42))(line),
        Join(Mul, Cross(None), Const(CNum(3.14))(line), Const(CLong(2))(line))(line))(line)
    ...
    "produce CUndefined in cases where the operation is undefined" in {
      ...
      val input = Join(Div, Cross(None), Const(CLong(42))(line), Const(CLong(0))(line))(line)
    ...
    "propagate through static computations CUndefined when produced at depth" in {
      ...
      val input = Join(
        ...
        Cross(None),
        Const(CLong(42))(line),
        Join(Div, Cross(None), Const(CNum(3.14))(line), Const(CLong(0))(line))(line))(line)
    ...
    "propagate through non-singleton computations CUndefined when produced at depth" >> {
      ...
      "right" >> {
        val input = Join(
          Add,
          Cross(None),
          Join(Div, Cross(None), Const(CNum(3.14))(line), Const(CLong(0))(line))(line),
    ...
    "reduce filters with static RHS" >> {
      ...
      "true" >> {
        ...
        inlineStatics(input, defaultEvaluationContext) mustEqual dag.AbsoluteLoad(
    ...
    "detect and resolve array" >> {
      ...
      "wrap" >> {
        ...
        inlineStatics(input, defaultEvaluationContext) mustEqual Const(RArray(CBoolean(false)))(
          line)
      }
    ...
    "detect and resolve object" >> {
      ...
      "wrap" >> {
        val input =
          Join(WrapObject, Cross(None), Const(CString("k"))(line), Const(CTrue)(line))(line)

        inlineStatics(input, defaultEvaluationContext) mustEqual Const(RObject("k" -> CTrue))(
          line)
      }
    ...
    "rewrite a filter with undefined to undefined" >> {
      "target" >> {
        val line = Line(1, 1, "")
        val input =
          Filter(IdentitySort, Const(CUndefined)(line), Const(CString("j"))(line))(line)
      ...
      "predicate" >> {
        val line = Line(1, 1, "")
        val input =
          Filter(IdentitySort, Const(CString("j"))(line), Const(CUndefined)(line))(line)
...
object StaticInlinerSpecs extends StaticInlinerSpecs[Need]
