...
object BsonCodec {
  ...
  object EJsonTypeSize {
    def apply(typ: String, size: Int): Bson =
      Bson.Doc(
        ListMap("_ejson.type" -> Bson.Text(typ), "_ejson.size" -> Bson.Int32(size)))
      ...
      (meta, value) match {
        ...
        case (EJsonTypeSize("_ejson.binary", size), Bson.Text(data)) =>
          if (size.isValidInt)
            ejson.z85
              .decode(data)
              .fold[PlannerError \/ Bson](NonRepresentableEJson(
                "“" + data + "” is not a valid Z85-encoded string").left)(bv =>
                Bson.Binary(ImmutableArray.fromArray(bv.take(size.toLong).toArray)).right)
          ...
          (extract(map.get("year"), Bson._int32) ⊛
            ...
            .orElse(
              (extract(map.get("year"), Bson._int32) ⊛
                ...
                LocalDate.of(y, m, d)))
            .flatMap(date =>
              Bson.Date.fromInstant(date.atStartOfDay.toInstant(ZoneOffset.UTC))) \/>
        ...
        case (EJsonType("_ejson.time"), Bson.Doc(map)) =>
          (extract(map.get("hour"), Bson._int32) ⊛
            ...
            extract(map.get("nanosecond"), Bson._int32))(
            (h, m, s, n) =>
              Bson.Text(
                pad2(h) + ":" +
                  pad2(m) + ":" +
        ...
        case (EJsonType("_ejson.timestamp"), Bson.Doc(map)) =>
          (extract(map.get("year"), Bson._int32) ⊛
            ...
            Bson.Date.fromInstant(
              LocalDateTime.of(y, mo, d, h, mi, s, n).toInstant(ZoneOffset.UTC))).join \/>
  ...
  /** Converts the parts of `Bson` that it can, then stores the rest in,
    ...
    */
  def toEJson[F[_]](implicit C: ejson.Common :<: F,
                    E: ejson.Extension :<: F): ElgotCoalgebra[Bson \/ ?, F, Bson] = {
    ...
    case Bson.Binary(value) =>
      E.inj(
          ejson.Meta(Bson.Text(ejson.z85.encode(ByteVector.view(value.toArray))),
                     EJsonTypeSize("_ejson.binary", value.size)))
        .right
  ...
  def fromData(data: Data): PlannerError \/ Bson =
    data.hyloM[PlannerError \/ ?, CoEnv[Data, EJson, ?], Bson](interpretM({
      case Data.NA => Bson.Undefined.right
      case data    => NonRepresentableData(data).left
    }, fromEJson), Data.toEJson[EJson].apply(_).right)

  def toData(bson: Bson): Data =
    bson.hylo[CoEnv[Bson, EJson, ?], Data](interpret(κ(Data.NA), Data.fromEJson),
