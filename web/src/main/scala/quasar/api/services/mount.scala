...
object mount {
      ...
      case GET -> AsPath(path) =>
        def err =
          ApiError.fromMsg(
            ...
            "path" := path)
        respondT(
          EitherT(M.lookupConfig(path).run.run ∘ (_ \/> (err) >>= (_
  ...
  private def move[S[_], T](
      ...
      S2: VCache :<: S): EitherT[Free[S, ?], ApiError, String] =
    parse(dstStr)
      ...
      .cata(
        dst =>
          ((refineType(src) ⊛ refineType(dst))((s, d) =>
            vcache
              .get(s) >>= (vc => (vcache.put(d, vc) >> vcache.delete(s)).liftM[OptionT])) | ()
  ...
  private def mount[S[_]](
      ...
      S3: VCache :<: S): EitherT[Free[S, ?], ApiError, Boolean] =
    for {
      body <- free.lift(EntityDecoder.decodeString(req)).into[S].liftM[ApiErrT]
      bConf <- EitherT.fromDisjunction[Free[S, ?]](
        Parse.decodeWith[ApiError \/ MountConfig, MountConfig](
          body,
          (_: MountConfig).right[ApiError],
          parseErrorMsg =>
            ApiError.fromMsg_(BadRequest withReason "Malformed input.", parseErrorMsg).left,
          (msg, _) => ApiError.fromMsg_(BadRequest, msg).left
        ))
      exists <- M.lookupType(path).run.isDefined.liftM[ApiErrT]
      _ <- (replaceIfExists && exists)
        .fold(M.replace(path, bConf), M.mount(path, bConf))
        .liftM[ApiErrT]
      maxAge = req.headers
        .get(`Cache-Control`)
        .flatMap(_.values.list.collectFirst(_ match {
          case `max-age`(s) => s
        }))
      _ <- (refineType(path).toOption ⊛ MountConfig.viewConfig
        .getOption(bConf)
        .map(MountConfig.ViewConfig.tupled) ⊛ maxAge)(createNewViewCache[S])
  ...
  private def createNewViewCache[S[_]](
      ...
      T: Timing.Ops[S],
      S0: Task :<: S,
      S1: VCache :<: S): EitherT[Free[S, ?], ApiError, Unit] =
    for {
      tempFile <- MF.tempFile(viewPath).leftMap(_.toApiError)
      timeStamp <- T.timestamp.liftM[ApiErrT]

      refreshAfter <- free
        .lift(Task.fromDisjunction(ViewCache.expireAt(timeStamp, maxAge)))
        .into
        .liftM[ApiErrT]
      ...
      _ <- vcache
        ...
        .fold(
          ...
          vcache.put(viewPath, newViewCache)
        )
        .join
        .liftM[ApiErrT]
    } yield ()
