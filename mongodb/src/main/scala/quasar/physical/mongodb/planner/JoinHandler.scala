...
final case class JoinHandler[WF[_], F[_]](
    run: (MongoJoinType, JoinSource[WF], JoinSource[WF]) => F[WorkflowBuilder[WF]]) {

  def apply(tpe: MongoJoinType,
            left: JoinSource[WF],
            right: JoinSource[WF]): F[WorkflowBuilder[WF]] =
    run(tpe, left, right)
}

final case class JoinSource[WF[_]](src: Fix[WorkflowBuilderF[WF, ?]],
                                   keys: List[WorkflowBuilder[WF]],
                                   js: Option[List[JsFn]])
...
object JoinHandler {
  ...
  def fallback[WF[_], F[_]: Monad](first: JoinHandler[WF, OptionT[F, ?]],
  ...
  def pipeline[WF[_]: Functor: Coalesce: Crush: Crystallize](
      ...
      ev3: ExprOpOps.Uni[ExprOp]): JoinHandler[WF, OptionT[WorkflowBuilder.M, ?]] =
    JoinHandler({ (tpe, left, right) =>
      ...
      def indexed(coll: Collection, field: BsonField): Boolean =
        indexes(coll).cata(_.exists(_.primary ≟ field), false)
      ...
      def lookup(lSrc: WorkflowBuilder[WF],
                 lKey: WorkflowBuilder[WF],
                 lName: BsonField.Name,
                 rColl: CollectionName,
                 rField: BsonField,
                 rName: BsonField.Name) = {
        ...
        def filterExists(wb: WorkflowBuilder[WF], field: BsonField): WorkflowBuilder[WF] =
          WB.filter(wb, List(ExprBuilder(wb, \/-($var(DocField(field))))), {
        ...
        lKey match {
          ...
          case _ =>
            for {
              ...
              left <- WB.objectConcat(WB.makeObject(lSrc, lName.asText),
              ...
              CollectionBuilder(
                chain[Fix[WF]](
                  ...
                  $project(Reshape(
                             ListMap(lName -> \/-($var(DocField(lName))),
      ...
      (tpe, left, right) match {
        case (set.InnerJoin, JoinSource(src, List(key), _), IsLookupFrom(coll, field))
            if unsharded(coll) && indexed(coll, field) && src
              .cata(sourceDb) ≟ coll.database.some =>
          lookup(src, key, LeftName, coll.collection, field, RightName).liftM[OptionT]
        ...
        case (set.InnerJoin, IsLookupFrom(coll, field), JoinSource(src, List(key), _))
            if unsharded(coll) && indexed(coll, field) && src
              .cata(sourceDb) ≟ coll.database.some =>
          lookup(src, key, RightName, coll.collection, field, LeftName).liftM[OptionT]
  ...
  object IsLookupFrom {
    ...
    def unapply[F[_]](arg: JoinSource[F])(
        ...
        ev1: Equal[Fix[WorkflowBuilderF[F, ?]]]): Option[(Collection, BsonField)] =
      arg match {
        case JoinSource(Fix(CollectionBuilderF(Fix(ev0($ReadF(coll))), Root(), None)),
  ...
  def mapReduce[WF[_]: Functor: Coalesce: Crush: Crystallize](
      ...
      ev2: ExprOpOps.Uni[ExprOp]): JoinHandler[WF, WorkflowBuilder.M] =
    JoinHandler({ (tpe, left0, right0) =>
      val ops = Ops[WF]
      import ops._
      ...
      def keyMap(keyExpr: List[JsFn],
                 rootField: BsonField.Name,
                 otherField: BsonField.Name): Js.AnonFunDecl =
        $MapF.mapKeyVal(
          ("key", "value"),
          keyExpr match {
            case Nil => Js.Null
            case _ =>
              jscore
                .Obj(
                  keyExpr
                    .map(_(jscore.ident("value")))
                    .zipWithIndex
                    .foldLeft[ListMap[jscore.Name, JsCore]](
                      ListMap[jscore.Name, JsCore]()) {
                      case (acc, (j, i)) => acc + (jscore.Name(i.toString) -> j)
                    })
                .toJs
          },
          Js.AnonObjDecl(
            List((otherField.asText, Js.AnonElem(Nil)),
                 (rootField.asText, Js.AnonElem(List(Js.Ident("value"))))))
        )

      def jsReduce(src: WorkflowBuilder[WF],
                   key: List[JsFn],
                   rootField: BsonField.Name,
                   otherField: BsonField.Name): (WorkflowBuilder[WF], FixOp[WF]) =
        (src, $map[WF](keyMap(key, rootField, otherField), ListMap()))

      def wbReduce(src: WorkflowBuilder[WF],
                   key: List[WorkflowBuilder[WF]],
                   rootField: BsonField.Name,
                   otherField: BsonField.Name): (WorkflowBuilder[WF], FixOp[WF]) =
        (DocBuilder(reduce(groupBy(src, key))($push(_)),
                    ListMap(rootField             -> \/-($$ROOT),
      ...
      def buildJoin(src: Fix[WF], tpe: MongoJoinType): Fix[WF] =
        tpe match {
          case set.FullOuterJoin =>
            chain(src, buildProjection(padEmpty(leftField), padEmpty(rightField)))
          ...
          case set.RightOuterJoin =>
            chain(
              src,
              $match[WF](
                Selector.Doc(ListMap(rightField.asInstanceOf[BsonField] -> nonEmpty))),
        ...
        AnonFunDecl(
          ...
          List(
            VarDef(
              List(
                ("result",
                 AnonObjDecl(List((leftField.asText, AnonElem(Nil)),
                                  (rightField.asText, AnonElem(Nil))))))),
            Call(
              Select(Ident("values"), "forEach"),
              List(
                AnonFunDecl(
                  List("value"),
  ...
  private def preferMapReduce[WF[_]: Coalesce: Crush: Crystallize: Functor](
      wb: WorkflowBuilder[WF])(implicit ev0: WorkflowOpCoreF :<: WF,
                               ev1: RenderTree[WorkflowBuilder[WF]],
                               ev2: ExprOpOps.Uni[ExprOp]): Boolean = {
    ...
    generateWorkflow[WF](wb).evalZero
      .fold(κ(false), wf => checkTask(task(Crystallize[WF].crystallize(wf._1))))
  }
}
