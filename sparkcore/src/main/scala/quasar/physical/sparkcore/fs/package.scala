...
package object fs {
  ...
  type SparkQScriptCP =
    QScriptCore[Fix, ?] :\: EquiJoin[Fix, ?] :/: Const[ShiftedRead[AFile], ?]
  ...
  final case class SparkFSDef[HS[_], S[_]](run: Free[HS, ?] ~> Free[S, ?],
                                           close: Free[S, Unit])
      ...
      Task
        ...
        .handleWith {
          case ex: SparkException if ex.getMessage.contains("SPARK-2243") =>
            "You can not mount second Spark based connector... Please unmount existing one first."
              .left[SparkContext]
              .point[Task]
        })
  ...
  def toQScript[M[_]: Monad](
    ...
    for {
      ...
      optQS = shifted.transHylo(optimize.optimize(reflNT[SparkQScript]),
                                Unicoalesce[Fix, SparkQScriptCP])

      _ <- EitherT(
        WriterT[M, PhaseResults, FileSystemError \/ Unit](
          (Vector(PhaseResult.tree("QScript (Spark)", optQS)), ().right[FileSystemError])
            .point[M]))
    } yield optQS
  }
    ...
    FileSystemDef.fromPF {
      case (`fsType`, uri) =>
        for {
          ...
          (sparkConf, t) = config
          res <- {
            sparkFsDef(sparkConf)
              .map {
                case SparkFSDef(run, close) =>
                  val fileSystemInterpreter = fsInterpret(t)
                  val queryFileIntereter = fileSystemInterpreter compose Inject[
                    QueryFile,
                    FileSystem]
                  val listContents
                    : ADir => Free[QueryFile, FileSystemError \/ Set[PathSegment]] =
                    (adir: ADir) => QueryFile.Ops[QueryFile].ls(adir).run
                  val analyze0: Analyze ~> Free[QueryFile, ?] =
                    Analyze
                      .defaultInterpreter[QueryFile, SparkQScript, Fix[SparkQScript]](
                        lp => toQScript(listContents)(lp).mapT(_.value))
