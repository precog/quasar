...
final case class TableContext[T](
    root: Option[T],
    full: () => T,
    subtables: Map[String, T])(implicit T: Corecursive.Aux[T, LP]) {
...
final case class Context[T](bindingContext: List[BindingContext[T]],
...
private object CompilerState {
  ...
  def rootTableReq[M[_], T](implicit
  ...
  def subtableReq[M[_], T](name: String)(implicit
                                         MErr: MonadError_[M, SemanticError],
                                         MState: MonadState[M, CompilerState[T]]): M[T] =
    subtable(name) >>=
...
final class Compiler[M[_], T: Equal](implicit TR: Recursive.Aux[T, LP],
  ...
  private def fail[A](error: SemanticError)(
      implicit m: MonadError_[M, SemanticError]): M[A] =
    m.raiseError(error)
  ...
  private def compile0(node: CoExpr)(implicit
                                     MErr: MonadError_[M, SemanticError],
                                     MState: MonadState[M, CompilerState[T]]): M[T] = {
    ...
    def compileCases(cases: List[Case[CoExpr]], default: T)(
        f: Case[CoExpr] => M[(T, T)]) =
      cases
        .traverse(f)
    ...
    def flattenJoins(term: T, relations: SqlRelation[CoExpr]): T = relations match {
      ...
      case JoinRelation(left, right, _, _) =>
        structural
          .ObjectConcat(flattenJoins(Left.projectFrom(term), left),
                        flattenJoins(Right.projectFrom(term), right))
          .embed
    }

    def buildJoinDirectionMap(
        relations: SqlRelation[CoExpr]): Map[String, List[JoinDir]] = {
      @SuppressWarnings(Array("org.wartremover.warts.Recursion"))
        ...
        current
          .map { current =>
            for {
              ...
              next2 <- CompilerState.contextual(bc,
                                                tableContext(lpr.free(stepName),
                                                             relations))(next)
    ...
    def compileFunction[N <: Nat](func: GenericFunc[N],
                                  args: Func.Input[CoExpr, N]): M[T] =
    ...
    node.tail match {
      case s @ Select(isDistinct, projections, relations, filter, groupBy, orderBy) =>
        ...
        val namesOrError: SemanticError \/ List[Option[String]] =
          projectionNames[Fix[Sql]](
            projections.map(
              _.map(forgetAnnotation[CoExpr, Fix[Sql], Sql, SA.Annotations])),
                        ...
                        val sort = orderBy.map(
                          orderBy =>
                            CompilerState.rootTableReq[M, T] >>= (t =>
                              nam
                                .fold(orderBy.keys.traverse(p => (t, p._1).point[M]))(n =>
                                  CompilerState.addFields(n.foldMap(_.toList))(
                                    orderBy.keys.traverse {
                                      case (ot, key) => compile0(key) strengthR ot
                                    }))
                                .map(ks =>
                                  lpr.sort(t, ks map {
                                  ...
                                  t =>
                                    if (syntheticNames.nonEmpty)
                                      set
                                        .DistinctBy(
                                          t,
                                          syntheticNames.foldLeft(t)((acc, field) =>
                                            structural
                                              .DeleteField(acc,
                                                           lpr.constant(Data.Str(field)))
                                              .embed))
                                        .embed
                                    else set.Distinct(t).embed)
                                .some
                            case _ => None
                          }
      ...
      case Splice(expr) =>
        expr.fold(CompilerState.fullTable.flatMap(_.map(emit _).getOrElse(fail(GenericError(
          ...
          func =>
            args
              ...
              .flatMap(
                func
                  .apply(_)
                  .cata(successfulInvoke => successfulInvoke.point[M],
...
object Compiler {
  def apply[M[_], T: Equal](implicit TR: Recursive.Aux[T, LP],
                            TC: Corecursive.Aux[T, LP]) =
    new Compiler[M, T]

  def trampoline[T: Equal](implicit TR: Recursive.Aux[T, LP],
                           TC: Corecursive.Aux[T, LP]) =
    apply[StateT[EitherT[scalaz.Free.Trampoline, SemanticError, ?], CompilerState[T], ?],
          T]
  ...
  /** Emulate SQL semantics by reducing any projection which trivially
    ...
    */
  def reduceGroupKeys[T: Equal](tree: T)(implicit TR: Recursive.Aux[T, LP],
                                         TC: Corecursive.Aux[T, LP]): T = {
