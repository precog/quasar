...
trait Planner[M[_], FMT, F[_]] {
  def plan[Q, V](
      implicit Q: Birecursive.Aux[Q, Query[V, ?]]): AlgebraM[M, F, Search[Q] \/ XQuery]
  ...
  def planXQuery(
    ...
    Kleisli(elimSearch[Q, Unit] _) <==< (plan[Q, Unit] <<< F.lift(
      (_: XQuery).right[Search[Q]]))
  }
...
sealed abstract class PlannerInstances extends PlannerInstances0 {
  implicit def coproduct[M[_], FMT, F[_], G[_]](
      implicit F: Lazy[Planner[M, FMT, F]],
      G: Lazy[Planner[M, FMT, G]]
...
sealed abstract class PlannerInstances0 extends PlannerInstances1 {
  ...
  implicit def qScriptCore[F[_]: Monad: QNameGenerator: PrologW: MonadPlanErr,
                           FMT: SearchOptions,
                           T[_[_]]: BirecursiveT](
      implicit
  ...
  implicit def thetaJoin[F[_]: Monad: QNameGenerator: PrologW: MonadPlanErr,
                         FMT: SearchOptions,
                         T[_[_]]: BirecursiveT](
      implicit
      SP: StructuralPlanner[F, FMT],
  ...
  implicit def constReadDir[F[_]: MonadPlanErr, FMT]
    : Planner[F, FMT, Const[Read[ADir], ?]] =
    new UnreachablePlanner[F, FMT, Const[Read[ADir], ?]]("Read[ADir]")

  implicit def constShiftedReadFile[F[_]: MonadPlanErr, FMT]
    : Planner[F, FMT, Const[ShiftedRead[AFile], ?]] =
    new UnreachablePlanner[F, FMT, Const[ShiftedRead[AFile], ?]]("ShiftedRead[AFile]")

  implicit def equiJoin[F[_]: MonadPlanErr, FMT, T[_[_]]]
    : Planner[F, FMT, EquiJoin[T, ?]] =
    new UnreachablePlanner[F, FMT, EquiJoin[T, ?]]("EquiJoin")

  implicit def projectBucket[F[_]: MonadPlanErr, FMT, T[_[_]]]
    : Planner[F, FMT, ProjectBucket[T, ?]] =
    new UnreachablePlanner[F, FMT, ProjectBucket[T, ?]]("ProjectBucket")
}

sealed abstract class PlannerInstances1 {
  implicit def eitherTPlanner[M[_]: Functor, FMT, F[_], E](
      implicit P: Planner[M, FMT, F]): Planner[EitherT[M, E, ?], FMT, F] =
    new Planner[EitherT[M, E, ?], FMT, F] {
      def plan[Q, V](implicit Q: Birecursive.Aux[Q, Query[V, ?]]) =
        fx => EitherT(P.plan.apply(fx) map (_.right[E]))
    }

  implicit def writerTPlanner[M[_]: Functor, FMT, F[_], W: Monoid](
      implicit P: Planner[M, FMT, F]): Planner[WriterT[M, W, ?], FMT, F] =
