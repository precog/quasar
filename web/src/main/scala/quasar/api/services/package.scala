...
package object services {
  ...
  def formattedZipDataResponse[S[_], E](
    ...
    val p =
      format.encode(data).map(str => ByteVector.view(str.getBytes(StandardCharsets.UTF_8)))
    val suffix = format match {
      case JsonContentType(_, _, _) => "json"
      case Csv(_, _) => "csv"
    }
    val f = currentDir[Sandboxed] </> file1[Sandboxed](
      fileName(filePath).changeExtension(κ(suffix)))
