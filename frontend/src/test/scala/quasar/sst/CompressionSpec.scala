...
final class CompressionSpec
    ...
    "combines consts with their primary type in unions" >> prop {
      (st0: SimpleType, sjs: ISet[LeafEjs]) =>
        ...
        val (matching, nonmatching) =
          ssts.partition(sstConst.exist(j => simpleTypeOf(j) exists (_ ≟ st)))
        val simplified =
          matching.map(x => envTType.set(TypeF.simple(st))(x.project).embed)
        val coalesced = (simpleSst :: simplified).suml

        val compressed =
          (simpleSst :: ssts).suml.transCata[S](compression.coalescePrimary[J, Real])
  ...
  "coalesceWithUnknown" >> {
    "merges known map entry with unknown entry when same primary type appears in unknown" >> prop {
      (head: (Char, LeafEjs),
       xs0: NonEmptyList[(Char, LeafEjs)],
       kv: (BigInt, LeafEjs)) =>
        val xs: IMap[Char, LeafEjs] = IMap.fromFoldable(xs0) + head

        val u1 =
          head.leftAs(SST.fromEJson(Real(1), E(ejson.char[J]('x')).embed)).map(_.toSST)
        val u2 =
          head.bimap(c =>
                       envT(TypeStat.fromEJson(Real(1), E(ejson.char[J](c)).embed).some,
    ...
    "has no effect on maps when all keys are known" >> prop {
  ...
  "narrowUnion" >> {
    "reduces the largest group of values having the same primary type" >> prop {
      (bs: ISet[Byte], c1: Char, c2: Char, c3: Char, d1: BigDecimal) =>
        ((bs.size > 3) && (ISet.fromFoldable(IList(c1, c2, c3)).size ≟ 3)) ==> {
          ...
          val bytes =
            bs.toIList.map(b => SST.fromEJson(Real(1), E(ejson.byte[J](b)).embed))
          val chars =
    ...
    "no effect on unions smaller or equal to maxSize" >> prop {
      (x: LeafEjs, y: LeafEjs, xs: IList[LeafEjs]) =>
        val union =
          envT(cnt1, TypeF.union[J, S](x.toSST, y.toSST, xs map (_.toSST))).embed
