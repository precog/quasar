...
class SetSpec extends quasar.Qspec with TypeArbitrary {
  ...
  "SetLib" should {
    ...
    "type inner join on false" in {
      val expr =
        InnerJoin.tpe(Func.Input3(Type.Int, Type.Int, Type.Const(Data.Bool(false))))
      expr should beSuccessful(Type.Const(Data.Set(Nil)))
    }

    "type inner join with empty left" in {
      val expr =
        InnerJoin.tpe(Func.Input3(Type.Const(Data.Set(Nil)), Type.Int, Type.Bool))
      expr should beSuccessful(Type.Const(Data.Set(Nil)))
    }

    "type inner join with empty right" in {
      val expr =
        InnerJoin.tpe(Func.Input3(Type.Int, Type.Const(Data.Set(Nil)), Type.Bool))
      expr should beSuccessful(Type.Const(Data.Set(Nil)))
    }

    "type left outer join with empty left" in {
      val expr =
        LeftOuterJoin.tpe(Func.Input3(Type.Const(Data.Set(Nil)), Type.Int, Type.Bool))
      expr should beSuccessful(Type.Const(Data.Set(Nil)))
    }

    "type right outer join with empty right" in {
      val expr =
        RightOuterJoin.tpe(Func.Input3(Type.Int, Type.Const(Data.Set(Nil)), Type.Bool))
      expr should beSuccessful(Type.Const(Data.Set(Nil)))
    }
