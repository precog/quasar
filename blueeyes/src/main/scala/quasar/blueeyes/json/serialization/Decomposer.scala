...
class ExtractorDecomposer[A](toJson: A => JValue,
                             fromJson: JValue => Validation[Extractor.Error, A])
    extends Extractor[A]
    with Decomposer[A] {
...
object ExtractorDecomposer {
  def makeOpt[A, B](fg: A => B, gf: B => Validation[Extractor.Error, A])(
      implicit ez: Extractor[B],
      dz: Decomposer[B]): ExtractorDecomposer[A] = {
  ...
  def make[A, B](fg: A => B, gf: B => A)(implicit ez: Extractor[B],
                                         dz: Decomposer[B]): ExtractorDecomposer[A] = {
    new ExtractorDecomposer[A](
      v => dz decompose fg(v),
  ...
  def by[A] = new {
    def apply[B](fg: A => B)(gf: B => A)(implicit ez: Extractor[B],
                                         dz: Decomposer[B]): ExtractorDecomposer[A] =
      make[A, B](fg, gf)
    def opt[B](fg: A => B)(gf: B => Validation[Extractor.Error, A])(
        implicit ez: Extractor[B],
        dz: Decomposer[B]): ExtractorDecomposer[A] = makeOpt[A, B](fg, gf)
  }
}
...
trait MiscSerializers {
  ...
  implicit val JPathExtractorDecomposer: ExtractorDecomposer[JPath] =
    ExtractorDecomposer
      .by[JPath]
      .opt(x => JString(x.toString): JValue)(_.validated[String] map (JPath(_)))

  implicit val InstantExtractorDecomposer = by[Instant](_.getMillis)(instant fromMillis _)
  implicit val DurationExtractorDecomposer =
    by[Duration](_.getMillis)(duration fromMillis _)
  implicit val UuidExtractorDecomposer = by[UUID](_.toString)(uuid)
  implicit val MimeTypeExtractorDecomposer =
    by[MimeType].opt(x => JString(x.toString): JValue)(jv =>
...
trait SerializationImplicits extends MiscSerializers {
  case class DeserializableJValue(jvalue: JValue) {
    ...
    def validated[T](implicit e: Extractor[T]): Validation[Extractor.Error, T] =
      e.validated(jvalue)
  ...
  implicit def JValueToTValue[T](jvalue: JValue): DeserializableJValue =
    DeserializableJValue(jvalue)
  implicit def TValueToJValue[T](tvalue: T): SerializableTValue[T] =
    SerializableTValue[T](tvalue)
}
...
object DefaultSerialization
    extends DefaultExtractors
    with DefaultDecomposers
    with SerializationImplicits {
  implicit val DateTimeExtractorDecomposer =
    by[LocalDateTime].opt(x => JNum(x.getMillis): JValue)(
      _.validated[Long] map (dateTime fromMillis _))
}

// when we want to serialize dates as ISO8601 not as numbers
object Iso8601Serialization
    extends DefaultExtractors
    with DefaultDecomposers
    with SerializationImplicits {
  import Extractor._
  implicit val TZDateTimeExtractorDecomposer =
    by[LocalDateTime].opt(d => JString(dateTime showIso d): JValue) {
      case JString(dt) =>
        (Thrown.apply _) <-: Validation.fromTryCatchNonFatal(dateTime fromIso dt)
