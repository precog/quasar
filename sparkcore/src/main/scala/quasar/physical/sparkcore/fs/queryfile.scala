...
object queryfile {
  ...
  def chrooted[S[_]](input: Input[S], fsType: FileSystemType, prefix: ADir)(
      implicit
      s0: Task :<: S,
      s1: SparkContextRead :<: S,
      s2: MonotonicSeq :<: S,
      s3: KeyValueStore[ResultHandle, RddState, ?] :<: S): QueryFile ~> Free[S, ?] =
    flatMapSNT(interpreter(input, fsType)) compose chroot.queryFile[QueryFile](prefix)

  def interpreter[S[_]](input: Input[S], fsType: FileSystemType)(
      implicit
      s0: Task :<: S,
      s1: SparkContextRead :<: S,
      s2: MonotonicSeq :<: S,
    ...
    def qsToProgram[T](
        exec: (Fix[SparkQScript]) => Free[
          S,
          EitherT[Writer[PhaseResults, ?], FileSystemError, T]],
        lp: Fix[LogicalPlan]
    ): Free[S, (PhaseResults, FileSystemError \/ T)] = {
      val qs = toQScript[Free[S, ?]](listContents(input, _))(lp) >>= (qs =>
        EitherT(WriterT(exec(qs).map(_.run.run))))
      qs.run.run
    }
  ...
  implicit def composedFunctor[F[_]: Functor, G[_]: Functor]: Functor[(F ∘ G)#λ] =
    new Functor[(F ∘ G)#λ] {
  ...
  private def explainPlan[S[_]](input: Input[S],
                                fsType: FileSystemType,
                                qs: Fix[SparkQScript],
                                lp: Fix[LogicalPlan])(implicit
                                                      s0: Task :<: S,
    ...
    read.asks { sc =>
      ...
      injectFT.apply {
        sparkStuff
          .flatMap(
            mrdd =>
              mrdd.bitraverse[(Task ∘ Writer[PhaseResults, ?])#λ,
                              FileSystemError,
                              ExecutionPlan](
                ...
                rdd => {
                  ...
                  val inputs =
                    qs.cata(ExtractPath[SparkQScript, APath].extractPath[DList])
                  Task.delay(
                    Writer(Vector(PhaseResult.detail("RDD", rddDebug)),
  ...
  private def executePlan[S[_]](input: Input[S],
                                qs: Fix[SparkQScript],
                                out: AFile,
                                lp: Fix[LogicalPlan])(
      implicit
    ...
    read.asks { sc =>
      ...
      sparkStuff >>= (mrdd =>
        mrdd
          .bitraverse[(Free[S, ?] ∘ Writer[PhaseResults, ?])#λ, FileSystemError, AFile](
            planningFailed(lp, _).point[Writer[PhaseResults, ?]].point[Free[S, ?]],
            rdd =>
              input
                .store(rdd, out)
                .as(Writer(Vector(PhaseResult.detail("RDD", rdd.toDebugString)), out))
          )
          .map(EitherT(_)))
  ...
  private def evaluatePlan[S[_]](input: Input[S],
                                 qs: Fix[SparkQScript],
                                 lp: Fix[LogicalPlan])(
      implicit
      s0: Task :<: S,
    ...
    open
      ...
      .map { disj =>
        EitherT(disj.traverse {
          case (rh, rdd) =>
            Writer(Vector(PhaseResult.detail("RDD", rdd.toDebugString)), rh)
        })
      }
  }

  private def more[S[_]](h: ResultHandle, step: Int)(
      implicit
      s0: Task :<: S,
      kvs: KeyValueStore.Ops[ResultHandle, RddState, S])
  ...
  private def close[S[_]](h: ResultHandle)(
      implicit
      kvs: KeyValueStore.Ops[ResultHandle, RddState, S]): Free[S, Unit] = kvs.delete(h)

  private def fileExists[S[_]](input: Input[S], f: AFile)(
      implicit
      s0: Task :<: S): Free[S, Boolean] = input.fileExists(f)

  private def listContents[S[_]](input: Input[S], d: ADir)(
      implicit
