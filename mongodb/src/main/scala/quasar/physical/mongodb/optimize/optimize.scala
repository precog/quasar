...
package object optimize {
  object pipeline {
    ...
    private def deleteUnusedFields0[F[_]: Functor: Refs](
        op: Fix[F],
        usedRefs: Option[Set[DocVar]])(implicit I: WorkflowOpCoreF :<: F): Fix[F] = {
      def getRefs[A](op: F[_], prev: Option[Set[DocVar]]): Option[Set[DocVar]] =
        op match {
      ...
      def getDefs[A](op: F[A]): Set[DocVar] =
        (I.prj(op) match {
      ...
      val pruned =
        usedRefs.fold(op.unFix) { usedRefs =>
          ...
          op.unFix match {
            ...
            case I(g @ $GroupF(_, _, _)) =>
              I.inj(g.deleteAll(unusedRefs.map(_.flatten.head)))
    ...
    def deleteUnusedFields[F[_]: Functor: Refs](op: Fix[F])(
        implicit ev: WorkflowOpCoreF :<: F): Fix[F] =
    ...
    /** Converts a \$group with fields that duplicate keys into a \$group
      ...
      * from the key.
      */
    private def simplifyGroupƒ[F[_]: Coalesce: Functor](
        implicit ev: WorkflowOpCoreF :<: F): F[Fix[F]] => Option[F[Fix[F]]] = {
      case ev($GroupF(src, Grouped(cont), id)) =>
        val (newCont, proj) =
          cont.foldLeft[(ListMap[BsonField.Name, AccumOp[Fix[ExprOp]]],
                         ListMap[BsonField.Name, Reshape.Shape[ExprOp]])](
            (ListMap(), ListMap())) {
        ...
        if (newCont == cont)
          ...
          chain(src,
                ...
                $project[F](Reshape(proj), IgnoreId)).project.some
      case _ => None
    }

    def simplifyGroup[F[_]: Coalesce: Functor](op: Fix[F])(
        implicit ev: WorkflowOpCoreF :<: F): Fix[F] =
      op.transCata[Fix[F]](orOriginal(simplifyGroupƒ[F]))

    private def reorderOpsƒ[F[_]: Coalesce: Functor](
        implicit I: WorkflowOpCoreF :<: F): F[Fix[F]] => Option[F[Fix[F]]] = {
...

      {
        case I($SkipF(Embed(I($ProjectF(src0, shape, id))), count)) =>
          chain(src0, $skip[F](count), $project[F](shape, id)).project.some
        case I(
            $SkipF(Embed(
                     I($SimpleMapF(src0, fn @ NonEmptyList(MapExpr(_), INil()), scope))),
                   count)) =>
          chain(src0, $skip[F](count), $simpleMap[F](fn, scope)).project.some

        case I($LimitF(Embed(I($ProjectF(src0, shape, id))), count)) =>
          chain(src0, $limit[F](count), $project[F](shape, id)).project.some
        case I(
            $LimitF(Embed(
                      I($SimpleMapF(src0, fn @ NonEmptyList(MapExpr(_), INil()), scope))),
                    count)) =>
          chain(src0, $limit[F](count), $simpleMap[F](fn, scope)).project.some
        ...
        case I($MatchF(Embed(I(p @ $ProjectF(src0, shape, id))), sel)) =>
          ...
          rewriteSelector(sel, defs).map(sel =>
            chain(src0, $match[F](sel), $project[F](shape, id)).project)

        case I(
            $MatchF(
              Embed(
                I($SimpleMapF(src0, exprs @ NonEmptyList(MapExpr(jsFn), INil()), scope))),
              sel)) => {
            ...
            expr.simplify match {
              case Obj(values) =>
                values.toList.collect {
                  case (n, Ident(jsFn.param)) =>
                    DocField(BsonField.Name(n.value)) -> DocVar.ROOT()
                  case (n, Access(Ident(jsFn.param), Literal(Js.Str(x)))) =>
                    DocField(BsonField.Name(n.value)) -> DocField(BsonField.Name(x))
                }.toMap
              case SpliceObjects(srcs) =>
                srcs.map(defs).foldLeft(Map[DocVar, DocVar]())(_ ++ _)
              case _ => Map.empty
            }
          rewriteSelector(sel, defs(jsFn.expr)).map(sel =>
            chain(src0, $match[F](sel), $simpleMap[F](exprs, scope)).project)
        }
    ...
    @SuppressWarnings(Array("org.wartremover.warts.Recursion"))
    def get0(leaves: List[BsonField.Name],
             rs: List[Reshape[ExprOp]]): Option[Reshape.Shape[ExprOp]] = {
      (leaves, rs) match {
        case (_, Nil) =>
          $var(BsonField(leaves).map(DocVar.ROOT(_)).getOrElse(DocVar.ROOT())).right.some
    ...
    private def fixExpr(e: Fix[ExprOp], rs: List[Reshape[ExprOp]]): Option[Fix[ExprOp]] =
      e.cataM[Option, Fix[ExprOp]] {
        case $varF(ref) => get0(ref.path, rs).flatMap(_.toOption)
        case x          => Fix(x).some
      }

    private def inlineProject0(r: Reshape[ExprOp],
                               rs: List[Reshape[ExprOp]]): Option[Reshape[ExprOp]] =
      inlineProject($ProjectF((), r, IgnoreId), rs)

    def inlineProject[A](p: $ProjectF[A],
                         rs: List[Reshape[ExprOp]]): Option[Reshape[ExprOp]] = {
      val map = p.getAll
        .map {
          case (k, v) =>
            k -> (v match {
              case $include() =>
                get0(k.flatten.toList, rs)
                  .map(_.right)
                  .orElse(if (k == IdName) ().left.some
                  else none)
    ...
    def renameProjectGroup(
        r: Reshape[ExprOp],
        g: Grouped[ExprOp]): Option[ListMap[BsonField.Name, List[BsonField.Name]]] = {
      ...
      def multiListMap[A, B](ts: List[(A, B)]): ListMap[A, List[B]] =
        ts.foldLeft(ListMap[A, List[B]]()) {
          case (map, (a, b)) => map + (a -> (map.get(a).getOrElse(List[B]()) :+ b))
        }

      s.map(multiListMap)
    }

    def inlineProjectGroup(r: Reshape[ExprOp],
                           g: Grouped[ExprOp]): Option[Grouped[ExprOp]] = {
      for {
    ...
    def inlineProjectUnwindGroup(
        r: Reshape[ExprOp],
        unwound: DocVar,
        g: Grouped[ExprOp]): Option[(DocVar, Grouped[ExprOp])] = {
      for {
        names <- renameProjectGroup(r, g)
    ...
    def inlineGroupProjects[F[_]: Functor](g: $GroupF[Fix[F]])(
      ...
      def collectShapes: GAlgebra[(Fix[F], ?), F, (List[Reshape[ExprOp]], Fix[F])] = {
        case I($ProjectF(src, shape, _)) =>
          ((x: List[Reshape[ExprOp]]) => shape :: x).first(src._2)
      ...
      if (src == g.src) None
        ...
        val by = g.by.fold(inlineProject0(_, rs).map(_.left), fixExpr(_, rs).map(\/-(_)))
