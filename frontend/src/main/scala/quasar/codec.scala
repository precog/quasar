    ...
    def encode(data: Data): Option[Json] = {
      ...
      data match {
        ...
        case Timestamp(value) =>
          Json
            .obj(TimestampKey -> jString(value.atZone(UTC).format(dateTimeFormatter)))
            .some
    ...
    def decode(json: Json): DataEncodingError \/ Data =
      json.fold(
        ...
        obj => {
          ...
          def unpack[A](a: Option[A],
                        expected: String)(f: A => DataEncodingError \/ Data) =
          ...
          obj.toList match {
            ...
            case (`IntervalKey`, value) :: Nil =>
              unpack(value.string, "string value for $interval")(
                parseInterval(_).leftMap(err => ParseError(err.message)))
  ...
  val Readable = new DataCodec {
    ...
    def encode(data: Data): Option[Json] = {
      ...
      data match {
        ...
        case Obj(value) =>
          Json
            .obj(value.toList.map({ case (k, v) => encode(v) strengthL k }).unite: _*)
            .some
        case Arr(value) => Json.array(value.map(encode).unite: _*).some
        case Set(_)     => None
