...
final case class JoinHandler[WF[_], F[_]](
    run: (JoinType, JoinSource[WF], JoinSource[WF]) => F[WorkflowBuilder[WF]]) {
...
object JoinHandler {
  ...
  def padEmpty(side: BsonField): Fix[ExprOp] =
    $cond(
      $eq($size($var(DocField(side))), $literal(Bson.Int32(0))),
  ...
  def buildProjection[WF[_]](
      ...
      leftField: BsonField.Name,
      l: BsonField.Name => Fix[ExprOp],
      rightField: BsonField.Name,
      r: BsonField.Name => Fix[ExprOp]): WorkflowBuilder[WF] =
    DocBuilder(src, ListMap(leftField -> \&/-(l(leftField)), rightField -> \&/-(r(rightField))))
  ...
  def pipeline[M[_]: Monad, WF[_]: Functor: Coalesce: Crush: Crystallize](
      ...
      ev3: ExprOpOps.Uni[ExprOp]): JoinHandler[WF, OptionT[M, ?]] =
    JoinHandler({ (tpe, left, right) =>
      ...
      def lookup(
        ...
        def filterExists(wb: WorkflowBuilder[WF], field: BsonField): WorkflowBuilder[WF] =
          WB.filter(wb, List(docVarToExpr(DocField(field))), {
        ...
        lKey match {
          case HasThat($var(DocVar(_, Some(lField)))) =>
            ...
            generateWorkflow[M, WF](filtered).map {
              case (left, _) =>
                CollectionBuilder(
                  chain[Fix[WF]](left, $lookup(rColl, lName \ lField, rField, rName)),
                  Root(),
                  None)
            }
          ...
          case _ =>
            for {
              t <- generateWorkflow[M, WF](
                DocBuilder(
                  lSrc,
                  ListMap(lName -> docVarToExpr(DocVar.ROOT()), BsonField.Name("0") -> lKey)))
              (src, _) = t
            } yield
              DocBuilder(
                CollectionBuilder(
                  chain[Fix[WF]](src, $lookup(rColl, BsonField.Name("0"), rField, rName)),
                  Root(),
                  None),
                ListMap(
  ...
  def mapReduce[M[_]: Monad, WF[_]: Functor: Coalesce: Crush: Crystallize](
      implicit M: MonadError[M, PlannerError],
      ev0: WorkflowOpCoreF :<: WF,
      ev1: RenderTree[WorkflowBuilder[WF]],
      ev2: ExprOpOps.Uni[ExprOp]): JoinHandler[WF, M] =
    JoinHandler({
        (tpe, left0, right0) =>
          val WB = WorkflowBuilder.Ops[WF]
            ...
            $MapF.mapKeyVal(
              ...
              keyExpr match {
                ...
                case _ =>
                  jscore
                    .Obj(keyExpr
                      .map(_(base.toDocVar.toJs(jscore.ident("value"))))
                      .zipWithIndex
                      .foldLeft[ListMap[jscore.Name, JsCore]](ListMap[jscore.Name, JsCore]()) {
              ...
              Js.AnonObjDecl(
                List(
                  ...
                  (
                    rootField.asText,
                    Js.AnonElem(List(base.toDocVar.toJs(jscore.ident("value")).toJs)))))
            )
          ...
          def jsReduce(
              ...
              key: List[JsFn],
              rootField: BsonField.Name,
              otherField: BsonField.Name): (WorkflowBuilder[WF], Base => FixOp[WF]) =
          ...
          val (left, right, leftField, rightField) =
            (
              left0.keys.map(HasThis.unapply).sequence,
              right0.keys.map(HasThis.unapply).sequence) match {
              case (Some(js), _)
          ...
          def buildJoin(src: WorkflowBuilder[WF], tpe: JoinType): WorkflowBuilder[WF] =
            tpe match {
              ...
              case JoinType.LeftOuter =>
                FlatteningBuilder(
                  buildProjection(
                    WB.filter(src, List(docVarToExpr(DocField(leftField))), {
              ...
              case JoinType.RightOuter =>
                FlatteningBuilder(
                  buildProjection(
                    WB.filter(src, List(docVarToExpr(DocField(rightField))), {
                    ...
                    AnonFunDecl(
                      ...
                      List(
                        BinOp(
                          ...
                          Call(
                            Select(Select(Ident("result"), leftField.asText), "concat"),
                        ...
                        BinOp(
                          ...
                          Call(
                            Select(Select(Ident("result"), rightField.asText), "concat"),
  ...
  private def preferMapReduce[WF[_]: Coalesce: Crush: Crystallize: Functor](
    ...
    generateWorkflow[PlannerError \/ ?, WF](wb)
