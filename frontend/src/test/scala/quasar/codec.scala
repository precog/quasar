...
import quasar.DataEncodingError.{UnescapedKeyError, UnrepresentableDataError}
...
class DataCodecSpecs extends quasar.Qspec {
  ...
  def roundTrip(d: Data)(implicit C: DataCodec): Option[DataEncodingError \/ Data] =
  ...
  "Precise" should {
    ...
    "render" should {
      ...
      "encode dec with no fractional part" in {
        DataCodec.render(Data.Dec(2.0)) must beSome("2.0")
      }
      "encode timestamp" in {
        DataCodec.render(Data.Timestamp(Instant.parse("2015-01-31T10:30:00Z"))) must beSome(
          """{ "$timestamp": "2015-01-31T10:30:00.000Z" }""")
      }
      "encode date" in {
        DataCodec.render(Data.Date(LocalDate.parse("2015-01-31"))) must beSome(
          """{ "$date": "2015-01-31" }""")
      }
      "encode time" in {
        DataCodec.render(Data.Time(LocalTime.parse("10:30:00.000"))) must beSome(
          """{ "$time": "10:30:00.000" }""")
      }
      "encode interval" in {
        DataCodec.render(Data.Interval(Duration.parse("PT12H34M"))) must beSome(
          """{ "$interval": "PT12H34M" }""")
      }
      "encode obj" in {
      ...
      "encode obj with leading '$'s" in {
        DataCodec.render(
          Data.Obj(
            ListMap(
              "$a" -> Data.Int(1),
              "$date" -> Data.Timestamp(Instant.parse("2015-01-31T10:30:00Z"))))) must
          beSome(
            """{ "$obj": { "$a": 1, "$date": { "$timestamp": "2015-01-31T10:30:00.000Z" } } }""")
      }
      ...
      "encode array" in {
        DataCodec.render(Data.Arr(List(Data.Int(0), Data.Int(1), Data.Int(2)))) must beSome(
          "[ 0, 1, 2 ]")
      }
      "encode set" in {
        DataCodec.render(Data.Set(List(Data.Int(0), Data.Int(1), Data.Int(2)))) must beNone
      }
      "encode binary" in {
        DataCodec.render(Data.Binary.fromArray(Array[Byte](76, 77, 78, 79))) must beSome(
          """{ "$binary": "TE1OTw==" }""")
      }
      "encode objectId" in {
        DataCodec.render(Data.Id("abc")) must beSome("""{ "$oid": "abc" }""")
      }
    ...
    "parse" should {
      ...
      "re-parse very large Int value as Dec" in {
        roundTrip(Data.Int(LargeInt)) must beSome(
          Data.Dec(new java.math.BigDecimal(LargeInt.underlying)).right[DataEncodingError])
      }
      ...
      "fail with unescaped leading '$'" in {
        DataCodec.parse("""{ "$a": 1 }""") must beLeftDisjunction(
          UnescapedKeyError(jSingleObject("$a", jNumber(1))))
      }
  ...
  "Readable" should {
    ...
    "render" should {
      ...
      "encode timestamp" in {
        DataCodec.render(Data.Timestamp(Instant.parse("2015-01-31T10:30:00Z"))) must beSome(
          "\"2015-01-31T10:30:00Z\"")
      }
      "encode date" in {
        DataCodec.render(Data.Date(LocalDate.parse("2015-01-31"))) must beSome("\"2015-01-31\"")
      }
      "encode time" in {
        DataCodec.render(Data.Time(LocalTime.parse("10:30:00.000"))) must beSome("\"10:30\"")
      }
      "encode interval" in {
        DataCodec.render(Data.Interval(Duration.parse("PT12H34M"))) must beSome("\"PT12H34M\"")
      }
      "encode obj" in {
        // NB: more than 4, to verify order is preserved
        DataCodec.render(
