...
class QScriptOptimizeSpec extends quasar.Qspec with QScriptHelpers {
  ...
  "optimizer" should {
    "move subset before map" in {
      val from: QS[FreeQS] =
        QC.inj(
          Map(Free.roll(QST[QS].inject(QC.inj(Filter(HoleQS, BoolLit(true))))),
      ...
      val output: QS[Fix[QS]] =
        QC.inj(
          Union(
            RootR.embed,
            Free.roll(
              QST[QS].inject(QC.inj(Filter(Free.roll(QST[QS].inject(lBranch)),
                                           EqR(AddR(HoleF, IntLit(1)), IntLit(5)))))),
            Free.roll(
              QST[QS].inject(QC.inj(Filter(Free.roll(QST[QS].inject(rBranch)),
                                           EqR(AddR(HoleF, IntLit(1)), IntLit(5))))))
          ))
