...
object CPathComparator {
  ...
  def apply[
      ...
      @specialized(Boolean, Long, Double, AnyRef) B](lCol: Int => A, rCol: Int => B)(
  ...
  def apply(lPath: CPath, lCol: Column, rPath: CPath, rCol: Column): CPathComparator =
    (lCol, rCol) match {
  ...
  def apply(
    ...
    (lCol.leafTpe, rCol.leafTpe) match {
      ...
      case (CNum, CNum) =>
        new ArrayCPathComparator[BigDecimal, BigDecimal](lPath, lCol, rPath, rCol)
      ...
      case (CDate, CDate) =>
        new ArrayCPathComparator[ZonedDateTime, ZonedDateTime](lPath, lCol, rPath, rCol)
      case (tpe1, tpe2) =>
        val ordering =
          MaybeOrdering.fromInt(implicitly[scalaz.Order[CType]].apply(lCol.tpe, rCol.tpe).toInt)
  ...
  def apply(
    ...
    (lCol.leafTpe, rCol) match {
      ...
      case (CDate, rCol: DateColumn) =>
        new HalfArrayCPathComparator[ZonedDateTime, ZonedDateTime](lPath, lCol, rCol(_))
      case (tpe1, _) =>
        val ordering =
          MaybeOrdering.fromInt(implicitly[scalaz.Order[CType]].apply(tpe1, rCol.tpe).toInt)
        new CPathComparator with ArrayCPathComparatorSupport {
...
private[yggdrasil] final class HalfArrayCPathComparator[
    @specialized(Boolean, Long, Double) A,
    @specialized(Boolean, Long, Double) B](
    lPath: CPath,
    lCol: HomogeneousArrayColumn[_],
 ...
 */
private[yggdrasil] final class ArraySelector[@specialized(Boolean, Long, Double) A](
    implicit m: CTag[A]) {
