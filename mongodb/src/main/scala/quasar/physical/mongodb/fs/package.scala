...
import quasar.connector.{EnvErr, EnvErrT, EnvironmentError}
...
package object fs {
  ...
  type MongoQScriptCP[T[_[_]]] =
    qs.QScriptCore[T, ?] :\: qs.EquiJoin[T, ?] :/: Const[qs.ShiftedRead[AFile], ?]
  ...
  def parseConfig(uri: ConnectionUri): DefErrT[Task, MongoConfig] =
    (for {
  ...
  def compile(cfg: MongoConfig): BackendDef.DefErrT[Task, (MongoM ~> Task, Task[Unit])] =
    (effToTask(cfg) map (i => (foldMapNT[Eff, Task](i), Task.delay(cfg.client.close).void)))
      .liftM[DefErrT]

  val listContents: ADir => EitherT[MongoDbIO, FileSystemError, Set[PathSegment]] =
    dir =>
      EitherT(dirName(dir) match {
        case Some(_) =>
  ...
  def wfExec(
      client: MongoClient): DefErrT[Free[Task, ?], WorkflowExecutor[MongoDbIO, BsonCursor]] = {
    val run: EnvErrT[MongoDbIO, ?] ~> EnvErrT[Task, ?] =
      Hoist[EnvErrT].hoist(MongoDbIO.runNT(client))
    val runWf: EnvErrT[Task, WorkflowExecutor[MongoDbIO, BsonCursor]] = run(
      WorkflowExecutor.mongoDb)
  ...
  private def freeFsEffToTask: Free[PhysFsEff, ?] ~> Task =
    foldMapNT[PhysFsEff, Task](fsEffToTask)

  private def fsEffToTask: PhysFsEff ~> Task =
    λ[PhysFsEff ~> Task](
      _.run.fold(
  ...
  private def findDefaultDb: MongoDbIO[Option[DefaultDb]] =
    (for {
      ...
      _ <- MongoDbIO.dropCollection(Collection(dbName, coll)).attempt.void.liftM[OptionT]
  ...
  private[fs] def asyncClientDef[S[_]](
      ...
      implicit
      S0: Task :<: S): DefErrT[Free[S, ?], MongoClient] = {
    ...
    util.createAsyncMongoClient[Eff](uri).foldMap[DefM](liftTask :+: evalEnvErr :+: evalCfgErr)
  }
}
