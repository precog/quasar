...
object CoreMap extends Serializable {
  ...
  def changeFreeMap[T[_[_]]: RecursiveT](f: FreeMap[T]): PlannerError \/ (Data => Data) =
    f.cataM(interpretM(κ(ι[Data].right[PlannerError]), change[T, Data]))

  def changeJoinFunc[T[_[_]]: RecursiveT](
      f: JoinFunc[T]): PlannerError \/ ((Data, Data) => Data) =
    f.cataM(
        interpretM[PlannerError \/ ?,
                   MapFuncCore[T, ?],
                   JoinSide,
                   ((Data, Data)) => Data]((js: JoinSide) =>
                                             (js match {
  ...
  def changeReduceFunc[T[_[_]]: RecursiveT](f: Free[MapFuncCore[T, ?], ReduceIndex])
    ...
    f.cataM(
        interpretM(_.idx.fold((_: (Data, List[Data]))._1)(i => _._2(i)).right,
                   change[T, (Data, List[Data])]))
      .map(f => (l: Data, r: List[Data]) => f((l, r)))
  ...
  def change[T[_[_]]: RecursiveT, A]
    ...
    case StartOfDay(f) =>
      (f >>> {
        case d @ Data.Timestamp(_) =>
          temporalTrunc(TemporalPart.Day, d).fold(κ(undefined), ι)
        case Data.Date(v) => Data.Timestamp(v.atStartOfDay(ZoneOffset.UTC).toInstant)
        case _            => undefined
      }).right
    case TemporalTrunc(p, f) =>
      (f >>> (d => temporalTrunc(p, d).fold(κ(undefined), ι))).right
    case TimeOfDay(f) =>
      (f >>> {
    ...
    case ExtractIsoDayOfWeek(f) =>
      (f >>> {
        case Data.Timestamp(v) =>
          fromDateTime(v)(dt => Data.Int(dt.getDayOfWeek().getValue()))
        case Data.Date(v) => Data.Int(v.getDayOfWeek().getValue())
        case _            => undefined
      }).right
    case ExtractIsoYear(f) =>
      (f >>> {
        case _ => undefined
      }).right
    case ExtractMicroseconds(f) =>
      (f >>> {
        case Data.Timestamp(v) =>
          fromDateTime(v) { dt =>
    ...
    case ExtractMonth(f) =>
      (f >>> {
        case Data.Timestamp(v) =>
          fromDateTime(v)(dt => Data.Int(dt.getMonth().getValue()))
        case Data.Date(v) => Data.Int(v.getMonth().getValue())
        case _            => undefined
      }).right
    case ExtractQuarter(f) =>
      (f >>> {
        case Data.Timestamp(v) =>
    ...
    case Integer(f) =>
      (f >>> {
        case Data.Str(a) =>
          \/.fromTryCatchNonFatal(Data.Int(BigInt(a))).fold(κ(Data.NA), ι)
        case _ => undefined
      }).right
    case Decimal(f) =>
      (f >>> {
        case Data.Str(a) =>
          \/.fromTryCatchNonFatal(Data.Dec(BigDecimal(a))).fold(κ(Data.NA), ι)
        case _ => undefined
      }).right
    case Null(f) =>
      (f >>> {
    ...
    case DeleteField(fSrc, fField) =>
      ((x: A) =>
        (fSrc(x), fField(x)) match {
          case (Data.Obj(m), Data.Str(field)) if m.isDefinedAt(field) =>
            Data.Obj(m - field)
          case _ => undefined
        }).right
    case Range(fFrom, fTo) =>
      ((x: A) =>
        (fFrom(x), fTo(x)) match {
          case (Data.Int(a), Data.Int(b)) if (a <= b) =>
            Data.Set((a to b).map(Data.Int(_)).toList)
        }).right
    case Guard(f1, fPattern, f2, ff3) => f2.right
    case TypeOf(f) =>
      (f >>> ((d: Data) =>
        d.dataType.toPrimaryType.fold(Data.NA: Data)(p => Data.Str(PrimaryType.name(p)))))
        .right[PlannerError]
  ...
  private def divide(d1: Data, d2: Data): Data = (d1, d2) match {
    ...
    case (Data.Dec(a), Data.Int(b)) =>
      Data.Dec(a(BigDecimal.defaultMathContext) / BigDecimal(b))
  ...
  private def between(d1: Data, d2: Data, d3: Data): Data = (d1, d2, d3) match {
    ...
    case (Data.Time(a), Data.Time(b), Data.Time(c)) =>
      Data.Bool(b.compareTo(a) <= 0 && a.compareTo(c) <= 0)
    case (Data.Bool(a), Data.Bool(b), Data.Bool(c)) =>
      (a, b, c) match {
    ...
    (dStr, dFrom, dCount) match {
      case (Data.Str(str), Data.Int(from), Data.Int(count)) =>
        \/.fromTryCatchNonFatal(
          Data.Str(StringLib.safeSubstring(str, from.toInt, count.toInt)))
          .fold(κ(Data.NA), ι)
      case _ => undefined
    }
