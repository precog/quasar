...
abstract class StructuralPlannerSpec[F[_]: Monad, FMT](
  ...
  implicit val arbitraryData: Arbitrary[Data] = {
    ...
    val genDate: Gen[LocalDate] = genSeconds map (s =>
      LocalDate.ofEpochDay(s / secondsInDay))
  ...
  implicit val arbMapEntry: Arbitrary[MapEntry] =
    Arbitrary(
      (genStrKey |@| arbitrary[Data])((key, value) => MapEntry(key.asString, value)))
  ...
  xquerySpec(bn => s"Structural Planner (${bn.name})") { evalM =>
    ...
    "arrayAppend" >> {
      "append to non-empty array" >> prop { (x0: Data, y0: Data) =>
        val r = evalF((lit(x0) |@| lit(y0))((x, y) =>
          SP.singletonArray(x) >>= (SP.arrayAppend(_, y))).join)
      ...
      "append to empty array" >> prop { x0: Data =>
        val r =
          evalF((lit(emptyArr) |@| lit(x0))((emp, x) => SP.arrayAppend(emp, x)).join)
    ...
    "arrayConcat" >> {
      "two non-empty arrays" >> prop { (x0: Data, y0: Data) =>
        val r = evalF(
          (lit(x0) |@| lit(y0))((x, y) =>
            (SP.singletonArray(x) |@| SP
              .singletonArray(y))(SP.arrayConcat(_, _)).join).join)
    ...
    "objectInsert" >> {
      "adds new assoc to non-empty object" >> prop {
        (testCase: WithoutKeyTestCase, y: Data) =>
          val res = evalF(
      ...
      "adds new assoc to empty object" >> prop { (key: StrKey, y: Data) =>
        val res = evalF(
          (lit(emptyObj) |@| key.asString.xs.point[F] |@| lit(y))(SP.objectInsert).join)
