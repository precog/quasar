...
trait StructuralLib extends Library {
  ...
  val MakeObject = BinaryFunc(
    ...
    partialUntyperV[nat._2] {
      case Const(Data.Obj(map)) => map.headOption match {
        case Some((key, value)) => success(Sized[IS](Const(Data.Str(key)), Const(value)))
      ...
      case Obj(map, uk) => map.headOption.fold(
        uk.fold[Func.VDomain[nat._2]](
          ...
          t => success(Sized[IS](Str, t)))) {
        case (key, value) => success(Sized[IS](Const(Data.Str(key)), value))
      }
    })
  ...
  val MakeArray = UnaryFunc(
    ...
    partialUntyper[nat._1] {
      case Const(Data.Arr(List(elem))) => Sized[IS](Const(elem))
      case Arr(List(elemType))         => Sized[IS](elemType)
      case FlexArr(_, _, elemType)     => Sized[IS](elemType)
    })
  ...
  val ObjectConcat: BinaryFunc = BinaryFunc(
    ...
    partialTyperV[nat._2] {
      ...
      case Sized(Const(o1 @ Data.Obj(_)), o2) => ObjectConcat.tpe(Sized[IS](o1.dataType, o2))
      case Sized(o1, Const(o2 @ Data.Obj(_))) => ObjectConcat.tpe(Sized[IS](o1, o2.dataType))
  ...
  val ArrayConcat: BinaryFunc = BinaryFunc(
    ...
    partialTyperV[nat._2] {
      ...
      case Sized(Const(a1 @ Data.Arr(_)), a2) => ArrayConcat.tpe(Sized[IS](a1.dataType, a2))
      case Sized(a1, Const(a2 @ Data.Arr(_))) => ArrayConcat.tpe(Sized[IS](a1, a2.dataType))
  ...
  val ConcatOp = BinaryFunc(
    ...
    AnyArray ⨿ Str,
    Sized[IS](AnyArray ⨿ Str, AnyArray ⨿ Str),
    noSimplification,
    partialTyperV[nat._2] {
      case Sized(t1, t2) if t1.arrayLike && t2.contains(AnyArray ⨿ Str) => ArrayConcat.tpe(Sized[IS](t1, FlexArr(0, None, Top)))
      case Sized(t1, t2) if t1.contains(AnyArray ⨿ Str) && t2.arrayLike => ArrayConcat.tpe(Sized[IS](FlexArr(0, None, Top), t2))
      case Sized(t1, t2) if t1.arrayLike && t2.arrayLike                => ArrayConcat.tpe(Sized[IS](t1, t2))
      ...
      case Sized(t1, t2) if Str.contains(t1) && Str.contains(t2)            => StringLib.Concat.tpe(Sized[IS](t1, t2))
    ...
    partialUntyperV[nat._2] {
      case x if x.contains(AnyArray ⨿ Str) => success(Sized[IS](AnyArray ⨿ Str, AnyArray ⨿ Str))
  ...
  val DeleteField: BinaryFunc = BinaryFunc(
    ...
    partialUntyperV[nat._2] {
      ...
      case Obj(map, _)              => success(Sized[IS](Obj(map, Some(Top)), Str))
    })
  ...
  val FlattenMap = UnaryFunc(
    ...
    untyper[nat._1](tpe => success(Sized[IS](Obj(Map(), Some(tpe))))))
  ...
  val FlattenArray = UnaryFunc(
    ...
    untyper[nat._1](tpe => success(Sized[IS](FlexArr(0, None, tpe)))))
  ...
  val FlattenMapKeys = UnaryFunc(
    ...
    untyper[nat._1](tpe => success(Sized[IS](Obj(Map(), Some(Top))))))
  ...
  val FlattenArrayIndices = UnaryFunc(
    ...
    partialUntyper[nat._1] {
      case Int => Sized[IS](FlexArr(0, None, Top))
    })
  ...
  object MakeObjectN {
    ...
    def apply[T[_[_]]: Corecursive](args: (T[LogicalPlan], T[LogicalPlan])*): LogicalPlan[T[LogicalPlan]] =
      args.toList match {
        ...
        case x :: xs  =>
          xs.foldLeft(MakeObject(Sized[IS](x._1, x._2)))((a, b) =>
            ObjectConcat(Sized[IS](a.embed, MakeObject(Sized[IS](b._1, b._2)).embed)))
      }
  ...
  object MakeArrayN {
    def apply[T[_[_]]: Corecursive](args: T[LogicalPlan]*): LogicalPlan[T[LogicalPlan]] =
      args.map(x => MakeArray(Sized[IS](x))) match {
        case Nil      => ConstantF(Data.Arr(Nil))
        case t :: Nil => t
        case mas      => mas.reduce((t, ma) => ArrayConcat(Sized[IS](t.embed, ma.embed)))
      }
