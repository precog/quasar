...
object WorkflowBuilderF {
  ...
  implicit def equal[F[_]: Coalesce](
      implicit ev: WorkflowOpCoreF :<: F): Delay[Equal, WorkflowBuilderF[F, ?]] =
    new Delay[Equal, WorkflowBuilderF[F, ?]] {
      def apply[A](eq: Equal[A]) = {
        implicit val eqA: Equal[A] = eq

        Equal.equal((v1: WorkflowBuilderF[F, A], v2: WorkflowBuilderF[F, A]) =>
          (v1, v2) match {
            case (CollectionBuilderF(g1, b1, s1), CollectionBuilderF(g2, b2, s2)) =>
              g1 == g2 && b1 == b2 && s1 ≟ s2
            case (
                v1 @ ShapePreservingBuilderF(s1, i1, _),
                v2 @ ShapePreservingBuilderF(s2, i2, _)) =>
  ...
  implicit def traverse[F[_]]: Traverse[WorkflowBuilderF[F, ?]] =
    new Traverse[WorkflowBuilderF[F, ?]] {
      def traverseImpl[G[_], A, B](fa: WorkflowBuilderF[F, A])(f: A => G[B])(
          implicit G: Applicative[G]): G[WorkflowBuilderF[F, B]] =
        fa match {
  ...
  implicit def renderTree[F[_]: Coalesce: Functor](
      implicit
      RG: RenderTree[ListMap[BsonField.Name, AccumOp[Fix[ExprOp]]]],
      RF: RenderTree[Fix[F]],
      ev: WorkflowOpCoreF :<: F): Delay[RenderTree, WorkflowBuilderF[F, ?]] =
    Delay.fromNT(λ[RenderTree ~> (RenderTree ∘ WorkflowBuilderF[F, ?])#λ](rt => {
      ...
      RenderTree.make {
        ...
        case spb @ ShapePreservingBuilderF(src, inputs, op) =>
          ...
          NonTerminal(nt, None, rt.render(src) :: (inputs.map(_.render) :+ spb.dummyOp.render))
        case ExprBuilderF(src, expr) =>
          NonTerminal("ExprBuilder" :: nodeType, None, rt.render(src) :: expr.render :: Nil)
        case DocBuilderF(src, shape) =>
          ...
          NonTerminal(
            ...
            None,
            rt.render(src) ::
              NonTerminal("Shape" :: nt, None, shape.toList.map {
              case (name, expr) =>
...
object WorkflowBuilder {
  ...
  def semiAlignExpr[F[_]: Traverse](fa: F[Expr]): Option[F[Fix[ExprOp]]] =
    fa.traverse(HasThat.unapply)

  def alignExpr[F[_]: Traverse](fa: F[Expr]): Option[F[JsFn] \/ F[Fix[ExprOp]]] =
    Bitraverse[\/].leftTraverse
      .sequence(fa.traverse(HasThat.unapply) \/> fa.traverse(HasThis.unapply))
   ...
   */
  final case class CollectionBuilderF[F[_]](src: Fix[F], base: Base, struct: Schema)
      extends WorkflowBuilderF[F, Nothing]
  ...
  final case class GroupBuilderF[F[_], A](src: A, keys: List[Expr], contents: GroupContents)
      extends WorkflowBuilderF[F, A]
  object GroupBuilder {
    def apply[F[_]](src: WorkflowBuilder[F], keys: List[Expr], contents: GroupContents) =
  ...
  object StructureType {
    final case class Array[A](field: A, includeIndex: IdStatus) extends StructureType[A]
    final case class Object[A](field: A, includeKey: IdStatus) extends StructureType[A]

    implicit val StructureTypeTraverse: Traverse[StructureType] =
      new Traverse[StructureType] {
        def traverseImpl[G[_], A, B](fa: StructureType[A])(f: A => G[B])(
            implicit G: Applicative[G]): G[StructureType[B]] =
          fa match {
  ...
  private def rewriteExpr(expr: Expr, base: Expr)(
      implicit exprOps: ExprOpOps.Uni[ExprOp]): Option[Expr] =
    (expr, base) match {
  ...
  def toWorkflow[M[_]: Monad, WF[_]: Coalesce](
      implicit M: MonadError_[M, PlannerError],
      ev0: WorkflowOpCoreF :<: WF,
      ev1: RenderTree[WorkflowBuilder[WF]],
      exprOps: ExprOpOps.Uni[ExprOp]): AlgebraM[M, WorkflowBuilderF[WF, ?], (Fix[WF], Base)] = {
      ...
      inputs match {
        ...
        case _ =>
          inputs.traverse {
            ...
            op.lift(pairs.map(_._1))
              .fold(M.raiseError[(Fix[WF], Base)](
                UnsupportedFunction(set.Filter, Some("failed to build operation"))))(
                op =>
                  toWorkflow[M, WF].apply(DocBuilderF(
                    (g, b),
                    pairs.toListMap + (srcName -> docVarToExpr(DocVar.ROOT())))) ∘ {
                    case (graph, _) => (chain(graph, op), Field(srcName))
                })
          }(
            op.lift(_)
              .fold(M.raiseError[(Fix[WF], Base)](
                UnsupportedFunction(set.Filter, Some("failed to build operation"))))(op =>
                (chain(g, op), b).point[M]))
      }
    ...
    case ExprBuilderF((g, b), expr) =>
      (rewriteExprPrefix(expr, b) match {
        ...
        case \&/.This(js) =>
          (chain(g, $simpleMap[WF](NonEmptyList(MapExpr(js)), ListMap())), Root())
      }).point[M]
    case DocBuilderF((wf, base), shape) =>
      alignExpr(rewriteDocPrefix(shape, base)).fold(
        ...
        s =>
          shape.keys.toList.toNel.fold(M.raiseError[(Fix[WF], Base)](
            InternalError fromMsg "A shape with no fields does not make sense"))(
            fields =>
              (
                chain(
                  wf,
                  s.fold(
                    jsExprs => {
                      $simpleMap[WF](NonEmptyList(MapExpr(JsFn(jsBase, jscore.Obj(jsExprs.map {
                        case (name, expr) =>
                          jscore.Name(name.asText) -> expr(jscore.Ident(jsBase))
                      })))), ListMap())
                    },
    ...
    case GroupBuilderF((gʹ, bʹ), keysʹ, content) =>
      ...
      semiAlignExpr(keys).fold {
        ...
        toWorkflow[M, WF].apply(
          DocBuilderF(
            (gʹ, Root()),
            ks.toListMap + (BsonField.Name("content") -> docVarToExpr(bʹ.toDocVar)))) strengthR
          ((Field(BsonField.Name("content")): Base, ks.map(key => $var(DocField(key._1)))))
      }(ks => ((gʹ, bʹ), (bʹ, ks)).point[M]) >>= {
        case ((g, b), (c, keys)) =>
          val key: Reshape.Shape[ExprOp] =
            (keys.all {
              case $literal(_) => true
              case _ => false
            }).fold(\/-($literal(Bson.Null)), -\/(Reshape(keys.zipWithIndex.map {
    ...
    case UnionBuilderF((lGraph, lBase), (rGraph, rBase)) =>
      if (lBase == rBase)
        ...
        (toWorkflow[M, WF].apply(
          DocBuilderF(
            (lGraph, Root()),
            ListMap(BsonField.Name("0") -> docVarToExpr(lBase.toDocVar)))) ⊛
          toWorkflow[M, WF].apply(
            DocBuilderF(
              (rGraph, Root()),
              ListMap(BsonField.Name("0") -> docVarToExpr(rBase.toDocVar)))))(
          (l, r) =>
            (
              $foldLeft(
                l._1,
    ...
    (base, struct) match {
      ...
      case (_, None) =>
        (
          chain(
            graph,
            $project[F](Reshape(ListMap(ExprName -> \/-($var(base.toDocVar)))), ExcludeId)),
          Field(ExprName))
      case (_, Some(fields)) =>
        (
          chain(
            ...
            $project[F](
              Reshape(
                fields.map(name => name -> \/-($var((base \ name).toDocVar))).toList.toListMap),
  ...
  def build[M[_]: Monad, F[_]: Coalesce](wb: WorkflowBuilder[F])(
      ...
      ev0: WorkflowOpCoreF :<: F,
      ev1: RenderTree[WorkflowBuilder[F]],
      ev2: ExprOpOps.Uni[ExprOp]): M[Fix[F]] =
    (wb: Fix[WorkflowBuilderF[F, ?]]).cataM(
      AlgebraMZip[M, WorkflowBuilderF[F, ?]].zip(toWorkflow[M, F], schema.generalizeM[M])) ∘ {
      case ((graph, Root()), _) => graph
      case ((graph, base), struct) => shift(base, struct, graph)._1
    }

  def asLiteral[F[_]](wb: WorkflowBuilder[F])(
      implicit ev0: WorkflowOpCoreF :<: F): Option[Bson] = wb.unFix match {
  ...
  final class Ops[F[_]: Coalesce](
      implicit ev0: WorkflowOpCoreF :<: F,
      ev1: ExprOpOps.Uni[ExprOp]) {
    ...
    def filter(
        src: WorkflowBuilder[F],
        those: List[Expr],
        sel: PartialFunction[List[BsonField], Selector]): WorkflowBuilder[F] =
    ...
    def groupBy(
        src: Fix[WorkflowBuilderF[F, ?]],
        keys: List[Expr],
        contents: GroupContents): WorkflowBuilder[F] =
      keys match {
        case List(HasThat($$ROOT)) =>
          findKeys(src).fold(
            ...
            case Subset(ks) =>
              GroupBuilder(src, ks.toList.map(k => docVarToExpr(DocField(k))), contents)
          }
    ...
    def sortBy(
        src: WorkflowBuilder[F],
        keys: List[Expr],
        sortTypes: List[SortDir]): WorkflowBuilder[F] =
  ...
  object Ops {
    implicit def apply[F[_]: Coalesce](
        implicit ev0: WorkflowOpCoreF :<: F,
        ev1: ExprOpOps.Uni[ExprOp]): Ops[F] =
      new Ops[F]
  }
}
