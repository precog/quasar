...
trait ReductionFinderSpecs[M[+ _]] extends EvaluatorSpecification[M] {
  ...
  "mega reduce" should {
    ...
    "in a reduction of a singleton" in {
      ...
      val megaR = dag.MegaReduce(List((trans.Leaf(trans.Source), List(input.red))),
    ...
    "in a single reduction" in {
      ...
      val input =
        dag.Reduce(Count, dag.AbsoluteLoad(Const(CString("/foo"))(line))(line))(line)
    ...
    "in joins where transpecs are eq, wrap object, operate, filter" in {
      ...
      val nonEqTrans =
        trans.Map1(trans.Equal(fooDerefTrans, trans.ConstLiteral(CNum(5), fooDerefTrans)),
                   Unary.Comp.f1)
      val objTrans = trans.WrapObject(trans.Leaf(trans.Source), "bar")
      val opTrans = op1ForUnOp(Neg).spec(
    ...
    "where three reductions use three different trans specs" in {
      ...
      val load =
        dag.AbsoluteLoad(Const(CString("/hom/heightWeightAcrossSlices"))(line))(line)
      ...
      val height =
        Join(DerefObject, Cross(None), load, Const(CString("height"))(line))(line)
      val weight =
        Join(DerefObject, Cross(None), load, Const(CString("weight"))(line))(line)
    ...
    "in a split that contains two reductions of the same dataset" in {
      ...
      val megaR =
        MegaReduce(List((trans.Leaf(trans.Source), List(red1.red, red2.red))), parent)
  ...
  "reduction finder" should {
    ...
    "in a join where only one side is a reduction" in {
      ...
      "left" in {
        ...
        val input =
          Join(Add, Cross(Some(CrossRight)), r, dag.Operate(Neg, load)(line))(line)
