...
object MetaStoreLocation {
  ...
  final class Ops[S[_]](implicit val ev: MetaStoreLocation :<: S)
      extends LiftedOps[MetaStoreLocation, S] {
  ...
  object impl {
    ...
    def default(
        ref: TaskRef[MetaStore],
        persist: DbConnectionConfig => MainTask[Unit]): MetaStoreLocation ~> Task =
      λ[MetaStoreLocation ~> Task] {
        ...
        case Set(conn, initialize) =>
          (for {
            ...
            m <- MetaStore
              .connect(conn, initialize || conn.isInMemory, currentSchemas)
              .leftMap(_.message)
            // Persist the change, if persisting fails, shutdown the new metastore connection and fail the change
            _ <- EitherT(
              persist(m.connectionInfo).foldM(
                persistFailure => m.shutdown.as(persistFailure.left),
                _ => ().right.point[Task]))
    ...
    def constant(config: DbConnectionConfig): MetaStoreLocation ~> Task =
      λ[MetaStoreLocation ~> Task] {
        ...
        case Set(conn, initialize) =>
          Task.fail(new Exception("This implementation does not allow changing MetaStore"))
      }
  }
}
