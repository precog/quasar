...
trait BlockStoreColumnarTableModule[M[+ _]] extends ColumnarTableModule[M] {
  ...
  trait BlockStoreColumnarTableCompanion extends ColumnarTableCompanion {
    ...
    case class WriteState(jdbmState: JDBMState,
                ...
                case (nextKeyTransform, kslice) => {
                  ...
                  if (keyColumnRefs.nonEmpty) {
                    ...
                    writeRawSlices(kslice,
                      ...
                      storeTransformed(newJdbmState,
                                       tail,
                                       (nextKeyTransform, streamId) :: updatedTransforms)
                    }
                  } else {
                    M.point(
                      (jdbmState, (nextKeyTransform, streamId) :: updatedTransforms))
                  }
                }
              }
    ...
    protected def writeRawSlices(kslice: Slice,
      ...
      jdbmState.indices.get(indexMapKey) map {
        ...
        case (index, jdbmState) =>
          ...
          jdbmState.copy(indices = jdbmState.indices + (indexMapKey -> newIndex),
        ...
        jdbmState.copy(indices = jdbmState.indices + (indexMapKey -> sortedSlice),
                       insertCount = 0)
      }
    }
    ...
    def loadTable(mergeEngine: MergeEngine[SortingKey, SortBlockData],
      ...
      def cellsMs: Stream[M[Option[CellState]]] =
        indices.values.toStream.zipWithIndex map {
          ...
          case (SliceIndex(name, dbFile, _, _, _, keyColumns, valColumns, count),
                index) =>
            val sortProjection = new JDBMRawSortProjection[M](dbFile,
                                                              name,
                                                              keyColumns,
                                                              valColumns,
                                                              sortOrder,
      ...
      def hashJoin(index: Slice, table: Table, flip: Boolean): M[Table] = {
        ...
        composeSliceTransform(indexKeySpec).advance(index) map {
          case (_, indexKey) =>
            ...
            Table(joinWithHash(table.slices, initKeyTrans, initJoinTrans, hashed),
                  UnknownSize)
        }
      }
  ...
  class SingletonTable(slices0: StreamT[M, Slice]) extends Table(slices0, ExactSize(1)) {
    ...
    override def compact(spec: TransSpec1, definedness: Definedness = AnyDefined): Table =
      this
