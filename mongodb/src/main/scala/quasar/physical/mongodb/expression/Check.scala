...
/** Runtime type checks, exploiting MongoDB's total ordering for Bson values,
  ...
  */
final class Check[T, EX[_]](implicit
...
object Check {
  ...
  def unapply[T: Equal, EX[_]](expr: T)(implicit TR: Recursive.Aux[T, EX],
                                        TC: Corecursive.Aux[T, EX],
                                        EX: Functor[EX],
                                        ev1: ExprOpCoreF :<: EX): Option[(T, Type)] = {
    ...
    expr match {
      ...
      case IsBetween(Bson.Arr(Nil), x, Bson.Binary(`minBinary`)) =>
        (x, Type.AnyArray).some
      case IsBetween(Bson.Binary(`minBinary`), x, Bson.ObjectId(`minOid`)) =>
        (x, Type.Binary).some
      ...
      case IsBetween(Bson.Doc(`nilMap`), x, Bson.Binary(`minBinary`)) =>
        (x, Type.AnyObject ⨿ Type.AnyArray).some
      case IsBetween(Bson.Binary(`minBinary`), x, `minRegex`) =>
        (x, Type.Binary ⨿ Type.Id ⨿ Type.Bool ⨿ Type.Date ⨿ Type.Timestamp).some
      case IsBetween(Bson.Bool(false), x, `minRegex`) =>
        (x, Type.Bool ⨿ Type.Date ⨿ Type.Timestamp).some
      case IsBetweenExcl(Bson.Null, x, Bson.Doc(`nilMap`)) =>
        (x, Type.Numeric ⨿ Type.Str).some

      case $or($lt(x1, $literal(Bson.Doc(`nilMap`))),
               IsBetween(Bson.ObjectId(`minOid`), x2, `minRegex`)) if x1 ≟ x2 =>
        (x1,
         Type.Null ⨿ Type.Numeric ⨿ Type.Str ⨿ Type.Id ⨿ Type.Bool ⨿ Type.Date ⨿ Type.Timestamp).some
  ...
  object IsBetween {
    def unapply[T: Equal, EX[_]](expr: T)(
        implicit TR: Recursive.Aux[T, EX],
        TC: Corecursive.Aux[T, EX],
        EX: Functor[EX],
        ev1: ExprOpCoreF :<: EX): Option[(Bson, T, Bson)] =
      expr match {
        case $and($lte($literal(const1), x1), $lt(x2, $literal(const2))) if x1 ≟ x2 =>
  ...
  object IsBetweenExcl {
    def unapply[T: Equal, EX[_]](expr: T)(
        ...
        ev1: ExprOpCoreF :<: EX): Option[(Bson, T, Bson)] =
      expr match {
        case $and($lt($literal(const1), x1), $lt(x2, $literal(const2))) if x1 ≟ x2 =>
          (const1, x1, const2).some
        case _ => None
      }
  }
  ...
  object IsBetweenIncl {
    def unapply[T: Equal, EX[_]](expr: T)(
        ...
        ev1: ExprOpCoreF :<: EX): Option[(Bson, T, Bson)] =
      expr match {
        case $and($lte($literal(const1), x1), $lte(x2, $literal(const2))) if x1 ≟ x2 =>
          (const1, x1, const2).some
        case _ => None
      }
  }
