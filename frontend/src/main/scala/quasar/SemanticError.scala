...
object SemanticError {
  ...
  final case class DomainError(data: Data, hint: Option[String]) extends SemanticError {
    def message =
      s"The data '${data.shows}' did not fall within its expected domain" + hint
        .map(": " + _)
        .getOrElse("")
  }

  final case class AmbiguousFunctionInvoke(name: CIName, from: List[(CIName, ADir)])
      extends SemanticError {
    def fullyQualifiedFuncs = from.map {
      case (name, dir) => posixCodec.printPath(dir) + name.value
    }
    def message = {
  ...
  final case class FunctionNotFound(name: CIName) extends SemanticError {
    def message =
      s"The function '${name.shows}' could not be found in the standard library"
  }
  final case class InvalidFunctionDefinition(funcDef: FunctionDecl[Fix[Sql]],
                                             reason: String)
      extends SemanticError {
    def message = s"The function '${funcDef.name.shows}' is invalid because: $reason"
  }
  final case class TypeError(expected: Type, actual: Type, hint: Option[String])
      extends SemanticError {
    def message =
      s"Expected type ${expected.shows} but found ${actual.shows}" + hint
        .map(": " + _)
        .getOrElse("")
  }
  final case class VariableParseError(vari: VarName,
                                      value: VarValue,
                                      cause: quasar.sql.ParsingError)
      extends SemanticError {
    def message =
      s"The variable ${vari.toString} should contain a SQL expression but was `${value.value}` (${cause.message})"
  }
  ...
  final case class WrongArgumentCount(funcName: CIName, expected: Int, actual: Int)
      extends SemanticError {
    def message =
      s"Wrong number of arguments for function '${funcName.shows}': expected $expected but found $actual"
  }
  final case class InvalidStringCoercion(str: String, expected: String \/ List[String])
      extends SemanticError {
    def message = {
      val expectedString =
  ...
  final case class AmbiguousReference(node: Fix[Sql], relations: List[SqlRelation[Unit]])
      extends SemanticError {
    def message =
      "The expression '" + pprint(node) + "' is ambiguous and might refer to any of the tables " + relations
        .mkString(", ")
  }
  ...
  final case class DateFormatError[N <: Nat](func: GenericFunc[N],
                                             str: String,
                                             hint: Option[String])
      extends SemanticError {
    def message =
      s"Date/time string could not be parsed as ${func.shows}: $str" + hint
        .map(" (" + _ + ")")
        .getOrElse("")
  }
  final case class InvalidPathError(path: Path[_, File, _], hint: Option[String])
      extends SemanticError {
    def message =
      "Invalid path: " + posixCodec.unsafePrintPath(path) + hint
        .map(" (" + _ + ")")
        .getOrElse("")
  }
  final case class UnexpectedDatePart(part: String) extends SemanticError {
    def message =
      s"""Invalid selector for DATE_PART: $part (expected "century", "day", etc.)"""
  }
  ...
  val duplicateAlias: Prism[SemanticError, String] =
    Prism.partial[SemanticError, String] {
      case DuplicateAlias(name) => name
    }(DuplicateAlias(_))

  val wrongArgumentCount: Prism[SemanticError, (CIName, Int, Int)] =
    Prism.partial[SemanticError, (CIName, Int, Int)] {
      case WrongArgumentCount(name, expected, found) => (name, expected, found)
    }(WrongArgumentCount.tupled)

  val compiledSubtableMissing: Prism[SemanticError, String] =
    Prism.partial[SemanticError, String] {
