...
import quasar.physical.couchbase._, common.ContextReader, N1QL.{Id, Union, Unreferenced, _},
planner.Planner._, Select.{Filter, Value, _}, Case._
...
final class QScriptCorePlanner[
  ...
  def plan: AlgebraM[F, QScriptCore[T, ?], T[N1QL]] = {
    ...
    case LeftShift(src, struct, id, repair) =>
      for {
        ...
        id2 <- genId[T[N1QL], F]
        id3 <- genId[T[N1QL], F]
        s <- struct.cataM(interpretM(κ(id1.embed.η[F]), mapFuncPlanner[T, F].plan))
        sr = ArrRange(int(0), LengthArr(s).embed, none)
        u <- (id match {
          case IdOnly =>
            IfNull(ObjNames(s).embed, sr.embed)
          case IncludeId =>
            Case(
    ...
    case qscript.Reduce(src, bucket, reducers, repair) =>
      for {
        ...
        red <- reducers.traverse(
          _.traverse(processFreeMap(_, id1)) >>=
            reduceFuncPlanner[T, F].plan)
        rep <- repair.cataM(
          interpretM(
            _.idx
              .fold(
                i => SelectElem(SelectElem(ArrAgg(b).embed, int(0)).embed, int(i)).embed,
                red(_))
              .point[F],
            mapFuncPlanner[T, F].plan))
      } yield {
    ...
    case qscript.Union(src, lBranch, rBranch) =>
      for {
        ...
        idRB <- genId[T[N1QL], F]
        lb <- lBranch.cataM(
          interpretM(κ(idLB.embed.η[F]), Planner[T, F, QScriptTotal[T, ?]].plan))
        rb <- rBranch.cataM(
          interpretM(κ(idRB.embed.η[F]), Planner[T, F, QScriptTotal[T, ?]].plan))
      } yield Union(SelectField(src, lb).embed, SelectField(src, rb).embed).embed
  ...
  def takeOrDrop(
      src: T[N1QL],
      from: FreeQS[T],
      takeOrDrop: FreeQS[T] \/ FreeQS[T]): F[T[N1QL]] =
    for {
      id1 <- genId[T[N1QL], F]
      id2 <- genId[T[N1QL], F]
      id3 <- genId[T[N1QL], F]
      id4 <- genId[T[N1QL], F]
      id5 <- genId[T[N1QL], F]
      f <- from.cataM(interpretM(κ(id1.embed.η[F]), Planner[T, F, QScriptTotal[T, ?]].plan))
      c <- takeOrDrop.merge.cataM(
        interpretM(κ(id1.embed.η[F]), Planner[T, F, QScriptTotal[T, ?]].plan))
    } yield {
      val s =
        Select(
          Value(false),
          nels(ResultExpr(f, id2.some), ResultExpr(c, id3.some)),
          Keyspace(src, id1.some).some,
          join = none,
          unnest = none,
          let = nil,
      ...
      val slc =
        takeOrDrop
          .bimap(
            κ(Slice(int(0), Least(LengthArr(id2.embed).embed, cnt).embed.some)),
            κ(Slice(cnt, none))
          )
          .merge
          .embed
